
BugGrabberDB = {
	["lastSanitation"] = 3,
	["session"] = 1133,
	["errors"] = {
		{
			["message"] = "[string \"Error in: Chain Distance 2':'custom text\"]:33: attempt to concatenate a nil value",
			["time"] = "2021/06/12 00:01:59",
			["locals"] = "Skipped (In Encounter)",
			["stack"] = "[string \"=[C]\"]: in function `xpcall'\n[string \"@Interface\\AddOns\\WeakAuras\\WeakAuras-3.4.2.lua\"]:4113: in function `RunCustomTextFunc'\n[string \"@Interface\\AddOns\\WeakAuras\\SubRegionTypes\\SubText.lua\"]:288: in function `?'\n[string \"@Interface\\AddOns\\WeakAuras\\RegionTypes\\RegionPrototype.lua\"]:34: in function `Notify'\n[string \"@Interface\\AddOns\\WeakAuras\\WeakAuras-3.4.2.lua\"]:3884: in function <Interface\\AddOns\\WeakAuras\\WeakAuras.lua:3881>\n[string \"@Interface\\AddOns\\WeakAuras\\WeakAuras-3.4.2.lua\"]:3971: in function <Interface\\AddOns\\WeakAuras\\WeakAuras.lua:3938>\n[string \"@Interface\\AddOns\\WeakAuras\\WeakAuras-3.4.2.lua\"]:4054: in function `UpdatedTriggerState'\n[string \"@Interface\\AddOns\\WeakAuras\\BuffTrigger2.lua\"]:1397: in function <Interface\\AddOns\\WeakAuras\\BuffTrigger2.lua:1389>\n[string \"@Interface\\AddOns\\WeakAuras\\BuffTrigger2.lua\"]:1716: in function <Interface\\AddOns\\WeakAuras\\BuffTrigger2.lua:1709>",
			["session"] = 1115,
			["counter"] = 409,
		}, -- [1]
		{
			["message"] = "[ADDON_ACTION_BLOCKED] AddOn 'Dominos' tried to call the protected function 'MultiBarBottomLeftButton3:Show()'.",
			["time"] = "2021/06/12 00:15:50",
			["locals"] = "Skipped (In Encounter)",
			["stack"] = "[string \"@Interface\\AddOns\\!BugGrabber\\BugGrabber.lua\"]:519: in function <Interface\\AddOns\\!BugGrabber\\BugGrabber.lua:519>\n[string \"=[C]\"]: in function `Show'\n[string \"@Interface\\FrameXML\\ActionButton.lua\"]:348: in function <Interface\\FrameXML\\ActionButton.lua:332>\n[string \"=[C]\"]: in function `Update'\n[string \"@Interface\\FrameXML\\ActionButton.lua\"]:854: in function `OnEvent'\n[string \"@Interface\\FrameXML\\ActionButton.lua\"]:265: in function <Interface\\FrameXML\\ActionButton.lua:257>",
			["session"] = 1116,
			["counter"] = 1,
		}, -- [2]
		{
			["message"] = "[ADDON_ACTION_BLOCKED] AddOn 'Dominos' tried to call the protected function 'OverrideActionBarButton1:Show()'.",
			["time"] = "2021/06/12 00:54:55",
			["locals"] = "Skipped (In Encounter)",
			["stack"] = "[string \"@Interface\\AddOns\\!BugGrabber\\BugGrabber.lua\"]:519: in function <Interface\\AddOns\\!BugGrabber\\BugGrabber.lua:519>\n[string \"=[C]\"]: in function `Show'\n[string \"@Interface\\FrameXML\\ActionButton.lua\"]:348: in function <Interface\\FrameXML\\ActionButton.lua:332>\n[string \"=[C]\"]: in function `Update'\n[string \"@Interface\\FrameXML\\ActionButton.lua\"]:759: in function `OnEvent'\n[string \"@Interface\\FrameXML\\ActionButton.lua\"]:217: in function <Interface\\FrameXML\\ActionButton.lua:214>",
			["session"] = 1117,
			["counter"] = 1,
		}, -- [3]
		{
			["message"] = "[ADDON_ACTION_BLOCKED] AddOn 'Dominos' tried to call the protected function 'MultiBarLeftButton4:Show()'.",
			["time"] = "2021/06/12 02:40:30",
			["locals"] = "Skipped (In Encounter)",
			["stack"] = "[string \"@Interface\\AddOns\\!BugGrabber\\BugGrabber.lua\"]:519: in function <Interface\\AddOns\\!BugGrabber\\BugGrabber.lua:519>\n[string \"=[C]\"]: in function `Show'\n[string \"@Interface\\FrameXML\\ActionButton.lua\"]:348: in function <Interface\\FrameXML\\ActionButton.lua:332>\n[string \"=[C]\"]: in function `Update'\n[string \"@Interface\\FrameXML\\ActionButton.lua\"]:854: in function `OnEvent'\n[string \"@Interface\\FrameXML\\ActionButton.lua\"]:265: in function <Interface\\FrameXML\\ActionButton.lua:257>",
			["session"] = 1121,
			["counter"] = 1,
		}, -- [4]
		{
			["message"] = "Interface\\AddOns\\Details\\Libs\\DF\\cooltip.lua:1036: GameCooltipMainButton1_StatusBar_TextRight:SetFontObject(): Couldn't find font named Bazooka",
			["time"] = "2021/06/18 21:19:30",
			["locals"] = "(*temporary) = GameCooltipMainButton1_StatusBar_TextRight {\n 0 = <userdata>\n}\n(*temporary) = \"Bazooka\"\n",
			["stack"] = "[string \"=[C]\"]: in function `SetFontObject'\n[string \"@Interface\\AddOns\\Details\\Libs\\DF\\cooltip.lua\"]:1036: in function `TextAndIcon'\n[string \"@Interface\\AddOns\\Details\\Libs\\DF\\cooltip.lua\"]:1595: in function <Interface\\AddOns\\Details\\Libs\\DF\\cooltip.lua:1557>\n[string \"=(tail call)\"]: ?\n[string \"@Interface\\AddOns\\Details\\Libs\\DF\\dropdown.lua\"]:586: in function <Interface\\AddOns\\Details\\Libs\\DF\\dropdown.lua:572>",
			["session"] = 1124,
			["counter"] = 1,
		}, -- [5]
		{
			["message"] = "[ADDON_ACTION_BLOCKED] AddOn 'Dominos' tried to call the protected function 'ActionButton5:Show()'.",
			["time"] = "2021/06/18 23:04:25",
			["locals"] = "Skipped (In Encounter)",
			["stack"] = "[string \"@Interface\\AddOns\\!BugGrabber\\BugGrabber.lua\"]:519: in function <Interface\\AddOns\\!BugGrabber\\BugGrabber.lua:519>\n[string \"=[C]\"]: in function `Show'\n[string \"@Interface\\FrameXML\\ActionButton.lua\"]:348: in function <Interface\\FrameXML\\ActionButton.lua:332>\n[string \"=[C]\"]: in function `Update'\n[string \"@Interface\\FrameXML\\ActionButton.lua\"]:854: in function `OnEvent'\n[string \"@Interface\\FrameXML\\ActionButton.lua\"]:265: in function <Interface\\FrameXML\\ActionButton.lua:257>",
			["session"] = 1131,
			["counter"] = 2,
		}, -- [6]
		{
			["message"] = "AceLocale-3.0-6: OmniCD: Missing entry for 'A new update is available. (%s)'",
			["time"] = "2021/06/26 01:04:04",
			["stack"] = "[string \"@Interface\\AddOns\\OmniCD\\Core\\load.lua\"]:131: in function <Interface\\AddOns\\OmniCD\\Core\\load.lua:121>",
			["session"] = 1133,
			["counter"] = 1,
		}, -- [7]
		{
			["message"] = "[ADDON_ACTION_BLOCKED] AddOn 'Dominos' tried to call the protected function 'MultiBarLeftButton6:Show()'.",
			["time"] = "2021/06/26 01:34:50",
			["locals"] = "Skipped (In Encounter)",
			["stack"] = "[string \"@Interface\\AddOns\\!BugGrabber\\BugGrabber.lua\"]:519: in function <Interface\\AddOns\\!BugGrabber\\BugGrabber.lua:519>\n[string \"=[C]\"]: in function `Show'\n[string \"@Interface\\FrameXML\\ActionButton.lua\"]:348: in function <Interface\\FrameXML\\ActionButton.lua:332>\n[string \"=[C]\"]: in function `Update'\n[string \"@Interface\\FrameXML\\ActionButton.lua\"]:854: in function `OnEvent'\n[string \"@Interface\\FrameXML\\ActionButton.lua\"]:265: in function <Interface\\FrameXML\\ActionButton.lua:257>",
			["session"] = 1133,
			["counter"] = 1,
		}, -- [8]
	},
}
