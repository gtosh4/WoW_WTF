
WeakAurasSaved = {
	["dynamicIconCache"] = {
		["Flame Shock"] = {
			[188389] = 135813,
		},
	},
	["editor_tab_spaces"] = 4,
	["login_squelch_time"] = 10,
	["lastArchiveClear"] = 1595234684,
	["minimap"] = {
		["minimapPos"] = 208.9286600903307,
		["hide"] = true,
	},
	["lastUpgrade"] = 1618278840,
	["dbVersion"] = 45,
	["displays"] = {
		["09 Heart Rend - Teal - disabled"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 4,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						0, -- [1]
						0.94509803921569, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.25,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = true,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2417",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "A((uIFK)7Vv",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["useMatch_count"] = true,
						["auranames"] = {
						},
						["groupclone"] = true,
						["matchesShowOn"] = "showOnActive",
						["name_info"] = "aura",
						["unit"] = "group",
						["use_tooltip"] = false,
						["subeventPrefix"] = "SPELL",
						["match_countOperator"] = ">=",
						["match_count"] = "1",
						["buffShowOn"] = "showOnActive",
						["unevent"] = "timed",
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["useExactSpellId"] = true,
						["custom_type"] = "stateupdate",
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["event"] = "Health",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HARMFUL",
						["use_specific_unit"] = false,
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["auraspellids"] = {
							"334765", -- [1]
						},
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["duration"] = "1",
						["useGroup_count"] = false,
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["text2Font"] = "Friz Quadrata TT",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text1"] = "%n",
			["text1Point"] = "CENTER",
			["anchorFrameType"] = "SCREEN",
			["semver"] = "1.0.26",
			["justify"] = "LEFT",
			["zoom"] = 0,
			["id"] = "09 Heart Rend - Teal - disabled",
			["config"] = {
			},
			["frameStrata"] = 9,
			["width"] = 6.9999866485596,
			["text1Containment"] = "INSIDE",
			["wordWrap"] = "WordWrap",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["Dancing Fever"] = {
			["sparkWidth"] = 10,
			["iconSource"] = -1,
			["xOffset"] = 0,
			["displayText"] = "%c1 %c2 - %s",
			["yOffset"] = 0,
			["displayText_format_p_time_dynamic"] = false,
			["sparkRotation"] = 0,
			["url"] = "https://wago.io/H4Beysoli/3",
			["backgroundColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				0.5, -- [4]
			},
			["icon_color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["displayText_format_c1_format"] = "none",
			["barColor"] = {
				1, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["sparkOffsetY"] = 0,
			["load"] = {
				["use_encounterid"] = true,
				["zoneIds"] = "",
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2412",
				["class"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["shadowXOffset"] = 1,
			["useAdjustededMin"] = false,
			["regionType"] = "aurabar",
			["texture"] = "Smooth v2",
			["sparkTexture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
			["auto"] = true,
			["tocversion"] = 90002,
			["alpha"] = 1,
			["sparkColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["fixedWidth"] = 200,
			["outline"] = "OUTLINE",
			["sparkOffsetX"] = 0,
			["color"] = {
				1, -- [1]
				0.98039215686275, -- [2]
				0.94901960784314, -- [3]
				1, -- [4]
			},
			["customText"] = "function()\n    if aura_env.state and aura_env.state.unitName then\n        return WA_ClassColorName(aura_env.state.unitName)\n    end\nend\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["sparkRotationMode"] = "AUTO",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["showClones"] = true,
						["type"] = "aura2",
						["subeventSuffix"] = "_CAST_START",
						["auranames"] = {
							"Mark of the Wild", -- [1]
						},
						["subeventPrefix"] = "SPELL",
						["fetchTooltip"] = false,
						["event"] = "Health",
						["names"] = {
						},
						["auraspellids"] = {
							"347350", -- [1]
						},
						["match_countOperator"] = ">=",
						["spellIds"] = {
						},
						["useExactSpellId"] = true,
						["match_count"] = "1",
						["useMatch_count"] = true,
						["unit"] = "group",
						["debuffType"] = "HARMFUL",
					},
					["untrigger"] = {
					},
				}, -- [1]
				{
					["trigger"] = {
						["type"] = "custom",
						["subeventSuffix"] = "_CAST_START",
						["duration"] = "0.01",
						["event"] = "Chat Message",
						["subeventPrefix"] = "SPELL",
						["events"] = "RAID_TARGET_UPDATE",
						["custom_type"] = "stateupdate",
						["check"] = "event",
						["custom"] = "function()\n    return true\nend",
						["unevent"] = "timed",
						["custom_hide"] = "timed",
					},
					["untrigger"] = {
					},
				}, -- [2]
				["disjunctive"] = "custom",
				["customTriggerLogic"] = "function(t)\nreturn t[1]\nend",
				["activeTriggerMode"] = -10,
			},
			["displayText_format_p_format"] = "timed",
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["displayText_format_c2_format"] = "none",
			["version"] = 3,
			["subRegions"] = {
				{
					["type"] = "aurabar_bar",
				}, -- [1]
				{
					["text_text_format_n_format"] = "none",
					["text_text_format_s_format"] = "none",
					["text_text"] = "%c1",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Expressway",
					["text_shadowYOffset"] = -1,
					["text_fontType"] = "OUTLINE",
					["text_wordWrap"] = "WordWrap",
					["text_visible"] = true,
					["text_anchorPoint"] = "INNER_LEFT",
					["text_shadowXOffset"] = 1,
					["text_text_format_c1_format"] = "none",
					["text_fontSize"] = 14,
					["anchorXOffset"] = 0,
					["text_text_format_c2_format"] = "none",
				}, -- [2]
				{
					["text_text_format_n_format"] = "none",
					["text_text_format_s_format"] = "none",
					["text_text"] = "%s",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_anchorXOffset"] = -2,
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Expressway",
					["text_shadowYOffset"] = -1,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "None",
					["text_anchorPoint"] = "INNER_RIGHT",
					["text_shadowXOffset"] = 1,
					["text_fontSize"] = 18,
					["anchorXOffset"] = 0,
					["text_visible"] = true,
				}, -- [3]
				{
					["border_size"] = 2,
					["border_anchor"] = "bar",
					["border_offset"] = 0,
					["border_color"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["border_visible"] = true,
					["border_edge"] = "Square Full White",
					["type"] = "subborder",
				}, -- [4]
			},
			["height"] = 25,
			["displayText_format_s_format"] = "none",
			["useAdjustededMax"] = false,
			["fontSize"] = 20,
			["config"] = {
			},
			["parent"] = "07 Dancing Fever List",
			["uid"] = "1j81R2L5dQh",
			["orientation"] = "HORIZONTAL",
			["icon"] = false,
			["authorOptions"] = {
			},
			["selfPoint"] = "BOTTOM",
			["actions"] = {
				["start"] = {
				},
				["finish"] = {
				},
				["init"] = {
					["custom"] = "aura_env.icons= { \n    [8] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_8:12\\124t\",\n    [7] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_7:12\\124t\",\n    [6] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_6:12\\124t\",\n    [5] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_5:12\\124t\",\n    [4] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_4:12\\124t\",\n    [3] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_3:12\\124t\",\n    [2] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_2:12\\124t\",\n    [1]  = \n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_1:12\\124t\",       \n}\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
					["do_custom"] = true,
				},
			},
			["icon_side"] = "RIGHT",
			["width"] = 200,
			["sparkHeight"] = 30,
			["displayText_format_p_time_precision"] = 0,
			["customTextUpdate"] = "event",
			["id"] = "Dancing Fever",
			["justify"] = "CENTER",
			["semver"] = "1.0.2",
			["anchorPoint"] = "CENTER",
			["sparkHidden"] = "NEVER",
			["zoom"] = 0,
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["sparkBlendMode"] = "ADD",
			["spark"] = false,
			["inverse"] = false,
			["wordWrap"] = "WordWrap",
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["value"] = "2",
						["variable"] = "stacks",
					},
					["changes"] = {
						{
							["value"] = {
								1, -- [1]
								0.45882352941176, -- [2]
								0, -- [3]
								1, -- [4]
							},
							["property"] = "barColor",
						}, -- [1]
					},
				}, -- [1]
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["value"] = "1",
						["variable"] = "stacks",
					},
					["changes"] = {
						{
							["value"] = {
								1, -- [1]
								0.92156862745098, -- [2]
								0, -- [3]
								1, -- [4]
							},
							["property"] = "barColor",
						}, -- [1]
					},
				}, -- [2]
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["07 Dark Recital Partner - Yellow "] = {
			["outline"] = "OUTLINE",
			["authorOptions"] = {
				{
					["type"] = "select",
					["values"] = {
						"Standard", -- [1]
						"Pixel", -- [2]
						"AutoCast", -- [3]
					},
					["default"] = 3,
					["key"] = "glowType",
					["name"] = "Type",
					["width"] = 1,
				}, -- [1]
				{
					["type"] = "color",
					["key"] = "glowColor",
					["default"] = {
						1, -- [1]
						0.96862745098039, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["name"] = "Color",
					["width"] = 1,
				}, -- [2]
				{
					["softMin"] = 1,
					["type"] = "range",
					["bigStep"] = 1,
					["max"] = 50,
					["step"] = 1,
					["width"] = 1,
					["min"] = 1,
					["key"] = "glowParticules",
					["desc"] = "Number of particle groups (each groups contains 4 particles)",
					["softMax"] = 50,
					["useDesc"] = true,
					["name"] = "Particules",
					["default"] = 20,
				}, -- [3]
				{
					["min"] = 0,
					["type"] = "range",
					["name"] = "Frequency",
					["default"] = 0.3,
					["max"] = 1,
					["key"] = "glowFrequency",
					["step"] = 0.05,
					["width"] = 1,
				}, -- [4]
				{
					["min"] = -9999,
					["type"] = "number",
					["name"] = "xOffset",
					["default"] = 0,
					["max"] = 9999,
					["key"] = "glowxOffset",
					["step"] = 0.05,
					["width"] = 1,
				}, -- [5]
				{
					["min"] = -9999,
					["type"] = "number",
					["name"] = "yOffset",
					["default"] = 0,
					["max"] = 9999,
					["key"] = "glowyOffset",
					["step"] = 0.05,
					["width"] = 1,
				}, -- [6]
				{
					["fontSize"] = "medium",
					["key"] = "option13",
					["name"] = "Option #13",
					["text"] = "AutoCast Options",
					["type"] = "description",
					["width"] = 1,
				}, -- [7]
				{
					["type"] = "range",
					["useDesc"] = true,
					["max"] = 10,
					["step"] = 0.05,
					["width"] = 1,
					["min"] = 0,
					["key"] = "glowScale",
					["desc"] = "Scale of particles",
					["name"] = "Scale",
					["default"] = 2,
				}, -- [8]
				{
					["fontSize"] = "medium",
					["key"] = "option14",
					["name"] = "Option #14",
					["text"] = "Pixel Options",
					["type"] = "description",
					["width"] = 1,
				}, -- [9]
				{
					["type"] = "number",
					["useDesc"] = true,
					["max"] = 20,
					["step"] = 0.05,
					["width"] = 1,
					["min"] = 0,
					["key"] = "glowLength",
					["desc"] = "Length of lines",
					["name"] = "Length",
					["default"] = 8,
				}, -- [10]
				{
					["fontSize"] = "small",
					["key"] = "option14",
					["name"] = "Option #14",
					["text"] = " ",
					["type"] = "description",
					["width"] = 1,
				}, -- [11]
				{
					["type"] = "number",
					["useDesc"] = true,
					["max"] = 10,
					["step"] = 0.05,
					["width"] = 1,
					["min"] = 0,
					["key"] = "glowThickness",
					["desc"] = "Thickness of lines",
					["name"] = "Thickness",
					["default"] = 2,
				}, -- [12]
				{
					["fontSize"] = "small",
					["key"] = "option15",
					["name"] = "Option #15",
					["text"] = " ",
					["type"] = "description",
					["width"] = 1,
				}, -- [13]
				{
					["type"] = "toggle",
					["key"] = "glowBorder",
					["desc"] = "Create border under lines",
					["default"] = false,
					["useDesc"] = true,
					["name"] = "Border",
					["width"] = 1,
				}, -- [14]
			},
			["displayText"] = " ",
			["customText"] = "\n\n",
			["yOffset"] = 304.00006103516,
			["anchorPoint"] = "CENTER",
			["cooldownSwipe"] = true,
			["customTextUpdate"] = "event",
			["url"] = "https://wago.io/NathriaGlows/27",
			["icon"] = true,
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "aura2",
						["spellId"] = "",
						["useExactSpellId"] = true,
						["subeventPrefix"] = "SPELL",
						["duration"] = "1",
						["event"] = "Combat Log",
						["names"] = {
						},
						["unevent"] = "timed",
						["use_spellId"] = false,
						["spellIds"] = {
						},
						["subeventSuffix"] = "_CAST_START",
						["unit"] = "player",
						["auranames"] = {
						},
						["auraspellids"] = {
							"331637", -- [1]
							"331636", -- [2]
						},
						["debuffType"] = "HARMFUL",
					},
					["untrigger"] = {
					},
				}, -- [1]
				{
					["trigger"] = {
						["type"] = "custom",
						["unevent"] = "timed",
						["duration"] = "20",
						["event"] = "Chat Message",
						["subeventPrefix"] = "SPELL",
						["events"] = "UNIT_SPELLCAST_START:boss CLEU:SPELL_AURA_APPLIED:SPELL_AURA_REMOVED ENCOUNTER_START, ENCOUNTER_END",
						["custom_type"] = "event",
						["custom"] = "function(event, ...)\n    \n    if event == \"UNIT_SPELLCAST_START\" and select(3, ...) == 331634 then\n        aura_env.debuffs1 = {}\n        aura_env.debuffs2 = {}\n        \n    elseif event == \"COMBAT_LOG_EVENT_UNFILTERED\" then\n        local _,  subevent, hideCaster, sourceGUID, sourceName, sourceFlags, sourceRaidFlags, destGUID, destName, destFlags, destRaidFlags, spellID, spellName, spellSchool, extraSpellId = ...\n        \n        if subevent == \"SPELL_AURA_REMOVED\" and (spellID == aura_env.debuff1 or spellID == aura_env.debuff2) then\n            local frame = aura_env.GetFrame(destName)\n            if frame then\n                aura_env.Glow(frame, false)\n            end\n        end\n        \n        if subevent == \"SPELL_AURA_APPLIED\" then \n            if spellID == aura_env.debuff1 then\n                local destName = gsub(destName, \"%-[^|]+\", \"\")\n                if not UnitExists(destName) or not UnitIsVisible(destName) then return end\n                table.insert(aura_env.debuffs1, destName)\n                if UnitIsUnit(destName, \"player\") and aura_env.debuffs2[#aura_env.debuffs1] then\n                    local frame = aura_env.GetFrame(aura_env.debuffs2[#aura_env.debuffs1])\n                    if frame then\n                        aura_env.Glow(frame, true)\n                    end\n                    return true\n                elseif aura_env.debuffs2[#aura_env.debuffs1] and UnitIsUnit(aura_env.debuffs2[#aura_env.debuffs1], \"player\") then\n                    local frame = aura_env.GetFrame(aura_env.debuffs1[#aura_env.debuffs2])\n                    if frame then\n                        aura_env.Glow(frame, true)\n                    end\n                    return true\n                end\n            elseif spellID == aura_env.debuff2 then\n                local destName = gsub(destName, \"%-[^|]+\", \"\")\n                if not UnitExists(destName) or not UnitIsVisible(destName) then return end\n                table.insert(aura_env.debuffs2, destName)\n                if UnitIsUnit(destName, \"player\") and aura_env.debuffs1[#aura_env.debuffs2] then\n                    local frame = aura_env.GetFrame(aura_env.debuffs1[#aura_env.debuffs2])\n                    if frame then\n                        aura_env.Glow(frame, true)\n                    end\n                    return true\n                elseif aura_env.debuffs1[#aura_env.debuffs2] and UnitIsUnit(aura_env.debuffs1[#aura_env.debuffs2], \"player\") then\n                    local frame = aura_env.GetFrame(aura_env.debuffs2[#aura_env.debuffs1])\n                    if frame then\n                        aura_env.Glow(frame, true)\n                    end\n                    return true\n                end\n            end\n        end\n        \n    elseif event == \"ENCOUNTER_START\" then\n        aura_env.debuffs1 = {}\n        aura_env.debuffs2 = {}\n        \n    elseif event == \"ENCOUNTER_END\" then\n        for unit in WA_IterateGroupMembers() do\n            local frame = aura_env.GetFrame(unit)\n            if frame then\n                aura_env.Glow(frame, false)\n            end \n        end\n    end\n    \n    \nend\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
						["subeventSuffix"] = "_CAST_START",
						["custom_hide"] = "timed",
					},
					["untrigger"] = {
					},
				}, -- [2]
				["disjunctive"] = "all",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["keepAspectRatio"] = false,
			["wordWrap"] = "WordWrap",
			["auto"] = false,
			["fixedWidth"] = 200,
			["stickyDuration"] = false,
			["font"] = "Friz Quadrata TT",
			["version"] = 27,
			["subRegions"] = {
			},
			["height"] = 75,
			["parent"] = "Castle Nathria Glows",
			["load"] = {
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["use_encounterid"] = true,
				["difficulty"] = {
					["single"] = "mythic",
					["multi"] = {
						["mythic"] = true,
					},
				},
				["role"] = {
					["single"] = "TANK",
					["multi"] = {
						["TANK"] = true,
					},
				},
				["encounterid"] = "2412",
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["displayIcon"] = 135740,
			["fontSize"] = 12,
			["shadowYOffset"] = -1,
			["xOffset"] = -147.99987792969,
			["shadowXOffset"] = 1,
			["automaticWidth"] = "Auto",
			["config"] = {
				["glowFrequency"] = 0.3,
				["glowxOffset"] = 0,
				["glowyOffset"] = 0,
				["glowLength"] = 8,
				["glowColor"] = {
					1, -- [1]
					0.96862745098039, -- [2]
					0, -- [3]
					1, -- [4]
				},
				["glowParticules"] = 20,
				["glowThickness"] = 2,
				["glowType"] = 3,
				["glowScale"] = 2,
				["glowBorder"] = false,
			},
			["desaturate"] = false,
			["regionType"] = "text",
			["anchorFrameType"] = "SCREEN",
			["frameStrata"] = 1,
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["cooldownEdge"] = false,
			["cooldownTextDisabled"] = false,
			["semver"] = "1.0.26",
			["zoom"] = 0.3,
			["justify"] = "LEFT",
			["tocversion"] = 90002,
			["id"] = "07 Dark Recital Partner - Yellow ",
			["preferToUpdate"] = false,
			["alpha"] = 1,
			["width"] = 75,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["uid"] = "uQmnWy8mpUe",
			["inverse"] = false,
			["actions"] = {
				["start"] = {
					["do_glow"] = false,
					["do_custom"] = false,
					["sound"] = "Interface\\Addons\\SharedMedia_Causese\\sound\\Debuff.ogg",
					["do_sound"] = false,
				},
				["init"] = {
					["custom"] = "aura_env.debuff1 = 331637\naura_env.debuff2 = 331636\naura_env.debuffs1 = {}\naura_env.debuffs2 = {}\naura_env.debug = false\n\n\nif aura_env.debug then\n    aura_env.debuff1 = 774\n    aura_env.debuff2 = 8936\n    aura_env.cast = 48438\nend\n\n\n\naura_env.GetFrame = WeakAuras.GetUnitFrame\n\n\n-- Custom Glow\n\nlocal LCG = LibStub(\"LibCustomGlow-1.0\")\nlocal glowTypes = {\"Standard\",\"Pixel\",\"AutoCast\"}\nlocal glowType = glowTypes[aura_env.config.glowType]\n\naura_env.Glow = function(frame,show)\n    if show then\n        if glowType == \"AutoCast\" then\n            LCG.AutoCastGlow_Start(\n                frame,\n                aura_env.config.glowColor,\n                aura_env.config.glowParticules,\n                aura_env.config.glowFrequency,\n                aura_env.config.glowScale,\n                aura_env.config.glowxOffset,\n                aura_env.config.glowyOffset,\n                aura_env.id\n            )\n        elseif glowType == \"Pixel\" then\n            LCG.PixelGlow_Start(\n                frame,\n                aura_env.config.glowColor,\n                aura_env.config.glowParticules,\n                aura_env.config.glowFrequency,\n                aura_env.config.glowLength,\n                aura_env.config.glowThickness,\n                aura_env.config.glowxOffset,\n                aura_env.config.glowyOffset,\n                aura_env.config.glowBorder,\n                aura_env.id\n            )\n        elseif glowType == \"Standard\" then\n            LCG.ButtonGlow_Start(\n                frame,\n                aura_env.config.glowColor,\n                aura_env.config.glowFrequency\n            )\n        end\n    else\n        if glowType == \"AutoCast\" then\n            LCG.AutoCastGlow_Stop(frame, aura_env.id)\n        elseif glowType == \"Pixel\" then\n            LCG.PixelGlow_Stop(frame, aura_env.id)\n        elseif glowType == \"Standard\" then\n            LCG.ButtonGlow_Stop(frame)\n        end\n    end\nend\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
					["do_custom"] = true,
				},
				["finish"] = {
					["hide_all_glows"] = true,
				},
			},
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["cooldown"] = true,
			["selfPoint"] = "CENTER",
		},
		["02 Sinseeker Targeted - Pink"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 4,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.25,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = true,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2418",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "cWR5jsxFmOS",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["useMatch_count"] = true,
						["auranames"] = {
						},
						["groupclone"] = true,
						["matchesShowOn"] = "showOnActive",
						["name_info"] = "aura",
						["unit"] = "group",
						["use_tooltip"] = false,
						["subeventPrefix"] = "SPELL",
						["match_countOperator"] = ">=",
						["match_count"] = "1",
						["buffShowOn"] = "showOnActive",
						["unevent"] = "timed",
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["useExactSpellId"] = true,
						["custom_type"] = "stateupdate",
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["event"] = "Health",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HARMFUL",
						["use_specific_unit"] = false,
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["auraspellids"] = {
							"335113", -- [1]
							"335112", -- [2]
							"335111", -- [3]
						},
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["duration"] = "1",
						["useGroup_count"] = false,
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["wordWrap"] = "WordWrap",
			["config"] = {
			},
			["text1"] = "%n",
			["text1Containment"] = "INSIDE",
			["width"] = 6.9999866485596,
			["justify"] = "LEFT",
			["semver"] = "1.0.26",
			["zoom"] = 0,
			["id"] = "02 Sinseeker Targeted - Pink",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["frameStrata"] = 9,
			["anchorFrameType"] = "SCREEN",
			["text1Point"] = "CENTER",
			["text2Font"] = "Friz Quadrata TT",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["circle_leaf_fill"] = {
			["iconSource"] = 0,
			["xOffset"] = 0,
			["preferToUpdate"] = true,
			["customText"] = "function()\n    if aura_env.showText then\n        return aura_env.order\n    else\n        return \"\"\n    end\nend",
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["cooldownSwipe"] = true,
			["cooldownEdge"] = false,
			["icon"] = true,
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "custom",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HELPFUL",
						["event"] = "Health",
						["unit"] = "player",
						["custom_type"] = "status",
						["spellIds"] = {
						},
						["events"] = "ZONE_CHANGED, PLAYER_LOGIN, PLAYER_ENTERING_WORLD",
						["names"] = {
						},
						["check"] = "event",
						["custom"] = "function()\n    WeakAuras.ScanEvents(\"TirnaScitheButtonData\", 0, aura_env.frameData)\n    if aura_env.debugmode then \n        return true \n    end\n    local text = GetMinimapZoneText()\n    if text == aura_env.mistZone then\n        return true\n    else\n        return false\n    end\nend",
						["subeventPrefix"] = "SPELL",
						["custom_hide"] = "custom",
					},
					["untrigger"] = {
					},
				}, -- [1]
				{
					["trigger"] = {
						["debuffType"] = "HELPFUL",
						["type"] = "custom",
						["custom"] = "function(event,eid,...)\n    if event == \"ENCOUNTER_END\" then \n        if eid == 2392 and aura_env.showEncounter == false then \n            return false \n        end\n        return true \n    elseif event == \"ENCOUNTER_START\" then \n        if aura_env.showEncounter == false then \n            return false\n        else\n            return true\n        end\n    end\n    return true\nend",
						["custom_type"] = "status",
						["check"] = "event",
						["customName"] = "function()\n    return \nend",
						["events"] = "ENCOUNTER_START, ENCOUNTER_END, PLAYER_ENTERING_WORLD",
						["unit"] = "player",
					},
					["untrigger"] = {
					},
				}, -- [2]
				{
					["trigger"] = {
						["type"] = "custom",
						["custom_type"] = "status",
						["unit"] = "player",
						["customName"] = "function()\n    return aura_env.order            \nend",
						["events"] = "TirnaScitheButtonOrder",
						["check"] = "event",
						["debuffType"] = "HELPFUL",
						["custom"] = "function(_,button,order)\n    if button == 0 then \n        aura_env.order = order\n        if order == 9 then \n            aura_env.order = \"\"\n        end\n        return true\n    elseif button == 9 then\n        aura_env.order = \"\"\n        return true\n    end\n    return true\nend\n\n",
						["custom_hide"] = "timed",
					},
					["untrigger"] = {
					},
				}, -- [3]
				["activeTriggerMode"] = -10,
			},
			["internalVersion"] = 45,
			["keepAspectRatio"] = false,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["desaturate"] = false,
			["version"] = 67,
			["subRegions"] = {
				{
					["glowFrequency"] = 0.25,
					["type"] = "subglow",
					["useGlowColor"] = false,
					["glowType"] = "buttonOverlay",
					["glowLength"] = 10,
					["glowYOffset"] = 0,
					["glowColor"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["glowXOffset"] = 0,
					["glowScale"] = 1,
					["glowThickness"] = 1,
					["glow"] = false,
					["glowLines"] = 8,
					["glowBorder"] = false,
				}, -- [1]
				{
					["text_text_format_p_time_precision"] = 1,
					["text_text"] = "%c",
					["text_text_format_p_format"] = "timed",
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["text_text_format_p_time_format"] = 0,
					["type"] = "subtext",
					["text_text_format_p_time_dynamic_threshold"] = 60,
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Friz Quadrata TT",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_shadowYOffset"] = 0,
					["text_visible"] = true,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "INNER_BOTTOMRIGHT",
					["text_text_format_n_format"] = "none",
					["text_text_format_4.n_format"] = "none",
					["text_fontSize"] = 15,
					["anchorXOffset"] = 0,
					["text_shadowXOffset"] = 0,
				}, -- [2]
			},
			["height"] = 80,
			["load"] = {
				["ingroup"] = {
					["single"] = "group",
					["multi"] = {
						["group"] = true,
					},
				},
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["use_encounterid"] = false,
				["difficulty"] = {
					["multi"] = {
						["mythic"] = true,
						["challenge"] = true,
					},
				},
				["use_zone"] = false,
				["zoneIds"] = "1669",
				["use_zoneIds"] = true,
				["zone"] = "",
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["config"] = {
				["encounter"] = false,
				["noCommunication"] = false,
				["debug"] = false,
				["orderText"] = true,
			},
			["actions"] = {
				["start"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
				},
				["init"] = {
					["custom"] = "aura_env.debugmode = aura_env.config['debug']\nlocal debug = aura_env.debugmode\naura_env.showEncounter = aura_env.config['encounter']\naura_env.locale=GetLocale()\naura_env.mistZone = \"Mistveil Tangle\"\naura_env.order = \"\"\naura_env.showText = aura_env.config['orderText']\naura_env.noCommunication = aura_env.config['noCommunication']\nif aura_env.locale == \"koKR\" then\n    aura_env.mistZone = \"안개장막 덩굴숲\"\nelseif aura_env.locale == \"frFR\" then\n    aura_env.mistZone = \"Maquis Voile-de-Brume\"\nelseif aura_env.locale == \"deDE\" then\n    aura_env.mistZone = \"Nebelschleierdickicht\"\nelseif aura_env.locale == \"zhCN\" then\n    aura_env.mistZone = \"纱雾迷结\"\nelseif aura_env.locale == \"esES\" then\n    aura_env.mistZone = \"Espesura Velo de Niebla\"\nelseif aura_env.locale == \"zhTW\" then\n    aura_env.mistZone = \"霧紗密林\"\nelseif aura_env.locale == \"esMX\" then\n    aura_env.mistZone = \"Espesura Veloniebla\"\nelseif aura_env.locale == \"ruRU\" then\n    aura_env.mistZone = \"Туманная чащоба\"\nelseif aura_env.locale == \"ptBR\" then\n    aura_env.mistZone = \"Enleio do Véu da Névoa\"\nelseif aura_env.locale == \"itIT\" then\n    aura_env.mistZone = \"Intrico Velofosco\"\nelseif aura_env.locale == \"ptPT\" then\n    aura_env.mistZone = \"Enleio do Véu da Névoa\"        \nend\nlocal values = {0, 1, 0, 0}\nlocal message = string.format(\"%d %d %d %d\", values[1], values[2], values[3], values[4])\nlocal name, _ = UnitName(\"player\")\n\nif not aura_env.frameData then\n    local b = CreateFrame(\"Button\", nil, WeakAuras.regions[aura_env.id].region)\n    b:SetAllPoints(WeakAuras.regions[aura_env.id].region)\n    b:RegisterForClicks(\"LeftButtonDown\", \"RightButtonDown\")\n    aura_env.frameData = b\n    WeakAuras.ScanEvents(\"TirnaScitheButtonData\", 0, b)\nend\nif aura_env.noCommunication then\n    aura_env.frameData:SetScript(\"OnClick\", function(self, button)\n            if debug then print(\"Click Registered - true\") end\n            WeakAuras.ScanEvents(\"GlowUpdate\")\n            C_ChatInfo.RegisterAddonMessagePrefix(\"Tirna Ver2\") \n            if debug then \n                print(\"noCommunication sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"WHISPER\", name)) \n            else\n                C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"WHISPER\", name)\n            end\n            \n    end)\nelse\n    aura_env.frameData:SetScript(\"OnClick\", function(self, button)\n            if debug then print(\"Click Registered - true\") end\n            WeakAuras.ScanEvents(\"GlowUpdate\")\n            C_ChatInfo.RegisterAddonMessagePrefix(\"Tirna Ver2\")\n            if IsPartyLFG() then            \n                if debug then \n                    print(\"LFG sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"INSTANCE_CHAT\")) \n                else\n                    C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"INSTANCE_CHAT\")\n                end\n            elseif UnitInParty(\"player\") then            \n                if debug then \n                    print(\"party sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"PARTY\")) \n                else\n                    C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"PARTY\")\n                end\n            else         \n                if debug then \n                    print(\"solo sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"WHISPER\", name)) \n                else\n                    C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"WHISPER\", name)\n                end\n            end\n    end)\nend",
					["do_custom"] = true,
				},
			},
			["authorOptions"] = {
				{
					["type"] = "toggle",
					["key"] = "orderText",
					["default"] = true,
					["name"] = "Show Order Text",
					["useDesc"] = false,
					["width"] = 1,
				}, -- [1]
				{
					["type"] = "toggle",
					["key"] = "encounter",
					["default"] = false,
					["name"] = "Show during Boss",
					["useDesc"] = false,
					["width"] = 1,
				}, -- [2]
				{
					["type"] = "toggle",
					["key"] = "noCommunication",
					["default"] = false,
					["name"] = "Disable Cross-Communication with others running the WA",
					["useDesc"] = false,
					["width"] = 2,
				}, -- [3]
				{
					["type"] = "toggle",
					["key"] = "debug",
					["default"] = false,
					["name"] = "|cffff0000Debug testing|r (requires reload for full debug)",
					["useDesc"] = false,
					["width"] = 2,
				}, -- [4]
			},
			["regionType"] = "icon",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["cooldown"] = false,
			["conditions"] = {
			},
			["parent"] = "Mists of Tirna Scithe guessing game v2",
			["semver"] = "2.0.2",
			["anchorFrameType"] = "SCREEN",
			["zoom"] = 0,
			["auto"] = false,
			["tocversion"] = 90005,
			["id"] = "circle_leaf_fill",
			["alpha"] = 1,
			["frameStrata"] = 1,
			["width"] = 80,
			["cooldownTextDisabled"] = false,
			["uid"] = "qSPR4rlzXJY",
			["inverse"] = false,
			["selfPoint"] = "CENTER",
			["displayIcon"] = "Interface\\AddOns\\guess\\circle_leaf_fill.tga",
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["url"] = "https://wago.io/hVLym_eLv/67",
		},
		["DH Sigil of Silence 2"] = {
			["sparkWidth"] = 10,
			["iconSource"] = -1,
			["xOffset"] = 0,
			["preferToUpdate"] = false,
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["sparkRotation"] = 0,
			["url"] = "https://wago.io/ZG34ifiRf/1",
			["icon"] = true,
			["fontFlags"] = "OUTLINE",
			["icon_color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["selfPoint"] = "CENTER",
			["barColor"] = {
				0.35686274509804, -- [1]
				0.94117647058824, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["desaturate"] = false,
			["sparkOffsetY"] = 0,
			["load"] = {
				["ingroup"] = {
					["multi"] = {
					},
				},
				["use_never"] = false,
				["class"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["level"] = "100",
				["size"] = {
					["single"] = "party",
					["multi"] = {
						["ratedpvp"] = true,
						["arena"] = true,
						["ratedarena"] = true,
						["pvp"] = true,
					},
				},
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_level"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["difficulty"] = {
					["single"] = "challenge",
					["multi"] = {
						["challenge"] = true,
					},
				},
				["zoneIds"] = "",
				["faction"] = {
					["multi"] = {
					},
				},
				["use_size"] = false,
				["race"] = {
					["multi"] = {
					},
				},
				["use_zone"] = false,
				["pvptalent"] = {
					["multi"] = {
					},
				},
			},
			["useAdjustededMin"] = false,
			["regionType"] = "aurabar",
			["texture"] = "Glamour4",
			["sparkTexture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
			["spark"] = false,
			["tocversion"] = 80300,
			["alpha"] = 1,
			["sparkColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["displayIcon"] = "Interface\\Icons\\spell_holy_rebuke",
			["stacksPoint"] = "BOTTOMRIGHT",
			["borderBackdrop"] = "Blizzard Tooltip",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["sparkRotationMode"] = "AUTO",
			["triggers"] = {
				{
					["trigger"] = {
						["spellId"] = "",
						["use_genericShowOn"] = true,
						["spellName"] = "Sigil of Silence",
						["subeventSuffix"] = "_CAST_SUCCESS",
						["use_showOn"] = true,
						["event"] = "Combat Log",
						["sourceFlags2"] = "Hostile",
						["use_name"] = true,
						["use_spellId"] = false,
						["name"] = "Flask of the warm sun ",
						["use_sourceUnit"] = false,
						["use_destunit"] = false,
						["use_destUnit"] = false,
						["genericShowOn"] = "showOnCooldown",
						["names"] = {
							"Greater Draenic Intellect Flask", -- [1]
						},
						["unit"] = "player",
						["use_destName"] = false,
						["use_cloneId"] = false,
						["debuffType"] = "HELPFUL",
						["custom_hide"] = "timed",
						["type"] = "combatlog",
						["sourceName"] = "",
						["unevent"] = "timed",
						["ownOnly"] = true,
						["use_unit"] = true,
						["spellIds"] = {
						},
						["subeventPrefix"] = "SPELL",
						["realSpellName"] = "Sigil of Silence",
						["use_spellName"] = true,
						["inverse"] = true,
						["use_specific_sourceunit"] = true,
						["duration"] = "60",
						["use_sourceName"] = false,
						["sourceUnit"] = "Benzikura",
						["use_sourceFlags2"] = true,
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
			},
			["backdropInFront"] = false,
			["stickyDuration"] = false,
			["version"] = 1,
			["subRegions"] = {
				{
					["type"] = "subborder",
					["border_anchor"] = "bar",
					["border_size"] = 16,
					["border_color"] = {
						0.40392156862745, -- [1]
						0.89411764705882, -- [2]
						1, -- [3]
						0.5, -- [4]
					},
					["border_visible"] = true,
					["border_edge"] = "1 Pixel",
					["border_offset"] = 2,
				}, -- [1]
				{
					["type"] = "aurabar_bar",
				}, -- [2]
				{
					["text_shadowXOffset"] = 1,
					["text_text"] = "%p",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Vixar",
					["text_text_format_p_time_format"] = 0,
					["text_shadowYOffset"] = -1,
					["text_text_format_p_time_dynamic_threshold"] = 3,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "ICON_CENTER",
					["text_text_format_p_format"] = "timed",
					["text_visible"] = true,
					["text_fontSize"] = 20,
					["anchorXOffset"] = 0,
					["text_text_format_p_time_precision"] = 1,
				}, -- [3]
			},
			["height"] = 30,
			["sparkBlendMode"] = "ADD",
			["useAdjustededMax"] = false,
			["displayStacks"] = "%c",
			["uid"] = "dvnxH)2uLI0",
			["config"] = {
			},
			["actions"] = {
				["start"] = {
					["do_message"] = false,
					["message_type"] = "SAY",
				},
				["init"] = {
					["do_custom"] = true,
				},
				["finish"] = {
				},
			},
			["borderInFront"] = false,
			["sparkOffsetX"] = 0,
			["icon_side"] = "LEFT",
			["authorOptions"] = {
			},
			["zoom"] = 0.28,
			["sparkHeight"] = 30,
			["backgroundColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				0.45673036575317, -- [4]
			},
			["stacksContainment"] = "INSIDE",
			["auto"] = true,
			["semver"] = "1.0.0",
			["width"] = 30,
			["sparkHidden"] = "NEVER",
			["parent"] = "Enemy Interrupt Icons",
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["id"] = "DH Sigil of Silence 2",
			["backdropColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0.5, -- [4]
			},
			["inverse"] = false,
			["sparkDesature"] = false,
			["orientation"] = "HORIZONTAL",
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["customTextUpdate"] = "update",
		},
		["02 Sinseeker - White"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 3,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_frequency"] = 0.25,
					["custom"] = "",
					["glow_border"] = true,
					["glow_lines"] = 12,
					["glow_type"] = "Pixel",
					["use_glow_color"] = true,
					["do_custom"] = false,
					["do_glow"] = true,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = true,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2418",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "5sIOdD3(9EN",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["useMatch_count"] = true,
						["auranames"] = {
						},
						["groupclone"] = true,
						["matchesShowOn"] = "showOnActive",
						["name_info"] = "aura",
						["unit"] = "group",
						["use_tooltip"] = false,
						["subeventPrefix"] = "SPELL",
						["match_countOperator"] = ">=",
						["match_count"] = "1",
						["buffShowOn"] = "showOnActive",
						["unevent"] = "timed",
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["useExactSpellId"] = true,
						["custom_type"] = "stateupdate",
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["event"] = "Health",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HARMFUL",
						["use_specific_unit"] = false,
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["auraspellids"] = {
							"335304", -- [1]
						},
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["duration"] = "1",
						["useGroup_count"] = false,
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["wordWrap"] = "WordWrap",
			["config"] = {
			},
			["text1"] = "%n",
			["text1Containment"] = "INSIDE",
			["width"] = 6.9999866485596,
			["justify"] = "LEFT",
			["semver"] = "1.0.26",
			["zoom"] = 0,
			["id"] = "02 Sinseeker - White",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["frameStrata"] = 9,
			["anchorFrameType"] = "SCREEN",
			["text1Point"] = "CENTER",
			["text2Font"] = "Friz Quadrata TT",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["05 Volatile Ejection targeted - White"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 4,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						1, -- [1]
						0.9921568627451, -- [2]
						0.9921568627451, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.25,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "C_ChatInfo.RegisterAddonMessagePrefix(\"RELOE_AURA_SYNCH\")\n\n\n\n",
					["do_custom"] = true,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = true,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2383",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "DrVkZteRvVN",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["useGroup_count"] = false,
						["groupclone"] = true,
						["duration"] = "1",
						["name_info"] = "aura",
						["subeventPrefix"] = "SPELL",
						["use_tooltip"] = false,
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["buffShowOn"] = "showOnActive",
						["auraspellids"] = {
							"306163", -- [1]
						},
						["debuffType"] = "HARMFUL",
						["custom"] = "function(states, e, ...)\n    \n    if e == \"CHAT_MSG_ADDON\" then\n        local prefix, msg, _, sender = ...\n        if prefix == \"RELOE_AURA_SYNCH\" then\n            sender = sender or UnitName(\"player\")\n            sender = gsub(sender, \"%-[^|]+\", \"\")\n            if not UnitExists(sender) or not UnitIsVisible(sender) then return end\n            \n            local spell, G, u = strsplit(\" \", msg)\n            if tonumber(spell) == 334064 then\n                states[G] = {\n                    show = true,\n                    unit = u,\n                    duration = 4.5,\n                    expirationTime = GetTime()+4.5,\n                    autoHide = true,\n                    changed = true,\n                }\n                return true\n            end\n        end\n    end\nend\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "custom",
						["use_debuffClass"] = false,
						["subeventSuffix"] = "_CAST_START",
						["events"] = "CHAT_MSG_ADDON",
						["matchesShowOn"] = "showOnActive",
						["event"] = "Combat Log",
						["unit"] = "group",
						["auranames"] = {
						},
						["unevent"] = "timed",
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["custom_type"] = "stateupdate",
						["check"] = "event",
						["combineMatches"] = "showLowest",
						["useExactSpellId"] = true,
						["use_specific_unit"] = false,
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["wordWrap"] = "WordWrap",
			["config"] = {
			},
			["text1"] = "%n",
			["text1Containment"] = "INSIDE",
			["width"] = 6.9999866485596,
			["justify"] = "LEFT",
			["semver"] = "1.0.26",
			["zoom"] = 0,
			["id"] = "05 Volatile Ejection targeted - White",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["frameStrata"] = 9,
			["anchorFrameType"] = "SCREEN",
			["text1Point"] = "CENTER",
			["text2Font"] = "Friz Quadrata TT",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["Warlock 2"] = {
			["sparkWidth"] = 10,
			["iconSource"] = -1,
			["authorOptions"] = {
			},
			["preferToUpdate"] = false,
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["sparkRotation"] = 0,
			["url"] = "https://wago.io/ZG34ifiRf/1",
			["backgroundColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				0.4738245010376, -- [4]
			},
			["fontFlags"] = "OUTLINE",
			["icon_color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["selfPoint"] = "CENTER",
			["barColor"] = {
				0.48235294117647, -- [1]
				0.33333333333333, -- [2]
				0.67058823529412, -- [3]
				1, -- [4]
			},
			["desaturate"] = false,
			["sparkOffsetY"] = 0,
			["load"] = {
				["ingroup"] = {
					["multi"] = {
					},
				},
				["use_never"] = false,
				["class"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["level"] = "100",
				["size"] = {
					["single"] = "party",
					["multi"] = {
						["ratedpvp"] = true,
						["arena"] = true,
						["ratedarena"] = true,
						["pvp"] = true,
					},
				},
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_level"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["difficulty"] = {
					["single"] = "challenge",
					["multi"] = {
						["challenge"] = true,
					},
				},
				["zoneIds"] = "",
				["faction"] = {
					["multi"] = {
					},
				},
				["use_size"] = false,
				["use_zone"] = false,
				["race"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
			},
			["useAdjustededMin"] = false,
			["regionType"] = "aurabar",
			["texture"] = "Glamour4",
			["zoom"] = 0.28,
			["auto"] = true,
			["tocversion"] = 80300,
			["alpha"] = 1,
			["sparkColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["displayIcon"] = "Interface\\Icons\\spell_holy_rebuke",
			["stacksPoint"] = "BOTTOMRIGHT",
			["borderBackdrop"] = "Blizzard Tooltip",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customTextUpdate"] = "update",
			["triggers"] = {
				{
					["trigger"] = {
						["spellId"] = "Flask of the warm sun",
						["behavior"] = "assist",
						["duration"] = "24",
						["spellName"] = "Spell Lock",
						["subeventSuffix"] = "_CAST_SUCCESS",
						["use_showOn"] = true,
						["event"] = "Combat Log",
						["sourceFlags2"] = "Hostile",
						["use_name"] = true,
						["name"] = "Flask of the warm sun ",
						["use_sourceUnit"] = false,
						["use_destunit"] = false,
						["genericShowOn"] = "showOnCooldown",
						["unit"] = "player",
						["names"] = {
							"Greater Draenic Intellect Flask", -- [1]
						},
						["use_unit"] = true,
						["use_cloneId"] = true,
						["debuffType"] = "HELPFUL",
						["custom_hide"] = "timed",
						["use_sourceName"] = false,
						["use_genericShowOn"] = true,
						["unevent"] = "timed",
						["subeventPrefix"] = "SPELL",
						["spellIds"] = {
						},
						["type"] = "combatlog",
						["sourceName"] = "",
						["realSpellName"] = "Shadow Lock",
						["use_spellName"] = true,
						["inverse"] = true,
						["use_specific_sourceunit"] = true,
						["ownOnly"] = true,
						["use_sourceFlags2"] = true,
						["sourceUnit"] = "pet",
						["use_behavior"] = true,
					},
					["untrigger"] = {
						["behavior"] = "assist",
						["spellName"] = 171138,
					},
				}, -- [1]
				{
					["trigger"] = {
						["type"] = "combatlog",
						["subeventSuffix"] = "_CAST_SUCCESS",
						["duration"] = "24",
						["genericShowOn"] = "showOnActive",
						["subeventPrefix"] = "SPELL",
						["use_spellName"] = true,
						["spellName"] = "Spell Lock",
						["unevent"] = "timed",
						["event"] = "Combat Log",
						["use_cloneId"] = true,
						["custom_hide"] = "timed",
					},
					["untrigger"] = {
					},
				}, -- [2]
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
			},
			["backdropInFront"] = false,
			["stickyDuration"] = false,
			["version"] = 1,
			["subRegions"] = {
				{
					["type"] = "subborder",
					["border_anchor"] = "bar",
					["border_size"] = 16,
					["border_color"] = {
						0.49019607843137, -- [1]
						0.30980392156863, -- [2]
						0.67843137254902, -- [3]
						0.5, -- [4]
					},
					["border_visible"] = true,
					["border_edge"] = "1 Pixel",
					["border_offset"] = 2,
				}, -- [1]
				{
					["type"] = "aurabar_bar",
				}, -- [2]
				{
					["text_shadowXOffset"] = 1,
					["text_text"] = "%p",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Vixar",
					["text_text_format_p_time_format"] = 0,
					["text_shadowYOffset"] = -1,
					["text_text_format_p_time_dynamic_threshold"] = 3,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "ICON_CENTER",
					["text_text_format_p_format"] = "timed",
					["text_visible"] = true,
					["text_fontSize"] = 20,
					["anchorXOffset"] = 0,
					["text_text_format_p_time_precision"] = 1,
				}, -- [3]
			},
			["height"] = 30,
			["sparkBlendMode"] = "ADD",
			["useAdjustededMax"] = false,
			["displayStacks"] = "%c",
			["config"] = {
			},
			["sparkTexture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
			["actions"] = {
				["start"] = {
					["do_message"] = false,
					["message_type"] = "SAY",
				},
				["init"] = {
				},
				["finish"] = {
				},
			},
			["borderInFront"] = false,
			["sparkOffsetX"] = 0,
			["icon_side"] = "LEFT",
			["xOffset"] = 0,
			["spark"] = false,
			["sparkHeight"] = 30,
			["icon"] = true,
			["stacksContainment"] = "INSIDE",
			["uid"] = "0jXQIUDnrXo",
			["semver"] = "1.0.0",
			["width"] = 30,
			["sparkHidden"] = "NEVER",
			["parent"] = "Enemy Interrupt Icons",
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["id"] = "Warlock 2",
			["backdropColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0.5, -- [4]
			},
			["inverse"] = false,
			["sparkDesature"] = false,
			["orientation"] = "HORIZONTAL",
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["sparkRotationMode"] = "AUTO",
		},
		["01 Blood Lantern - Green"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 2,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						0, -- [1]
						1, -- [2]
						0.03921568627451, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.2,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = true,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2398",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "b)ifpvyyiNB",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["useMatch_count"] = true,
						["auranames"] = {
						},
						["groupclone"] = true,
						["matchesShowOn"] = "showOnActive",
						["name_info"] = "aura",
						["unit"] = "group",
						["use_tooltip"] = false,
						["subeventPrefix"] = "SPELL",
						["match_countOperator"] = ">=",
						["match_count"] = "1",
						["buffShowOn"] = "showOnActive",
						["unevent"] = "timed",
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["useExactSpellId"] = true,
						["custom_type"] = "stateupdate",
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["event"] = "Health",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HARMFUL",
						["use_specific_unit"] = false,
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["auraspellids"] = {
							"343303", -- [1]
						},
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["duration"] = "1",
						["useGroup_count"] = false,
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["wordWrap"] = "WordWrap",
			["config"] = {
			},
			["text1"] = "%n",
			["text1Containment"] = "INSIDE",
			["width"] = 6.9999866485596,
			["justify"] = "LEFT",
			["semver"] = "1.0.26",
			["zoom"] = 0,
			["id"] = "01 Blood Lantern - Green",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["frameStrata"] = 9,
			["anchorFrameType"] = "SCREEN",
			["text1Point"] = "CENTER",
			["text2Font"] = "Friz Quadrata TT",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["Priest 2"] = {
			["sparkWidth"] = 10,
			["iconSource"] = -1,
			["authorOptions"] = {
			},
			["preferToUpdate"] = false,
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["sparkRotation"] = 0,
			["url"] = "https://wago.io/ZG34ifiRf/1",
			["backgroundColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				0.43963617086411, -- [4]
			},
			["fontFlags"] = "OUTLINE",
			["icon_color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["selfPoint"] = "CENTER",
			["barColor"] = {
				0.96862745098039, -- [1]
				0.99607843137255, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["desaturate"] = false,
			["sparkOffsetY"] = 0,
			["load"] = {
				["ingroup"] = {
					["multi"] = {
					},
				},
				["use_never"] = false,
				["class"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["level"] = "100",
				["size"] = {
					["single"] = "party",
					["multi"] = {
						["ratedpvp"] = true,
						["arena"] = true,
						["ratedarena"] = true,
						["pvp"] = true,
					},
				},
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_level"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["difficulty"] = {
					["single"] = "challenge",
					["multi"] = {
						["challenge"] = true,
					},
				},
				["zoneIds"] = "",
				["faction"] = {
					["multi"] = {
					},
				},
				["use_size"] = false,
				["use_zone"] = false,
				["race"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
			},
			["useAdjustededMin"] = false,
			["regionType"] = "aurabar",
			["texture"] = "Glamour4",
			["sparkTexture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
			["auto"] = true,
			["tocversion"] = 80300,
			["alpha"] = 1,
			["sparkColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["displayIcon"] = "Interface\\Icons\\spell_holy_rebuke",
			["stacksPoint"] = "BOTTOMRIGHT",
			["sparkOffsetX"] = 0,
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["sparkRotationMode"] = "AUTO",
			["triggers"] = {
				{
					["trigger"] = {
						["ownOnly"] = true,
						["names"] = {
							"Greater Draenic Intellect Flask", -- [1]
						},
						["spellId"] = "",
						["name"] = "Flask of the warm sun ",
						["use_destName"] = false,
						["duration"] = "45",
						["genericShowOn"] = "showOnActive",
						["subeventPrefix"] = "SPELL",
						["use_specific_sourceunit"] = true,
						["inverse"] = true,
						["use_spellName"] = true,
						["use_sourceName"] = false,
						["sourceFlags2"] = "Hostile",
						["use_sourceFlags"] = false,
						["use_cloneId"] = true,
						["spellName"] = "Silence",
						["type"] = "combatlog",
						["sourceName"] = "Draemii",
						["unevent"] = "timed",
						["subeventSuffix"] = "_CAST_SUCCESS",
						["custom_hide"] = "timed",
						["debuffType"] = "HELPFUL",
						["event"] = "Combat Log",
						["use_sourceFlags2"] = true,
						["use_name"] = true,
						["use_spellId"] = false,
						["spellIds"] = {
						},
						["use_sourceUnit"] = false,
						["use_destunit"] = false,
						["use_destUnit"] = false,
						["sourceUnit"] = "Benzikura",
						["unit"] = "player",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
			},
			["backdropInFront"] = false,
			["stickyDuration"] = false,
			["version"] = 1,
			["subRegions"] = {
				{
					["type"] = "subborder",
					["border_anchor"] = "bar",
					["border_size"] = 16,
					["border_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						0.5, -- [4]
					},
					["border_visible"] = true,
					["border_edge"] = "1 Pixel",
					["border_offset"] = 2,
				}, -- [1]
				{
					["type"] = "aurabar_bar",
				}, -- [2]
				{
					["text_shadowXOffset"] = 1,
					["text_text"] = "%p",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Vixar",
					["text_text_format_p_time_format"] = 0,
					["text_shadowYOffset"] = -1,
					["text_text_format_p_time_dynamic_threshold"] = 3,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "ICON_CENTER",
					["text_text_format_p_format"] = "timed",
					["text_visible"] = true,
					["text_fontSize"] = 20,
					["anchorXOffset"] = 0,
					["text_text_format_p_time_precision"] = 1,
				}, -- [3]
			},
			["height"] = 30,
			["sparkBlendMode"] = "ADD",
			["backdropColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0.5, -- [4]
			},
			["displayStacks"] = "%c",
			["spark"] = false,
			["config"] = {
			},
			["uid"] = "ZYJfdWbaPhS",
			["borderInFront"] = false,
			["borderBackdrop"] = "Blizzard Tooltip",
			["icon_side"] = "LEFT",
			["xOffset"] = 0,
			["zoom"] = 0.28,
			["sparkHeight"] = 30,
			["actions"] = {
				["start"] = {
					["do_message"] = false,
					["message_type"] = "SAY",
				},
				["init"] = {
				},
				["finish"] = {
				},
			},
			["stacksContainment"] = "INSIDE",
			["parent"] = "Enemy Interrupt Icons",
			["semver"] = "1.0.0",
			["width"] = 30,
			["id"] = "Priest 2",
			["icon"] = true,
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["sparkHidden"] = "NEVER",
			["useAdjustededMax"] = false,
			["inverse"] = false,
			["sparkDesature"] = false,
			["orientation"] = "HORIZONTAL",
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["customTextUpdate"] = "update",
		},
		["Volatile Miasma"] = {
			["sparkWidth"] = 10,
			["iconSource"] = -1,
			["authorOptions"] = {
				{
					["type"] = "select",
					["values"] = {
						"Buff", -- [1]
						"Debuff", -- [2]
					},
					["default"] = 2,
					["key"] = "auratype",
					["useDesc"] = false,
					["name"] = "Aura type",
					["width"] = 0.5,
				}, -- [1]
				{
					["type"] = "number",
					["useDesc"] = false,
					["max"] = 9999999999,
					["step"] = 1,
					["width"] = 1.5,
					["min"] = 0,
					["key"] = "spellId",
					["name"] = "Spell ID",
					["default"] = 329298,
				}, -- [2]
				{
					["type"] = "select",
					["values"] = {
						"|TInterface\\TargetingFrame\\UI-RaidTargetingIcon_1:0|t", -- [1]
						"|TInterface\\TargetingFrame\\UI-RaidTargetingIcon_2:0|t", -- [2]
						"|TInterface\\TargetingFrame\\UI-RaidTargetingIcon_3:0|t", -- [3]
						"|TInterface\\TargetingFrame\\UI-RaidTargetingIcon_4:0|t", -- [4]
						"|TInterface\\TargetingFrame\\UI-RaidTargetingIcon_5:0|t", -- [5]
						"|TInterface\\TargetingFrame\\UI-RaidTargetingIcon_6:0|t", -- [6]
						"|TInterface\\TargetingFrame\\UI-RaidTargetingIcon_7:0|t", -- [7]
						"|TInterface\\TargetingFrame\\UI-RaidTargetingIcon_8:0|t", -- [8]
					},
					["default"] = 1,
					["key"] = "mark",
					["useDesc"] = false,
					["name"] = "Mark",
					["width"] = 0.5,
				}, -- [3]
				{
					["type"] = "toggle",
					["key"] = "ert",
					["desc"] = "When enabled, attempt to fetch mark assignment from active ERT note. This overwrites the manual mark choice.",
					["default"] = true,
					["useDesc"] = true,
					["name"] = "Fetch mark from ERT note",
					["width"] = 1.5,
				}, -- [4]
				{
					["type"] = "toggle",
					["key"] = "clip",
					["desc"] = "If selected, the absorb size is kept inside the bar (similar to how unit frames display them.|n|nIf not toggled, absorb size will grow outside the bar.",
					["default"] = true,
					["useDesc"] = true,
					["name"] = "Clip absorbs",
					["width"] = 1,
				}, -- [5]
			},
			["preferToUpdate"] = false,
			["customText"] = "function()\n    if WeakAuras.IsOptionsOpen() then\n        return ICON_LIST[GetRaidTargetIndex(\"player\") or 1] .. \"0|t\", WA_ClassColorName(UnitName(\"player\"))\n    end\n    \n    if aura_env and aura_env.state and aura_env.state.mark and aura_env.state.name then\n        return aura_env.state.mark, WA_ClassColorName(Ambiguate(aura_env.state.name, \"short\"))\n    end\nend",
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["sparkRotation"] = 0,
			["sparkRotationMode"] = "AUTO",
			["url"] = "https://wago.io/PescorusMiasma/9",
			["backgroundColor"] = {
				0.050980392156863, -- [1]
				0.13725490196078, -- [2]
				0.03921568627451, -- [3]
				1, -- [4]
			},
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "custom",
						["subeventSuffix"] = "_CAST_START",
						["customVariables"] = "{\n    additionalProgress = 1\n}",
						["event"] = "Health",
						["unit"] = "player",
						["subeventPrefix"] = "SPELL",
						["spellIds"] = {
						},
						["events"] = "ENCOUNTER_START, UNIT_HEALTH:group, UNIT_MAXHEALTH:group, CLEU:SPELL_AURA_APPLIED:SPELL_AURA_REMOVED, RAID_TARGET_UPDATE, UNIT_ABSORB_AMOUNT_CHANGED:group, UNIT_HEAL_ABSORB_AMOUNT_CHANGED:group",
						["names"] = {
						},
						["check"] = "event",
						["custom"] = "function(s, event, ...)\n    if event == \"ENCOUNTER_START\" then\n        aura_env.affected = {}\n        aura_env.auratype = (aura_env.config.auratype == 1) and \"HELPFUL\" or \"HARMFUL\"\n        \n        if aura_env.config.ert and IsAddOnLoaded(\"ExRT\")  and _G.VExRT.Note.Text1 then\n            local text = _G.VExRT.Note.Text1\n            \n            for line in text:gmatch('[^\\r\\n]+') do\n                local chunks = {}\n                for substring in line:gmatch(\"%S+\") do table.insert(chunks, substring) end\n                \n                if chunks[1] == \"Miasma\" and chunks[2] and line:lower():match(UnitName(\"player\"):lower()) then\n                    for k, v in pairs(ICON_TAG_LIST) do\n                        if chunks[2]:match(\"{\" .. k .. \"}\") then\n                            aura_env.config.mark = v\n                            break\n                        end\n                    end\n                end\n            end\n        end\n    elseif event == \"COMBAT_LOG_EVENT_UNFILTERED\" and ... then\n        local _, subEvent, _, _, _, _, _, _, destName, _, _, spellId = ...\n        if not (spellId == aura_env.config.spellId) then return false end\n        \n        if subEvent == \"SPELL_AURA_APPLIED\" then\n            aura_env.affected[destName] = true\n            \n            -- Check if unit already had the correct marker when the aura was applied\n            -- In this case RAID_TARGET_UPDATE won't fire so we manually fire it\n            local mark = GetRaidTargetIndex(destName)\n            if mark and (mark == aura_env.config.mark) then\n                C_Timer.After(0.1, function() WeakAuras.ScanEvents(\"RAID_TARGET_UPDATE\") end)\n            end\n        else -- \"SPELL_AURA_REMOVED\"\n            aura_env.affected[destName] = nil\n            \n            if s and s[\"\"] and s[\"\"].name and s[\"\"].name == destName then\n                s[\"\"].show = false\n                s[\"\"].changed = true\n                return true\n            end\n        end\n    elseif event == \"RAID_TARGET_UPDATE\" then\n        for name, _ in pairs(aura_env.affected) do\n            local mark = GetRaidTargetIndex(name)\n            \n            if mark and mark == aura_env.config.mark then\n                local _, _, _, _, _, expirationTime = WA_GetUnitAura(name, aura_env.config.spellId, aura_env.auratype)                \n                local duration = expirationTime - GetTime()\n                local currentHealth = UnitHealth(name)\n                local maxHealth = UnitHealthMax(name)\n                local currentAbsorb = UnitGetTotalAbsorbs(name)\n                \n                local total = maxHealth\n                if aura_env.config.clip then\n                    total = (currentHealth + currentAbsorb < maxHealth) and maxHealth or (currentHealth + currentAbsorb)\n                end\n                \n                s[\"\"] = {\n                    show = true,\n                    changed = true,\n                    progressType = \"static\",\n                    value = currentHealth + currentAbsorb,\n                    total = total,\n                    duration = duration,\n                    expirationTime = expirationTime,\n                    autoHide = true,\n                    name = name,\n                    mark = ICON_LIST[mark] .. \"0|t\",\n                    additionalProgress = {\n                        {\n                            min = currentHealth,\n                            max = currentHealth + currentAbsorb\n                        }\n                    }\n                }\n                \n                return true\n            end\n        end\n        \n        -- Check if mark was removed or changed\n        local mark = s[\"\"] and s[\"\"].name and GetRaidTargetIndex(s[\"\"].name)\n        if s and s[\"\"] and s[\"\"].show and s[\"\"].name and (not mark or not (mark == aura_env.config.mark)) then\n            s[\"\"].show = false\n            s[\"\"].changed = true\n            return true\n        end\n    elseif ... then -- UNIT_HEALTH/UNIT_MAXHEALTH/UNIT_ABSORB_AMOUNT_CHANGED/UNIT_HEAL_ABSORB_AMOUNT_CHANGED\n        local unit = ...\n        \n        if s and s[\"\"] and s[\"\"].name and UnitIsUnit(unit, s[\"\"].name) then\n            local currentHealth = UnitHealth(unit)\n            local maxHealth = UnitHealthMax(unit)\n            local currentAbsorb = UnitGetTotalAbsorbs(unit)\n            \n            local total = maxHealth\n            if aura_env.config.clip then\n                total = (currentHealth + currentAbsorb < maxHealth) and maxHealth or (currentHealth + currentAbsorb)\n            end\n            \n            s[\"\"].changed = true\n            s[\"\"].value = currentHealth + currentAbsorb\n            s[\"\"].total = total\n            s[\"\"].additionalProgress = {\n                {\n                    min = currentHealth,\n                    max = currentHealth + currentAbsorb\n                }\n            }\n            \n            return true\n        end\n    end\n    \n    return false\nend\n\n\n",
						["custom_type"] = "stateupdate",
						["debuffType"] = "HELPFUL",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["icon_color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["internalVersion"] = 45,
			["selfPoint"] = "CENTER",
			["barColor"] = {
				0.25490196078431, -- [1]
				0.67843137254902, -- [2]
				0.18823529411765, -- [3]
				1, -- [4]
			},
			["desaturate"] = false,
			["zoom"] = 0,
			["version"] = 9,
			["subRegions"] = {
				{
					["type"] = "aurabar_bar",
				}, -- [1]
				{
					["text_text_format_p_time_precision"] = 1,
					["text_text"] = "%c1 %c2",
					["text_text_format_p_format"] = "timed",
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["text_text_format_mark_format"] = "none",
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["text_text_format_p_time_format"] = 0,
					["type"] = "subtext",
					["text_text_format_p_time_dynamic_threshold"] = 60,
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "PT Sans Narrow",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_shadowYOffset"] = -1,
					["text_text_format_c2_format"] = "none",
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "None",
					["text_anchorPoint"] = "INNER_LEFT",
					["text_text_format_c1_format"] = "none",
					["text_visible"] = true,
					["text_fontSize"] = 16,
					["anchorXOffset"] = 0,
					["text_shadowXOffset"] = 1,
				}, -- [2]
				{
					["text_text_format_p_time_precision"] = 1,
					["text_text_format_p_big_number_format"] = "AbbreviateNumbers",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "PT Sans Narrow",
					["text_text_format_p_format"] = "BigNumber",
					["text_shadowYOffset"] = -1,
					["text_fontType"] = "None",
					["text_wordWrap"] = "WordWrap",
					["text_visible"] = true,
					["text_anchorPoint"] = "INNER_RIGHT",
					["text_text_format_n_format"] = "none",
					["text_text"] = "%p",
					["text_fontSize"] = 16,
					["anchorXOffset"] = 0,
					["text_shadowXOffset"] = 1,
				}, -- [3]
				{
					["type"] = "subborder",
					["border_anchor"] = "bar",
					["border_offset"] = 1,
					["border_color"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["border_visible"] = true,
					["border_edge"] = "Square Full White",
					["border_size"] = 1,
				}, -- [4]
				{
					["tick_blend_mode"] = "ADD",
					["tick_rotation"] = 0,
					["tick_xOffset"] = 0,
					["tick_desaturate"] = false,
					["automatic_length"] = true,
					["tick_thickness"] = 2,
					["tick_color"] = {
						1, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["use_texture"] = false,
					["tick_placement_mode"] = "AtValue",
					["tick_visible"] = true,
					["tick_placement"] = "14848",
					["tick_texture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
					["tick_yOffset"] = 0,
					["tick_length"] = 30,
					["tick_mirror"] = false,
					["type"] = "subtick",
				}, -- [5]
			},
			["height"] = 24,
			["parent"] = "Miasma soak group",
			["load"] = {
				["difficulty"] = {
				},
				["use_encounterid"] = true,
				["zoneIds"] = "",
				["talent"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2383",
				["class"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["sparkBlendMode"] = "ADD",
			["useAdjustededMax"] = false,
			["sparkOffsetY"] = 0,
			["icon"] = false,
			["actions"] = {
				["start"] = {
				},
				["init"] = {
					["custom"] = "\n\n\n",
					["do_custom"] = false,
				},
				["finish"] = {
				},
			},
			["xOffset"] = 0,
			["overlays"] = {
				{
					0.30980392156863, -- [1]
					0.91372549019608, -- [2]
					1, -- [3]
					1, -- [4]
				}, -- [1]
			},
			["uid"] = "GfVW25ZUCvR",
			["useAdjustededMin"] = false,
			["regionType"] = "aurabar",
			["sparkColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["anchorFrameType"] = "SCREEN",
			["icon_side"] = "RIGHT",
			["alpha"] = 1,
			["sparkHeight"] = 30,
			["texture"] = "Smooth v2",
			["id"] = "Volatile Miasma",
			["sparkTexture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
			["spark"] = false,
			["tocversion"] = 90002,
			["sparkHidden"] = "NEVER",
			["semver"] = "1.0.8",
			["frameStrata"] = 1,
			["width"] = 240,
			["overlayclip"] = false,
			["config"] = {
				["ert"] = true,
				["spellId"] = 329298,
				["auratype"] = 2,
				["clip"] = true,
				["mark"] = 1,
			},
			["inverse"] = false,
			["sparkOffsetX"] = 0,
			["orientation"] = "HORIZONTAL",
			["conditions"] = {
			},
			["information"] = {
			},
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
		},
		["Spiteful"] = {
			["arcLength"] = 360,
			["controlledChildren"] = {
				"Spiteful Countdown", -- [1]
			},
			["borderBackdrop"] = "Blizzard Tooltip",
			["authorOptions"] = {
			},
			["preferToUpdate"] = false,
			["yOffset"] = 234.99975585938,
			["gridType"] = "RD",
			["borderColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["space"] = 2,
			["url"] = "https://wago.io/yOeM0FVzB/2",
			["actions"] = {
				["start"] = {
				},
				["finish"] = {
				},
				["init"] = {
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["names"] = {
						},
						["type"] = "aura2",
						["spellIds"] = {
						},
						["subeventSuffix"] = "_CAST_START",
						["subeventPrefix"] = "SPELL",
						["debuffType"] = "HELPFUL",
						["event"] = "Health",
						["unit"] = "player",
					},
					["untrigger"] = {
					},
				}, -- [1]
			},
			["columnSpace"] = 1,
			["radius"] = 200,
			["useLimit"] = false,
			["align"] = "CENTER",
			["rotation"] = 0,
			["version"] = 2,
			["subRegions"] = {
			},
			["uid"] = "(TVy)skwDOZ",
			["load"] = {
				["talent"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["spec"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["grow"] = "DOWN",
			["backdropColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0.5, -- [4]
			},
			["stagger"] = 0,
			["animate"] = false,
			["selfPoint"] = "TOP",
			["scale"] = 1,
			["fullCircle"] = true,
			["border"] = false,
			["borderEdge"] = "Square Full White",
			["regionType"] = "dynamicgroup",
			["borderSize"] = 2,
			["sort"] = "none",
			["xOffset"] = 191.66625976563,
			["anchorPoint"] = "CENTER",
			["constantFactor"] = "RADIUS",
			["borderInset"] = 1,
			["borderOffset"] = 4,
			["semver"] = "1.0.1",
			["tocversion"] = 90002,
			["id"] = "Spiteful",
			["frameStrata"] = 1,
			["gridWidth"] = 5,
			["anchorFrameType"] = "SCREEN",
			["limit"] = 5,
			["config"] = {
			},
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["internalVersion"] = 45,
			["conditions"] = {
			},
			["information"] = {
			},
			["rowSpace"] = 1,
		},
		["Autofollow Follower"] = {
			["outline"] = "OUTLINE",
			["iconSource"] = -1,
			["authorOptions"] = {
				{
					["type"] = "input",
					["useDesc"] = true,
					["width"] = 1,
					["key"] = "leader",
					["default"] = "",
					["desc"] = "If configured, your characters will only receive commands from this character.",
					["multiline"] = false,
					["length"] = 10,
					["name"] = "Leader",
					["useLength"] = false,
				}, -- [1]
			},
			["displayText"] = "%c",
			["customText"] = "function()\n    if (aura_env.leader) then\n        FollowUnit(leader)\n    end\nend",
			["yOffset"] = 40,
			["anchorPoint"] = "TOP",
			["cooldownSwipe"] = true,
			["displayText_format_p_time_format"] = 0,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["icon"] = true,
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "custom",
						["subeventSuffix"] = "_CAST_START",
						["custom_hide"] = "timed",
						["customVariables"] = "\n\n",
						["event"] = "Health",
						["subeventPrefix"] = "SPELL",
						["unit"] = "nameplate",
						["custom"] = "--Honorax@Illidan\n\nfunction(event, ...)\n    if not (UnitIsGroupLeader(\"player\")) then\n        if (event == \"CHAT_MSG_ADDON\") then\n            local prefix, text, _, sender = ...\n            if (prefix == \"AuF\") then\n                local msg = {}\n                for entry in text:gmatch(\"[^,]+\") do\n                    table.insert(msg, entry)\n                end\n                \n                local action = msg[1]\n                local payload = msg[2]\n                if (action == \"F\") then --Follow\n                    aura_env.leader = payload\n                    FollowUnit(payload)\n                elseif (action == \"M\") then -- Mount\n                    if (not IsMounted()) then\n                        C_MountJournal.SummonByID(payload)\n                    end\n                elseif(action == \"Q\") then -- Quest ping\n                    C_ChatInfo.SendAddonMessage(\"AuF\", \"QA\")\n                end\n            end\n        elseif (event == \"QUEST_DETAIL\") then\n            AcceptQuest()\n        end\n    elseif (event == \"PLAYER_REGEN_ENABLED\") then\n        if (aura_env.leader) then\n            FollowUnit(aura_env.leader)\n        end\n    end\nend\n\n\n\n",
						["spellIds"] = {
						},
						["events"] = "CHAT_MSG_ADDON, PLAYER_REGEN_ENABLED, QUEST_DETAIL",
						["check"] = "event",
						["names"] = {
						},
						["custom_type"] = "event",
						["debuffType"] = "HELPFUL",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["displayText_format_p_format"] = "timed",
			["internalVersion"] = 45,
			["keepAspectRatio"] = false,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["wordWrap"] = "WordWrap",
			["desaturate"] = false,
			["font"] = "Oswald",
			["version"] = 11,
			["subRegions"] = {
			},
			["height"] = 1,
			["displayText_format_p_time_dynamic_threshold"] = 60,
			["load"] = {
				["ingroup"] = {
					["single"] = "group",
				},
				["use_never"] = false,
				["use_ignoreNameRealm"] = true,
				["class"] = {
					["multi"] = {
					},
				},
				["use_ingroup"] = true,
				["ignoreNameRealm"] = "Honorax",
				["zoneIds"] = "",
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["cooldown"] = false,
			["conditions"] = {
			},
			["fontSize"] = 36,
			["xOffset"] = 0,
			["displayText_format_n_format"] = "none",
			["shadowXOffset"] = 1,
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["preferToUpdate"] = true,
			["uid"] = "EAGXePwu6IA",
			["regionType"] = "text",
			["parent"] = "Autofollow",
			["anchorFrameType"] = "SCREEN",
			["alpha"] = 1,
			["shadowYOffset"] = -1,
			["displayText_format_p_time_precision"] = 1,
			["cooldownTextDisabled"] = false,
			["justify"] = "LEFT",
			["zoom"] = 0,
			["semver"] = "1.0.10",
			["tocversion"] = 90002,
			["id"] = "Autofollow Follower",
			["selfPoint"] = "TOP",
			["frameStrata"] = 9,
			["width"] = 1,
			["cooldownEdge"] = false,
			["config"] = {
				["leader"] = "",
			},
			["inverse"] = false,
			["actions"] = {
				["start"] = {
					["do_custom"] = false,
				},
				["init"] = {
					["custom"] = "C_ChatInfo.RegisterAddonMessagePrefix(\"AuF\")\naura_env.leader = nil\n\nif not aura_env.clickableFrame then\n    local r = WeakAuras.regions['Autofollow Follower'].region\n    aura_env.clickableFrame = CreateFrame(\"Button\",\"TargetingButton\", r, \"SecureActionButtonTemplate\")  \nend\n\naura_env.clickableFrame:SetAllPoints()",
					["do_custom"] = true,
				},
				["finish"] = {
				},
			},
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["fixedWidth"] = 200,
			["information"] = {
			},
			["url"] = "https://wago.io/UW5MyMEHT/11",
		},
		["Council of Blood - Dreadbolt Volley Cast 3"] = {
			["sparkWidth"] = 10,
			["sparkOffsetX"] = 0,
			["authorOptions"] = {
			},
			["preferToUpdate"] = false,
			["yOffset"] = 238.33312988281,
			["anchorPoint"] = "CENTER",
			["sparkRotation"] = 0,
			["sparkRotationMode"] = "AUTO",
			["url"] = "https://wago.io/VqAXD3338/1",
			["actions"] = {
				["start"] = {
					["do_sound"] = false,
				},
				["finish"] = {
				},
				["init"] = {
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["spellId"] = "328789",
						["duration"] = "3",
						["use_spell"] = true,
						["subeventPrefix"] = "SPELL",
						["use_unit"] = true,
						["names"] = {
						},
						["use_specific_unit"] = true,
						["spellName"] = "Dreadbolt Volley",
						["auraspellids"] = {
							"329834", -- [1]
						},
						["spell"] = "Dreadbolt Volley",
						["useExactSpellId"] = true,
						["unevent"] = "auto",
						["subeventSuffix"] = "_CAST_START",
						["use_spellName"] = true,
						["event"] = "Cast",
						["type"] = "unit",
						["realSpellName"] = "Dreadbolt Volley",
						["use_spellId"] = false,
						["spellIds"] = {
						},
						["use_genericShowOn"] = true,
						["genericShowOn"] = "showOnCooldown",
						["debuffType"] = "HELPFUL",
						["use_track"] = true,
						["unit"] = "boss3",
					},
					["untrigger"] = {
						["use_specific_unit"] = true,
						["unit"] = "boss3",
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["icon_color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["internalVersion"] = 45,
			["selfPoint"] = "CENTER",
			["barColor"] = {
				0.5529411764705883, -- [1]
				0.1058823529411765, -- [2]
				0.03137254901960784, -- [3]
				1, -- [4]
			},
			["desaturate"] = false,
			["version"] = 1,
			["subRegions"] = {
				{
					["type"] = "aurabar_bar",
				}, -- [1]
				{
					["text_text_format_n_format"] = "none",
					["text_text"] = "Dreadbolt Volley",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Accidental Presidency",
					["text_shadowYOffset"] = -1,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "None",
					["text_anchorPoint"] = "INNER_RIGHT",
					["text_shadowXOffset"] = 1,
					["text_fontSize"] = 25,
					["anchorXOffset"] = 0,
					["text_visible"] = true,
				}, -- [2]
				{
					["text_text_format_p_time_precision"] = 1,
					["text_text_format_s_format"] = "none",
					["text_text"] = "%p",
					["text_text_format_p_format"] = "timed",
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Accidental Presidency",
					["text_text_format_p_time_format"] = 0,
					["text_shadowYOffset"] = -1,
					["text_text_format_p_time_dynamic_threshold"] = 60,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "None",
					["text_anchorPoint"] = "INNER_LEFT",
					["text_visible"] = true,
					["text_shadowXOffset"] = 1,
					["text_fontSize"] = 25,
					["anchorXOffset"] = 0,
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
				}, -- [3]
				{
					["type"] = "subborder",
					["border_anchor"] = "bar",
					["border_size"] = 2,
					["border_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["border_visible"] = true,
					["border_edge"] = "Square Full White",
					["border_offset"] = 0,
				}, -- [4]
			},
			["height"] = 43,
			["anchorFrameType"] = "SCREEN",
			["load"] = {
				["use_encounterid"] = true,
				["zoneIds"] = "",
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2412",
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["sparkBlendMode"] = "ADD",
			["useAdjustededMax"] = false,
			["icon"] = false,
			["sparkOffsetY"] = 0,
			["backgroundColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				0.5, -- [4]
			},
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["iconSource"] = -1,
			["useAdjustededMin"] = false,
			["regionType"] = "aurabar",
			["config"] = {
			},
			["uid"] = "AS0bdTGVJiq",
			["icon_side"] = "RIGHT",
			["sparkHidden"] = "NEVER",
			["sparkHeight"] = 30,
			["texture"] = "Solid",
			["alpha"] = 1,
			["zoom"] = 0,
			["semver"] = "1.0.0",
			["tocversion"] = 90002,
			["id"] = "Council of Blood - Dreadbolt Volley Cast 3",
			["spark"] = false,
			["frameStrata"] = 1,
			["width"] = 360,
			["sparkTexture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
			["sparkColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["inverse"] = false,
			["xOffset"] = -457.50018310547,
			["orientation"] = "HORIZONTAL",
			["conditions"] = {
			},
			["information"] = {
			},
			["parent"] = "Council of Blood - Dreadbolt Volley - Cast Bar",
		},
		["10 Searing Censure - Teal"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 2,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						0, -- [1]
						0.89411764705882, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.25,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = true,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2407",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "bW28OeyFk5j",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["useMatch_count"] = true,
						["auranames"] = {
						},
						["groupclone"] = true,
						["matchesShowOn"] = "showOnActive",
						["name_info"] = "aura",
						["unit"] = "group",
						["use_tooltip"] = false,
						["subeventPrefix"] = "SPELL",
						["match_countOperator"] = ">=",
						["match_count"] = "1",
						["buffShowOn"] = "showOnActive",
						["unevent"] = "timed",
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["useExactSpellId"] = true,
						["custom_type"] = "stateupdate",
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["event"] = "Health",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HARMFUL",
						["use_specific_unit"] = false,
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["auraspellids"] = {
							"341732", -- [1]
							"341426", -- [2]
						},
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["duration"] = "1",
						["useGroup_count"] = false,
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["text2Font"] = "Friz Quadrata TT",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text1"] = "%n",
			["text1Point"] = "CENTER",
			["anchorFrameType"] = "SCREEN",
			["semver"] = "1.0.26",
			["justify"] = "LEFT",
			["zoom"] = 0,
			["id"] = "10 Searing Censure - Teal",
			["config"] = {
			},
			["frameStrata"] = 9,
			["width"] = 6.9999866485596,
			["text1Containment"] = "INSIDE",
			["wordWrap"] = "WordWrap",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["Generals Soak Assign/Backup"] = {
			["outline"] = "OUTLINE",
			["xOffset"] = 0,
			["displayText"] = "%c",
			["customText"] = "function()\n    if aura_env.state and aura_env.state.name and aura_env.config[\"list\"] then\n        if UnitExists(aura_env.state.name) then\n            return aura_env.state.mark..\" \"..WA_ClassColorName(aura_env.state.name)\n        else\n            return aura_env.state.mark..\" \"..aura_env.state.name\n        end\n    end\nend",
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["displayText_format_p_time_format"] = 0,
			["customTextUpdate"] = "event",
			["url"] = "https://wago.io/GtuQCwZ2-/8",
			["actions"] = {
				["start"] = {
				},
				["finish"] = {
				},
				["init"] = {
					["custom"] = "aura_env.icons= { \n    [1]  = \n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_1:12\\124t\",\n    [2] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_2:12\\124t\", \n    [3] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_3:12\\124t\", \n    [4] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_4:12\\124t\", \n    [5] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_5:12\\124t\", \n    [6] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_6:12\\124t\",   \n    [7] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_7:12\\124t\",\n    [8] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_8:12\\124t\",\n    [9] =\n    \" Right of \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_1:12\\124t\",\n    [10] =\n    \" 10\",\n    [11] =\n    \" 11\",\n    [12] =\n    \" 12\",\n    [13] =\n    \" 13\",\n    [14] =\n    \" 14\",\n    [15] =\n    \" 15\",\n}\n\naura_env.list = aura_env.config[\"list\"]\naura_env.max = 9\naura_env.cast = 344496\naura_env.unit = \"boss2target\"\naura_env.players = {}\naura_env.backup = {}\naura_env.num = 0\naura_env.alert = 1\naura_env.count = 1\naura_env.countb = 1\naura_env.assign = false\n\nif aura_env.config[\"debug\"] then\n    aura_env.cast = 8936 -- debug\n    aura_env.unit = \"target\"\nend\n\naura_env.colorcode = function(name)\n    local length = string.len(name)\n    if string.sub(name, 2, 3) == \"|c\" then -- checking if name starts with colorcode\n        if string.sub(name, length-1) == \"|r\" then -- checking if end of name ends the colorcode or not (if same color is used twice in a row it might not end the color until 2nd name)\n            return string.sub(name, 12, length-3)\n        else\n            return string.sub(name, 12)\n        end\n    elseif string.sub(name, length-1) == \"|r\" then -- checking if only end of colorcode is present\n        return string.sub(name, 1, length-3)\n    else\n        return name\n    end\nend\n\n\naura_env.ERT = function()\n    local text = _G.VExRT.Note.Text1\n    aura_env.assign = false\n    for line in string.gmatch(text,'[^\\r\\n]+') do\n        local start = strsplit(\" \", line)\n        if string.lower(start) == \"end\" then\n            aura_env.assign = false\n            aura_env.assign2 = false\n        elseif string.lower(start) == \"backup\" then\n            aura_env.assign = false\n            aura_env.assign2 = true\n        elseif aura_env.assign2 then\n            aura_env.backup[aura_env.countb] = aura_env.colorcode(start)\n            aura_env.countb = aura_env.countb+1\n        elseif aura_env.assign then\n            aura_env.players[aura_env.count] = aura_env.colorcode(start)\n            aura_env.count = aura_env.count+1\n            \n            \n        elseif string.lower(start) == \"start\" then\n            aura_env.players = {}\n            aura_env.backup = {}\n            aura_env.num = 0\n            aura_env.alert = 1\n            aura_env.count = 1\n            aura_env.countb = 1\n            aura_env.assign = true\n            aura_env.assign2 = false\n        end\n    end\nend",
					["do_custom"] = true,
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "custom",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HELPFUL",
						["custom_type"] = "stateupdate",
						["event"] = "Health",
						["subeventPrefix"] = "SPELL",
						["names"] = {
						},
						["spellIds"] = {
						},
						["custom"] = "function(states, event, ...)\n    if event == \"OPTIONS\" then\n        states[\"\"] = {\n            show = true,\n            changed = true,\n            progressType = \"static\",\n            name = UnitName(\"player\"),\n            mark = aura_env.icons[1], \n            index = 1,\n        }\n    elseif event == \"ENCOUNTER_START\" then\n        aura_env.ERT()\n        aura_env.num = 0\n        aura_env.alert = 1\n        aura_env.looped = false\n        aura_env.assigned = {}\n    elseif event == \"RELOE_STATE_UPDATE\" then\n        if ... then\n            if aura_env.list then\n                for i=1, aura_env.num do\n                    if aura_env.players[i] then\n                        states[i] = {\n                            show = true,\n                            changed = true,\n                            progressType = \"static\",\n                            name = aura_env.players[i],\n                            mark = aura_env.icons[i], \n                            index = i,\n                        }\n                    end\n                end\n                return true\n            end\n        else\n            if UnitExists(aura_env.unit) then\n                WeakAuras.ScanEvents(\"RELOE_GLOW_ALERT\", aura_env.unit)\n                if UnitIsUnit(aura_env.unit, \"player\") then\n                    WeakAuras.ScanEvents(\"RELOE_CAST_ALERT\", aura_env.alert)\n                end\n            end\n            for i=1, aura_env.num do\n                if aura_env.num == aura_env.max and aura_env.looped and i == aura_env.alert then -- preventing soak alert for a person that currently has no zone on the ground\n                else\n                    if (not aura_env.players[i]) or aura_env.players[i] == UnitName(aura_env.unit) or UnitIsDead(aura_env.players[i]) then -- needs backup\n                        for j=1, #aura_env.backup+1 do\n                            local unit = aura_env.backup[j]\n                            if unit then -- checking if a backup still exists\n                                if (not (UnitIsDead(unit))) and (not (UnitName(aura_env.unit) == unit)) and (not (aura_env.assigned[unit])) then -- check if backup is viable\n                                    WeakAuras.ScanEvents(\"RELOE_GLOW_ALERT\", unit)\n                                    if UnitName(\"player\") == UnitName(unit) then\n                                        WeakAuras.ScanEvents(\"RELOE_BACKUP_ALERT\", i)\n                                    end\n                                    aura_env.assigned[unit] = true\n                                    break\n                                end\n                            else\n                                WeakAuras.ScanEvents(\"RELOE_BACKUP_ALERT2\", i)\n                                break\n                            end\n                        end\n                    else -- all is fine, player can soak\n                        WeakAuras.ScanEvents(\"RELOE_GLOW_ALERT\", aura_env.players[i])\n                        if aura_env.players[i] == UnitName(\"player\") then\n                            WeakAuras.ScanEvents(\"RELOE_SOAK_ALERT\", i)\n                        end\n                    end\n                end\n            end\n            return true\n        end\n    else\n        local timestamp, subevent, hideCaster, sourceGUID, sourceName, sourceFlags, sourceRaidFlags, destGUID, destName, destFlags, destRaidFlags, spellID = ...\n        if subevent == \"SPELL_CAST_SUCCESS\" and spellID == aura_env.cast then\n            aura_env.num = (aura_env.num < aura_env.max and aura_env.num+1) or aura_env.num\n            aura_env.alert = (aura_env.alert < aura_env.max and aura_env.alert+1) or 1\n            aura_env.looped = (aura_env.alert == aura_env.max and true) or aura_env.looped -- checking if all 8 zones are dropped already\n            WeakAuras.ScanEvents(\"RELOE_STATE_UPDATE\", true)\n            \n        elseif subevent == \"SPELL_CAST_START\" and spellID == aura_env.cast then\n            for unit in WA_IterateGroupMembers() do\n                aura_env.assigned[UnitName(unit)] = false\n            end\n            C_Timer.After(0.2, function()\n                    WeakAuras.ScanEvents(\"RELOE_STATE_UPDATE\", false)\n            end)\n        end\n    end\nend\n\n\n",
						["events"] = "CLEU:SPELL_CAST_START:SPELL_CAST_SUCCESS ENCOUNTER_START RELOE_STATE_UPDATE",
						["check"] = "event",
						["unit"] = "player",
						["customVariables"] = "{\n    alpha = \"bool\"\n    }\n    \n    ",
						["custom_hide"] = "timed",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["displayText_format_p_format"] = "timed",
			["internalVersion"] = 45,
			["wordWrap"] = "WordWrap",
			["displayText_format_name_format"] = "none",
			["font"] = "Friz Quadrata TT",
			["version"] = 8,
			["subRegions"] = {
			},
			["load"] = {
				["use_encounterid"] = true,
				["zoneIds"] = "",
				["talent"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2417",
				["class"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["fontSize"] = 29,
			["shadowXOffset"] = 1,
			["displayText_format_mark_format"] = "none",
			["displayText_format_p_time_dynamic_threshold"] = 60,
			["regionType"] = "text",
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["parent"] = "09 Generals Soak Main",
			["conditions"] = {
			},
			["displayText_format_p_time_precision"] = 1,
			["authorOptions"] = {
				{
					["type"] = "toggle",
					["key"] = "list",
					["default"] = true,
					["name"] = "Display List?",
					["useDesc"] = false,
					["width"] = 1,
				}, -- [1]
				{
					["type"] = "toggle",
					["key"] = "debug",
					["default"] = false,
					["name"] = "Debug Mode",
					["useDesc"] = false,
					["width"] = 1,
				}, -- [2]
			},
			["automaticWidth"] = "Auto",
			["config"] = {
				["debug"] = false,
				["list"] = true,
			},
			["justify"] = "LEFT",
			["tocversion"] = 90002,
			["id"] = "Generals Soak Assign/Backup",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["selfPoint"] = "BOTTOM",
			["uid"] = "EHM8LwjaT5b",
			["semver"] = "1.0.7",
			["shadowYOffset"] = -1,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["fixedWidth"] = 200,
			["information"] = {
			},
			["preferToUpdate"] = false,
		},
		["06 Shared Cognition - White"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 3,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.25,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = true,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2406",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "AQfauLjD2bf",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["useMatch_count"] = true,
						["auranames"] = {
						},
						["groupclone"] = true,
						["matchesShowOn"] = "showOnActive",
						["name_info"] = "aura",
						["unit"] = "group",
						["use_tooltip"] = false,
						["subeventPrefix"] = "SPELL",
						["match_countOperator"] = ">=",
						["match_count"] = "1",
						["buffShowOn"] = "showOnActive",
						["unevent"] = "timed",
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["useExactSpellId"] = true,
						["custom_type"] = "stateupdate",
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["event"] = "Health",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HARMFUL",
						["use_specific_unit"] = false,
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["auraspellids"] = {
							"325908", -- [1]
							"325936", -- [2]
						},
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["duration"] = "1",
						["useGroup_count"] = false,
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["wordWrap"] = "WordWrap",
			["config"] = {
			},
			["text1"] = "%n",
			["text1Containment"] = "INSIDE",
			["width"] = 6.9999866485596,
			["justify"] = "LEFT",
			["semver"] = "1.0.26",
			["zoom"] = 0,
			["id"] = "06 Shared Cognition - White",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["frameStrata"] = 9,
			["anchorFrameType"] = "SCREEN",
			["text1Point"] = "CENTER",
			["text2Font"] = "Friz Quadrata TT",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["Golden Mission Table Helper"] = {
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["displayText"] = "%c",
			["customText"] = "function()\n    if aura_env.state and aura_env.state.percent then\n        if aura_env.state.select then\n            return \"SELECT TROOPS\"\n        end\n        \n        local percent = aura_env.state.percent\n        local result = percent > 0 and \"WIN\" or percent == 0 and \"DRAW\" or \"LOSE\"\n        local digit = percent > 0 and \"+\" or \"\"\n        return (\"%s: %s%s\"):format(result, digit, percent)\n    end\nend",
			["yOffset"] = 0,
			["anchorPoint"] = "TOP",
			["displayText_format_p_time_format"] = 0,
			["customTextUpdate"] = "event",
			["url"] = "https://wago.io/FTT0P82zx/12",
			["actions"] = {
				["start"] = {
				},
				["init"] = {
					["custom"] = "if not aura_env.region.hook then\n    aura_env.region.hook = true\n    local func = function() \n        C_Timer.After(0.1, function() WeakAuras.ScanEvents(\"GO_FARM_ANIME\", true) end)\n    end\n    \n    LoadAddOn(\"Blizzard_GarrisonUI\")\n    \n    CovenantMissionFrame.MissionTab.MissionPage.Board:HookScript('OnShow', func)\n    CovenantMissionFrame.MissionTab.MissionPage.Board:HookScript('OnHide', func)\nend",
					["do_custom"] = true,
				},
				["finish"] = {
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "custom",
						["subeventSuffix"] = "_CAST_START",
						["customVariables"] = "{\n    percent = \"number\",\n    select = \"bool\",\n}",
						["unit"] = "player",
						["event"] = "Health",
						["subeventPrefix"] = "SPELL",
						["custom"] = "function(s, e, ...)\n    if e == \"GARRISON_MISSION_NPC_OPENED\"\n    or e == \"GARRISON_SHIPYARD_NPC_CLOSED\"\n    or e == \"GARRISON_FOLLOWER_LIST_UPDATE\"\n    then\n        C_Timer.After(0.1, function() WeakAuras.ScanEvents(\"GO_FARM_ANIME\", true) end)\n    elseif e == \"GO_FARM_ANIME\" and ... then\n        if CovenantMissionFrame\n        and CovenantMissionFrame:IsVisible()\n        and CovenantMissionFrame.MissionTab\n        and CovenantMissionFrame.MissionTab.MissionPage\n        and CovenantMissionFrame.MissionTab.MissionPage.Stage\n        and CovenantMissionFrame.MissionTab.MissionPage.Stage:IsVisible()\n        and CovenantMissionFrame.MissionTab.MissionPage.Board\n        and CovenantMissionFrame.MissionTab.MissionPage.Board:IsVisible()\n        then\n            local stage = CovenantMissionFrame.MissionTab.MissionPage.Stage\n            local board = CovenantMissionFrame.MissionTab.MissionPage.Board\n            local percent = 0\n            \n            local eHP = stage.EnemyHealthValue and stage.EnemyHealthValue:GetText():gsub(',','') or 0\n            local eDMG = stage.EnemyPowerValue and stage.EnemyPowerValue:GetText():gsub(',','') or 0\n            \n            local aHP = board.AllyHealthValue and board.AllyHealthValue:GetText():gsub(',','') or 0\n            local aDMG = board.AllyPowerValue and board.AllyPowerValue:GetText():gsub(',','') or 0\n            \n            \n            if tonumber(aDMG) > 0 then\n                if ( (aHP/eDMG) - (eHP/aDMG) ) >= 0 then\n                    percent = math.ceil( (aHP/eDMG) - (eHP/aDMG) )\n                else\n                    percent = math.floor( (aHP/eDMG) - (eHP/aDMG) )\n                end\n            end\n            \n            s[\"\"] = {\n                percent = percent,\n                select = tonumber(aDMG) == 0,\n                progressType = \"static\",\n                changed = true,\n                show = true,\n            }\n            return true\n        elseif s[\"\"] then\n            s[\"\"] = {\n                changed = true,\n                show = false,\n            }\n            return true\n        end\n    end\nend",
						["customName"] = "\n\n",
						["spellIds"] = {
						},
						["names"] = {
						},
						["check"] = "event",
						["events"] = "GARRISON_MISSION_NPC_OPENED GARRISON_SHIPYARD_NPC_CLOSED GARRISON_FOLLOWER_LIST_UPDATE GO_FARM_ANIME",
						["custom_type"] = "stateupdate",
						["debuffType"] = "HELPFUL",
					},
					["untrigger"] = {
						["custom"] = "",
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["displayText_format_p_format"] = "timed",
			["internalVersion"] = 45,
			["wordWrap"] = "WordWrap",
			["font"] = "1Bignoodle",
			["version"] = 12,
			["subRegions"] = {
			},
			["load"] = {
				["use_zone"] = false,
				["use_zoneIds"] = true,
				["talent"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "1698,1707,1699,1701,1565,1533",
				["class"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["fontSize"] = 40,
			["shadowXOffset"] = 1,
			["anchorFrameFrame"] = "CovenantMissionFrame",
			["regionType"] = "text",
			["conditions"] = {
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "<",
						["variable"] = "percent",
						["value"] = "0",
					},
					["linked"] = false,
					["changes"] = {
						{
							["value"] = {
								1, -- [1]
								0, -- [2]
								0, -- [3]
								1, -- [4]
							},
							["property"] = "color",
						}, -- [1]
					},
				}, -- [1]
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["variable"] = "percent",
						["value"] = "0",
					},
					["linked"] = true,
					["changes"] = {
						{
							["value"] = {
								1, -- [1]
								1, -- [2]
								0, -- [3]
								1, -- [4]
							},
							["property"] = "color",
						}, -- [1]
					},
				}, -- [2]
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = ">",
						["variable"] = "percent",
						["value"] = "0",
					},
					["linked"] = true,
					["changes"] = {
						{
							["value"] = {
								0, -- [1]
								1, -- [2]
								0, -- [3]
								1, -- [4]
							},
							["property"] = "color",
						}, -- [1]
					},
				}, -- [3]
				{
					["check"] = {
						["trigger"] = 1,
						["variable"] = "select",
						["value"] = 1,
					},
					["changes"] = {
						{
							["value"] = {
								0.61960784313725, -- [1]
								0.61960784313725, -- [2]
								0.61960784313725, -- [3]
								1, -- [4]
							},
							["property"] = "color",
						}, -- [1]
					},
				}, -- [4]
			},
			["displayText_format_p_time_dynamic_threshold"] = 60,
			["selfPoint"] = "BOTTOM",
			["displayText_format_p_time_precision"] = 1,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["shadowYOffset"] = -1,
			["xOffset"] = 0,
			["semver"] = "1.0.11",
			["tocversion"] = 90002,
			["id"] = "Golden Mission Table Helper",
			["config"] = {
			},
			["frameStrata"] = 1,
			["anchorFrameType"] = "SELECTFRAME",
			["justify"] = "LEFT",
			["uid"] = "LTx0bgl)k0q",
			["authorOptions"] = {
			},
			["automaticWidth"] = "Auto",
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["fixedWidth"] = 200,
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = true,
		},
		["Soak Alert"] = {
			["sparkWidth"] = 10,
			["iconSource"] = -1,
			["xOffset"] = 0,
			["displayText"] = "%c",
			["yOffset"] = 200,
			["anchorPoint"] = "CENTER",
			["sparkRotation"] = 0,
			["url"] = "https://wago.io/kJFQFy0V8/3",
			["actions"] = {
				["start"] = {
					["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\AirHorn.ogg",
					["do_sound"] = false,
				},
				["finish"] = {
				},
				["init"] = {
					["custom"] = "aura_env.icons= { \n    [1]  = \n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_1:12\\124t\",\n    [2] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_2:12\\124t\", \n    [3] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_3:12\\124t\", \n    [4] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_4:12\\124t\", \n    [5] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_5:12\\124t\", \n    [6] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_6:12\\124t\",   \n    [7] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_7:12\\124t\",\n    [8] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_8:12\\124t\",\n    [9] =\n    \" Right of \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_1:12\\124t\",\n    [10] =\n    \" 10\",\n    [11] =\n    \" 11\",\n    [12] =\n    \" 12\",\n    [13] =\n    \" 13\",\n    [14] =\n    \" 14\",\n    [15] =\n    \" 15\",\n}",
					["do_custom"] = true,
				},
			},
			["icon_color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["selfPoint"] = "BOTTOM",
			["barColor"] = {
				1, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["sparkOffsetY"] = 0,
			["load"] = {
				["use_encounterid"] = true,
				["zoneIds"] = "",
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2417",
				["spec"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["shadowXOffset"] = 1,
			["useAdjustededMin"] = false,
			["regionType"] = "aurabar",
			["texture"] = "Details Flat",
			["sparkTexture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
			["spark"] = false,
			["tocversion"] = 90002,
			["alpha"] = 1,
			["sparkColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["fixedWidth"] = 200,
			["outline"] = "OUTLINE",
			["sparkOffsetX"] = 0,
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "function()\n    aura_env.mark = aura_env.mark or \"\"\n    if aura_env.state and aura_env.state.marker then\n        return \"SOAK:\"..aura_env.icons[aura_env.state.marker]\n    end\nend",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["debuffType"] = "HELPFUL",
						["type"] = "custom",
						["subeventSuffix"] = "_CAST_START",
						["custom_type"] = "stateupdate",
						["names"] = {
						},
						["duration"] = "5.2",
						["event"] = "Health",
						["subeventPrefix"] = "SPELL",
						["dynamicDuration"] = false,
						["custom"] = "function(states, event, mark)\n    if mark then \n        states [\"\"] = {\n            show = true,\n            progressType = \"timed\",\n            duration = 5.3,\n            expirationTime = GetTime()+5.3,\n            autoHide = true,\n            changed = true,\n            marker = mark\n        }\n        return true\n    end\nend",
						["events"] = "RELOE_SOAK_ALERT",
						["spellIds"] = {
						},
						["check"] = "event",
						["unit"] = "player",
						["customVariables"] = "{\n    marker = \"number\"\n    }\n    \n    ",
						["custom_hide"] = "timed",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["displayText_format_p_format"] = "timed",
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["version"] = 3,
			["subRegions"] = {
				{
					["type"] = "aurabar_bar",
				}, -- [1]
				{
					["text_text_format_n_format"] = "none",
					["text_text"] = "%c",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_anchorXOffset"] = 0,
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "ExpressWay",
					["text_shadowYOffset"] = -1,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "None",
					["text_anchorPoint"] = "INNER_TOP",
					["text_anchorYOffset"] = 40,
					["text_shadowXOffset"] = 1,
					["text_fontSize"] = 40,
					["anchorXOffset"] = 0,
					["text_visible"] = true,
				}, -- [2]
				{
					["text_text_format_p_time_precision"] = 1,
					["text_text"] = "%p",
					["text_text_format_p_format"] = "Number",
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["text_text_format_p_decimal_precision"] = 1,
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "ExpressWay",
					["text_shadowYOffset"] = -1,
					["text_shadowXOffset"] = 1,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "None",
					["text_anchorPoint"] = "INNER_RIGHT",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_text_format_n_format"] = "none",
					["text_fontSize"] = 20,
					["anchorXOffset"] = 0,
					["text_visible"] = true,
				}, -- [3]
				{
					["type"] = "subborder",
					["border_anchor"] = "bar",
					["border_size"] = 2,
					["border_color"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["border_visible"] = true,
					["border_edge"] = "Square Full White",
					["border_offset"] = 0,
				}, -- [4]
			},
			["height"] = 40,
			["sparkBlendMode"] = "ADD",
			["useAdjustededMax"] = false,
			["fontSize"] = 60,
			["preferToUpdate"] = false,
			["parent"] = "09 Generals Soak Alerts",
			["uid"] = "pfC7F1fP0E3",
			["orientation"] = "HORIZONTAL",
			["displayText_format_p_time_dynamic"] = false,
			["icon_side"] = "RIGHT",
			["config"] = {
			},
			["displayText_format_p_time_precision"] = 1,
			["sparkHeight"] = 30,
			["sparkRotationMode"] = "AUTO",
			["anchorFrameType"] = "SCREEN",
			["zoom"] = 0,
			["semver"] = "1.0.2",
			["justify"] = "LEFT",
			["id"] = "Soak Alert",
			["sparkHidden"] = "NEVER",
			["frameStrata"] = 1,
			["width"] = 350,
			["backgroundColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				0.5, -- [4]
			},
			["icon"] = false,
			["inverse"] = false,
			["authorOptions"] = {
			},
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["value"] = "1",
						["variable"] = "marker",
					},
					["changes"] = {
						{
							["value"] = {
								0.97254901960784, -- [1]
								1, -- [2]
								0, -- [3]
								1, -- [4]
							},
							["property"] = "barColor",
						}, -- [1]
						{
							["value"] = {
								["sound_type"] = "Play",
								["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\Star.ogg",
								["sound_channel"] = "Master",
							},
							["property"] = "sound",
						}, -- [2]
					},
				}, -- [1]
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["value"] = "2",
						["variable"] = "marker",
					},
					["changes"] = {
						{
							["value"] = {
								1, -- [1]
								0.53725490196078, -- [2]
								0, -- [3]
								1, -- [4]
							},
							["property"] = "barColor",
						}, -- [1]
						{
							["value"] = {
								["sound_type"] = "Play",
								["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\Circle.ogg",
								["sound_channel"] = "Master",
							},
							["property"] = "sound",
						}, -- [2]
					},
				}, -- [2]
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["value"] = "3",
						["variable"] = "marker",
					},
					["changes"] = {
						{
							["value"] = {
								0.56862745098039, -- [1]
								0, -- [2]
								1, -- [3]
								1, -- [4]
							},
							["property"] = "barColor",
						}, -- [1]
						{
							["value"] = {
								["sound_type"] = "Play",
								["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\Diamond.ogg",
								["sound_channel"] = "Master",
							},
							["property"] = "sound",
						}, -- [2]
					},
				}, -- [3]
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["value"] = "4",
						["variable"] = "marker",
					},
					["changes"] = {
						{
							["value"] = {
								0.12549019607843, -- [1]
								1, -- [2]
								0, -- [3]
								1, -- [4]
							},
							["property"] = "barColor",
						}, -- [1]
						{
							["value"] = {
								["sound_type"] = "Play",
								["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\Triangle.ogg",
								["sound_channel"] = "Master",
							},
							["property"] = "sound",
						}, -- [2]
					},
				}, -- [4]
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["value"] = "5",
						["variable"] = "marker",
					},
					["changes"] = {
						{
							["value"] = {
								0.52156862745098, -- [1]
								1, -- [2]
								0.95686274509804, -- [3]
								1, -- [4]
							},
							["property"] = "barColor",
						}, -- [1]
						{
							["value"] = {
								["sound_type"] = "Play",
								["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\Moon.ogg",
								["sound_channel"] = "Master",
							},
							["property"] = "sound",
						}, -- [2]
					},
				}, -- [5]
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["value"] = "6",
						["variable"] = "marker",
					},
					["changes"] = {
						{
							["value"] = {
								0, -- [1]
								0.40392156862745, -- [2]
								1, -- [3]
								1, -- [4]
							},
							["property"] = "barColor",
						}, -- [1]
						{
							["value"] = {
								["sound_type"] = "Play",
								["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\Square.ogg",
								["sound_channel"] = "Master",
							},
							["property"] = "sound",
						}, -- [2]
					},
				}, -- [6]
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["value"] = "7",
						["variable"] = "marker",
					},
					["changes"] = {
						{
							["value"] = {
								1, -- [1]
								0.066666666666667, -- [2]
								0, -- [3]
								1, -- [4]
							},
							["property"] = "barColor",
						}, -- [1]
						{
							["value"] = {
								["sound_type"] = "Play",
								["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\Cross.ogg",
								["sound_channel"] = "Master",
							},
							["property"] = "sound",
						}, -- [2]
					},
				}, -- [7]
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["value"] = "8",
						["variable"] = "marker",
					},
					["changes"] = {
						{
							["property"] = "barColor",
						}, -- [1]
						{
							["value"] = {
								["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\Skull.ogg",
								["sound_type"] = "Play",
							},
							["property"] = "sound",
						}, -- [2]
					},
				}, -- [8]
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["value"] = "9",
						["variable"] = "marker",
					},
					["changes"] = {
						{
							["value"] = {
								0.95686274509804, -- [1]
								1, -- [2]
								0, -- [3]
								1, -- [4]
							},
							["property"] = "barColor",
						}, -- [1]
						{
							["value"] = {
								["sound_type"] = "Play",
								["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\Star.ogg",
								["sound_channel"] = "Master",
							},
							["property"] = "sound",
						}, -- [2]
					},
				}, -- [9]
			},
			["information"] = {
			},
			["wordWrap"] = "WordWrap",
		},
		["Autofollow Leader"] = {
			["outline"] = "OUTLINE",
			["iconSource"] = -1,
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["displayText"] = "",
			["customText"] = "function()\n    if (aura_env.leader) then\n        FollowUnit(leader)\n    end\nend",
			["yOffset"] = 13,
			["anchorPoint"] = "TOP",
			["cooldownSwipe"] = true,
			["customTextUpdate"] = "event",
			["cooldownEdge"] = false,
			["icon"] = true,
			["triggers"] = {
				{
					["trigger"] = {
						["use_alwaystrue"] = true,
						["duration"] = "1",
						["use_unit"] = true,
						["debuffType"] = "HELPFUL",
						["type"] = "unit",
						["unevent"] = "auto",
						["custom_hide"] = "timed",
						["event"] = "Conditions",
						["subeventPrefix"] = "SPELL",
						["subeventSuffix"] = "_CAST_START",
						["spellIds"] = {
						},
						["events"] = "PARTY_LEADER_CHANGED, GROUP_ROSTER_UPDATE",
						["names"] = {
						},
						["check"] = "event",
						["custom_type"] = "event",
						["unit"] = "nameplate",
						["use_absorbMode"] = true,
					},
					["untrigger"] = {
					},
				}, -- [1]
				{
					["trigger"] = {
						["use_genericShowOn"] = true,
						["genericShowOn"] = "showOnCooldown",
						["subeventPrefix"] = "SPELL",
						["debuffType"] = "HELPFUL",
						["custom_hide"] = "timed",
						["type"] = "custom",
						["sourceUnit"] = "player",
						["subeventSuffix"] = "_CAST_START",
						["unit"] = "player",
						["custom_type"] = "event",
						["event"] = "Combat Log",
						["events"] = "UNIT_SPELLCAST_START, QUEST_ACCEPTED,CHAT_MSG_ADDON",
						["realSpellName"] = 0,
						["use_spellName"] = false,
						["custom"] = "function(event, ...)\n    if (UnitIsGroupLeader(\"player\")) then\n        if (event == \"UNIT_SPELLCAST_START\") then\n            local unit, _, spell = ...\n            if unit == \"player\" then\n                local spellName = GetSpellInfo(spell)\n                local mountIDs = C_MountJournal.GetMountIDs()    \n                for i = 1, #mountIDs do\n                    local creatureName, _, _, _, _, _, _, _, _, _, _, mountID = C_MountJournal.GetMountInfoByID(mountIDs[i])\n                    if spellName == creatureName then\n                        C_ChatInfo.SendAddonMessage(\"AuF\", \"M,\" .. tostring(mountIDs[i]))\n                        return\n                    end\n                end    \n            end\n        elseif (event == \"QUEST_ACCEPTED\") then\n            local quest = ...\n            if (quest) then\n                C_ChatInfo.SendAddonMessage(\"AuF\", \"Q\")\n                aura_env.pendingQuest = quest\n            end\n        elseif (event == \"CHAT_MSG_ADDON\") then\n            local prefix, text = ...\n            if (prefix == \"AuF\" and text == \"QA\") then\n                if (aura_env.pendingQuest) then\n                    C_QuestLog.SetSelectedQuest(aura_env.pendingQuest)\n                    QuestLogPushQuest()\n                    aura_env.pendingQuest = nil\n                end\n                \n            end\n        end\n    end\nend\n\n\n\n\n\n\n\n",
						["use_sourceUnit"] = true,
						["duration"] = "1",
						["spellName"] = 0,
						["use_track"] = true,
						["unevent"] = "timed",
					},
					["untrigger"] = {
					},
				}, -- [2]
				["disjunctive"] = "custom",
				["customTriggerLogic"] = "function(t)\n    return t[1]\nend",
				["activeTriggerMode"] = -10,
			},
			["displayText_format_p_format"] = "timed",
			["internalVersion"] = 45,
			["keepAspectRatio"] = false,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["semver"] = "1.0.10",
			["automaticWidth"] = "Auto",
			["desaturate"] = false,
			["cooldown"] = false,
			["font"] = "Oswald",
			["version"] = 11,
			["subRegions"] = {
			},
			["height"] = 64,
			["displayIcon"] = 450907,
			["load"] = {
				["zoneIds"] = "",
				["ingroup"] = {
					["single"] = "group",
				},
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["use_ingroup"] = true,
				["spec"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["preferToUpdate"] = true,
			["parent"] = "Autofollow",
			["fontSize"] = 36,
			["wordWrap"] = "WordWrap",
			["displayText_format_n_format"] = "none",
			["shadowXOffset"] = 1,
			["uid"] = "qXaOqb12Grz",
			["url"] = "https://wago.io/UW5MyMEHT/11",
			["width"] = 64,
			["regionType"] = "icon",
			["frameStrata"] = 1,
			["conditions"] = {
			},
			["shadowYOffset"] = -1,
			["displayText_format_p_time_precision"] = 1,
			["anchorFrameParent"] = false,
			["cooldownTextDisabled"] = false,
			["selfPoint"] = "CENTER",
			["zoom"] = 0,
			["justify"] = "LEFT",
			["tocversion"] = 90002,
			["id"] = "Autofollow Leader",
			["authorOptions"] = {
			},
			["alpha"] = 1,
			["anchorFrameType"] = "SCREEN",
			["actions"] = {
				["start"] = {
					["custom"] = "if not aura_env.clickableFrame then\n    local r = WeakAuras.regions['Autofollow Leader'].region\n    aura_env.clickableFrame = CreateFrame(\"Button\",\"FollowButton\", r, \"SecureActionButtonTemplate\")  \nend\n\naura_env.clickableFrame:SetAllPoints()\naura_env.clickableFrame:SetAttribute(\"type\", \"macro\")\naura_env.clickableFrame:SetAttribute(\"macrotext\", \"/run C_ChatInfo.SendAddonMessage(\\\"AuF\\\", \\\"F,\\\" .. UnitName(\\\"player\\\"))\")",
					["do_custom"] = true,
				},
				["init"] = {
					["custom"] = "C_ChatInfo.RegisterAddonMessagePrefix(\"AuF\")",
					["do_custom"] = true,
				},
				["finish"] = {
				},
			},
			["config"] = {
			},
			["inverse"] = false,
			["displayText_format_p_time_dynamic"] = false,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["fixedWidth"] = 200,
			["information"] = {
			},
			["xOffset"] = -490,
		},
		["Enemy Interrupt Icons"] = {
			["grow"] = "GRID",
			["controlledChildren"] = {
				"Death_Knight 2", -- [1]
				"Druid_Skull_Bash 2", -- [2]
				"Druid_Solar_Beam 2", -- [3]
				"Demon_Hunter 2", -- [4]
				"DH Sigil of Silence 2", -- [5]
				"Hunter 2", -- [6]
				"Hunter Survival 2", -- [7]
				"Mage 2", -- [8]
				"Monk 2", -- [9]
				"Paladin 2", -- [10]
				"Priest 2", -- [11]
				"Rogue 2", -- [12]
				"Shaman 2", -- [13]
				"Warlock 2", -- [14]
				"Warrior 2", -- [15]
			},
			["borderBackdrop"] = "Blizzard Tooltip",
			["xOffset"] = -182,
			["preferToUpdate"] = false,
			["yOffset"] = -107.6656188964844,
			["gridType"] = "RU",
			["borderColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["space"] = 6,
			["url"] = "https://wago.io/ZG34ifiRf/1",
			["actions"] = {
				["start"] = {
				},
				["init"] = {
				},
				["finish"] = {
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "aura",
						["subeventSuffix"] = "_CAST_START",
						["event"] = "Health",
						["unit"] = "player",
						["spellIds"] = {
						},
						["debuffType"] = "HELPFUL",
						["names"] = {
						},
						["subeventPrefix"] = "SPELL",
						["buffShowOn"] = "showOnActive",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = 1,
			},
			["columnSpace"] = 6,
			["internalVersion"] = 45,
			["useLimit"] = false,
			["align"] = "LEFT",
			["rotation"] = 0,
			["version"] = 1,
			["borderInset"] = 0,
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["ingroup"] = {
					["multi"] = {
					},
				},
				["use_class"] = false,
				["role"] = {
					["multi"] = {
					},
				},
				["difficulty"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["faction"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
			},
			["backdropColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0.5, -- [4]
			},
			["selfPoint"] = "BOTTOMLEFT",
			["animate"] = false,
			["arcLength"] = 360,
			["scale"] = 1,
			["authorOptions"] = {
			},
			["border"] = false,
			["borderEdge"] = "Square Full White",
			["regionType"] = "dynamicgroup",
			["borderSize"] = 2,
			["limit"] = 5,
			["rowSpace"] = 6,
			["fullCircle"] = true,
			["constantFactor"] = "RADIUS",
			["sort"] = "descending",
			["borderOffset"] = 16,
			["semver"] = "1.0.0",
			["tocversion"] = 80300,
			["id"] = "Enemy Interrupt Icons",
			["frameStrata"] = 1,
			["gridWidth"] = 8,
			["anchorFrameType"] = "SCREEN",
			["stagger"] = 0,
			["config"] = {
			},
			["anchorPoint"] = "CENTER",
			["uid"] = "INKAARHre9c",
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["radius"] = 200,
		},
		["Targeted Spells"] = {
			["text2Point"] = "CENTER",
			["iconSource"] = -1,
			["text1FontSize"] = 12,
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["preferToUpdate"] = true,
			["customText"] = "function()\n    if aura_env.state and aura_env.state.stacks and aura_env.state.stacks > 1 then\n        return aura_env.state.stacks\n    end\nend",
			["yOffset"] = 0,
			["anchorPoint"] = "LEFT",
			["text1Font"] = "Friz Quadrata TT",
			["cooldownSwipe"] = true,
			["parent"] = "Dungeon - Targeted Spells",
			["customTextUpdate"] = "event",
			["url"] = "https://wago.io/BFADungeonTargetedSpells/68",
			["actions"] = {
				["start"] = {
					["do_glow"] = false,
					["custom"] = "\n\n",
					["glow_action"] = "show",
					["do_sound"] = false,
					["sound"] = " custom",
					["do_custom"] = false,
				},
				["finish"] = {
					["hide_all_glows"] = true,
					["custom"] = "",
					["do_custom"] = false,
				},
				["init"] = {
					["custom"] = "-- update 25/04/20 -- by Buds - https://wago.io/BFADungeonTargetedSpells\n\n-- Spell list\naura_env.spells = {}\nfor i, v in ipairs(aura_env.config.spells) do\n    if v.spellId ~= 0 then\n        local opt = v.options\n        aura_env.spells[v.spellId] = { icon = opt[1], glow = opt[2], sound = opt[3], bigIcon = opt[4] }\n    end\nend\n\n\n-- TODO move this in aura_env.config\naura_env.blacklist_npc = {\n    [120651] = true, -- explosive orb affix\n}\naura_env.blacklist_spell = {\n    \n}\n\n-- DO NOT EDIT BELOW --\n\naura_env.loaded = false\n\naura_env.isBlackListed = function(spellId, unit)\n    local guid = UnitGUID(unit)\n    local npc_id= select(6,strsplit(\"-\",guid))\n    return aura_env.blacklist_spell[spellId] or aura_env.blacklist_npc[npc_id]\nend\n\naura_env.allcasts = {}\n\naura_env.loaded = true",
					["do_custom"] = true,
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["duration"] = "1",
						["genericShowOn"] = "showOnActive",
						["names"] = {
						},
						["debuffType"] = "HELPFUL",
						["type"] = "custom",
						["unevent"] = "timed",
						["custom_hide"] = "timed",
						["subeventPrefix"] = "SPELL",
						["event"] = "Health",
						["subeventSuffix"] = "_CAST_START",
						["unit"] = "player",
						["custom"] = "function(allstates, event, sourceUnit)\n    if not aura_env.loaded then return false end\n    if event == \"ENCOUNTER_END\" then\n        -- reset aura state to avoid ghost icons between pulls\n        aura_env.allcasts = {}\n        for _,state in pairs(allstates) do\n            state.show = false\n            state.changed = true\n        end\n        return true\n    end\n    if sourceUnit and UnitIsEnemy(sourceUnit, \"player\") then\n        local allcasts = aura_env.allcasts\n        local sourceGUID = UnitGUID(sourceUnit)\n        local iconChanged = false\n        local cast = allcasts[sourceGUID]\n        \n        if event == \"UNIT_SPELLCAST_START\"\n        or event == \"UNIT_SPELLCAST_DELAYED\" \n        or event == \"UNIT_SPELLCAST_CHANNEL_START\"\n        or event == \"UNIT_SPELLCAST_CHANNEL_UPDATE\"\n        or event == \"UNIT_TARGET\"\n        then\n            if not cast then\n                -- check if unit is casting\n                local castType\n                local name,_,texture,castStart,castEnd,_,_,notInterruptible,spellId = UnitCastingInfo(sourceUnit)\n                if name then\n                    castType = \"cast\"\n                else\n                    name,_,texture,castStart,castEnd,_,notInterruptible,spellId = UnitChannelInfo(sourceUnit)\n                    if name then\n                        castType = \"channel\"\n                    end\n                end\n                \n                if spellId and not aura_env.isBlackListed(spellId, sourceUnit) then\n                    local spellInList = aura_env.spells[spellId]\n                    -- find npc's target\n                    local targetUnit = sourceUnit..\"target\"\n                    if UnitExists(targetUnit) then\n                        for groupmember in WA_IterateGroupMembers() do\n                            if UnitIsUnit(targetUnit, groupmember) then\n                                local targetGUID = UnitGUID(groupmember)\n                                local targeted = targetGUID == WeakAuras.myGUID\n                                local showIcon = (spellInList and spellInList.icon) or (not spellInList and aura_env.config.icon.enable)\n                                local showGlow = spellInList and spellInList.glow\n                                local playSound = targeted and spellInList and spellInList.sound\n                                local bigIcon = targeted and spellInList and spellInList.bigIcon\n                                allcasts[sourceGUID] = {\n                                    name = name,\n                                    icon = texture,\n                                    start = castStart/1000,\n                                    expirationTime = castEnd/1000,\n                                    spellId = spellId,\n                                    target = groupmember,\n                                    targetGUID = targetGUID,\n                                    targeted = targeted,\n                                    spellInList = spellInList,\n                                    notInterruptible = notInterruptible,\n                                    castType = castType,\n                                    showIcon = showIcon,\n                                    showGlow = showGlow,\n                                    bigIcon = bigIcon,\n                                    playSound = playSound\n                                }\n                                iconChanged = true\n                                break\n                            end\n                        end\n                    end\n                end\n            else\n                if UnitExists(sourceUnit) then\n                    if event == \"UNIT_SPELLCAST_DELAYED\" \n                    or event == \"UNIT_SPELLCAST_CHANNEL_UPDATE\"\n                    or event == \"UNIT_SPELLCAST_CHANNEL_START\"\n                    then\n                        local castType\n                        local name,_,_,castStart,castEnd,_,_,notInterruptible,spellId = UnitCastingInfo(sourceUnit)\n                        if name then\n                            castType = \"cast\"\n                        else\n                            name,_,_,castStart,castEnd,_,notInterruptible,spellId = UnitChannelInfo(sourceUnit)\n                            if name then\n                                castType = \"channel\"\n                            end\n                        end\n                        if spellId then\n                            cast.notInterruptible = notInterruptible\n                            cast.castType = castType\n                            cast.start = castStart/1000\n                            cast.expirationTime = castEnd/1000\n                            cast.changed = true\n                            iconChanged = true\n                        end\n                    elseif event == \"UNIT_TARGET\" then\n                        local targetUnit = sourceUnit..\"target\"\n                        if UnitExists(targetUnit) then\n                            for groupmember in WA_IterateGroupMembers() do\n                                if UnitIsUnit(targetUnit, groupmember) then\n                                    local targetGUID = UnitGUID(groupmember)\n                                    local targeted = targetGUID == WeakAuras.myGUID\n                                    local spellId = cast.spellId\n                                    cast.changed = true\n                                    cast.target = groupmember\n                                    cast.targetGUID = targetGUID\n                                    cast.targeted = targeted\n                                    \n                                    iconChanged = true\n                                    break\n                                end\n                            end\n                        end\n                    end\n                end\n            end\n        elseif cast then\n            if event == \"UNIT_SPELLCAST_STOP\" --  or event == \"UNIT_SPELLCAST_SUCCEEDED\"\n            or event == \"UNIT_SPELLCAST_INTERRUPTED\"\n            or event == \"UNIT_SPELLCAST_FAILED\"\n            or event == \"UNIT_SPELLCAST_FAILED_QUIET\"\n            or event == \"UNIT_SPELLCAST_CHANNEL_STOP\"\n            then\n                allcasts[sourceGUID] = nil\n                iconChanged = true\n            end\n        end\n        \n        if iconChanged then\n            local change = false\n            -- update allstates from allcasts\n            for sourceGUID, cast in pairs(allcasts) do\n                -- index for allstates is \"spellId_targetGUID\"\n                local index = (\"%s_%s\"):format(cast.spellId, cast.targetGUID)\n                \n                local state = allstates[index]\n                \n                if state and state.show then\n                    state.casts[sourceGUID] = true\n                    if cast.expirationTime > state.expirationTime then\n                        state.expirationTime = cast.expirationTime\n                        state.changed = true\n                        change = true\n                    end\n                else\n                    allstates[index] = {\n                        show = true,\n                        name = cast.name,\n                        icon = cast.icon,\n                        changed = true,\n                        autoHide = true,\n                        progressType = \"timed\",\n                        duration = cast.expirationTime - cast.start,\n                        expirationTime = cast.expirationTime,\n                        spellId = cast.spellId,\n                        castType = cast.castType,\n                        notInterruptible = cast.notInterruptible,\n                        target = cast.target,\n                        unit = cast.showIcon and cast.target,\n                        targetGUID = cast.targetGUID,\n                        casts = {\n                            [sourceGUID] = true\n                        },\n                        targeted = cast.targeted,\n                        showGlow = cast.showGlow,\n                        playSound = cast.playSound,\n                        bigIcon = cast.bigIcon,\n                        showIcon = cast.showIcon,\n                    }\n                    change = true\n                end\n            end\n            \n            -- count how much of the same cast is showing each icon\n            -- remove casts stopped from state.casts\n            -- remove state if state.casts is empty\n            for index, state in pairs(allstates) do \n                if state.show and state.showIcon then\n                    local countcasts = 0\n                    for sourceGUID,_ in pairs(state.casts) do\n                        local cast = allcasts[sourceGUID]\n                        if not cast\n                        or (cast and cast.targetGUID ~= state.targetGUID)\n                        then\n                            state.casts[sourceGUID] = nil\n                        else \n                            countcasts = countcasts + 1\n                        end\n                    end\n                    if state.stacks ~= countcasts then\n                        state.stacks = countcasts\n                        state.changed = true\n                        change = true\n                    end\n                    -- hide if no cast\n                    if countcasts == 0 then\n                        state.show = false \n                        state.changed = true\n                        change = true\n                    end\n                end\n            end\n            if change then\n                return true \n            end\n        end\n    end\nend",
						["spellIds"] = {
						},
						["events"] = "UNIT_SPELLCAST_CHANNEL_START UNIT_SPELLCAST_CHANNEL_STOP UNIT_SPELLCAST_CHANNEL_UPDATE UNIT_SPELLCAST_DELAYED UNIT_SPELLCAST_FAILED UNIT_SPELLCAST_FAILED_QUIET UNIT_SPELLCAST_INTERRUPTED UNIT_SPELLCAST_START UNIT_SPELLCAST_STOP UNIT_TARGET ENCOUNTER_END",
						["check"] = "event",
						["custom_type"] = "stateupdate",
						["buffShowOn"] = "showOnActive",
						["customVariables"] = "{\n    targeted = {\n        display = \"Targeted\",\n        type = \"bool\"\n    },\n    playSound = {\n        display = \"Play Sound\",\n        type = \"bool\"\n    },\n    showGlow = {\n        display = \"Glow\",\n        type = \"bool\"\n    },\n    bigIcon = {\n        display = \"Big Icon\",\n        type = \"bool\"\n    },\n    spellId = {\n        display = \"Spell Id\",\n        type = \"number\"\n    },\n    notInterruptible = {\n        display = \"Not Interruptible\",\n        type = \"bool\"\n    },\n    castType = {\n        display = \"Cast Type\",\n        type = \"select\",\n        values = {\n            [\"cast\"] = \"Cast\",\n            [\"channel\"] = \"Channeling\"\n        }\n    }\n}",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = -10,
			},
			["cooldownEdge"] = false,
			["internalVersion"] = 45,
			["keepAspectRatio"] = false,
			["selfPoint"] = "CENTER",
			["cooldownTextEnabled"] = true,
			["icon"] = true,
			["zoom"] = 0,
			["desaturate"] = false,
			["text1Enabled"] = true,
			["text1Point"] = "BOTTOM",
			["version"] = 68,
			["text2FontFlags"] = "OUTLINE",
			["height"] = 20,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["load"] = {
				["ingroup"] = {
					["multi"] = {
					},
				},
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_size"] = false,
				["difficulty"] = {
					["multi"] = {
						["challenge"] = true,
						["heroic"] = true,
						["lfr"] = true,
						["timewalking"] = true,
						["none"] = true,
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["faction"] = {
					["multi"] = {
					},
				},
				["talent2"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["single"] = "party",
					["multi"] = {
						["scenario"] = true,
						["ten"] = true,
						["twentyfive"] = true,
						["fortyman"] = true,
						["ratedpvp"] = true,
						["flexible"] = true,
						["ratedarena"] = true,
						["pvp"] = true,
						["arena"] = true,
						["party"] = true,
						["none"] = true,
					},
				},
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["xOffset"] = 0,
			["stickyDuration"] = false,
			["text2Containment"] = "INSIDE",
			["text2Font"] = "Friz Quadrata TT",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["subRegions"] = {
				{
					["text_shadowXOffset"] = 0,
					["text_text"] = "%c",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "2002",
					["text_shadowYOffset"] = 0,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "OUTER_BOTTOM",
					["text_fontSize"] = 13,
					["anchorXOffset"] = 0,
					["text_visible"] = true,
				}, -- [1]
			},
			["auto"] = true,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["regionType"] = "icon",
			["text1Containment"] = "OUTSIDE",
			["uid"] = "pWfZPrRiJzV",
			["text2FontSize"] = 24,
			["text2Enabled"] = false,
			["text1FontFlags"] = "OUTLINE",
			["text1"] = "%c",
			["anchorFrameType"] = "SCREEN",
			["frameStrata"] = 6,
			["cooldownTextDisabled"] = true,
			["useGlowColor"] = true,
			["tocversion"] = 90002,
			["id"] = "Targeted Spells",
			["authorOptions"] = {
				{
					["subOptions"] = {
						{
							["type"] = "toggle",
							["key"] = "enable",
							["width"] = 2,
							["name"] = "Show (default)",
							["useDesc"] = true,
							["default"] = true,
							["desc"] = "Show cast's icons on Unit Frames",
						}, -- [1]
					},
					["type"] = "group",
					["width"] = 2,
					["useCollapse"] = true,
					["name"] = "Cast icons on Unit Frames",
					["key"] = "icon",
					["limitType"] = "none",
					["groupType"] = "simple",
					["collapse"] = false,
					["size"] = 10,
				}, -- [1]
				{
					["subOptions"] = {
						{
							["type"] = "number",
							["useDesc"] = false,
							["max"] = 999999,
							["step"] = 1,
							["width"] = 1,
							["min"] = 0,
							["key"] = "spellId",
							["name"] = "Spell Id",
							["default"] = 0,
						}, -- [1]
						{
							["type"] = "input",
							["useDesc"] = false,
							["width"] = 1,
							["key"] = "desc",
							["multiline"] = false,
							["default"] = "",
							["length"] = 10,
							["name"] = "Description",
							["useLength"] = false,
						}, -- [2]
						{
							["type"] = "multiselect",
							["values"] = {
								"Icon", -- [1]
								"Glow", -- [2]
								"Sound", -- [3]
								"Big Icon", -- [4]
							},
							["default"] = {
								true, -- [1]
								true, -- [2]
								true, -- [3]
								true, -- [4]
							},
							["key"] = "options",
							["useDesc"] = false,
							["name"] = "Options",
							["width"] = 1,
						}, -- [3]
					},
					["type"] = "group",
					["useDesc"] = false,
					["nameSource"] = 2,
					["width"] = 1,
					["useCollapse"] = true,
					["hideReorder"] = false,
					["name"] = "Spells",
					["collapse"] = false,
					["limitType"] = "none",
					["groupType"] = "array",
					["key"] = "spells",
					["size"] = 10,
				}, -- [2]
				{
					["type"] = "space",
					["variableWidth"] = true,
					["height"] = 10,
					["width"] = 2,
					["useHeight"] = true,
				}, -- [3]
			},
			["alpha"] = 1,
			["width"] = 20,
			["semver"] = "3.0.6",
			["config"] = {
				["icon"] = {
					["enable"] = true,
				},
				["spells"] = {
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 259832,
						["desc"] = "Massive Glaive - Stormbound Conqueror (Warport Wastari, Zuldazar, for testing purpose only)",
					}, -- [1]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							false, -- [4]
						},
						["spellId"] = 0,
						["desc"] = "== Atal'Dazar ==",
					}, -- [2]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							false, -- [4]
						},
						["spellId"] = 253239,
						["desc"] = "Dazar'ai Juggernaut - Merciless Assault",
					}, -- [3]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							false, -- [4]
						},
						["spellId"] = 256846,
						["desc"] = "Dinomancer Kish'o - Deadeye Aim",
					}, -- [4]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 257407,
						["desc"] = "Rezan - Pursuit",
					}, -- [5]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							false, -- [4]
						},
						["spellId"] = 0,
						["desc"] = "== Freehold ==",
					}, -- [6]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 257739,
						["desc"] = "Blacktooth Scrapper - Blind Rage",
					}, -- [7]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 258338,
						["desc"] = "Captain Raoul - Blackout Barrel",
					}, -- [8]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 256979,
						["desc"] = "Captain Eudora - Powder Shot",
					}, -- [9]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							false, -- [4]
						},
						["spellId"] = 0,
						["desc"] = "== Kings'Rest ==",
					}, -- [10]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 266231,
						["desc"] = "Kula the Butcher - Severing Axe",
					}, -- [11]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 270507,
						["desc"] = "Spectral Beastmaster - Poison Barrage",
					}, -- [12]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 265773,
						["desc"] = "The Golden Serpent - Spit Gold",
					}, -- [13]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 270506,
						["desc"] = "Spectral Beastmaster - Deadeye Shot",
					}, -- [14]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 0,
						["desc"] = "== Shrine of the Storm ==",
					}, -- [15]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 264166,
						["desc"] = "Aqu'sirr - Undertow",
					}, -- [16]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 268214,
						["desc"] = "Runecarver Sorn - Carve Flesh",
					}, -- [17]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 0,
						["desc"] = "== Siege of Boralus ==",
					}, -- [18]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							false, -- [4]
						},
						["spellId"] = 257641,
						["desc"] = "Kul Tiran Marksman - Molten Slug",
					}, -- [19]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 272874,
						["desc"] = "Ashvane Commander - Trample",
					}, -- [20]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							false, -- [4]
						},
						["spellId"] = 272581,
						["desc"] = "Bilge Rat Tempest - Water Spray",
					}, -- [21]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							false, -- [4]
						},
						["spellId"] = 272528,
						["desc"] = "Ashvane Sniper - Shoot",
					}, -- [22]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 272542,
						["desc"] = "Ashvane Sniper - Ricochet",
					}, -- [23]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 0,
						["desc"] = "== Temple of Sethraliss ==",
					}, -- [24]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 268703,
						["desc"] = "Charged Dust Devil - Lightning Bolt",
					}, -- [25]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 272670,
						["desc"] = "Sandswept Marksman - Shoot",
					}, -- [26]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 267278,
						["desc"] = "Static-charged Dervish - Electrocute",
					}, -- [27]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 272820,
						["desc"] = "Spark Channeler - Shock",
					}, -- [28]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 274642,
						["desc"] = "Hoodoo Hexer - Lava Burst",
					}, -- [29]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 268061,
						["desc"] = "Plague Doctor - Chain Lightning",
					}, -- [30]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 0,
						["desc"] = "== The Motherlode!! ==",
					}, -- [31]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 268185,
						["desc"] = "Refreshment Vendor, Iced Spritzer",
					}, -- [32]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							false, -- [4]
						},
						["spellId"] = 258674,
						["desc"] = "Off-Duty Laborer - Throw Wrench",
					}, -- [33]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							false, -- [4]
						},
						["spellId"] = 276304,
						["desc"] = "Rowdy Reveler - Penny For Your Thoughts",
					}, -- [34]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 263628,
						["desc"] = "Mechanized Peacekeeper - Charged Claw",
					}, -- [35]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							false, -- [4]
						},
						["spellId"] = 263209,
						["desc"] = "Mine Rat - Throw Rock",
					}, -- [36]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 263202,
						["desc"] = "Venture Co. Earthshaper - Rock Lance",
					}, -- [37]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							false, -- [4]
						},
						["spellId"] = 262794,
						["desc"] = "Venture Co. Mastermind - Energy Lash",
					}, -- [38]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 260669,
						["desc"] = "Rixxa Fluxflame - Propellant Blast",
					}, -- [39]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 0,
						["desc"] = "== The Underrot ==",
					}, -- [40]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 265376,
						["desc"] = "Fanatical Headhunter - Barbed Spear",
					}, -- [41]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							false, -- [4]
						},
						["spellId"] = 265084,
						["desc"] = "Devout Blood Priest - Blood Bolt",
					}, -- [42]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 265625,
						["desc"] = "Befouled Spirit - Dark Omen",
					}, -- [43]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 0,
						["desc"] = "== Tol Dagor ==",
					}, -- [44]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 256039,
						["desc"] = "Overseer Korgus - Deadeye",
					}, -- [45]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							false, -- [4]
						},
						["spellId"] = 185857,
						["desc"] = "Ashvane Spotter - Shoot",
					}, -- [46]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 0,
						["desc"] = "== Waycrest Manor ==",
					}, -- [47]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							false, -- [4]
						},
						["spellId"] = 263891,
						["desc"] = "Heartsbane Vinetwister - Grasping Thorns",
					}, -- [48]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							false, -- [4]
						},
						["spellId"] = 264510,
						["desc"] = "Crazed Marksman - Shoot",
					}, -- [49]
					{
						["options"] = {
							true, -- [1]
							false, -- [2]
							true, -- [3]
							false, -- [4]
						},
						["spellId"] = 260699,
						["desc"] = "Coven Diviner - Soul Bolt",
					}, -- [50]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 260551,
						["desc"] = "Soulbound Goliath - Soul Thorns",
					}, -- [51]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 260741,
						["desc"] = "Heartsbane Triad - Jagged Nettles",
					}, -- [52]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 268202,
						["desc"] = "Gorak Tul - Death Lens",
					}, -- [53]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							true, -- [4]
						},
						["spellId"] = 0,
						["desc"] = "== Season 4 Affix ==",
					}, -- [54]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							false, -- [4]
						},
						["spellId"] = 314406,
						["desc"] = "Crippling Pestilence",
					}, -- [55]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							false, -- [4]
						},
						["spellId"] = 314411,
						["desc"] = "Lingering Doubt",
					}, -- [56]
					{
						["options"] = {
							true, -- [1]
							true, -- [2]
							true, -- [3]
							false, -- [4]
						},
						["spellId"] = 314592,
						["desc"] = "Mind Flay",
					}, -- [57]
				},
			},
			["inverse"] = false,
			["glowParticules"] = 4,
			["text2"] = "%p",
			["conditions"] = {
				{
					["check"] = {
						["trigger"] = 1,
						["variable"] = "playSound",
						["value"] = 1,
						["checks"] = {
							{
								["trigger"] = 1,
								["variable"] = "targeted",
								["value"] = 1,
							}, -- [1]
							{
								["trigger"] = 1,
								["variable"] = "playSound",
								["value"] = 1,
							}, -- [2]
						},
					},
					["changes"] = {
						{
							["value"] = {
								["sound_kit_id"] = "12889",
								["sound_type"] = "Play",
								["sound"] = " KitID",
								["sound_channel"] = "Master",
							},
							["property"] = "sound",
						}, -- [1]
					},
				}, -- [1]
				{
					["check"] = {
						["trigger"] = 1,
						["variable"] = "showGlow",
						["value"] = 1,
					},
					["changes"] = {
						{
							["value"] = {
								["glow_frame_type"] = "UNITFRAME",
								["glow_type"] = "Pixel",
								["use_glow_color"] = true,
								["glow_action"] = "show",
							},
							["property"] = "glowexternal",
						}, -- [1]
					},
				}, -- [2]
				{
					["check"] = {
						["trigger"] = 1,
						["variable"] = "bigIcon",
						["value"] = 1,
					},
					["changes"] = {
						{
							["value"] = {
								["custom"] = "-- Send event for aura \"BigIcon when targeted\"\nWeakAuras.ScanEvents(\n    \"SHOW_BIGICON\",\n    aura_env.state.icon,\n    aura_env.state.spellId,\n    aura_env.state.duration\n)",
							},
							["property"] = "customcode",
						}, -- [1]
					},
				}, -- [3]
			},
			["cooldown"] = true,
			["anchorFrameFrame"] = "UIParent",
		},
		["09 Wicked Laceration >= 2 stacks - Orange"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 3,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						1, -- [1]
						0.62352941176471, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.25,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = true,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2417",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "Jsftk)mAei5",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["custom_type"] = "stateupdate",
						["useMatch_count"] = true,
						["useStacks"] = true,
						["useGroup_count"] = false,
						["groupclone"] = true,
						["matchesShowOn"] = "showOnActive",
						["name_info"] = "aura",
						["subeventPrefix"] = "SPELL",
						["useExactSpellId"] = true,
						["stacks"] = "2",
						["debuffType"] = "HARMFUL",
						["subeventSuffix"] = "_CAST_START",
						["match_count"] = "1",
						["stacksOperator"] = ">=",
						["auraspellids"] = {
							"333913", -- [1]
						},
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["match_countOperator"] = ">=",
						["unit"] = "group",
						["duration"] = "1",
						["event"] = "Health",
						["use_specific_unit"] = false,
						["unevent"] = "timed",
						["use_tooltip"] = false,
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["buffShowOn"] = "showOnActive",
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["auranames"] = {
						},
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["text2Font"] = "Friz Quadrata TT",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text1"] = "%n",
			["text1Point"] = "CENTER",
			["anchorFrameType"] = "SCREEN",
			["semver"] = "1.0.26",
			["justify"] = "LEFT",
			["zoom"] = 0,
			["id"] = "09 Wicked Laceration >= 2 stacks - Orange",
			["config"] = {
			},
			["frameStrata"] = 9,
			["width"] = 6.9999866485596,
			["text1Containment"] = "INSIDE",
			["wordWrap"] = "WordWrap",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["06 Concentrate Anima - Green"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 2,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						0, -- [1]
						1, -- [2]
						0.019607843137255, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.25,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = true,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2406",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "Tarb(Qq3Z05",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["useMatch_count"] = true,
						["auranames"] = {
						},
						["groupclone"] = true,
						["matchesShowOn"] = "showOnActive",
						["name_info"] = "aura",
						["unit"] = "group",
						["use_tooltip"] = false,
						["subeventPrefix"] = "SPELL",
						["match_countOperator"] = ">=",
						["match_count"] = "1",
						["buffShowOn"] = "showOnActive",
						["unevent"] = "timed",
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["useExactSpellId"] = true,
						["custom_type"] = "stateupdate",
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["event"] = "Health",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HARMFUL",
						["use_specific_unit"] = false,
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["auraspellids"] = {
							"332664", -- [1]
							"340477", -- [2]
						},
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["duration"] = "1",
						["useGroup_count"] = false,
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["wordWrap"] = "WordWrap",
			["config"] = {
			},
			["text1"] = "%n",
			["text1Containment"] = "INSIDE",
			["width"] = 6.9999866485596,
			["justify"] = "LEFT",
			["semver"] = "1.0.26",
			["zoom"] = 0,
			["id"] = "06 Concentrate Anima - Green",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["frameStrata"] = 9,
			["anchorFrameType"] = "SCREEN",
			["text1Point"] = "CENTER",
			["text2Font"] = "Friz Quadrata TT",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["Quaking Current Cast Warning"] = {
			["outline"] = "OUTLINE",
			["xOffset"] = 0.99981689453125,
			["displayText"] = "CANCEL CAST",
			["yOffset"] = 193,
			["anchorPoint"] = "CENTER",
			["customTextUpdate"] = "event",
			["url"] = "https://wago.io/ByQSAbqBm/5",
			["actions"] = {
				["start"] = {
					["do_loop"] = false,
					["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\BikeHorn.ogg",
					["do_sound"] = true,
				},
				["finish"] = {
				},
				["init"] = {
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "custom",
						["subeventSuffix"] = "_CAST_START",
						["custom_hide"] = "custom",
						["genericShowOn"] = "showOnActive",
						["names"] = {
						},
						["subeventPrefix"] = "SPELL",
						["custom_type"] = "status",
						["custom"] = "function()\n    --By Seolfer-Antonidas (EU)\n    local buff = \"\"\n    local expires, spellId\n    local count = 1\n    \n    --search for a debuff with quakes spell id\n    while true do\n        buff, _, _, _, _, expires, _, _, _, spellId = UnitAura(\"player\", count, \"HARMFUL\")\n        if not buff then break end\n        if spellId == 240447 then break end\n        count = count + 1\n    end\n    \n    \n    if buff then\n        spell, _, _, _, endTime, _, _, _ = UnitCastingInfo(\"player\");\n        \n        if spell then\n            \n            --compare expiration Time of Quaking to end time of cast\n            --UnitAura rounds to 1/100 s, UnitCastingInfo to 1/1000 - add 1/100 s to prevent mistakes from this\n            if expires*1000 < endTime + 10 then\n                return true\n            end\n        end\n    end\nend\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
						["event"] = "Health",
						["check"] = "update",
						["spellIds"] = {
						},
						["unit"] = "player",
						["debuffType"] = "HELPFUL",
					},
					["untrigger"] = {
						["custom"] = "function()\n    local buff = \"\"\n    local expires, spellId\n    local count = 1\n    \n    --go search for a debuff with quakes spell id\n    while true do\n        buff, _, _, _, _, expires, _, _, _, spellId = UnitAura(\"player\", count, \"HARMFUL\")\n        if not buff then break end\n        if spellId == 240447 then break end\n        count = count + 1\n    end\n    \n    \n    if spell then\n        \n        if buff then\n            \n            if expires*1000 > endTime + 10 then\n                return true\n            end\n        else\n            return true\n        end\n    else \n        return true\n    end\nend\n\n\n\n\n\n",
					},
				}, -- [1]
				["disjunctive"] = "all",
				["activeTriggerMode"] = -10,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["font"] = "Friz Quadrata TT",
			["version"] = 5,
			["height"] = 72.000015258789,
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["ingroup"] = {
					["multi"] = {
					},
				},
				["difficulty"] = {
					["multi"] = {
						["mythic"] = true,
						["challenge"] = true,
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["faction"] = {
					["multi"] = {
					},
				},
				["use_size"] = true,
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["single"] = "party",
					["multi"] = {
						["party"] = true,
					},
				},
			},
			["fontSize"] = 72,
			["shadowXOffset"] = 1,
			["regionType"] = "text",
			["fixedWidth"] = 200,
			["color"] = {
				1, -- [1]
				0.054901960784314, -- [2]
				0.63137254901961, -- [3]
				1, -- [4]
			},
			["automaticWidth"] = "Auto",
			["shadowYOffset"] = -1,
			["wordWrap"] = "WordWrap",
			["authorOptions"] = {
				{
					["type"] = "toggle",
					["key"] = "option1",
					["default"] = true,
					["name"] = "Play sound",
					["useDesc"] = false,
					["width"] = 1,
				}, -- [1]
			},
			["semver"] = "1.0.1",
			["tocversion"] = 80300,
			["id"] = "Quaking Current Cast Warning",
			["config"] = {
				["option1"] = false,
			},
			["frameStrata"] = 1,
			["width"] = 502.00006103516,
			["anchorFrameType"] = "SCREEN",
			["uid"] = "BfiauuKJdu5",
			["justify"] = "LEFT",
			["selfPoint"] = "BOTTOM",
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
				{
					["check"] = {
					},
					["changes"] = {
						{
						}, -- [1]
					},
				}, -- [1]
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["08 Hateful Gaze - Pink"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 2,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						1, -- [1]
						0, -- [2]
						0.8156862745098, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.25,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = true,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2399",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "DHWe74mdyAI",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["useMatch_count"] = true,
						["auranames"] = {
						},
						["groupclone"] = true,
						["matchesShowOn"] = "showOnActive",
						["name_info"] = "aura",
						["unit"] = "group",
						["use_tooltip"] = false,
						["subeventPrefix"] = "SPELL",
						["match_countOperator"] = ">=",
						["match_count"] = "1",
						["buffShowOn"] = "showOnActive",
						["unevent"] = "timed",
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["useExactSpellId"] = true,
						["custom_type"] = "stateupdate",
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["event"] = "Health",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HARMFUL",
						["use_specific_unit"] = false,
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["auraspellids"] = {
							"331209", -- [1]
						},
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["duration"] = "1",
						["useGroup_count"] = false,
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["text2Font"] = "Friz Quadrata TT",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text1"] = "%n",
			["text1Point"] = "CENTER",
			["anchorFrameType"] = "SCREEN",
			["semver"] = "1.0.26",
			["justify"] = "LEFT",
			["zoom"] = 0,
			["id"] = "08 Hateful Gaze - Pink",
			["config"] = {
			},
			["frameStrata"] = 9,
			["width"] = 6.9999866485596,
			["text1Containment"] = "INSIDE",
			["wordWrap"] = "WordWrap",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["03 Ember Blast - Green"] = {
			["outline"] = "OUTLINE",
			["xOffset"] = 0,
			["displayText"] = " ",
			["customText"] = "\n",
			["shadowYOffset"] = -1,
			["anchorPoint"] = "CENTER",
			["cooldownSwipe"] = true,
			["displayText_format_p_time_format"] = 0,
			["customTextUpdate"] = "event",
			["cooldownEdge"] = false,
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 2,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						0, -- [1]
						1, -- [2]
						0.082352941176471, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.25,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["custom_hide"] = "timed",
						["type"] = "custom",
						["customVariables"] = "",
						["subeventSuffix"] = "_CAST_START",
						["subeventPrefix"] = "SPELL",
						["duration"] = "3.5",
						["event"] = "Chat Message",
						["names"] = {
						},
						["unevent"] = "timed",
						["spellIds"] = {
						},
						["events"] = "UNIT_TARGET:boss UNIT_SPELLCAST_SUCCEEDED:boss UNIT_SPELLCAST_STOP:boss UNIT_SPELLCAST_FAILED:boss",
						["custom"] = "function(allstates, event, unit, _, spellID)    \n    if event == \"UNIT_TARGET\" and unit and UnitExists(unit..\"target\") then\n        local _, _, _, _, expires, _, _, _, castID = UnitCastingInfo(unit)\n        if castID == 325877 then\n            local duration = expires/1000 - GetTime()\n            if not allstates[\"\"] then\n                allstates[\"\"] = {\n                    show = true,\n                    changed =  true,\n                    progressType = \"timed\", \n                    duration = duration,\n                    expirationTime = duration+GetTime(),\n                    autoHide = true,\n                    unit = unit..\"target\",\n                }\n            else\n                allstates[\"\"].show = true\n                allstates[\"\"].duration = duration\n                allstates[\"\"].expirationTime = duration+GetTime()\n                allstates[\"\"].unit = unit..\"target\"\n                allstates[\"\"].changed = true\n            end\n            return true\n        end\n    end\n    if event == \"UNIT_SPELLCAST_SUCCEEDED\" or event == \"UNIT_SPELLCAST_STOP\" or event == \"UNIT_SPELLCAST_FAILED\" then\n        if UnitExists(unit) and spellID == 325877 then\n            if allstates[\"\"] then\n                allstates[\"\"].show = false\n                allstates[\"\"].changed = true\n                return true\n            end\n        end\n    end\nend",
						["check"] = "event",
						["unit"] = "player",
						["custom_type"] = "stateupdate",
						["debuffType"] = "HELPFUL",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = -10,
			},
			["displayText_format_p_format"] = "timed",
			["internalVersion"] = 45,
			["keepAspectRatio"] = false,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["zoom"] = 0.3,
			["displayText_format_p_time_dynamic_threshold"] = 60,
			["desaturate"] = false,
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["font"] = "Friz Quadrata TT",
			["version"] = 27,
			["subRegions"] = {
			},
			["height"] = 75,
			["displayIcon"] = 607865,
			["load"] = {
				["use_encounterid"] = true,
				["role"] = {
					["single"] = "TANK",
					["multi"] = {
						["TANK"] = true,
					},
				},
				["use_never"] = true,
				["talent"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2402",
				["class"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["icon"] = true,
			["stickyDuration"] = false,
			["fontSize"] = 12,
			["parent"] = "Castle Nathria Glows",
			["uid"] = "7UopV8f635g",
			["shadowXOffset"] = 1,
			["url"] = "https://wago.io/NathriaGlows/27",
			["anchorFrameType"] = "SCREEN",
			["alpha"] = 1,
			["regionType"] = "text",
			["wordWrap"] = "WordWrap",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["semver"] = "1.0.26",
			["auto"] = false,
			["displayText_format_p_time_precision"] = 1,
			["selfPoint"] = "CENTER",
			["automaticWidth"] = "Auto",
			["cooldownTextDisabled"] = false,
			["justify"] = "LEFT",
			["tocversion"] = 90002,
			["id"] = "03 Ember Blast - Green",
			["authorOptions"] = {
			},
			["frameStrata"] = 1,
			["width"] = 75,
			["yOffset"] = 0,
			["config"] = {
			},
			["inverse"] = false,
			["conditions"] = {
			},
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["fixedWidth"] = 200,
			["cooldown"] = true,
			["preferToUpdate"] = false,
		},
		["04 Withering Touch - White"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 2,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.25,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2405",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "eiJ6Jua9CYI",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["useMatch_count"] = true,
						["auranames"] = {
						},
						["groupclone"] = true,
						["duration"] = "1",
						["name_info"] = "aura",
						["use_specific_unit"] = false,
						["use_tooltip"] = false,
						["unit"] = "group",
						["matchesShowOn"] = "showOnActive",
						["match_count"] = "1",
						["useExactSpellId"] = true,
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HARMFUL",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["unevent"] = "timed",
						["custom_hide"] = "timed",
						["auraspellids"] = {
							"340860", -- [1]
						},
						["event"] = "Health",
						["match_countOperator"] = ">=",
						["buffShowOn"] = "showOnActive",
						["custom_type"] = "stateupdate",
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["useGroup_count"] = false,
						["subeventPrefix"] = "SPELL",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["wordWrap"] = "WordWrap",
			["config"] = {
			},
			["text1"] = "%n",
			["text1Containment"] = "INSIDE",
			["width"] = 6.9999866485596,
			["justify"] = "LEFT",
			["semver"] = "1.0.26",
			["zoom"] = 0,
			["id"] = "04 Withering Touch - White",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["frameStrata"] = 9,
			["anchorFrameType"] = "SCREEN",
			["text1Point"] = "CENTER",
			["text2Font"] = "Friz Quadrata TT",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["Pescorus - Miasma priority marker"] = {
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["displayText"] = "",
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["displayText_format_p_time_format"] = 0,
			["customTextUpdate"] = "event",
			["url"] = "https://wago.io/PescorusMiasmaMarks/5",
			["actions"] = {
				["start"] = {
				},
				["finish"] = {
				},
				["init"] = {
					["custom"] = "aura_env.priority = {}\nfor unit in aura_env.config.priority:gmatch(\"%S+\") do\n    if UnitExists(unit) then\n        local name, realm = UnitName(unit)\n        if realm then name = name .. \"-\" .. realm end -- Use Name-Realm to match CLEU destName\n        aura_env.priority[name] = true\n    end\nend",
					["do_custom"] = true,
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "custom",
						["debuffType"] = "HELPFUL",
						["unevent"] = "timed",
						["custom_type"] = "stateupdate",
						["duration"] = "1",
						["event"] = "Combat Log",
						["unit"] = "player",
						["names"] = {
						},
						["events"] = "CLEU:SPELL_AURA_APPLIED, ENCOUNTER_START",
						["custom"] = "function(s, event, _, _, _, _, _, _, _, _, destName, _, _, spellId)\n    if event == \"ENCOUNTER_START\" then\n        aura_env.count = 0\n        aura_env.marks = {}\n        aura_env.assignments = {}\n        aura_env.affected = {}\n        aura_env.valid = false -- Whether the ERT note is valid, if not the aura does nothing\n        \n        if IsAddOnLoaded(\"ExRT\")  and _G.VExRT.Note.Text1 then\n            local text = _G.VExRT.Note.Text1\n            local mark_count = 0\n            \n            for line in text:gmatch('[^\\r\\n]+') do\n                local chunks = {}\n                for substring in line:gmatch(\"%S+\") do table.insert(chunks, substring) end\n                \n                if chunks[1] == \"Miasma\" and chunks[2] then\n                    local mark = nil\n                    for k, v in pairs(ICON_TAG_LIST) do\n                        if chunks[2]:match(\"{\" .. k .. \"}\") then\n                            mark = v\n                            aura_env.marks[v] = true\n                            break\n                        end\n                    end\n                    \n                    if mark then\n                        mark_count = mark_count + 1\n                        for unit in WA_IterateGroupMembers() do\n                            local name, realm = UnitName(unit) \n                            \n                            if line:lower():match(name:lower()) then\n                                if realm then name = name .. \"-\" .. realm end -- Use Name-Realm to match CLEU destName\n                                aura_env.assignments[name] = mark\n                            end\n                        end\n                    end\n                end\n            end\n            \n            aura_env.valid = (mark_count == 4)\n        end\n    elseif spellId == 329298 and aura_env.valid then\n        aura_env.affected[destName] = true\n        aura_env.count = aura_env.count + 1\n        \n        if aura_env.count == 4 then\n            -- Copy the marks\n            local remaining = {}\n            for k, _ in pairs(aura_env.marks) do\n                remaining[k] = true\n            end\n            \n            -- First check priority players\n            for name, _ in pairs(aura_env.affected) do\n                local pref_mark = aura_env.assignments[name]\n                \n                -- If unit has priority and they are assigned a preferred mark\n                if aura_env.priority[name] and pref_mark then\n                    SetRaidTarget(name, pref_mark)\n                    aura_env.affected[name] = nil\n                    remaining[pref_mark] = nil\n                    break\n                end\n            end\n            \n            for name, _ in pairs(aura_env.affected) do\n                if aura_env.assignments[name] then\n                    local pref_mark = aura_env.assignments[name]\n                    \n                    -- If their preferred mark is still available, use it\n                    if remaining[pref_mark] then\n                        SetRaidTarget(name, pref_mark)\n                        remaining[pref_mark] = nil -- Their mark is no longer available\n                        aura_env.affected[name] = nil -- And we no longer have to mark them\n                    end\n                end\n            end\n            \n            -- Only duplicates left at this point, just assign them in order\n            for name, _ in pairs(aura_env.affected) do\n                for mark, _ in pairs(remaining) do\n                    SetRaidTarget(name, mark)\n                    aura_env.affected[name] = nil\n                    remaining[mark] = nil\n                    break\n                end\n            end\n            \n            aura_env.affected = {} -- Just to be sure\n            aura_env.count = 0\n        end\n    end\nend",
						["spellIds"] = {
						},
						["check"] = "event",
						["subeventSuffix"] = "_CAST_START",
						["subeventPrefix"] = "SPELL",
						["custom_hide"] = "timed",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["displayText_format_p_format"] = "timed",
			["internalVersion"] = 45,
			["wordWrap"] = "WordWrap",
			["font"] = "Friz Quadrata TT",
			["version"] = 5,
			["subRegions"] = {
			},
			["load"] = {
				["talent"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["use_encounterid"] = true,
				["difficulty"] = {
					["single"] = "mythic",
				},
				["zoneIds"] = "",
				["spec"] = {
					["multi"] = {
					},
				},
				["use_difficulty"] = true,
				["encounterid"] = "2383",
				["size"] = {
					["multi"] = {
					},
				},
			},
			["fontSize"] = 12,
			["shadowXOffset"] = 1,
			["regionType"] = "text",
			["conditions"] = {
			},
			["displayText_format_p_time_dynamic_threshold"] = 60,
			["preferToUpdate"] = false,
			["displayText_format_p_time_precision"] = 1,
			["authorOptions"] = {
				{
					["type"] = "input",
					["useDesc"] = false,
					["width"] = 2,
					["key"] = "priority",
					["default"] = "John Steve James",
					["name"] = "Priority players (realm name not required)",
					["length"] = 10,
					["multiline"] = false,
					["useLength"] = false,
				}, -- [1]
			},
			["xOffset"] = 0,
			["shadowYOffset"] = -1,
			["semver"] = "1.0.4",
			["tocversion"] = 90002,
			["id"] = "Pescorus - Miasma priority marker",
			["config"] = {
				["priority"] = "John Steve James",
			},
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["automaticWidth"] = "Auto",
			["uid"] = "(mkC6skXALV",
			["justify"] = "LEFT",
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["fixedWidth"] = 200,
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["selfPoint"] = "BOTTOM",
		},
		["05 Volatile Ejection targeted AND Miasma - Big"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 4,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						1, -- [1]
						0.9921568627451, -- [2]
						0.9921568627451, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "ACShine",
					["glow_frequency"] = 0.25,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "C_ChatInfo.RegisterAddonMessagePrefix(\"RELOE_AURA_SYNCH\")\n\n\n\n",
					["do_custom"] = true,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2383",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "B2VVGzOK3he",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["useGroup_count"] = false,
						["groupclone"] = true,
						["duration"] = "1",
						["name_info"] = "aura",
						["subeventPrefix"] = "SPELL",
						["use_tooltip"] = false,
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["buffShowOn"] = "showOnActive",
						["auraspellids"] = {
							"306163", -- [1]
						},
						["debuffType"] = "HARMFUL",
						["custom"] = "function(states, e, ...)\n    \n    if e == \"CHAT_MSG_ADDON\" then\n        local prefix, msg, _, sender = ...\n        if prefix == \"RELOE_AURA_SYNCH\" then\n            sender = sender or UnitName(\"player\")\n            sender = gsub(sender, \"%-[^|]+\", \"\")\n            if not UnitExists(sender) or not UnitIsVisible(sender) then return end\n            \n            local spell, G, u = strsplit(\" \", msg)\n            \n            if tonumber(spell) == 334064 and WA_GetUnitDebuff(u, 329298) then\n                states[G] = {\n                    show = true,\n                    unit = u,\n                    duration = 4.5,\n                    expirationTime = GetTime()+4.5,\n                    autoHide = true,\n                    changed = true,\n                }\n                return true\n            end\n        end\n    end\nend\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "custom",
						["use_debuffClass"] = false,
						["subeventSuffix"] = "_CAST_START",
						["events"] = "CHAT_MSG_ADDON",
						["matchesShowOn"] = "showOnActive",
						["event"] = "Combat Log",
						["unit"] = "group",
						["auranames"] = {
						},
						["unevent"] = "timed",
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["custom_type"] = "stateupdate",
						["check"] = "event",
						["combineMatches"] = "showLowest",
						["useExactSpellId"] = true,
						["use_specific_unit"] = false,
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["wordWrap"] = "WordWrap",
			["config"] = {
			},
			["text1"] = "%n",
			["text1Containment"] = "INSIDE",
			["width"] = 6.9999866485596,
			["justify"] = "LEFT",
			["semver"] = "1.0.26",
			["zoom"] = 0,
			["id"] = "05 Volatile Ejection targeted AND Miasma - Big",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["frameStrata"] = 9,
			["anchorFrameType"] = "SCREEN",
			["text1Point"] = "CENTER",
			["text2Font"] = "Friz Quadrata TT",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["Position"] = {
			["xOffset"] = 0,
			["preferToUpdate"] = false,
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/sFnb6lvjO/5",
			["actions"] = {
				["start"] = {
					["do_message"] = false,
				},
				["finish"] = {
				},
				["init"] = {
					["custom"] = "aura_env.index = 1\naura_env.lastEvent = 0\naura_env.debuffId = 329951\n\nif aura_env.config.debug then\n    aura_env.debuffId = 48438\nend",
					["do_custom"] = true,
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "custom",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HELPFUL",
						["event"] = "Health",
						["unit"] = "player",
						["events"] = "CLEU:SPELL_AURA_APPLIED",
						["custom"] = "function(states,event, timestamp, subEvent, hideCaster, sourceGUID, sourceName, sourceFlags, sourceRaidFlags, destGUID, destName, destFlags, destRaidFlags, spellId)\n    \n    if (WeakAuras.IsOptionsOpen()) then\n        for i=1, 4 do\n            states[i] = states[i] or {\n                show = true,\n                changed = true,\n                position = i,\n                progressType = \"timed\",\n                expirationTime = GetTime() + 6,\n                duration = 6,\n                me = i == 2\n            }\n            \n        end \n    end\n    \n    if (subEvent == \"SPELL_AURA_APPLIED\" and spellId == aura_env.debuffId) then\n        aura_env.index = GetTime() - aura_env.lastEvent > 10 and 1 or aura_env.index + 1\n        aura_env.lastEvent = GetTime()\n        states[timestamp .. destName] = states[timestamp] or {\n            show = true,\n            changed = true,\n            position = aura_env.index,\n            progressType = \"timed\",\n            expirationTime = GetTime() + 6,\n            duration = 6,\n            autoHide = true,\n            me = UnitIsUnit(destName, 'player')\n            \n        }\n    end\n    \n    \n    local have = false\n    \n    for _, state in pairs(states) do\n        if state.show and state.me then\n            have = true\n        end\n    end\n    \n    return have\nend",
						["spellIds"] = {
						},
						["names"] = {
						},
						["check"] = "event",
						["subeventPrefix"] = "SPELL",
						["custom_type"] = "stateupdate",
						["customVariables"] = "{\n  me = \"bool\"\n}",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["colorR"] = 1,
					["use_rotate"] = true,
					["scalex"] = 1,
					["alphaType"] = "custom",
					["colorA"] = 1,
					["colorG"] = 1,
					["alphaFunc"] = "function(progress, start, delta)\n    if (aura_env.state and aura_env.state.me) then\n    local angle = (progress * 2 * math.pi) - (math.pi / 2)\n    return start + (((math.sin(angle) + 1)/2) * delta)\nend\nreturn start\nend",
					["use_alpha"] = true,
					["type"] = "custom",
					["easeType"] = "none",
					["scaley"] = 1,
					["alpha"] = 0.5,
					["duration"] = "0.6",
					["y"] = 0,
					["x"] = 0,
					["rotateType"] = "custom",
					["rotate"] = 0,
					["easeStrength"] = 3,
					["rotateFunc"] = "function(progress, start, delta)\n    if (aura_env.state and aura_env.state.position) then\n        local pos = aura_env.state.position\n        if (pos == 1 ) then\n            return start + 270\n        elseif (pos == 2) then\n            return start + 180\n        elseif (pos == 3) then\n            return start + 90\n        elseif (pos == 4) then\n            return start \n        end\n    end\n    \n    return start\nend",
					["colorB"] = 1,
					["duration_type"] = "seconds",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["desaturate"] = false,
			["rotation"] = 0,
			["version"] = 5,
			["subRegions"] = {
				{
					["text_text_format_n_format"] = "none",
					["text_text"] = "%position",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						0.96862745098039, -- [1]
						0.92156862745098, -- [2]
						1, -- [3]
						0.69221988320351, -- [4]
					},
					["text_font"] = "Expressway",
					["text_shadowYOffset"] = -1,
					["text_shadowXOffset"] = 1,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "None",
					["text_anchorPoint"] = "CENTER",
					["text_text_format_position_format"] = "none",
					["text_fontSize"] = 30,
					["text_text_format_postion_format"] = "none",
					["anchorXOffset"] = 0,
					["text_visible"] = true,
				}, -- [1]
				{
					["text_text_format_n_format"] = "none",
					["text_text"] = "%p",
					["text_text_format_p_format"] = "timed",
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["text_text_format_p_time_precision"] = 1,
					["text_text_format_p_time_dynamic_threshold"] = 60,
					["type"] = "subtext",
					["text_anchorXOffset"] = 0,
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Expressway",
					["text_shadowXOffset"] = 1,
					["text_anchorYOffset"] = -30,
					["text_shadowYOffset"] = -1,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "None",
					["text_anchorPoint"] = "CENTER",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_visible"] = false,
					["text_fontSize"] = 15,
					["anchorXOffset"] = 0,
					["text_text_format_p_time_format"] = 0,
				}, -- [2]
			},
			["height"] = 100,
			["rotate"] = true,
			["load"] = {
				["talent"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2407",
				["use_encounterid"] = true,
				["difficulty"] = {
					["single"] = "mythic",
				},
				["zoneIds"] = "",
				["spec"] = {
					["multi"] = {
					},
				},
				["use_difficulty"] = true,
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["textureWrapMode"] = "CLAMPTOBLACKADDITIVE",
			["mirror"] = false,
			["regionType"] = "texture",
			["blendMode"] = "BLEND",
			["anchorFrameType"] = "SCREEN",
			["texture"] = "Interface\\AddOns\\WeakAuras\\Media\\Textures\\Triangle45",
			["parent"] = "Impale Positions",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0.49999964237213, -- [4]
			},
			["semver"] = "1.0.4",
			["tocversion"] = 90005,
			["id"] = "Position",
			["selfPoint"] = "CENTER",
			["frameStrata"] = 1,
			["width"] = 100,
			["uid"] = "psVbcrIKQ5V",
			["config"] = {
				["debug"] = false,
			},
			["authorOptions"] = {
				{
					["type"] = "toggle",
					["key"] = "debug",
					["default"] = false,
					["name"] = "Debug",
					["useDesc"] = false,
					["width"] = 1,
				}, -- [1]
			},
			["alpha"] = 1,
			["conditions"] = {
				{
					["check"] = {
						["trigger"] = 1,
						["variable"] = "me",
						["value"] = 1,
					},
					["changes"] = {
						{
							["value"] = {
								1, -- [1]
								0.007843137254902, -- [2]
								0, -- [3]
								1, -- [4]
							},
							["property"] = "sub.1.text_color",
						}, -- [1]
						{
							["value"] = 53,
							["property"] = "sub.1.text_fontSize",
						}, -- [2]
						{
							["value"] = {
								0.96862745098039, -- [1]
								1, -- [2]
								0, -- [3]
								1, -- [4]
							},
							["property"] = "color",
						}, -- [3]
					},
				}, -- [1]
			},
			["information"] = {
			},
			["discrete_rotation"] = 0,
		},
		["Innervate Send"] = {
			["outline"] = "OUTLINE",
			["iconSource"] = -1,
			["xOffset"] = 0,
			["displayText"] = "",
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["cooldownSwipe"] = true,
			["displayText_format_p_time_format"] = 0,
			["customTextUpdate"] = "event",
			["cooldownEdge"] = false,
			["actions"] = {
				["start"] = {
				},
				["finish"] = {
				},
				["init"] = {
					["do_custom"] = true,
					["custom"] = "local AceComm = LibStub(\"AceComm-3.0\")\n\nfunction aura_env:Request()\n    local dist = \"PARTY\"\n    if IsInRaid() then\n        dist = \"RAID\"\n    elseif IsInGroup() == \"LE_PARTY_CATEGORY_INSTANCE\" then\n        dist = \"INSTANCE_CHAT\"\n    end\n    AceComm:SendCommMessage(\"TOSH_WA\", \"INNRV:me\", dist)\nend",
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "custom",
						["subeventSuffix"] = "_CAST_START",
						["duration"] = "1",
						["event"] = "Health",
						["subeventPrefix"] = "SPELL",
						["debuffType"] = "HELPFUL",
						["unit"] = "player",
						["spellIds"] = {
						},
						["names"] = {
						},
						["custom_type"] = "event",
						["custom"] = "function(event, ...)\n    if event == \"TOSH_WA_INNRV_ASK\" then\n        aura_env:Request()\n    end\nend\n",
						["events"] = "TOSH_WA_INNRV_ASK",
						["custom_hide"] = "timed",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["displayText_format_p_format"] = "timed",
			["internalVersion"] = 45,
			["keepAspectRatio"] = false,
			["wordWrap"] = "WordWrap",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["subRegions"] = {
			},
			["height"] = 64,
			["load"] = {
				["talent"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["class"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["frameStrata"] = 1,
			["fontSize"] = 12,
			["displayText_format_p_time_dynamic_threshold"] = 60,
			["information"] = {
			},
			["shadowXOffset"] = 1,
			["conditions"] = {
			},
			["automaticWidth"] = "Auto",
			["url"] = "",
			["regionType"] = "text",
			["authorOptions"] = {
				{
					["fontSize"] = "large",
					["type"] = "description",
					["text"] = "To use, add to a macro (or create a new one):\n\n/script WeakAuras.ScanEvents(\"TOSH_WA_INNRV_ASK\")\n",
					["width"] = 2,
				}, -- [1]
			},
			["uid"] = "QOZljllHaQn",
			["icon"] = true,
			["width"] = 64,
			["displayText_format_p_time_precision"] = 1,
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["cooldownTextDisabled"] = false,
			["zoom"] = 0,
			["justify"] = "LEFT",
			["tocversion"] = 90002,
			["id"] = "Innervate Send",
			["selfPoint"] = "CENTER",
			["alpha"] = 1,
			["anchorFrameType"] = "SCREEN",
			["shadowYOffset"] = -1,
			["config"] = {
			},
			["inverse"] = false,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["fixedWidth"] = 200,
			["cooldown"] = false,
			["preferToUpdate"] = false,
		},
		["09 Wicked Blade - White"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 4,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.25,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = true,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2417",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "7DVVseMGXho",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["useMatch_count"] = true,
						["auranames"] = {
						},
						["groupclone"] = true,
						["matchesShowOn"] = "showOnActive",
						["name_info"] = "aura",
						["unit"] = "group",
						["use_tooltip"] = false,
						["subeventPrefix"] = "SPELL",
						["match_countOperator"] = ">=",
						["match_count"] = "1",
						["buffShowOn"] = "showOnActive",
						["unevent"] = "timed",
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["useExactSpellId"] = true,
						["custom_type"] = "stateupdate",
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["event"] = "Health",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HARMFUL",
						["use_specific_unit"] = false,
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["auraspellids"] = {
							"333377", -- [1]
						},
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["duration"] = "1",
						["useGroup_count"] = false,
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["text2Font"] = "Friz Quadrata TT",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text1"] = "%n",
			["text1Point"] = "CENTER",
			["anchorFrameType"] = "SCREEN",
			["semver"] = "1.0.26",
			["justify"] = "LEFT",
			["zoom"] = 0,
			["id"] = "09 Wicked Blade - White",
			["config"] = {
			},
			["frameStrata"] = 9,
			["width"] = 6.9999866485596,
			["text1Containment"] = "INSIDE",
			["wordWrap"] = "WordWrap",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["Druid_Solar_Beam 2"] = {
			["sparkWidth"] = 10,
			["iconSource"] = -1,
			["authorOptions"] = {
			},
			["preferToUpdate"] = false,
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["sparkRotation"] = 0,
			["url"] = "https://wago.io/ZG34ifiRf/1",
			["backgroundColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				0.4909183382988, -- [4]
			},
			["fontFlags"] = "OUTLINE",
			["icon_color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["selfPoint"] = "CENTER",
			["barColor"] = {
				1, -- [1]
				0.49803921568628, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["desaturate"] = false,
			["sparkOffsetY"] = 0,
			["load"] = {
				["ingroup"] = {
					["multi"] = {
					},
				},
				["use_never"] = false,
				["class"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["level"] = "100",
				["size"] = {
					["single"] = "party",
					["multi"] = {
						["ratedpvp"] = true,
						["arena"] = true,
						["ratedarena"] = true,
						["pvp"] = true,
					},
				},
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_level"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["difficulty"] = {
					["single"] = "challenge",
					["multi"] = {
						["challenge"] = true,
					},
				},
				["zoneIds"] = "",
				["faction"] = {
					["multi"] = {
					},
				},
				["use_size"] = false,
				["use_zone"] = false,
				["race"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
			},
			["useAdjustededMin"] = false,
			["regionType"] = "aurabar",
			["texture"] = "Glamour4",
			["sparkTexture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
			["spark"] = false,
			["tocversion"] = 80300,
			["alpha"] = 1,
			["uid"] = "dkWo8rdOZxV",
			["displayIcon"] = "Interface\\Icons\\spell_holy_rebuke",
			["stacksPoint"] = "BOTTOMRIGHT",
			["sparkOffsetX"] = 0,
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["sparkRotationMode"] = "AUTO",
			["triggers"] = {
				{
					["trigger"] = {
						["duration"] = "60",
						["spellId"] = "",
						["names"] = {
							"Greater Draenic Intellect Flask", -- [1]
						},
						["use_sourceName"] = false,
						["ownOnly"] = true,
						["genericShowOn"] = "showOnActive",
						["subeventPrefix"] = "SPELL",
						["custom_hide"] = "timed",
						["use_sourceUnit"] = false,
						["spellIds"] = {
						},
						["use_spellId"] = false,
						["type"] = "combatlog",
						["use_destName"] = false,
						["use_cloneId"] = true,
						["debuffType"] = "HELPFUL",
						["use_sourceFlags2"] = true,
						["sourceName"] = "",
						["unevent"] = "timed",
						["subeventSuffix"] = "_CAST_SUCCESS",
						["inverse"] = true,
						["spellName"] = "Solar Beam",
						["event"] = "Combat Log",
						["sourceFlags2"] = "Hostile",
						["use_name"] = true,
						["use_spellName"] = true,
						["name"] = "Flask of the warm sun ",
						["use_specific_sourceunit"] = true,
						["use_destunit"] = false,
						["use_destUnit"] = false,
						["sourceUnit"] = "Benzikura",
						["unit"] = "player",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
			},
			["backdropInFront"] = false,
			["stickyDuration"] = false,
			["version"] = 1,
			["subRegions"] = {
				{
					["type"] = "subborder",
					["border_anchor"] = "bar",
					["border_size"] = 16,
					["border_color"] = {
						1, -- [1]
						0.57254901960784, -- [2]
						0, -- [3]
						0.5, -- [4]
					},
					["border_visible"] = true,
					["border_edge"] = "1 Pixel",
					["border_offset"] = 2,
				}, -- [1]
				{
					["type"] = "aurabar_bar",
				}, -- [2]
				{
					["text_shadowXOffset"] = 1,
					["text_text"] = "%p",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Vixar",
					["text_text_format_p_time_format"] = 0,
					["text_shadowYOffset"] = -1,
					["text_text_format_p_time_dynamic_threshold"] = 3,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "ICON_CENTER",
					["text_text_format_p_format"] = "timed",
					["text_visible"] = true,
					["text_fontSize"] = 20,
					["anchorXOffset"] = 0,
					["text_text_format_p_time_precision"] = 1,
				}, -- [3]
			},
			["height"] = 30,
			["sparkBlendMode"] = "ADD",
			["backdropColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0.5, -- [4]
			},
			["displayStacks"] = "%c",
			["config"] = {
			},
			["sparkColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["actions"] = {
				["start"] = {
					["do_message"] = false,
					["message_type"] = "SAY",
				},
				["init"] = {
				},
				["finish"] = {
				},
			},
			["borderInFront"] = false,
			["borderBackdrop"] = "Blizzard Tooltip",
			["icon_side"] = "LEFT",
			["xOffset"] = 0,
			["zoom"] = 0.28,
			["sparkHeight"] = 30,
			["icon"] = true,
			["stacksContainment"] = "INSIDE",
			["auto"] = true,
			["semver"] = "1.0.0",
			["anchorFrameType"] = "SCREEN",
			["id"] = "Druid_Solar_Beam 2",
			["parent"] = "Enemy Interrupt Icons",
			["frameStrata"] = 1,
			["width"] = 30,
			["sparkHidden"] = "NEVER",
			["useAdjustededMax"] = false,
			["inverse"] = false,
			["sparkDesature"] = false,
			["orientation"] = "HORIZONTAL",
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["customTextUpdate"] = "update",
		},
		["Castle Nathria Glows"] = {
			["controlledChildren"] = {
				"01 Echolocation - White", -- [1]
				"01 Blood Lantern - Green", -- [2]
				"01 Horrified - Red", -- [3]
				"01 Exsanguinated High Stacks - Black", -- [4]
				"02 Sinseeker - White", -- [5]
				"02 Sinseeker Targeted - Pink", -- [6]
				"02 Vicious Lunge - Green", -- [7]
				"02 Vicious Wound - Red", -- [8]
				"02 Jagged Claws High Stacks - Orange", -- [9]
				"02 Petrifying Howl - White", -- [10]
				"03 Greater Castigation - White", -- [11]
				"03 Crimson Torment - Red", -- [12]
				"03 Lingering Embers >1stack - Orange", -- [13]
				"03 Ember Blast - Green", -- [14]
				"04 Displacement Cypher - Green", -- [15]
				"04 Glyph of Destruction - Yellow", -- [16]
				"04 Withering Touch - White", -- [17]
				"05 Gluttonous Miasma - Black", -- [18]
				"05 Volatile Ejection targeted - White", -- [19]
				"05 Volatile Ejection targeted AND Miasma - Big", -- [20]
				"06 Change of Heart - Orange", -- [21]
				"06 Shared Cognition - White", -- [22]
				"06 Shared Suffering - Yellow", -- [23]
				"06 Concentrate Anima - Green", -- [24]
				"07 Drain Essence - White", -- [25]
				"07 Dark Recital Partner - Yellow ", -- [26]
				"08 Hateful Gaze - Pink", -- [27]
				"08 Chain Slam - Green", -- [28]
				"09 Heart Rend - Teal - disabled", -- [29]
				"09 Rended Heart - Red", -- [30]
				"09 Wicked Blade - White", -- [31]
				"09 Ravenous Feast - Pink", -- [32]
				"09 Wicked Laceration >= 2 stacks - Orange", -- [33]
				"09 Volatile Anima Infusion - Green", -- [34]
				"10 Night Hunter - Green", -- [35]
				"10 Impale - Red", -- [36]
				"10 Searing Censure - Teal", -- [37]
				"10 Final Finesse - White", -- [38]
			},
			["borderBackdrop"] = "Blizzard Tooltip",
			["xOffset"] = 0,
			["preferToUpdate"] = false,
			["groupIcon"] = 3614361,
			["anchorPoint"] = "CENTER",
			["borderColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
				},
				["init"] = {
				},
				["finish"] = {
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["debuffType"] = "HELPFUL",
						["type"] = "aura2",
						["spellIds"] = {
						},
						["subeventSuffix"] = "_CAST_START",
						["subeventPrefix"] = "SPELL",
						["names"] = {
						},
						["event"] = "Health",
						["unit"] = "player",
					},
					["untrigger"] = {
					},
				}, -- [1]
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["version"] = 27,
			["subRegions"] = {
			},
			["load"] = {
				["talent"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["spec"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["backdropColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0.5, -- [4]
			},
			["scale"] = 1,
			["border"] = false,
			["borderEdge"] = "Square Full White",
			["regionType"] = "group",
			["borderSize"] = 2,
			["borderOffset"] = 4,
			["semver"] = "1.0.26",
			["tocversion"] = 90002,
			["id"] = "Castle Nathria Glows",
			["borderInset"] = 1,
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["selfPoint"] = "BOTTOMLEFT",
			["uid"] = "EazKeH6nUbn",
			["yOffset"] = 0,
			["config"] = {
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
				["groupOffset"] = true,
			},
			["authorOptions"] = {
			},
		},
		["Cast Targeted Alert"] = {
			["sparkWidth"] = 10,
			["iconSource"] = -1,
			["xOffset"] = 0,
			["displayText"] = "%c",
			["yOffset"] = 200,
			["anchorPoint"] = "CENTER",
			["sparkRotation"] = 0,
			["url"] = "https://wago.io/kJFQFy0V8/3",
			["actions"] = {
				["start"] = {
					["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\AirHorn.ogg",
					["do_sound"] = false,
				},
				["finish"] = {
				},
				["init"] = {
					["custom"] = "aura_env.icons= { \n    [1]  = \n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_1:12\\124t\",\n    [2] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_2:12\\124t\", \n    [3] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_3:12\\124t\", \n    [4] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_4:12\\124t\", \n    [5] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_5:12\\124t\", \n    [6] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_6:12\\124t\",   \n    [7] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_7:12\\124t\",\n    [8] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_8:12\\124t\",\n    [9] =\n    \" Right of \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_1:12\\124t\",\n    [10] =\n    \" 10\",\n    [11] =\n    \" 11\",\n    [12] =\n    \" 12\",\n    [13] =\n    \" 13\",\n    [14] =\n    \" 14\",\n    [15] =\n    \" 15\",\n}",
					["do_custom"] = true,
				},
			},
			["icon_color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["selfPoint"] = "BOTTOM",
			["barColor"] = {
				1, -- [1]
				0.031372549019608, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["sparkOffsetY"] = 0,
			["load"] = {
				["use_encounterid"] = true,
				["zoneIds"] = "",
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2417",
				["spec"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["shadowXOffset"] = 1,
			["useAdjustededMin"] = false,
			["regionType"] = "aurabar",
			["texture"] = "Details Flat",
			["sparkTexture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
			["spark"] = false,
			["tocversion"] = 90002,
			["alpha"] = 1,
			["sparkColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["fixedWidth"] = 200,
			["outline"] = "OUTLINE",
			["sparkOffsetX"] = 0,
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "function()\n    aura_env.mark = aura_env.mark or \"\"\n    if aura_env.state and aura_env.state.marker then\n        return \"GO TO:\"..aura_env.icons[aura_env.state.marker]\n    end\nend",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["debuffType"] = "HELPFUL",
						["type"] = "custom",
						["subeventSuffix"] = "_CAST_START",
						["custom_type"] = "stateupdate",
						["names"] = {
						},
						["duration"] = "5.2",
						["event"] = "Health",
						["subeventPrefix"] = "SPELL",
						["dynamicDuration"] = false,
						["custom"] = "function(states, event, mark)\n    if mark then \n        states [\"\"] = {\n            show = true,\n            progressType = \"timed\",\n            duration = 5.3,\n            expirationTime = GetTime()+5.3,\n            autoHide = true,\n            changed = true,\n            marker = mark\n        }\n        return true\n    end\nend",
						["events"] = "RELOE_CAST_ALERT",
						["spellIds"] = {
						},
						["check"] = "event",
						["unit"] = "player",
						["customVariables"] = "{\n    marker = \"number\"\n    }\n    \n    ",
						["custom_hide"] = "timed",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["displayText_format_p_format"] = "timed",
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["version"] = 3,
			["subRegions"] = {
				{
					["type"] = "aurabar_bar",
				}, -- [1]
				{
					["text_text_format_n_format"] = "none",
					["text_text"] = "%c",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_anchorXOffset"] = 0,
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Friz Quadrata TT",
					["text_shadowYOffset"] = -1,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "None",
					["text_anchorPoint"] = "INNER_TOP",
					["text_anchorYOffset"] = 40,
					["text_shadowXOffset"] = 1,
					["text_fontSize"] = 40,
					["anchorXOffset"] = 0,
					["text_visible"] = true,
				}, -- [2]
				{
					["text_text_format_p_time_precision"] = 1,
					["text_text"] = "%p",
					["text_text_format_p_format"] = "Number",
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["text_text_format_p_decimal_precision"] = 1,
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "ExpressWay",
					["text_shadowYOffset"] = -1,
					["text_shadowXOffset"] = 1,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "None",
					["text_anchorPoint"] = "INNER_RIGHT",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_text_format_n_format"] = "none",
					["text_fontSize"] = 20,
					["anchorXOffset"] = 0,
					["text_visible"] = true,
				}, -- [3]
				{
					["type"] = "subborder",
					["border_anchor"] = "bar",
					["border_size"] = 2,
					["border_color"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["border_visible"] = true,
					["border_edge"] = "Square Full White",
					["border_offset"] = 0,
				}, -- [4]
			},
			["height"] = 40,
			["sparkBlendMode"] = "ADD",
			["useAdjustededMax"] = false,
			["fontSize"] = 60,
			["preferToUpdate"] = false,
			["parent"] = "09 Generals Soak Alerts",
			["uid"] = "XWS4jUTXJUn",
			["orientation"] = "HORIZONTAL",
			["displayText_format_p_time_dynamic"] = false,
			["icon_side"] = "RIGHT",
			["config"] = {
			},
			["displayText_format_p_time_precision"] = 1,
			["sparkHeight"] = 30,
			["sparkRotationMode"] = "AUTO",
			["anchorFrameType"] = "SCREEN",
			["zoom"] = 0,
			["semver"] = "1.0.2",
			["justify"] = "LEFT",
			["id"] = "Cast Targeted Alert",
			["sparkHidden"] = "NEVER",
			["frameStrata"] = 1,
			["width"] = 350,
			["backgroundColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				0.5, -- [4]
			},
			["icon"] = false,
			["inverse"] = false,
			["authorOptions"] = {
			},
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["value"] = "1",
						["variable"] = "marker",
					},
					["changes"] = {
						{
							["value"] = {
								0.97254901960784, -- [1]
								1, -- [2]
								0, -- [3]
								1, -- [4]
							},
							["property"] = "barColor",
						}, -- [1]
						{
							["value"] = {
								["sound_type"] = "Play",
								["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\Star.ogg",
								["sound_channel"] = "Master",
							},
							["property"] = "sound",
						}, -- [2]
					},
				}, -- [1]
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["value"] = "2",
						["variable"] = "marker",
					},
					["changes"] = {
						{
							["value"] = {
								1, -- [1]
								0.53725490196078, -- [2]
								0, -- [3]
								1, -- [4]
							},
							["property"] = "barColor",
						}, -- [1]
						{
							["value"] = {
								["sound_type"] = "Play",
								["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\Circle.ogg",
								["sound_channel"] = "Master",
							},
							["property"] = "sound",
						}, -- [2]
					},
				}, -- [2]
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["value"] = "3",
						["variable"] = "marker",
					},
					["changes"] = {
						{
							["value"] = {
								0.56862745098039, -- [1]
								0, -- [2]
								1, -- [3]
								1, -- [4]
							},
							["property"] = "barColor",
						}, -- [1]
						{
							["value"] = {
								["sound_type"] = "Play",
								["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\Diamond.ogg",
								["sound_channel"] = "Master",
							},
							["property"] = "sound",
						}, -- [2]
					},
				}, -- [3]
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["value"] = "4",
						["variable"] = "marker",
					},
					["changes"] = {
						{
							["value"] = {
								0.12549019607843, -- [1]
								1, -- [2]
								0, -- [3]
								1, -- [4]
							},
							["property"] = "barColor",
						}, -- [1]
						{
							["value"] = {
								["sound_type"] = "Play",
								["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\Triangle.ogg",
								["sound_channel"] = "Master",
							},
							["property"] = "sound",
						}, -- [2]
					},
				}, -- [4]
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["value"] = "5",
						["variable"] = "marker",
					},
					["changes"] = {
						{
							["value"] = {
								0.52156862745098, -- [1]
								1, -- [2]
								0.95686274509804, -- [3]
								1, -- [4]
							},
							["property"] = "barColor",
						}, -- [1]
						{
							["value"] = {
								["sound_type"] = "Play",
								["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\Moon.ogg",
								["sound_channel"] = "Master",
							},
							["property"] = "sound",
						}, -- [2]
					},
				}, -- [5]
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["value"] = "6",
						["variable"] = "marker",
					},
					["changes"] = {
						{
							["value"] = {
								0, -- [1]
								0.40392156862745, -- [2]
								1, -- [3]
								1, -- [4]
							},
							["property"] = "barColor",
						}, -- [1]
						{
							["value"] = {
								["sound_type"] = "Play",
								["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\Square.ogg",
								["sound_channel"] = "Master",
							},
							["property"] = "sound",
						}, -- [2]
					},
				}, -- [6]
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["value"] = "7",
						["variable"] = "marker",
					},
					["changes"] = {
						{
							["value"] = {
								1, -- [1]
								0.066666666666667, -- [2]
								0, -- [3]
								1, -- [4]
							},
							["property"] = "barColor",
						}, -- [1]
						{
							["value"] = {
								["sound_type"] = "Play",
								["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\Cross.ogg",
								["sound_channel"] = "Master",
							},
							["property"] = "sound",
						}, -- [2]
					},
				}, -- [7]
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["value"] = "8",
						["variable"] = "marker",
					},
					["changes"] = {
						{
							["property"] = "barColor",
						}, -- [1]
						{
							["value"] = {
								["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\Skull.ogg",
								["sound_type"] = "Play",
							},
							["property"] = "sound",
						}, -- [2]
					},
				}, -- [8]
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["value"] = "9",
						["variable"] = "marker",
					},
					["changes"] = {
						{
							["value"] = {
								0.95686274509804, -- [1]
								1, -- [2]
								0, -- [3]
								1, -- [4]
							},
							["property"] = "barColor",
						}, -- [1]
						{
							["value"] = {
								["sound_type"] = "Play",
								["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\Star.ogg",
								["sound_channel"] = "Master",
							},
							["property"] = "sound",
						}, -- [2]
					},
				}, -- [9]
			},
			["information"] = {
			},
			["wordWrap"] = "WordWrap",
		},
		["07 Seismic Upheaval Left"] = {
			["iconSource"] = -1,
			["xOffset"] = 0,
			["preferToUpdate"] = false,
			["yOffset"] = 200,
			["anchorPoint"] = "CENTER",
			["cooldownSwipe"] = true,
			["url"] = "https://wago.io/HcLsyDhV7/2",
			["icon"] = true,
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "combatlog",
						["spellId"] = "334498",
						["unevent"] = "timed",
						["duration"] = "5.1",
						["event"] = "Combat Log",
						["subeventPrefix"] = "SPELL",
						["destUnit"] = "player",
						["use_spellId"] = true,
						["spellIds"] = {
						},
						["subeventSuffix"] = "_AURA_APPLIED",
						["names"] = {
						},
						["use_destUnit"] = true,
						["unit"] = "player",
						["debuffType"] = "HARMFUL",
					},
					["untrigger"] = {
					},
				}, -- [1]
				{
					["trigger"] = {
						["type"] = "aura2",
						["match_countOperator"] = ">=",
						["unit"] = "group",
						["debuffType"] = "HARMFUL",
						["auraspellids"] = {
							"334498", -- [1]
						},
						["match_count"] = "1",
						["useMatch_count"] = true,
						["useExactSpellId"] = true,
						["useIgnoreName"] = false,
					},
					["untrigger"] = {
					},
				}, -- [2]
				["disjunctive"] = "custom",
				["customTriggerLogic"] = "function(t)\n    return t[2] and not t[1]\nend",
				["activeTriggerMode"] = -10,
			},
			["internalVersion"] = 45,
			["keepAspectRatio"] = false,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["desaturate"] = false,
			["rotation"] = 270,
			["version"] = 2,
			["subRegions"] = {
			},
			["height"] = 100,
			["rotate"] = true,
			["load"] = {
				["size"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["talent"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2417",
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
			},
			["textureWrapMode"] = "CLAMPTOBLACKADDITIVE",
			["config"] = {
			},
			["color"] = {
				1, -- [1]
				0, -- [2]
				0.031372549019608, -- [3]
				1, -- [4]
			},
			["actions"] = {
				["start"] = {
					["do_custom"] = true,
					["custom"] = "if DBM then\n    aura_env.region.texture:SetTexture(\"Interface\\\\Addons\\\\DBM-Core\\\\textures\\\\arrows\\\\arrow.blp\", false)\n    aura_env.region.texture:SetTexCoord(0, 1/9, 3/12-1/50, 4/12-1/50)\nend",
					["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\Left.ogg",
					["do_sound"] = true,
				},
				["init"] = {
				},
				["finish"] = {
				},
			},
			["mirror"] = false,
			["authorOptions"] = {
			},
			["regionType"] = "texture",
			["information"] = {
			},
			["blendMode"] = "BLEND",
			["selfPoint"] = "CENTER",
			["cooldownEdge"] = false,
			["texture"] = "Interface\\AddOns\\WeakAuras\\Media\\Textures\\targeting-mark.tga",
			["alpha"] = 1,
			["zoom"] = 0,
			["semver"] = "1.0.1",
			["tocversion"] = 90005,
			["id"] = "07 Seismic Upheaval Left",
			["width"] = 100,
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["cooldownTextDisabled"] = false,
			["uid"] = "TO6Luokukyt",
			["inverse"] = false,
			["discrete_rotation"] = 0,
			["conditions"] = {
			},
			["cooldown"] = false,
			["parent"] = "09 Seismic Upheaval Group",
		},
		["10 Crescendo"] = {
			["iconSource"] = 0,
			["authorOptions"] = {
			},
			["preferToUpdate"] = false,
			["yOffset"] = 360.8335571289063,
			["anchorPoint"] = "CENTER",
			["cooldownSwipe"] = true,
			["cooldownEdge"] = false,
			["actions"] = {
				["start"] = {
					["sound"] = " custom",
					["do_sound"] = false,
				},
				["init"] = {
					["custom"] = "aura_env.GUIDS = {\n    [\"173161\"] = true,\n    [\"174161\"] = true,\n    [\"174134\"] = true,\n    [\"173162\"] = true,\n    [\"174126\"] = true,\n    [\"173163\"] = true,\n    [\"174162\"] = true,\n    [\"173164\"] = true,\n    [\"169196\"] = true,\n    [\"169470\"] = true,\n}",
					["do_custom"] = true,
				},
				["finish"] = {
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["spellId"] = "",
						["auranames"] = {
						},
						["duration"] = "2",
						["subeventPrefix"] = "UNIT_DIED",
						["names"] = {
						},
						["custom_hide"] = "timed",
						["type"] = "custom",
						["use_cloneId"] = false,
						["debuffType"] = "HARMFUL",
						["unevent"] = "timed",
						["useName"] = true,
						["use_destNpcId"] = false,
						["custom_type"] = "event",
						["events"] = "CLEU:UNIT_DIED",
						["fetchTooltip"] = false,
						["event"] = "Combat Log",
						["custom"] = "function(event, ...)\n    if event == \"COMBAT_LOG_EVENT_UNFILTERED\" then\n        local timestamp, subEvent, hideCaster, sourceGUID, sourceName, sourceFlags, sourceRaidFlags, destGUID, destName, destFlags, destRaidFlags, spellID, _,_, auraType = ...\n        if subEvent == \"UNIT_DIED\" \n        and aura_env.GUIDS[select(6, strsplit(\"-\", destGUID))] then\n            return true\n        end\n    end\nend",
						["unit"] = "player",
						["use_spellId"] = false,
						["spellIds"] = {
						},
						["destNpcId"] = "",
						["check"] = "event",
						["use_destUnit"] = false,
						["subeventSuffix"] = "",
						["customVariables"] = "{expirationTime = true}",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["activeTriggerMode"] = -10,
			},
			["internalVersion"] = 45,
			["keepAspectRatio"] = false,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["colorR"] = 1,
					["use_scale"] = false,
					["colorA"] = 1,
					["colorG"] = 0.07843137254902,
					["colorB"] = 0,
					["duration_type"] = "seconds",
					["use_translate"] = false,
					["easeStrength"] = 3,
					["scaleType"] = "custom",
					["type"] = "none",
					["preset"] = "alphaPulse",
					["easeType"] = "none",
					["translateFunc"] = "",
					["use_color"] = false,
					["alpha"] = 0,
					["x"] = 0.9,
					["y"] = 0,
					["colorType"] = "custom",
					["translateType"] = "custom",
					["scaley"] = 1,
					["colorFunc"] = "function(progress, r1, g1, b1, a1, r2, g2, b2, a2)\n    local angle = (progress * 4 * math.pi) - (math.pi / 2)\n    local newProgress = ((math.sin(angle) + 1)/2);\n    return WeakAuras.GetHSVTransition(newProgress, r1, g1, b1, a1, r2, g2, b2, a2)\nend",
					["rotate"] = 0,
					["scaleFunc"] = "function(progress, startX, startY, scaleX, scaleY)\n    return startX + (progress * (scaleX - startX)), startY + (progress * (scaleY - startY))\nend\n",
					["scalex"] = 1,
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["desaturate"] = false,
			["version"] = 132,
			["subRegions"] = {
				{
					["text_shadowXOffset"] = 0,
					["text_text_format_s_format"] = "none",
					["text_text"] = "Pool",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_text_format_tooltip1_format"] = "none",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Expressway",
					["text_shadowYOffset"] = 0,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "OUTER_BOTTOM",
					["text_fontSize"] = 16,
					["anchorXOffset"] = 0,
					["text_visible"] = true,
				}, -- [1]
				{
					["text_text_format_p_time_precision"] = 1,
					["text_text"] = "%p",
					["text_text_format_p_format"] = "timed",
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Expressway",
					["text_text_format_p_time_format"] = 0,
					["text_shadowYOffset"] = 0,
					["text_text_format_p_time_dynamic_threshold"] = 0,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "THICKOUTLINE",
					["text_anchorPoint"] = "CENTER",
					["text_visible"] = true,
					["text_shadowXOffset"] = 0,
					["text_fontSize"] = 25,
					["anchorXOffset"] = 0,
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
				}, -- [2]
				{
					["text_shadowXOffset"] = 0,
					["text_text_format_s_format"] = "none",
					["text_text"] = "%s",
					["text_text_format_p_format"] = "timed",
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["text_text_format_p_time_format"] = 0,
					["type"] = "subtext",
					["text_anchorXOffset"] = 6,
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Arial Narrow",
					["text_text_format_p_time_dynamic_threshold"] = 60,
					["text_shadowYOffset"] = 0,
					["text_visible"] = false,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "INNER_BOTTOMRIGHT",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_text_format_p_time_precision"] = 1,
					["text_fontSize"] = 25,
					["anchorXOffset"] = 0,
					["text_anchorYOffset"] = -6,
				}, -- [3]
			},
			["height"] = 80,
			["load"] = {
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
						["mythic"] = true,
						["heroic"] = true,
					},
				},
				["zoneIds"] = "",
				["encounterid"] = "2407",
				["use_difficulty"] = false,
				["spec"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["uid"] = "b4DDaTGg9YW",
			["useTooltip"] = false,
			["xOffset"] = -263.3328857421875,
			["regionType"] = "icon",
			["url"] = "https://wago.io/slraid1/132",
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["displayIcon"] = 3528310,
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["auto"] = false,
			["anchorFrameType"] = "SCREEN",
			["cooldownTextDisabled"] = true,
			["semver"] = "1.0.131",
			["tocversion"] = 90002,
			["id"] = "10 Crescendo",
			["alpha"] = 1,
			["frameStrata"] = 1,
			["width"] = 80,
			["zoom"] = 0.3,
			["config"] = {
			},
			["inverse"] = false,
			["icon"] = true,
			["conditions"] = {
			},
			["cooldown"] = true,
			["selfPoint"] = "CENTER",
		},
		["07 Dark Recital Group"] = {
			["controlledChildren"] = {
				"07 Dark Recital Right", -- [1]
				"07 Dark Recital Left", -- [2]
				"07 Dark Recital Stack Up Alert", -- [3]
			},
			["borderBackdrop"] = "Blizzard Tooltip",
			["authorOptions"] = {
			},
			["preferToUpdate"] = false,
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["borderColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["url"] = "https://wago.io/seLAtvE5B/1",
			["actions"] = {
				["start"] = {
				},
				["finish"] = {
				},
				["init"] = {
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["debuffType"] = "HELPFUL",
						["type"] = "aura2",
						["spellIds"] = {
						},
						["subeventSuffix"] = "_CAST_START",
						["subeventPrefix"] = "SPELL",
						["unit"] = "player",
						["event"] = "Health",
						["names"] = {
						},
					},
					["untrigger"] = {
					},
				}, -- [1]
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["version"] = 1,
			["subRegions"] = {
			},
			["load"] = {
				["talent"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["spec"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["backdropColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0.5, -- [4]
			},
			["scale"] = 1,
			["border"] = false,
			["borderEdge"] = "Square Full White",
			["regionType"] = "group",
			["borderSize"] = 2,
			["borderOffset"] = 4,
			["semver"] = "1.0.0",
			["tocversion"] = 90002,
			["id"] = "07 Dark Recital Group",
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["selfPoint"] = "CENTER",
			["uid"] = "Xp6Q2OY4xu1",
			["config"] = {
			},
			["borderInset"] = 1,
			["conditions"] = {
			},
			["information"] = {
			},
			["xOffset"] = 0,
		},
		["Mirror Text"] = {
			["outline"] = "OUTLINE",
			["xOffset"] = 0,
			["displayText"] = "Mirror",
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["displayText_format_p_time_format"] = 0,
			["customTextUpdate"] = "event",
			["url"] = "https://wago.io/sFnb6lvjO/5",
			["actions"] = {
				["start"] = {
				},
				["finish"] = {
				},
				["init"] = {
					["custom"] = "aura_env.region.header = true",
					["do_custom"] = true,
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "aura2",
						["subeventSuffix"] = "_CAST_START",
						["event"] = "Health",
						["names"] = {
						},
						["spellIds"] = {
						},
						["useName"] = true,
						["auranames"] = {
							"329951", -- [1]
						},
						["subeventPrefix"] = "SPELL",
						["unit"] = "player",
						["debuffType"] = "HARMFUL",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["displayText_format_p_format"] = "timed",
			["internalVersion"] = 45,
			["selfPoint"] = "BOTTOM",
			["font"] = "Expressway",
			["version"] = 5,
			["subRegions"] = {
			},
			["load"] = {
				["talent"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2407",
				["use_encounterid"] = true,
				["difficulty"] = {
					["single"] = "mythic",
				},
				["zoneIds"] = "",
				["spec"] = {
					["multi"] = {
					},
				},
				["use_difficulty"] = true,
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["fontSize"] = 24,
			["shadowXOffset"] = 1,
			["wordWrap"] = "WordWrap",
			["regionType"] = "text",
			["preferToUpdate"] = false,
			["authorOptions"] = {
			},
			["conditions"] = {
			},
			["displayText_format_p_time_precision"] = 1,
			["displayText_format_p_time_dynamic_threshold"] = 60,
			["color"] = {
				1, -- [1]
				0, -- [2]
				0.047058823529412, -- [3]
				1, -- [4]
			},
			["config"] = {
			},
			["semver"] = "1.0.4",
			["tocversion"] = 90005,
			["id"] = "Mirror Text",
			["automaticWidth"] = "Auto",
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["justify"] = "LEFT",
			["uid"] = "7oQt1teY69Z",
			["shadowYOffset"] = -1,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["fixedWidth"] = 200,
			["information"] = {
			},
			["parent"] = "Impale Positions",
		},
		["Infuser's Boon"] = {
			["iconSource"] = -1,
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["cooldownSwipe"] = true,
			["cooldownEdge"] = false,
			["icon"] = true,
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "aura2",
						["subeventSuffix"] = "_CAST_START",
						["event"] = "Health",
						["subeventPrefix"] = "SPELL",
						["spellIds"] = {
						},
						["auraspellids"] = {
							"326078", -- [1]
						},
						["useExactSpellId"] = true,
						["names"] = {
						},
						["unit"] = "player",
						["debuffType"] = "HELPFUL",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["internalVersion"] = 45,
			["keepAspectRatio"] = false,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["subRegions"] = {
				{
					["text_shadowXOffset"] = 0,
					["text_text_format_s_format"] = "none",
					["text_text"] = "%s",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Friz Quadrata TT",
					["text_shadowYOffset"] = 0,
					["text_wordWrap"] = "WordWrap",
					["text_visible"] = true,
					["text_anchorPoint"] = "INNER_BOTTOMRIGHT",
					["text_fontSize"] = 12,
					["anchorXOffset"] = 0,
					["text_fontType"] = "OUTLINE",
				}, -- [1]
				{
					["glowFrequency"] = 0.25,
					["type"] = "subglow",
					["useGlowColor"] = false,
					["glowType"] = "buttonOverlay",
					["glowLength"] = 10,
					["glowYOffset"] = 0,
					["glowColor"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["glowXOffset"] = 0,
					["glow"] = false,
					["glowScale"] = 1,
					["glowThickness"] = 1,
					["glowLines"] = 8,
					["glowBorder"] = false,
				}, -- [2]
			},
			["height"] = 64,
			["load"] = {
				["use_encounterid"] = true,
				["zoneIds"] = "",
				["talent"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2402",
				["size"] = {
					["multi"] = {
					},
				},
			},
			["regionType"] = "icon",
			["parent"] = "Tosh Nathria",
			["actions"] = {
				["start"] = {
				},
				["init"] = {
				},
				["finish"] = {
				},
			},
			["cooldown"] = false,
			["xOffset"] = 0,
			["zoom"] = 0,
			["authorOptions"] = {
			},
			["alpha"] = 1,
			["id"] = "Infuser's Boon",
			["config"] = {
			},
			["frameStrata"] = 1,
			["width"] = 64,
			["anchorFrameType"] = "SCREEN",
			["uid"] = "B78RKEQhoJA",
			["inverse"] = false,
			["cooldownTextDisabled"] = false,
			["conditions"] = {
			},
			["information"] = {
			},
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
		},
		["Dungeon - Targeted Spells"] = {
			["arcLength"] = 360,
			["controlledChildren"] = {
				"Targeted Spells", -- [1]
				"Sound when targeted by a boss", -- [2]
			},
			["xOffset"] = 8,
			["preferToUpdate"] = true,
			["groupIcon"] = 1033497,
			["anchorPoint"] = "TOP",
			["borderColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["rowSpace"] = 1,
			["url"] = "https://wago.io/BFADungeonTargetedSpells/68",
			["actions"] = {
				["start"] = {
				},
				["finish"] = {
				},
				["init"] = {
				},
			},
			["backgroundInset"] = 0,
			["wordWrap"] = "WordWrap",
			["stagger"] = 0,
			["font"] = "Friz Quadrata TT",
			["load"] = {
				["use_class"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["class"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["animate"] = false,
			["scale"] = 1,
			["regionType"] = "dynamicgroup",
			["constantFactor"] = "RADIUS",
			["borderOffset"] = 4,
			["tocversion"] = 90002,
			["config"] = {
			},
			["fixedWidth"] = 200,
			["grow"] = "HORIZONTAL",
			["borderBackdrop"] = "Blizzard Tooltip",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["fullCircle"] = true,
			["useAnchorPerUnit"] = true,
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["unit"] = "player",
						["type"] = "aura2",
						["spellIds"] = {
						},
						["subeventSuffix"] = "_CAST_START",
						["names"] = {
						},
						["subeventPrefix"] = "SPELL",
						["event"] = "Health",
						["debuffType"] = "HELPFUL",
					},
					["untrigger"] = {
					},
				}, -- [1]
			},
			["columnSpace"] = 1,
			["internalVersion"] = 45,
			["useLimit"] = true,
			["align"] = "CENTER",
			["version"] = 68,
			["subRegions"] = {
			},
			["backdropColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0.5, -- [4]
			},
			["fontSize"] = 12,
			["yOffset"] = 0,
			["customTextUpdate"] = "update",
			["uid"] = "zSrl30KxJ6i",
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["border"] = false,
			["borderEdge"] = "1 Pixel",
			["outline"] = "OUTLINE",
			["borderSize"] = 2,
			["limit"] = 3,
			["anchorPerUnit"] = "UNITFRAME",
			["borderInset"] = 1,
			["sort"] = "none",
			["gridType"] = "RD",
			["frameStrata"] = 6,
			["selfPoint"] = "CENTER",
			["justify"] = "LEFT",
			["semver"] = "3.0.6",
			["id"] = "Dungeon - Targeted Spells",
			["space"] = 1,
			["gridWidth"] = 5,
			["anchorFrameType"] = "SCREEN",
			["rotation"] = 0,
			["radius"] = 200,
			["authorOptions"] = {
			},
			["displayText"] = "%p",
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["background"] = "None",
		},
		["Sire Denathrius - Baleful Shadow 2"] = {
			["text2Point"] = "CENTER",
			["iconSource"] = 0,
			["text1FontSize"] = 12,
			["authorOptions"] = {
				{
					["type"] = "input",
					["key"] = "npcID",
					["useLength"] = false,
					["name"] = "npcID (comma seperated)",
					["length"] = 10,
					["default"] = "162432",
					["width"] = 1,
				}, -- [1]
				{
					["type"] = "input",
					["key"] = "spellID",
					["useLength"] = false,
					["name"] = "spellID (comma seperated)",
					["length"] = 10,
					["default"] = "316211",
					["width"] = 1,
				}, -- [2]
				{
					["text"] = "Reset after",
					["type"] = "description",
					["fontSize"] = "medium",
					["width"] = 0.4,
				}, -- [3]
				{
					["min"] = 0,
					["type"] = "range",
					["name"] = "",
					["default"] = 2,
					["max"] = 10,
					["key"] = "limit",
					["step"] = 1,
					["width"] = 0.6,
				}, -- [4]
				{
					["text"] = " interrupts/casts.",
					["type"] = "description",
					["fontSize"] = "medium",
					["width"] = 1,
				}, -- [5]
				{
					["min"] = 0,
					["type"] = "range",
					["name"] = "Specify your interrupt:",
					["default"] = 0,
					["max"] = 10,
					["key"] = "enableSpecified",
					["step"] = 1,
					["width"] = 1.05,
				}, -- [6]
				{
					["type"] = "toggle",
					["key"] = "enableIcon",
					["default"] = false,
					["name"] = "Show only if unit is affected by Raidicon",
					["width"] = 2,
				}, -- [7]
				{
					["type"] = "multiselect",
					["values"] = {
						"|TInterface\\TargetingFrame\\UI-RaidTargetingIcon_1:16|t", -- [1]
						"|TInterface\\TargetingFrame\\UI-RaidTargetingIcon_2:16|t", -- [2]
						"|TInterface\\TargetingFrame\\UI-RaidTargetingIcon_3:16|t", -- [3]
						"|TInterface\\TargetingFrame\\UI-RaidTargetingIcon_4:16|t", -- [4]
						"|TInterface\\TargetingFrame\\UI-RaidTargetingIcon_5:16|t", -- [5]
						"|TInterface\\TargetingFrame\\UI-RaidTargetingIcon_6:16|t", -- [6]
						"|TInterface\\TargetingFrame\\UI-RaidTargetingIcon_7:16|t", -- [7]
						"|TInterface\\TargetingFrame\\UI-RaidTargetingIcon_8:16|t", -- [8]
					},
					["default"] = {
						false, -- [1]
						false, -- [2]
						false, -- [3]
						false, -- [4]
						false, -- [5]
						false, -- [6]
						false, -- [7]
						false, -- [8]
					},
					["name"] = "Specify required Raidicon",
					["key"] = "specifiedIcon",
					["width"] = 1.4,
				}, -- [8]
				{
					["type"] = "toggle",
					["key"] = "overrideSettings",
					["width"] = 2,
					["name"] = "|TInterface\\ADdOns\\WeakAuras\\Media\\Textures\\exclamation-mark:16|tAllow ERT Note to override the settings above",
					["useDesc"] = true,
					["default"] = false,
					["desc"] = "example format:|n|nstartLine|n|TInterface\\TargetingFrame\\UI-RaidTargetingIcon_1:16|t|cffC41F3BJohnnyCash|r |cffFF7D0ANirvana|r|n|TInterface\\TargetingFrame\\UI-RaidTargetingIcon_2:16|t|cff00FF96PinkFloyd|r |cffF58CBAHeart|r |cff3ec5e9Causese|r |n|TInterface\\TargetingFrame\\UI-RaidTargetingIcon_3:16|t|cffC79C6EChuckBerry|r |cff8787EDJCole|r|n|TInterface\\TargetingFrame\\UI-RaidTargetingIcon_4:16|t|cffFFF569NotoriousBIG|r |cffA330C9Tool|r|nendLine|n|n|cffff0000Important:|r|nAdd startLine before the assignment and endLine after!|n|nIn the example above,|n|cffF58CBAHeart's|r reset after would be set to 3,|nspecified Interrupt to 2|nand raidicon to |TInterface\\TargetingFrame\\UI-RaidTargetingIcon_2:16|t",
				}, -- [9]
				{
					["text"] = "",
					["type"] = "header",
					["useName"] = false,
					["width"] = 1,
				}, -- [10]
				{
					["type"] = "toggle",
					["key"] = "enableGlow",
					["default"] = true,
					["name"] = "Glow if your interrupt is next",
					["width"] = 2,
				}, -- [11]
				{
					["type"] = "toggle",
					["key"] = "enableSound",
					["default"] = false,
					["name"] = "Play Sound if your interrupt is next",
					["width"] = 2,
				}, -- [12]
				{
					["type"] = "select",
					["values"] = {
						"Ringing Phone (from WeakAuras)", -- [1]
						"Next (from Causese Addon)", -- [2]
					},
					["default"] = 1,
					["name"] = "Specify Sound",
					["key"] = "specifiedSound",
					["width"] = 1.2,
				}, -- [13]
				{
					["type"] = "toggle",
					["key"] = "enableCastSound",
					["default"] = false,
					["name"] = "Play Sound when specified cast starts",
					["width"] = 2,
				}, -- [14]
				{
					["type"] = "select",
					["values"] = {
						"Air Horn (from WeakAuras)", -- [1]
						"Bike Horn (from WeakAuras)", -- [2]
						"Interrupt (from Causese Addon)", -- [3]
					},
					["default"] = 1,
					["name"] = "Specify Sound",
					["key"] = "specifiedCastSound",
					["width"] = 1.2,
				}, -- [15]
				{
					["text"] = "",
					["type"] = "header",
					["useName"] = false,
					["width"] = 1,
				}, -- [16]
				{
					["type"] = "toggle",
					["key"] = "printfails",
					["default"] = false,
					["name"] = "Print failed interrupts in chat (mark + count)",
					["width"] = 2,
				}, -- [17]
				{
					["type"] = "toggle",
					["key"] = "showText",
					["default"] = false,
					["name"] = "Show text when cast starts (requires wago.io/interruptwa)",
					["width"] = 2,
				}, -- [18]
			},
			["displayText"] = "%p",
			["yOffset"] = 342.00003051758,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/XY17WJYK0/1",
			["actions"] = {
				["start"] = {
					["do_sound"] = false,
				},
				["finish"] = {
				},
				["init"] = {
					["custom"] = "aura_env.translateERT = {\n    [\"{star}\"] = 1,\n    [\"{circle}\"] = 2,\n    [\"{diamond}\"] = 3,\n    [\"{triangle}\"] = 4,\n    [\"{moon}\"] = 5,\n    [\"{square}\"] = 6,\n    [\"{cross}\"] = 7,\n    [\"{skull}\"] = 8,\n}\n\naura_env.npcIDs = {}\naura_env.spellIDs = {}\naura_env.counter = aura_env.counter or {}\naura_env.caststart = {}\n\naura_env.sound = {\n    [1] = \"Interface\\\\AddOns\\\\WeakAuras\\\\Media\\\\Sounds\\\\RingingPhone.ogg\",\n    [2] = \"Interface\\\\AddOns\\\\SharedMedia_Causese\\\\sound\\\\Next.ogg\",\n}\n\naura_env.castsound = {\n    [1] = \"Interface\\\\AddOns\\\\WeakAuras\\\\Media\\\\Sounds\\\\AirHorn.ogg\",\n    [2] = \"Interface\\\\AddOns\\\\WeakAuras\\\\Media\\\\Sounds\\\\BikeHorn.ogg\",\n    [3] = \"Interface\\\\AddOns\\\\SharedMedia_Causese\\\\sound\\\\Interrupt.ogg\",\n}\n\naura_env.sendemote = {\n    [1] = \"STAR\",\n    [2] = \"CIRCLE\",\n    [4] = \"DIAMOND\",\n    [8] = \"TRIANGLE\",\n    [16] = \"MOON\",\n    [32] = \"SQUARE\",\n    [64] = \"CROSS\",\n    [128] = \"SKULL\",\n}\n\n--sourceRaidFlags to icon number \naura_env.translate = {\n    [1] = 1,\n    [2] = 2,\n    [4] = 3,\n    [8] = 4,\n    [16] = 5,\n    [32] = 6,\n    [64] = 7,\n    [128] = 8,\n}",
					["do_custom"] = true,
				},
			},
			["text2Font"] = "Friz Quadrata TT",
			["keepAspectRatio"] = false,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["difficulty"] = {
					["single"] = "mythic",
					["multi"] = {
						["mythic"] = true,
					},
				},
				["use_encounterid"] = true,
				["zoneIds"] = "",
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2407",
				["size"] = {
					["multi"] = {
					},
				},
			},
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "icon",
			["text2FontSize"] = 24,
			["cooldownTextDisabled"] = false,
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["config"] = {
				["overrideSettings"] = true,
				["specifiedCastSound"] = 1,
				["enableCastSound"] = true,
				["enableGlow"] = true,
				["limit"] = 5,
				["npcID"] = "175205",
				["enableIcon"] = false,
				["enableSound"] = true,
				["showText"] = false,
				["specifiedIcon"] = {
					false, -- [1]
					false, -- [2]
					false, -- [3]
					false, -- [4]
					false, -- [5]
					false, -- [6]
					false, -- [7]
					false, -- [8]
				},
				["printfails"] = true,
				["specifiedSound"] = 2,
				["enableSpecified"] = 4,
				["spellID"] = "344776",
			},
			["fixedWidth"] = 200,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "function()\n    if aura_env.state and aura_env.state.PassUnit then\n        local region = aura_env.region\n        local plate = C_NamePlate.GetNamePlateForUnit(aura_env.state.PassUnit)\n        if plate then\n            region:ClearAllPoints()\n            region:SetAnchor(\"BOTTOM\", plate, \"TOP\")\n            region:SetOffset(0, 0)\n            region:Show()\n        else\n            region:Hide()\n        end\n    end\n    if aura_env.state and aura_env.state.name then\n        if aura_env.config[\"enableGlow\"] and aura_env.config[\"enableSpecified\"] == aura_env.state.name then\n            WeakAuras.GetRegion(aura_env.id, aura_env.cloneId):SetGlow(true)\n        else\n            WeakAuras.GetRegion(aura_env.id, aura_env.cloneId):SetGlow(false)    \n        end\n    end\nend",
			["cooldownSwipe"] = true,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["debuffType"] = "HELPFUL",
						["type"] = "custom",
						["unevent"] = "auto",
						["custom_type"] = "stateupdate",
						["names"] = {
						},
						["duration"] = "1",
						["genericShowOn"] = "showOnActive",
						["unit"] = "player",
						["event"] = "Chat Message",
						["events"] = "ENCOUNTER_START, NAME_PLATE_UNIT_ADDED, NAME_PLATE_UNIT_REMOVED, CLEU:SPELL_CAST_START:SPELL_CAST_SUCCESS:SPELL_INTERRUPT:UNIT_DIED, RAID_TARGET_UPDATE",
						["spellIds"] = {
						},
						["custom"] = "function(allstates, event, unit, ...)\n    --iterates through visible nameplates and checks for specified raidicon. \n    if event == \"RAID_TARGET_UPDATE\" and aura_env.config[\"enableIcon\"] then\n        for i = 1,40 do\n            local u = \"nameplate\"..i\n            if UnitExists(u) then \n                local guid = UnitGUID(u)\n                if guid and not allstates[guid] and aura_env.npcIDs[select(6,strsplit(\"-\", guid))] then\n                    if aura_env.config[\"specifiedIcon\"][GetRaidTargetIndex(u)] then\n                        allstates[guid] = {\n                            show = true,\n                            changed = true,\n                            name = aura_env.counter[guid] or 1,\n                            PassUnit = u,\n                        }\n                        --sound only fired if nameplate is visible the moment that icon is applied, only relevant for VERY first interrupt.\n                        if aura_env.config.enableSound and allstates[guid].name == aura_env.config[\"enableSpecified\"] then\n                            PlaySoundFile(aura_env.sound[aura_env.config[\"specifiedSound\"]], \"MASTER\")\n                        end\n                    end\n                end\n            end\n        end\n    end\n    if event == \"COMBAT_LOG_EVENT_UNFILTERED\" then\n        local subEvent, hideCaster, sourceGUID, sourceName, sourceFlags, sourceRaidFlags, destGUID, destName, destFlags, destRaidFlags, spellID, spellName, spellschool, extraspellID, extraspellName  = ...\n        if subEvent == \"SPELL_CAST_SUCCESS\" and aura_env.spellIDs[tostring(spellID)] then\n            local state = allstates[sourceGUID]\n            --cancels text aura if counter matches your specified value\n            if aura_env.config[\"showText\"] and aura_env.caststart[sourceGUID] and aura_env.caststart[sourceGUID] == aura_env.config[\"enableSpecified\"] then\n                WeakAuras.ScanEvents(\"HIDE_TEXTAURA\", \"nilcheck\")\n            end\n            --count & print failed interrupts in chat (if option is actived)\n            if aura_env.counter[sourceGUID] then\n                aura_env.counter[sourceGUID] = aura_env.counter[sourceGUID] + 1\n                if aura_env.config.printfails and sourceRaidFlags and sourceRaidFlags > 0 then\n                    print((aura_env.counter[sourceGUID]-1)..\". interrupt failed on \"..aura_env.sendemote[sourceRaidFlags])\n                end\n            else aura_env.counter[sourceGUID] = 2 \n                if aura_env.config.printfails and sourceRaidFlags and sourceRaidFlags > 0 then\n                    print((aura_env.counter[sourceGUID]-1)..\". interrupt failed on \"..aura_env.sendemote[sourceRaidFlags])\n                end\n            end\n            --update state\n            if state then\n                state.changed = true\n                state.name = aura_env.counter[sourceGUID]\n            end\n            --check if current count == limit and reset to 1 & update TSU\n            if aura_env.counter[sourceGUID] == (aura_env.config[\"limit\"]+1) and aura_env.config[\"limit\"] > 0 then\n                aura_env.counter[sourceGUID] = 1\n                if state then\n                    state.changed = true\n                    state.name = aura_env.counter[sourceGUID]\n                end\n            end\n            --fire sound if it matches your specified value\n            if aura_env.config[\"enableSound\"] and aura_env.counter[sourceGUID] then\n                if aura_env.counter[sourceGUID] == aura_env.config[\"enableSpecified\"] then\n                    PlaySoundFile(aura_env.sound[aura_env.config[\"specifiedSound\"]], \"MASTER\")\n                end\n            end\n        end\n        if subEvent == \"SPELL_INTERRUPT\" and aura_env.spellIDs[tostring(extraspellID)] then\n            local state = allstates[destGUID]\n            --cancels text aura if counter matches your specified value\n            if aura_env.config[\"showText\"] and aura_env.caststart[destGUID] and aura_env.caststart[destGUID] == aura_env.config[\"enableSpecified\"] then\n                WeakAuras.ScanEvents(\"HIDE_TEXTAURA\", \"nilcheck\")\n                \n            end\n            --increase count\n            if aura_env.counter[destGUID] then\n                aura_env.counter[destGUID] = aura_env.counter[destGUID] + 1\n            else aura_env.counter[destGUID] = 2 \n            end\n            --update state with new count\n            if state then\n                state.changed = true\n                state.name = aura_env.counter[destGUID]\n            end\n            --check if current count == limit and reset to 1 & update TSU\n            if aura_env.counter[destGUID] == (aura_env.config[\"limit\"]+1) and aura_env.config[\"limit\"] > 0  then\n                aura_env.counter[destGUID] = 1\n                if state then\n                    state.changed = true\n                    state.name = aura_env.counter[destGUID]\n                end\n            end\n            -- fire sound if it matches your specified value\n            if aura_env.config[\"enableSound\"] and aura_env.counter[destGUID] then\n                if aura_env.counter[destGUID] == aura_env.config[\"enableSpecified\"]  then\n                    PlaySoundFile(aura_env.sound[aura_env.config[\"specifiedSound\"]], \"MASTER\")\n                end    \n            end\n        end\n        if subEvent == \"SPELL_CAST_START\" and aura_env.spellIDs[tostring(spellID)] then \n            if (aura_env.config.specifiedIcon\n                and aura_env.config.enableIcon\n                and sourceRaidFlags \n                and (aura_env.config.specifiedIcon[aura_env.translate[sourceRaidFlags]]\n                    or sourceRaidFlags == 0 and not aura_env.caststart[sourceGUID]))\n            or not aura_env.config.enableIcon then\n                if aura_env.caststart[sourceGUID] then\n                    aura_env.caststart[sourceGUID] = aura_env.caststart[sourceGUID] + 1\n                else \n                    aura_env.caststart[sourceGUID] = 1\n                end\n                \n                --reset cast start counter if counter = limit\n                if aura_env.caststart[sourceGUID] == (aura_env.config[\"limit\"]+1) and aura_env.config[\"limit\"] > 0 then\n                    aura_env.caststart[sourceGUID] = 1\n                end\n                --fire cast start sound if it matches your specified value + raidicon\n                if aura_env.config[\"enableCastSound\"] and aura_env.caststart[sourceGUID] then\n                    if (aura_env.caststart[sourceGUID] == aura_env.config[\"enableSpecified\"])  then\n                        PlaySoundFile(aura_env.castsound[aura_env.config[\"specifiedCastSound\"]], \"MASTER\")\n                    end\n                end\n                --starts text aura if it matches your specified value\n                if aura_env.config[\"showText\"] and aura_env.caststart[sourceGUID] and aura_env.caststart[sourceGUID] == aura_env.config[\"enableSpecified\"] then\n                    WeakAuras.ScanEvents(\"SHOW_TEXTAURA\", \"nilcheck\")\n                end\n            end\n        end\n        --hide anchor-to-nameplate when npc dies\n        if subEvent == \"UNIT_DIED\" then\n            local state = allstates[destGUID] \n            if state then\n                state.show = false\n                state.changed = true\n            end\n        end\n    end\n    if event == \"NAME_PLATE_UNIT_ADDED\" then\n        if unit then\n            local guid = UnitGUID(unit)\n            if guid then\n                local state = allstates[guid]\n                --update state with current count when nameplate shows\n                if state then\n                    state.changed = true\n                    state.PassUnit = unit\n                    state.name = aura_env.counter[guid] or 1\n                end\n                --create state when nameplate shows          \n                if not state and aura_env.npcIDs[select(6,strsplit(\"-\", guid))] then\n                    if aura_env.config[\"enableIcon\"] and aura_env.config[\"specifiedIcon\"][GetRaidTargetIndex(unit)] or not aura_env.config[\"enableIcon\"] then\n                        allstates[guid] = {\n                            PassUnit = unit,\n                            show = true,\n                            name = aura_env.counter[guid] or 1,\n                            changed = true,\n                        } \n                    end\n                end\n            end\n        end\n    end\n    if event == \"NAME_PLATE_UNIT_REMOVED\" then\n        if unit then\n            local guid = UnitGUID(unit)\n            if guid then\n                local state = allstates[guid]\n                if state then\n                    state.changed = true\n                    state.PassUnit = \"none\"\n                end\n            end\n        end\n    end\n    if event == \"ENCOUNTER_START\" then\n        aura_env.npcIDs = {}\n        aura_env.spellIDs = {}\n        for match in aura_env.config.npcID:gmatch(\"%d+\") do aura_env.npcIDs[match] = true end\n        for match in aura_env.config.spellID:gmatch(\"%d+\") do aura_env.spellIDs[match] = true end\n        aura_env.counter = {}\n        aura_env.caststart = {}\n        if IsAddOnLoaded(\"ExRT\") \n        and _G.VExRT.Note.Text1 \n        and aura_env.config.overrideSettings then\n            local text = _G.VExRT.Note.Text1\n            aura_env.count = 0\n            --counts lines except gaps\n            for line in string.gmatch(text,'[^\\r\\n]+') do\n                aura_env.count = aura_env.count + 1\n                --searches for \"startLine\" in Note\n                if string.match(line, \"startLine\") then\n                    aura_env.startCount = aura_env.count\n                end\n                --searches for \"endLine\" in Note\n                if string.match(line, \"endLine\") then\n                    aura_env.endCount = aura_env.count\n                end\n                --checks if your name is found inbetween startLine and endLine\n                if aura_env.startCount and (aura_env.count or aura_env.myCount and aura_env.myCount > aura_env.startCount) then\n                    if (string.match(line, UnitName(\"player\")) or aura_env.myCount) then\n                        if not aura_env.myCount then\n                            aura_env.myCount = aura_env.count\n                            aura_env.line = line\n                            aura_env.myMark = string.match(line, \"%{%a+%}\")\n                        elseif aura_env.myCount and aura_env.endCount and aura_env.endCount > aura_env.myCount then\n                            aura_env.resetAfter = 0\n                            for name in string.gmatch(aura_env.line, \"|c%x%x%x%x%x%x%x%x([%w-]+)|\") do\n                                aura_env.resetAfter = aura_env.resetAfter + 1\n                                if name == UnitName(\"player\") then\n                                    aura_env.config.enableSpecified = aura_env.resetAfter\n                                end\n                            end\n                        end\n                    end\n                end\n            end\n            if aura_env.resetAfter then\n                --set limit to amount of playerNames in your interruptorder\n                aura_env.config.limit = aura_env.resetAfter\n            end\n            if aura_env.myMark then\n                --disable all currently selected raidicons & enable raidicon from note\n                for i = 1,8 do\n                    aura_env.config[\"specifiedIcon\"][i] = false\n                end\n                aura_env.config[\"enableIcon\"] = true\n                aura_env.config[\"specifiedIcon\"][aura_env.translateERT[aura_env.myMark]] = true\n                --aura_env.config.enableSound = true --forces people to use a sound (disabled by default)\n            end\n        end\n    end\n    return true    \nend",
						["check"] = "event",
						["subeventPrefix"] = "SPELL",
						["subeventSuffix"] = "_CAST_START",
						["custom_hide"] = "timed",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "all",
				["activeTriggerMode"] = -10,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 1,
			["subRegions"] = {
				{
					["text_text_format_p_time_precision"] = 1,
					["text_text"] = "%name",
					["text_text_format_p_format"] = "timed",
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_text_format_name_format"] = "none",
					["text_color"] = {
						1, -- [1]
						0, -- [2]
						0.019607843137255, -- [3]
						1, -- [4]
					},
					["text_font"] = "Arial Narrow",
					["text_text_format_p_time_dynamic_threshold"] = 60,
					["text_shadowYOffset"] = 0,
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "CENTER",
					["text_shadowXOffset"] = 0,
					["text_visible"] = true,
					["text_fontSize"] = 45,
					["anchorXOffset"] = 0,
					["text_text_format_p_time_format"] = 0,
				}, -- [1]
				{
					["text_shadowXOffset"] = 0,
					["text_text"] = "%c",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Friz Quadrata TT",
					["text_shadowYOffset"] = 0,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "CENTER",
					["text_fontSize"] = 24,
					["anchorXOffset"] = 0,
					["text_visible"] = true,
				}, -- [2]
				{
					["glowFrequency"] = 0.25,
					["type"] = "subglow",
					["useGlowColor"] = false,
					["glowType"] = "buttonOverlay",
					["glowLength"] = 10,
					["glowYOffset"] = 0,
					["glowColor"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["glowXOffset"] = 0,
					["glowScale"] = 1,
					["glowThickness"] = 1,
					["glow"] = false,
					["glowLines"] = 8,
					["glowBorder"] = false,
				}, -- [3]
			},
			["height"] = 47.999893188477,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["xOffset"] = 5.0006103515625,
			["preferToUpdate"] = false,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["cooldown"] = false,
			["uid"] = "Y5jRsbRv3Qh",
			["alpha"] = 1,
			["icon"] = true,
			["zoom"] = 0,
			["text2"] = "%p",
			["text1"] = "%c",
			["anchorFrameType"] = "SCREEN",
			["text1Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["justify"] = "LEFT",
			["semver"] = "1.0.0",
			["id"] = "Sire Denathrius - Baleful Shadow 2",
			["text1Point"] = "BOTTOMRIGHT",
			["frameStrata"] = 1,
			["width"] = 48,
			["text1Enabled"] = true,
			["displayIcon"] = "Interface\\AddOns\\WeakAuras\\Media\\Textures\\Square_White_Border",
			["inverse"] = false,
			["wordWrap"] = "WordWrap",
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["cooldownEdge"] = false,
		},
		["Spiteful Countdown"] = {
			["user_y"] = 0,
			["iconSource"] = 0,
			["user_x"] = 0,
			["authorOptions"] = {
			},
			["displayText"] = "%p  - %custom",
			["yOffset"] = -2.5000057220459,
			["displayText_format_p_time_dynamic"] = false,
			["sparkRotation"] = 0,
			["sameTexture"] = true,
			["url"] = "https://wago.io/yOeM0FVzB/2",
			["icon"] = true,
			["icon_color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["keepAspectRatio"] = false,
			["wordWrap"] = "WordWrap",
			["barColor"] = {
				1, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["desaturate"] = false,
			["rotation"] = 0,
			["font"] = "Friz Quadrata TT",
			["sparkOffsetY"] = 0,
			["crop_y"] = 0.41,
			["textureWrapMode"] = "CLAMPTOBLACKADDITIVE",
			["startAngle"] = 0,
			["shadowXOffset"] = 1,
			["useAdjustededMin"] = false,
			["regionType"] = "icon",
			["blendMode"] = "BLEND",
			["slantMode"] = "INSIDE",
			["texture"] = "Blizzard",
			["sparkTexture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
			["auto"] = false,
			["compress"] = false,
			["alpha"] = 1,
			["uid"] = "DxLaP7mMrUn",
			["displayIcon"] = 237553,
			["backgroundOffset"] = 2,
			["outline"] = "OUTLINE",
			["sparkOffsetX"] = 0,
			["parent"] = "Spiteful",
			["preferToUpdate"] = false,
			["information"] = {
			},
			["shadowYOffset"] = -1,
			["crop_x"] = 0.41,
			["desaturateBackground"] = false,
			["cooldownSwipe"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["customTextUpdate"] = "event",
			["cooldownEdge"] = false,
			["desaturateForeground"] = false,
			["triggers"] = {
				{
					["trigger"] = {
						["use_absorbMode"] = true,
						["unit"] = "nameplate",
						["destUnit"] = "player",
						["custom_hide"] = "timed",
						["debuffType"] = "HELPFUL",
						["names"] = {
						},
						["type"] = "custom",
						["subeventSuffix"] = "_CAST_START",
						["custom_type"] = "stateupdate",
						["use_nameplateType"] = false,
						["duration"] = "1",
						["event"] = "Cast",
						["spellIds"] = {
						},
						["use_npcId"] = true,
						["subeventPrefix"] = "SPELL",
						["custom"] = "function(allstates, event, unit, ...)\n    if event == \"WA_NAMEPLATE_CLOSED\" then\n        for i = 1, 40 do\n            local unit = \"nameplate\" .. i\n            if UnitExists(unit) and UnitCanAttack(\"player\", unit) then\n                C_Timer.After(0.1, function() WeakAuras.ScanEvents(\"NAME_PLATE_AURA_NAMES\", unit) end)\n            end\n        end\n    end\n    \n    if event == \"NAME_PLATE_UNIT_ADDED\" and unit and UnitCanAttack(\"player\", unit)\n    or event == \"NAME_PLATE_AURA_NAMES\" and unit then\n        local npcID = select(6,strsplit(\"-\",UnitGUID(unit)))\n        if UnitGUID(unit) and npcID == \"174773\" then\n            aura_env.stored[UnitGUID(unit)] = unit\n            local target = unit..\"-target\"\n            if UnitExists(target) and UnitIsUnit(\"player\", target) then\n                allstates[UnitGUID(unit)] = {\n                    GUID = UnitGUID(unit),\n                    unit = unit,\n                    changed = true,\n                    show = true,\n                }\n                print(allstates[UnitGUID(unit)].GUID)\n                return true \n            end\n        end\n    elseif event == \"NAME_PLATE_UNIT_REMOVED\" and unit then\n        if allstates[UnitGUID(unit)] then\n            allstates[UnitGUID(unit)].show = false\n            allstates[UnitGUID(unit)].changed = true\n        end\n        if aura_env.stored[UnitGUID(unit)] then\n            aura_env.stored[UnitGUID(unit)] = nil\n        end\n    end\n    if event == \"FRAME_UPDATE\" then\n        if not aura_env.last or aura_env.last < GetTime() then\n            for GUID, unit in pairs(aura_env.stored) do\n                if GUID then\n                    local target = unit..\"-target\"\n                    if UnitExists(target) and UnitIsUnit(\"player\", target) then\n                        if allstates[GUID] then\n                            allstates[GUID].changed = true\n                        else\n                            allstates[GUID] = {\n                                GUID = GUID,\n                                unit = unit,\n                                progressType = \"timed\",\n                                duration = 12.5,\n                                expirationTime = GetTime() + 12.5,\n                                autoHide = true,\n                                changed = true,\n                                show = true,\n                            }\n                        end\n                    elseif allstates[GUID] then\n                        allstates[GUID].show = false\n                        allstates[GUID].changed = true\n                    end\n                end\n            end\n        end\n    end\n    return true\nend",
						["events"] = "WA_NAMEPLATE_CLOSED,NAME_PLATE_UNIT_ADDED,NAME_PLATE_UNIT_REMOVED,NAME_PLATE_AURA_NAMES,FRAME_UPDATE",
						["check"] = "event",
						["use_destUnit"] = true,
						["unevent"] = "auto",
						["use_unit"] = true,
					},
					["untrigger"] = {
						["unit"] = "nameplate",
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["endAngle"] = 360,
			["internalVersion"] = 45,
			["sparkWidth"] = 10,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["displayText_format_custom_format"] = "none",
			["actions"] = {
				["start"] = {
					["sound"] = "Interface\\Addons\\WeakAuras\\PowerAurasMedia\\Sounds\\aggro.ogg",
					["do_sound"] = true,
				},
				["finish"] = {
				},
				["init"] = {
					["custom"] = "aura_env.stored = {}",
					["do_custom"] = true,
				},
			},
			["desc"] = "Credit to Translit for the base code which this is build from.\nAuthor: Justbears-Illidan(US)",
			["tocversion"] = 90002,
			["anchorFrameType"] = "SCREEN",
			["displayText_format_p_format"] = "timed",
			["version"] = 2,
			["subRegions"] = {
				{
					["text_text_format_p_time_precision"] = 1,
					["text_text"] = "A shade is fixated on you!",
					["text_text_format_p_format"] = "timed",
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_text_format_custom_format"] = "none",
					["rotateText"] = "NONE",
					["text_text_format_p_time_format"] = 0,
					["type"] = "subtext",
					["text_anchorXOffset"] = 5,
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "vixar",
					["text_text_format_p_time_dynamic_threshold"] = 60,
					["text_shadowYOffset"] = 0,
					["text_visible"] = true,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "OUTER_RIGHT",
					["text_shadowXOffset"] = 0,
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_fontSize"] = 15,
					["anchorXOffset"] = 0,
					["text_justify"] = "CENTER",
				}, -- [1]
			},
			["height"] = 30,
			["backgroundTexture"] = "Interface\\Addons\\WeakAuras\\PowerAurasMedia\\Auras\\Aura3",
			["id"] = "Spiteful Countdown",
			["sparkBlendMode"] = "ADD",
			["useAdjustededMax"] = false,
			["fontSize"] = 20,
			["load"] = {
				["zoneIds"] = "",
				["affixes"] = {
					["single"] = 123,
					["multi"] = {
						[123] = true,
					},
				},
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["use_affixes"] = true,
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["displayText_format_n_format"] = "none",
			["justify"] = "LEFT",
			["zoom"] = 0.3,
			["mirror"] = false,
			["cooldownTextDisabled"] = false,
			["automaticWidth"] = "Auto",
			["sparkHeight"] = 30,
			["spark"] = false,
			["icon_side"] = "RIGHT",
			["sparkColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["backgroundColor"] = {
				0.5, -- [1]
				0.5, -- [2]
				0.5, -- [3]
				0.5, -- [4]
			},
			["displayText_format_p_time_precision"] = 1,
			["anchorPoint"] = "CENTER",
			["xOffset"] = -3.7499580383301,
			["sparkRotationMode"] = "AUTO",
			["semver"] = "1.0.1",
			["config"] = {
			},
			["sparkHidden"] = "NEVER",
			["foregroundColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["frameStrata"] = 1,
			["width"] = 30,
			["fixedWidth"] = 200,
			["foregroundTexture"] = "Interface\\Addons\\WeakAuras\\PowerAurasMedia\\Auras\\Aura3",
			["inverse"] = false,
			["selfPoint"] = "BOTTOM",
			["orientation"] = "VERTICAL",
			["conditions"] = {
			},
			["cooldown"] = true,
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
		},
		["My Raid-Mark"] = {
			["outline"] = "OUTLINE",
			["xOffset"] = -633.0001220703125,
			["displayText"] = "%p",
			["yOffset"] = -184.4994506835938,
			["displayText_format_p_time_dynamic"] = false,
			["customTextUpdate"] = "event",
			["url"] = "https://wago.io/WWPDzIdcG/2",
			["actions"] = {
				["start"] = {
				},
				["finish"] = {
				},
				["init"] = {
					["custom"] = "aura_env.marks = {\n    [1] = \"Interface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_1\",\n    [2] = \"Interface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_2\",\n    [3] = \"Interface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_3\",\n    [4] = \"Interface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_4\",\n    [5] = \"Interface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_5\",\n    [6] = \"Interface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_6\",\n    [7] = \"Interface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_7\",\n    [8] = \"Interface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_8\",\n}",
					["do_custom"] = true,
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "custom",
						["subeventSuffix"] = "_CAST_START",
						["event"] = "Health",
						["unit"] = "player",
						["names"] = {
						},
						["events"] = "RAID_TARGET_UPDATE",
						["custom"] = "function(states, event, ...)\n    local mark = GetRaidTargetIndex(\"player\")\n    if mark then\n        states[\"\"] = {\n            texture = aura_env.marks[mark],\n            show = true,\n            changed = true,\n            progressType = \"static\",\n            \n        }\n        return true\n    else\n        if states[\"\"] then\n            states[\"\"].show = false\n            states[\"\"].changed = true\n            return true\n        end\n    end\nend\n\n\n",
						["custom_type"] = "stateupdate",
						["check"] = "event",
						["subeventPrefix"] = "SPELL",
						["spellIds"] = {
						},
						["debuffType"] = "HELPFUL",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["displayText_format_p_format"] = "timed",
			["internalVersion"] = 45,
			["wordWrap"] = "WordWrap",
			["desaturate"] = false,
			["discrete_rotation"] = 0,
			["font"] = "Friz Quadrata TT",
			["version"] = 2,
			["subRegions"] = {
			},
			["height"] = 200,
			["rotate"] = true,
			["load"] = {
				["zoneIds"] = "",
				["talent"] = {
					["multi"] = {
					},
				},
				["use_combat"] = true,
				["spec"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["automaticWidth"] = "Auto",
			["fontSize"] = 12,
			["shadowYOffset"] = -1,
			["authorOptions"] = {
			},
			["shadowXOffset"] = 1,
			["conditions"] = {
			},
			["mirror"] = false,
			["preferToUpdate"] = false,
			["regionType"] = "texture",
			["textureWrapMode"] = "CLAMPTOBLACKADDITIVE",
			["blendMode"] = "BLEND",
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["config"] = {
			},
			["displayText_format_p_time_precision"] = 1,
			["texture"] = "Interface\\TargetingFrame\\UI-RaidTargetingIcon_1",
			["anchorFrameType"] = "SCREEN",
			["frameStrata"] = 1,
			["semver"] = "1.0.1",
			["tocversion"] = 90002,
			["id"] = "My Raid-Mark",
			["justify"] = "LEFT",
			["alpha"] = 1,
			["width"] = 200,
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["uid"] = "uEiy7PFG9NL",
			["anchorPoint"] = "CENTER",
			["rotation"] = 0,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["fixedWidth"] = 200,
			["information"] = {
			},
			["selfPoint"] = "BOTTOM",
		},
		["07 Dancing Fever List"] = {
			["arcLength"] = 360,
			["controlledChildren"] = {
				"Dancing Fever Time Left", -- [1]
				"Dancing Fever", -- [2]
			},
			["borderBackdrop"] = "Blizzard Tooltip",
			["authorOptions"] = {
			},
			["preferToUpdate"] = false,
			["yOffset"] = 71.12030029296875,
			["gridType"] = "RD",
			["fullCircle"] = true,
			["space"] = 1,
			["url"] = "https://wago.io/H4Beysoli/3",
			["actions"] = {
				["start"] = {
				},
				["finish"] = {
				},
				["init"] = {
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["debuffType"] = "HELPFUL",
						["type"] = "aura2",
						["spellIds"] = {
						},
						["subeventSuffix"] = "_CAST_START",
						["subeventPrefix"] = "SPELL",
						["unit"] = "player",
						["event"] = "Health",
						["names"] = {
						},
					},
					["untrigger"] = {
					},
				}, -- [1]
			},
			["columnSpace"] = 1,
			["radius"] = 200,
			["useLimit"] = false,
			["align"] = "LEFT",
			["stagger"] = 0,
			["version"] = 3,
			["subRegions"] = {
			},
			["borderInset"] = 1,
			["load"] = {
				["use_class"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["class"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["internalVersion"] = 45,
			["backdropColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0.5, -- [4]
			},
			["selfPoint"] = "TOPLEFT",
			["animate"] = false,
			["rowSpace"] = 1,
			["scale"] = 1,
			["xOffset"] = -399.0513916015625,
			["border"] = false,
			["borderEdge"] = "Square Full White",
			["regionType"] = "dynamicgroup",
			["borderSize"] = 2,
			["limit"] = 5,
			["borderColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["grow"] = "DOWN",
			["constantFactor"] = "RADIUS",
			["uid"] = ")NBoiB)vbCx",
			["borderOffset"] = 4,
			["semver"] = "1.0.2",
			["tocversion"] = 90002,
			["id"] = "07 Dancing Fever List",
			["frameStrata"] = 1,
			["gridWidth"] = 5,
			["anchorFrameType"] = "SCREEN",
			["sort"] = "none",
			["config"] = {
			},
			["rotation"] = 0,
			["anchorPoint"] = "CENTER",
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
		},
		["08 Chain Slam - Green"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 2,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						0, -- [1]
						1, -- [2]
						0.10196078431373, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.25,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2399",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "qllcPSF(ac2",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["useMatch_count"] = true,
						["auranames"] = {
						},
						["groupclone"] = true,
						["matchesShowOn"] = "showOnActive",
						["name_info"] = "aura",
						["unit"] = "group",
						["use_tooltip"] = false,
						["subeventPrefix"] = "SPELL",
						["match_countOperator"] = ">=",
						["match_count"] = "1",
						["buffShowOn"] = "showOnActive",
						["unevent"] = "timed",
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["useExactSpellId"] = true,
						["custom_type"] = "stateupdate",
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["event"] = "Health",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HARMFUL",
						["use_specific_unit"] = false,
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["auraspellids"] = {
							"335470", -- [1]
						},
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["duration"] = "1",
						["useGroup_count"] = false,
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["text2Font"] = "Friz Quadrata TT",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text1"] = "%n",
			["text1Point"] = "CENTER",
			["anchorFrameType"] = "SCREEN",
			["semver"] = "1.0.26",
			["justify"] = "LEFT",
			["zoom"] = 0,
			["id"] = "08 Chain Slam - Green",
			["config"] = {
			},
			["frameStrata"] = 9,
			["width"] = 6.9999866485596,
			["text1Containment"] = "INSIDE",
			["wordWrap"] = "WordWrap",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["Miasma soak group"] = {
			["grow"] = "DOWN",
			["controlledChildren"] = {
				"Volatile Miasma (duration)", -- [1]
				"Volatile Miasma", -- [2]
				"Essence Sap", -- [3]
			},
			["borderBackdrop"] = "Blizzard Tooltip",
			["authorOptions"] = {
			},
			["preferToUpdate"] = false,
			["groupIcon"] = "1390943",
			["anchorPoint"] = "CENTER",
			["borderColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["space"] = 3,
			["url"] = "https://wago.io/PescorusMiasma/9",
			["actions"] = {
				["start"] = {
				},
				["init"] = {
				},
				["finish"] = {
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["unit"] = "player",
						["type"] = "aura2",
						["spellIds"] = {
						},
						["subeventSuffix"] = "_CAST_START",
						["subeventPrefix"] = "SPELL",
						["names"] = {
						},
						["event"] = "Health",
						["debuffType"] = "HELPFUL",
					},
					["untrigger"] = {
					},
				}, -- [1]
			},
			["columnSpace"] = 1,
			["radius"] = 200,
			["useLimit"] = false,
			["align"] = "CENTER",
			["rotation"] = 0,
			["limit"] = 5,
			["version"] = 9,
			["subRegions"] = {
			},
			["config"] = {
			},
			["fullCircle"] = true,
			["load"] = {
				["talent"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["spec"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["customSort"] = "function(a, b)\n    if a.dataIndex == b.dataIndex then\n        if a.region.state.dead then return false end\n        if b.region.state.dead then return true end\n        \n        return (a.region.state.expirationTime or 0) > (b.region.state.expirationTime or 0)\n    end\n    \n    return a.dataIndex < b.dataIndex\nend",
			["backdropColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0.5, -- [4]
			},
			["xOffset"] = -348.9970703125,
			["animate"] = false,
			["yOffset"] = 75.3319091796875,
			["scale"] = 1,
			["gridType"] = "RD",
			["border"] = false,
			["borderEdge"] = "Square Full White",
			["regionType"] = "dynamicgroup",
			["borderSize"] = 2,
			["sort"] = "custom",
			["uid"] = "as1ZeFOHJ32",
			["selfPoint"] = "TOP",
			["constantFactor"] = "RADIUS",
			["arcLength"] = 360,
			["borderOffset"] = 4,
			["semver"] = "1.0.8",
			["tocversion"] = 90002,
			["id"] = "Miasma soak group",
			["frameStrata"] = 1,
			["gridWidth"] = 5,
			["anchorFrameType"] = "SCREEN",
			["internalVersion"] = 45,
			["borderInset"] = 1,
			["rowSpace"] = 1,
			["stagger"] = 0,
			["conditions"] = {
			},
			["information"] = {
			},
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
		},
		["10 Final Finesse - White"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 4,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						0.94901960784314, -- [1]
						1, -- [2]
						0.94901960784314, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.25,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = true,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2407",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "cHqP9bjK9W4",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["useMatch_count"] = true,
						["useGroup_count"] = false,
						["groupclone"] = true,
						["duration"] = "1",
						["name_info"] = "aura",
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["use_tooltip"] = false,
						["use_specific_unit"] = false,
						["auranames"] = {
						},
						["match_count"] = "1",
						["debuffType"] = "HARMFUL",
						["unevent"] = "timed",
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["subeventSuffix"] = "_CAST_START",
						["auraspellids"] = {
							"332797", -- [1]
							"332794", -- [2]
						},
						["unit"] = "group",
						["event"] = "Health",
						["subeventPrefix"] = "SPELL",
						["buffShowOn"] = "showOnActive",
						["match_countOperator"] = ">=",
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["useExactSpellId"] = true,
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["custom_type"] = "stateupdate",
						["matchesShowOn"] = "showOnActive",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["text2Font"] = "Friz Quadrata TT",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text1"] = "%n",
			["text1Point"] = "CENTER",
			["anchorFrameType"] = "SCREEN",
			["semver"] = "1.0.26",
			["justify"] = "LEFT",
			["zoom"] = 0,
			["id"] = "10 Final Finesse - White",
			["config"] = {
			},
			["frameStrata"] = 9,
			["width"] = 6.9999866485596,
			["text1Containment"] = "INSIDE",
			["wordWrap"] = "WordWrap",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["07 Drain Essence - White"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 4,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.25,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2412",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "nio6IXdDxlA",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["useMatch_count"] = true,
						["auranames"] = {
						},
						["groupclone"] = true,
						["matchesShowOn"] = "showOnActive",
						["name_info"] = "aura",
						["unit"] = "group",
						["use_tooltip"] = false,
						["subeventPrefix"] = "SPELL",
						["match_countOperator"] = ">=",
						["match_count"] = "1",
						["buffShowOn"] = "showOnActive",
						["unevent"] = "timed",
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["useExactSpellId"] = true,
						["custom_type"] = "stateupdate",
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["event"] = "Health",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HARMFUL",
						["use_specific_unit"] = false,
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["auraspellids"] = {
							"346651", -- [1]
						},
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["duration"] = "1",
						["useGroup_count"] = false,
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["wordWrap"] = "WordWrap",
			["config"] = {
			},
			["text1"] = "%n",
			["text1Containment"] = "INSIDE",
			["width"] = 6.9999866485596,
			["justify"] = "LEFT",
			["semver"] = "1.0.26",
			["zoom"] = 0,
			["id"] = "07 Drain Essence - White",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["frameStrata"] = 9,
			["anchorFrameType"] = "SCREEN",
			["text1Point"] = "CENTER",
			["text2Font"] = "Friz Quadrata TT",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["09 Generals Soak Main"] = {
			["arcLength"] = 360,
			["controlledChildren"] = {
				"Generals Soak Assign/Backup", -- [1]
			},
			["borderBackdrop"] = "Blizzard Tooltip",
			["authorOptions"] = {
			},
			["preferToUpdate"] = false,
			["groupIcon"] = "3614362",
			["anchorPoint"] = "CENTER",
			["fullCircle"] = true,
			["space"] = 2,
			["url"] = "https://wago.io/GtuQCwZ2-/8",
			["actions"] = {
				["start"] = {
				},
				["finish"] = {
				},
				["init"] = {
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["unit"] = "player",
						["type"] = "aura2",
						["spellIds"] = {
						},
						["subeventSuffix"] = "_CAST_START",
						["subeventPrefix"] = "SPELL",
						["names"] = {
						},
						["event"] = "Health",
						["debuffType"] = "HELPFUL",
					},
					["untrigger"] = {
					},
				}, -- [1]
			},
			["columnSpace"] = 1,
			["radius"] = 200,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["align"] = "LEFT",
			["stagger"] = 0,
			["version"] = 8,
			["subRegions"] = {
			},
			["xOffset"] = -683.00015258789,
			["uid"] = "j05Udq0tOgR",
			["load"] = {
				["talent"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["spec"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["rotation"] = 0,
			["backdropColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0.5, -- [4]
			},
			["config"] = {
			},
			["animate"] = false,
			["borderColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["scale"] = 1,
			["gridType"] = "RD",
			["border"] = false,
			["borderEdge"] = "Square Full White",
			["regionType"] = "dynamicgroup",
			["borderSize"] = 2,
			["limit"] = 5,
			["selfPoint"] = "TOPLEFT",
			["yOffset"] = 330.00036621094,
			["constantFactor"] = "RADIUS",
			["rowSpace"] = 1,
			["borderOffset"] = 4,
			["semver"] = "1.0.7",
			["tocversion"] = 90002,
			["id"] = "09 Generals Soak Main",
			["gridWidth"] = 5,
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["internalVersion"] = 45,
			["borderInset"] = 1,
			["sort"] = "ascending",
			["useLimit"] = false,
			["conditions"] = {
			},
			["information"] = {
			},
			["grow"] = "DOWN",
		},
		["Dancing Fever Time Left"] = {
			["outline"] = "OUTLINE",
			["authorOptions"] = {
			},
			["displayText"] = "%p s",
			["shadowYOffset"] = -1,
			["anchorPoint"] = "CENTER",
			["displayText_format_p_time_format"] = 0,
			["customTextUpdate"] = "event",
			["url"] = "https://wago.io/H4Beysoli/3",
			["actions"] = {
				["start"] = {
				},
				["finish"] = {
				},
				["init"] = {
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "combatlog",
						["spellId"] = "347350",
						["subeventSuffix"] = "_AURA_APPLIED",
						["use_absorbMode"] = true,
						["event"] = "Combat Log",
						["unit"] = "player",
						["duration"] = "30",
						["use_spellId"] = true,
						["spellIds"] = {
						},
						["use_unit"] = true,
						["names"] = {
						},
						["subeventPrefix"] = "SPELL",
						["unevent"] = "timed",
						["debuffType"] = "HELPFUL",
					},
					["untrigger"] = {
					},
				}, -- [1]
				{
					["trigger"] = {
						["group_countOperator"] = ">=",
						["type"] = "aura2",
						["debuffType"] = "HARMFUL",
						["useExactSpellId"] = true,
						["group_count"] = "1",
						["auraspellids"] = {
							"347350", -- [1]
						},
						["useGroup_count"] = true,
						["unit"] = "group",
					},
					["untrigger"] = {
					},
				}, -- [2]
				["activeTriggerMode"] = 1,
			},
			["displayText_format_p_format"] = "timed",
			["internalVersion"] = 45,
			["selfPoint"] = "BOTTOM",
			["font"] = "Friz Quadrata TT",
			["version"] = 3,
			["subRegions"] = {
			},
			["load"] = {
				["use_encounterid"] = true,
				["zoneIds"] = "",
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2412",
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["fontSize"] = 20,
			["shadowXOffset"] = 1,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["regionType"] = "text",
			["xOffset"] = 0,
			["preferToUpdate"] = false,
			["displayText_format_ps_format"] = "none",
			["displayText_format_p_time_dynamic_threshold"] = 0,
			["displayText_format_p_time_precision"] = 1,
			["conditions"] = {
			},
			["config"] = {
			},
			["wordWrap"] = "WordWrap",
			["semver"] = "1.0.2",
			["tocversion"] = 90002,
			["id"] = "Dancing Fever Time Left",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["justify"] = "LEFT",
			["uid"] = "SoCDNE8LqLa",
			["yOffset"] = 0,
			["parent"] = "07 Dancing Fever List",
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["fixedWidth"] = 200,
			["information"] = {
			},
			["automaticWidth"] = "Auto",
		},
		["07 Seismic Upheaval Right"] = {
			["iconSource"] = -1,
			["xOffset"] = 0,
			["preferToUpdate"] = false,
			["yOffset"] = 200,
			["anchorPoint"] = "CENTER",
			["cooldownSwipe"] = true,
			["url"] = "https://wago.io/HcLsyDhV7/2",
			["icon"] = true,
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "combatlog",
						["spellId"] = "334498",
						["unevent"] = "timed",
						["duration"] = "5.1",
						["event"] = "Combat Log",
						["subeventPrefix"] = "SPELL",
						["destUnit"] = "player",
						["use_spellId"] = true,
						["spellIds"] = {
						},
						["subeventSuffix"] = "_AURA_APPLIED",
						["names"] = {
						},
						["use_destUnit"] = true,
						["unit"] = "player",
						["debuffType"] = "HARMFUL",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["activeTriggerMode"] = -10,
			},
			["internalVersion"] = 45,
			["keepAspectRatio"] = false,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["desaturate"] = false,
			["rotation"] = 90,
			["version"] = 2,
			["subRegions"] = {
			},
			["height"] = 100,
			["rotate"] = true,
			["load"] = {
				["size"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["talent"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2417",
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
			},
			["textureWrapMode"] = "CLAMPTOBLACKADDITIVE",
			["config"] = {
			},
			["color"] = {
				0, -- [1]
				1, -- [2]
				0.26666666666667, -- [3]
				1, -- [4]
			},
			["actions"] = {
				["start"] = {
					["do_custom"] = true,
					["custom"] = "if DBM then\n    aura_env.region.texture:SetTexture(\"Interface\\\\Addons\\\\DBM-Core\\\\textures\\\\arrows\\\\arrow.blp\", false)\n    aura_env.region.texture:SetTexCoord(0, 1/9, 9/12-1/50, 10/12-1/50)\nend",
					["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\Right.ogg",
					["do_sound"] = true,
				},
				["init"] = {
				},
				["finish"] = {
				},
			},
			["mirror"] = false,
			["authorOptions"] = {
			},
			["regionType"] = "texture",
			["information"] = {
			},
			["blendMode"] = "BLEND",
			["selfPoint"] = "CENTER",
			["cooldownEdge"] = false,
			["texture"] = "Interface\\AddOns\\WeakAuras\\Media\\Textures\\targeting-mark.tga",
			["alpha"] = 1,
			["zoom"] = 0,
			["semver"] = "1.0.1",
			["tocversion"] = 90005,
			["id"] = "07 Seismic Upheaval Right",
			["width"] = 100,
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["cooldownTextDisabled"] = false,
			["uid"] = "y7tYbVvOuPb",
			["inverse"] = false,
			["discrete_rotation"] = 90,
			["conditions"] = {
			},
			["cooldown"] = false,
			["parent"] = "09 Seismic Upheaval Group",
		},
		["03 Lingering Embers >1stack - Orange"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 4,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						1, -- [1]
						0.44313725490196, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.25,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = true,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2402",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "JC2F6ej)Ktr",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["custom_type"] = "stateupdate",
						["useMatch_count"] = true,
						["useStacks"] = true,
						["useGroup_count"] = false,
						["groupclone"] = true,
						["matchesShowOn"] = "showOnActive",
						["name_info"] = "aura",
						["subeventPrefix"] = "SPELL",
						["useExactSpellId"] = true,
						["stacks"] = "1",
						["debuffType"] = "HARMFUL",
						["subeventSuffix"] = "_CAST_START",
						["match_count"] = "1",
						["stacksOperator"] = ">",
						["auraspellids"] = {
							"326430", -- [1]
						},
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["match_countOperator"] = ">=",
						["unit"] = "group",
						["duration"] = "1",
						["event"] = "Health",
						["use_specific_unit"] = false,
						["unevent"] = "timed",
						["use_tooltip"] = false,
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["buffShowOn"] = "showOnActive",
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["auranames"] = {
						},
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["wordWrap"] = "WordWrap",
			["config"] = {
			},
			["text1"] = "%n",
			["text1Containment"] = "INSIDE",
			["width"] = 6.9999866485596,
			["justify"] = "LEFT",
			["semver"] = "1.0.26",
			["zoom"] = 0,
			["id"] = "03 Lingering Embers >1stack - Orange",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["frameStrata"] = 9,
			["anchorFrameType"] = "SCREEN",
			["text1Point"] = "CENTER",
			["text2Font"] = "Friz Quadrata TT",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["nocircle_noleaf_fill"] = {
			["iconSource"] = 0,
			["xOffset"] = 0,
			["preferToUpdate"] = true,
			["customText"] = "function()\n    if aura_env.showText then\n        return aura_env.order\n    else\n        return \"\"\n    end\nend",
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["cooldownSwipe"] = true,
			["cooldownEdge"] = false,
			["icon"] = true,
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "custom",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HELPFUL",
						["event"] = "Health",
						["unit"] = "player",
						["custom_type"] = "status",
						["spellIds"] = {
						},
						["events"] = "ZONE_CHANGED, PLAYER_LOGIN, PLAYER_ENTERING_WORLD",
						["names"] = {
						},
						["check"] = "event",
						["custom"] = "function()\n    WeakAuras.ScanEvents(\"TirnaScitheButtonData\", 6, aura_env.frameData)\n    if aura_env.debugmode then \n        return true \n    end\n    local text = GetMinimapZoneText()\n    if text == aura_env.mistZone then\n        return true\n    else\n        return false\n    end\nend\n\n",
						["subeventPrefix"] = "SPELL",
						["custom_hide"] = "custom",
					},
					["untrigger"] = {
					},
				}, -- [1]
				{
					["trigger"] = {
						["debuffType"] = "HELPFUL",
						["type"] = "custom",
						["custom"] = "function(event,eid,...)\n    if event == \"ENCOUNTER_END\" then \n        if eid == 2392 and aura_env.showEncounter == false then \n            return false \n        end\n        return true \n    elseif event == \"ENCOUNTER_START\" then \n        if aura_env.showEncounter == false then \n            return false\n        else\n            return true\n        end\n    end\n    return true\nend\n\n",
						["custom_type"] = "status",
						["check"] = "event",
						["customName"] = "function()\n    return \nend",
						["events"] = "ENCOUNTER_START, ENCOUNTER_END, PLAYER_ENTERING_WORLD",
						["unit"] = "player",
					},
					["untrigger"] = {
					},
				}, -- [2]
				{
					["trigger"] = {
						["type"] = "custom",
						["custom_type"] = "status",
						["unit"] = "player",
						["customName"] = "function()\n    return aura_env.order            \nend",
						["events"] = "TirnaScitheButtonOrder",
						["check"] = "event",
						["debuffType"] = "HELPFUL",
						["custom"] = "function(_,button,order)\n    if button == 6 then \n        aura_env.order = order\n        if order == 9 then \n            aura_env.order = \"\"\n        end\n        return true\n    elseif button == 9 then\n        aura_env.order = \"\"\n        return true\n    end\n    return true\nend\n\n",
						["custom_hide"] = "timed",
					},
					["untrigger"] = {
					},
				}, -- [3]
				["activeTriggerMode"] = -10,
			},
			["internalVersion"] = 45,
			["keepAspectRatio"] = false,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["desaturate"] = false,
			["version"] = 67,
			["subRegions"] = {
				{
					["glowFrequency"] = 0.25,
					["type"] = "subglow",
					["useGlowColor"] = false,
					["glowType"] = "buttonOverlay",
					["glowLength"] = 10,
					["glowYOffset"] = 0,
					["glowColor"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["glowXOffset"] = 0,
					["glowScale"] = 1,
					["glowThickness"] = 1,
					["glow"] = false,
					["glowLines"] = 8,
					["glowBorder"] = false,
				}, -- [1]
				{
					["text_text_format_p_time_format"] = 0,
					["text_text"] = "%c",
					["text_text_format_p_format"] = "timed",
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Friz Quadrata TT",
					["text_text_format_p_time_precision"] = 1,
					["text_shadowYOffset"] = 0,
					["text_text_format_p_time_dynamic_threshold"] = 60,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "INNER_BOTTOMRIGHT",
					["text_shadowXOffset"] = 0,
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_fontSize"] = 15,
					["anchorXOffset"] = 0,
					["text_visible"] = true,
				}, -- [2]
			},
			["height"] = 80,
			["load"] = {
				["ingroup"] = {
					["single"] = "group",
					["multi"] = {
						["group"] = true,
					},
				},
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["use_encounterid"] = false,
				["difficulty"] = {
					["multi"] = {
						["mythic"] = true,
						["challenge"] = true,
					},
				},
				["use_zone"] = false,
				["zoneIds"] = "1669",
				["use_zoneIds"] = true,
				["zone"] = "",
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["config"] = {
				["encounter"] = false,
				["noCommunication"] = false,
				["debug"] = false,
				["orderText"] = true,
			},
			["actions"] = {
				["start"] = {
				},
				["finish"] = {
				},
				["init"] = {
					["custom"] = "aura_env.debugmode = aura_env.config['debug']\nlocal debug = aura_env.debugmode\naura_env.showEncounter = aura_env.config['encounter']\naura_env.locale=GetLocale()\naura_env.mistZone = \"Mistveil Tangle\"\naura_env.order = \"\"\naura_env.showText = aura_env.config['orderText']\naura_env.noCommunication = aura_env.config['noCommunication']\nif aura_env.locale == \"koKR\" then\n    aura_env.mistZone = \"안개장막 덩굴숲\"\nelseif aura_env.locale == \"frFR\" then\n    aura_env.mistZone = \"Maquis Voile-de-Brume\"\nelseif aura_env.locale == \"deDE\" then\n    aura_env.mistZone = \"Nebelschleierdickicht\"\nelseif aura_env.locale == \"zhCN\" then\n    aura_env.mistZone = \"纱雾迷结\"\nelseif aura_env.locale == \"esES\" then\n    aura_env.mistZone = \"Espesura Velo de Niebla\"\nelseif aura_env.locale == \"zhTW\" then\n    aura_env.mistZone = \"霧紗密林\"\nelseif aura_env.locale == \"esMX\" then\n    aura_env.mistZone = \"Espesura Veloniebla\"\nelseif aura_env.locale == \"ruRU\" then\n    aura_env.mistZone = \"Туманная чащоба\"\nelseif aura_env.locale == \"ptBR\" then\n    aura_env.mistZone = \"Enleio do Véu da Névoa\"\nelseif aura_env.locale == \"itIT\" then\n    aura_env.mistZone = \"Intrico Velofosco\"\nelseif aura_env.locale == \"ptPT\" then\n    aura_env.mistZone = \"Enleio do Véu da Névoa\"        \nend\nlocal values = {1, 0, 0, 6}\nlocal message = string.format(\"%d %d %d %d\", values[1], values[2], values[3], values[4])\nlocal name, _ = UnitName(\"player\")\n\nif not aura_env.frameData then\n    local b = CreateFrame(\"Button\", nil, WeakAuras.regions[aura_env.id].region)\n    b:SetAllPoints(WeakAuras.regions[aura_env.id].region)\n    b:RegisterForClicks(\"LeftButtonDown\", \"RightButtonDown\")\n    aura_env.frameData = b\n    WeakAuras.ScanEvents(\"TirnaScitheButtonData\", 6, b)\nend\nif aura_env.noCommunication then\n    aura_env.frameData:SetScript(\"OnClick\", function(self, button)\n            if debug then print(\"Click Registered - true\") end\n            WeakAuras.ScanEvents(\"GlowUpdate\")\n            C_ChatInfo.RegisterAddonMessagePrefix(\"Tirna Ver2\") \n            if debug then \n                print(\"noCommunication sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"WHISPER\", name)) \n            else\n                C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"WHISPER\", name)\n            end\n            \n    end)\nelse\n    aura_env.frameData:SetScript(\"OnClick\", function(self, button)\n            if debug then print(\"Click Registered - true\") end\n            WeakAuras.ScanEvents(\"GlowUpdate\")\n            C_ChatInfo.RegisterAddonMessagePrefix(\"Tirna Ver2\")\n            if IsPartyLFG() then            \n                if debug then \n                    print(\"LFG sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"INSTANCE_CHAT\")) \n                else\n                    C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"INSTANCE_CHAT\")\n                end\n            elseif UnitInParty(\"player\") then            \n                if debug then \n                    print(\"party sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"PARTY\")) \n                else\n                    C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"PARTY\")\n                end\n            else         \n                if debug then \n                    print(\"solo sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"WHISPER\", name)) \n                else\n                    C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"WHISPER\", name)\n                end\n            end\n    end)\nend",
					["do_custom"] = true,
				},
			},
			["authorOptions"] = {
				{
					["type"] = "toggle",
					["key"] = "orderText",
					["default"] = true,
					["name"] = "Show Order Text",
					["useDesc"] = false,
					["width"] = 1,
				}, -- [1]
				{
					["type"] = "toggle",
					["key"] = "encounter",
					["default"] = false,
					["name"] = "Show during Boss",
					["useDesc"] = false,
					["width"] = 1,
				}, -- [2]
				{
					["type"] = "toggle",
					["key"] = "noCommunication",
					["default"] = false,
					["name"] = "Disable Cross-Communication with others running the WA",
					["useDesc"] = false,
					["width"] = 2,
				}, -- [3]
				{
					["type"] = "toggle",
					["key"] = "debug",
					["default"] = false,
					["name"] = "|cffff0000Debug testing|r (requires reload for full debug)",
					["useDesc"] = false,
					["width"] = 2,
				}, -- [4]
			},
			["regionType"] = "icon",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["cooldown"] = false,
			["conditions"] = {
			},
			["parent"] = "Mists of Tirna Scithe guessing game v2",
			["semver"] = "2.0.2",
			["anchorFrameType"] = "SCREEN",
			["zoom"] = 0,
			["auto"] = false,
			["tocversion"] = 90005,
			["id"] = "nocircle_noleaf_fill",
			["alpha"] = 1,
			["frameStrata"] = 1,
			["width"] = 80,
			["cooldownTextDisabled"] = false,
			["uid"] = "dhAZI5EjoCP",
			["inverse"] = false,
			["selfPoint"] = "CENTER",
			["displayIcon"] = "Interface\\AddOns\\guess\\nocircle_lotus_fill.tga",
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["url"] = "https://wago.io/hVLym_eLv/67",
		},
		["Innervate"] = {
			["iconSource"] = -1,
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["yOffset"] = -107.4995727539063,
			["anchorPoint"] = "CENTER",
			["cooldownSwipe"] = true,
			["cooldownEdge"] = false,
			["icon"] = true,
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "aura2",
						["subeventSuffix"] = "_CAST_START",
						["event"] = "Health",
						["subeventPrefix"] = "SPELL",
						["spellIds"] = {
						},
						["auranames"] = {
							"Innervate", -- [1]
						},
						["useName"] = true,
						["names"] = {
						},
						["unit"] = "player",
						["debuffType"] = "HELPFUL",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["internalVersion"] = 45,
			["keepAspectRatio"] = false,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["subRegions"] = {
				{
					["text_shadowXOffset"] = 0,
					["text_text_format_s_format"] = "none",
					["text_text"] = "%p",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Friz Quadrata TT",
					["text_text_format_p_time_format"] = 0,
					["text_shadowYOffset"] = 0,
					["text_text_format_p_time_dynamic_threshold"] = 60,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "INNER_BOTTOMRIGHT",
					["text_text_format_p_format"] = "timed",
					["text_text_format_p_time_precision"] = 1,
					["text_fontSize"] = 20,
					["anchorXOffset"] = 0,
					["text_visible"] = true,
				}, -- [1]
				{
					["glowFrequency"] = 0.25,
					["type"] = "subglow",
					["useGlowColor"] = false,
					["glowType"] = "ACShine",
					["glowLength"] = 10,
					["glowYOffset"] = 0,
					["glowColor"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["glowXOffset"] = 0,
					["glow"] = true,
					["glowScale"] = 1.5,
					["glowThickness"] = 1,
					["glowLines"] = 8,
					["glowBorder"] = false,
				}, -- [2]
			},
			["height"] = 64,
			["load"] = {
				["talent"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["class"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["regionType"] = "icon",
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["actions"] = {
				["start"] = {
					["sound"] = "Interface\\Addons\\WeakAuras\\PowerAurasMedia\\Sounds\\ESPARK1.ogg",
					["do_sound"] = true,
				},
				["init"] = {
				},
				["finish"] = {
				},
			},
			["cooldown"] = false,
			["cooldownTextDisabled"] = false,
			["xOffset"] = -242.4989013671875,
			["frameStrata"] = 1,
			["id"] = "Innervate",
			["width"] = 64,
			["alpha"] = 0.5,
			["anchorFrameType"] = "SCREEN",
			["config"] = {
			},
			["uid"] = "Ggw(qsGorOV",
			["inverse"] = false,
			["zoom"] = 0,
			["conditions"] = {
			},
			["information"] = {
			},
			["authorOptions"] = {
			},
		},
		["Warrior 2"] = {
			["sparkWidth"] = 10,
			["iconSource"] = -1,
			["xOffset"] = 0,
			["preferToUpdate"] = false,
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["sparkRotation"] = 0,
			["url"] = "https://wago.io/ZG34ifiRf/1",
			["backgroundColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				0.48237186670303, -- [4]
			},
			["fontFlags"] = "OUTLINE",
			["icon_color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["selfPoint"] = "CENTER",
			["barColor"] = {
				0.73333333333333, -- [1]
				0.49411764705882, -- [2]
				0.40392156862745, -- [3]
				1, -- [4]
			},
			["desaturate"] = false,
			["sparkOffsetY"] = 0,
			["load"] = {
				["ingroup"] = {
					["multi"] = {
					},
				},
				["use_never"] = false,
				["class"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["level"] = "100",
				["size"] = {
					["single"] = "party",
					["multi"] = {
						["ratedpvp"] = true,
						["arena"] = true,
						["ratedarena"] = true,
						["pvp"] = true,
					},
				},
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_level"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["difficulty"] = {
					["single"] = "challenge",
					["multi"] = {
						["challenge"] = true,
					},
				},
				["zoneIds"] = "",
				["faction"] = {
					["multi"] = {
					},
				},
				["use_size"] = false,
				["race"] = {
					["multi"] = {
					},
				},
				["use_zone"] = false,
				["pvptalent"] = {
					["multi"] = {
					},
				},
			},
			["useAdjustededMin"] = false,
			["regionType"] = "aurabar",
			["texture"] = "Glamour4",
			["zoom"] = 0.28,
			["auto"] = true,
			["tocversion"] = 80300,
			["alpha"] = 1,
			["uid"] = "3CM4V4fIAtv",
			["displayIcon"] = "Interface\\Icons\\spell_holy_rebuke",
			["stacksPoint"] = "BOTTOMRIGHT",
			["borderBackdrop"] = "Blizzard Tooltip",
			["parent"] = "Enemy Interrupt Icons",
			["customTextUpdate"] = "update",
			["triggers"] = {
				{
					["trigger"] = {
						["spellId"] = "Flask of the warm sun",
						["ownOnly"] = true,
						["genericShowOn"] = "showOnActive",
						["subeventPrefix"] = "SPELL",
						["duration"] = "15",
						["names"] = {
							"Greater Draenic Intellect Flask", -- [1]
						},
						["name"] = "Flask of the warm sun ",
						["sourceName"] = "",
						["use_specific_sourceunit"] = true,
						["inverse"] = true,
						["use_cloneId"] = true,
						["spellName"] = "Pummel",
						["type"] = "combatlog",
						["use_sourceName"] = false,
						["unevent"] = "timed",
						["subeventSuffix"] = "_CAST_SUCCESS",
						["use_sourceFlags2"] = true,
						["debuffType"] = "HELPFUL",
						["event"] = "Combat Log",
						["sourceFlags2"] = "Hostile",
						["use_name"] = true,
						["use_spellName"] = true,
						["spellIds"] = {
						},
						["use_sourceUnit"] = false,
						["use_destunit"] = false,
						["custom_hide"] = "timed",
						["sourceUnit"] = "Benzikura",
						["unit"] = "player",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
			},
			["backdropInFront"] = false,
			["stickyDuration"] = false,
			["version"] = 1,
			["subRegions"] = {
				{
					["type"] = "subborder",
					["border_anchor"] = "bar",
					["border_size"] = 16,
					["border_color"] = {
						0.68627450980392, -- [1]
						0.45490196078431, -- [2]
						0.3843137254902, -- [3]
						0.5, -- [4]
					},
					["border_visible"] = true,
					["border_edge"] = "1 Pixel",
					["border_offset"] = 2,
				}, -- [1]
				{
					["type"] = "aurabar_bar",
				}, -- [2]
				{
					["text_shadowXOffset"] = 1,
					["text_text"] = "%p",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Vixar",
					["text_text_format_p_time_format"] = 0,
					["text_shadowYOffset"] = -1,
					["text_text_format_p_time_dynamic_threshold"] = 3,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "ICON_CENTER",
					["text_text_format_p_format"] = "timed",
					["text_visible"] = true,
					["text_fontSize"] = 20,
					["anchorXOffset"] = 0,
					["text_text_format_p_time_precision"] = 1,
				}, -- [3]
			},
			["height"] = 30,
			["sparkBlendMode"] = "ADD",
			["useAdjustededMax"] = false,
			["displayStacks"] = "%c",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["sparkColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["sparkOffsetX"] = 0,
			["borderInFront"] = false,
			["spark"] = false,
			["icon_side"] = "LEFT",
			["authorOptions"] = {
			},
			["actions"] = {
				["start"] = {
					["do_message"] = false,
					["message_type"] = "SAY",
				},
				["init"] = {
				},
				["finish"] = {
				},
			},
			["sparkHeight"] = 30,
			["backdropColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0.5, -- [4]
			},
			["stacksContainment"] = "INSIDE",
			["config"] = {
			},
			["semver"] = "1.0.0",
			["width"] = 30,
			["sparkHidden"] = "NEVER",
			["id"] = "Warrior 2",
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["sparkTexture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
			["icon"] = true,
			["inverse"] = false,
			["sparkDesature"] = false,
			["orientation"] = "HORIZONTAL",
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["sparkRotationMode"] = "AUTO",
		},
		["Reset"] = {
			["iconSource"] = 0,
			["xOffset"] = 10,
			["preferToUpdate"] = true,
			["customText"] = "function()\n    return aura_env.resetText\nend",
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["cooldownSwipe"] = true,
			["cooldownEdge"] = false,
			["icon"] = true,
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "custom",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HELPFUL",
						["event"] = "Health",
						["unit"] = "player",
						["custom_type"] = "status",
						["spellIds"] = {
						},
						["events"] = "ZONE_CHANGED, PLAYER_LOGIN, PLAYER_ENTERING_WORLD",
						["names"] = {
						},
						["check"] = "event",
						["custom"] = "function()\n    if aura_env.debugmode then \n        return true \n    end\n    local text = GetMinimapZoneText()\n    if text == aura_env.mistZone then\n        return true\n    else\n        return false\n    end\nend",
						["subeventPrefix"] = "SPELL",
						["custom_hide"] = "custom",
					},
					["untrigger"] = {
					},
				}, -- [1]
				{
					["trigger"] = {
						["debuffType"] = "HELPFUL",
						["type"] = "custom",
						["custom"] = "function(event,eid,...)\n    if event == \"ENCOUNTER_END\" then \n        if eid == 2392 and aura_env.showEncounter == false then \n            return false \n        end\n        return true \n    elseif event == \"ENCOUNTER_START\" then \n        if aura_env.showEncounter == false then \n            return false\n        else\n            return true\n        end\n    end\n    return true\nend",
						["custom_type"] = "status",
						["check"] = "event",
						["customName"] = "function()\n    return \nend",
						["events"] = "ENCOUNTER_START, ENCOUNTER_END, PLAYER_ENTERING_WORLD",
						["unit"] = "player",
					},
					["untrigger"] = {
					},
				}, -- [2]
				["disjunctive"] = "all",
				["activeTriggerMode"] = -10,
			},
			["internalVersion"] = 45,
			["keepAspectRatio"] = false,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["desaturate"] = false,
			["discrete_rotation"] = 0,
			["version"] = 67,
			["subRegions"] = {
				{
					["text_shadowXOffset"] = -1,
					["text_text_format_s_format"] = "none",
					["text_text"] = "%c",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Arial Narrow",
					["text_shadowYOffset"] = 1,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "CENTER",
					["text_fontSize"] = 22,
					["anchorXOffset"] = 0,
					["text_visible"] = true,
				}, -- [1]
				{
					["type"] = "subborder",
					["border_size"] = 2,
					["border_color"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["border_visible"] = true,
					["border_edge"] = "Square Full White",
					["border_offset"] = 2,
				}, -- [2]
			},
			["height"] = 25,
			["rotate"] = true,
			["load"] = {
				["ingroup"] = {
					["single"] = "group",
					["multi"] = {
						["group"] = true,
					},
				},
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["use_encounterid"] = false,
				["difficulty"] = {
					["multi"] = {
						["mythic"] = true,
						["challenge"] = true,
					},
				},
				["use_zone"] = false,
				["zoneIds"] = "1669",
				["use_zoneIds"] = true,
				["zone"] = "",
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["anchorFrameType"] = "SCREEN",
			["textureWrapMode"] = "CLAMPTOBLACKADDITIVE",
			["cooldownTextDisabled"] = false,
			["selfPoint"] = "CENTER",
			["parent"] = "Mists of Tirna Scithe guessing game v2",
			["cooldown"] = false,
			["mirror"] = false,
			["conditions"] = {
			},
			["regionType"] = "icon",
			["rotation"] = 0,
			["blendMode"] = "BLEND",
			["actions"] = {
				["start"] = {
				},
				["finish"] = {
				},
				["init"] = {
					["custom"] = "aura_env.debugmode = aura_env.config['debug']\nlocal debug = aura_env.debugmode\naura_env.showEncounter = aura_env.config['encounter']\naura_env.noCommunication = aura_env.config['noCommunication']\naura_env.useEng = aura_env.config['useEng']\naura_env.eLang = aura_env.config['eLang']\naura_env.locale=GetLocale()\naura_env.mistZone = \"Mistveil Tangle\"\naura_env.resetText = \"Reset\"\nif aura_env.locale == \"koKR\" then\n    aura_env.mistZone = \"안개장막 덩굴숲\"\n    aura_env.resetText = \"초기화\"\nelseif aura_env.locale == \"frFR\" then\n    aura_env.mistZone = \"Maquis Voile-de-Brume\"\n    aura_env.resetText = \"Reset\"\nelseif aura_env.locale == \"deDE\" then\n    aura_env.mistZone = \"Nebelschleierdickicht\"\n    aura_env.resetText = \"Zurücksetzen\"\nelseif aura_env.locale == \"zhCN\" then\n    aura_env.mistZone = \"纱雾迷结\"\n    aura_env.resetText = \"重置\"\nelseif aura_env.locale == \"esES\" then\n    aura_env.mistZone = \"Espesura Velo de Niebla\"\n    aura_env.resetText = \"Resetear\"\nelseif aura_env.locale == \"zhTW\" then\n    aura_env.mistZone = \"霧紗密林\"\n    aura_env.resetText = \"重置\"\nelseif aura_env.locale == \"esMX\" then\n    aura_env.mistZone = \"Espesura Veloniebla\"\n    aura_env.resetText = \"Reiniciar\"\nelseif aura_env.locale == \"ruRU\" then\n    aura_env.mistZone = \"Туманная чащоба\"\n    aura_env.resetText = \"Сброс\"\nelseif aura_env.locale == \"ptBR\" then\n    aura_env.mistZone = \"Enleio do Véu da Névoa\"\n    aura_env.resetText = \"Resetar\"\nelseif aura_env.locale == \"itIT\" then\n    aura_env.mistZone = \"Intrico Velofosco\"\n    aura_env.resetText = \"Reset\"\nelseif aura_env.locale == \"ptPT\" then\n    aura_env.mistZone = \"Enleio do Véu da Névoa\" \nend\nif aura_env.eLang == 2 and not aura_env.useEng then --Dutch\n    aura_env.resetText = \"Reset\"\nelseif aura_env.eLang == 3 and not aura_env.useEng then --Norwegian\n    aura_env.resetText = \"Tilbakestill\"\nelseif aura_env.eLang == 4 and not aura_env.useEng then --Hungarian\n    aura_env.resetText = \"Töröl\"\nend\nif aura_env.useEng then \n    aura_env.resetText = \"Reset\" \nend\n\nlocal values = {9, 9, 9, 9}\nlocal message = string.format(\"%d %d %d %d\", values[1], values[2], values[3], values[4])\nlocal name, _ = UnitName(\"player\")\nif not aura_env.frameData then\n    local b = CreateFrame(\"Button\", nil, WeakAuras.regions[aura_env.id].region)\n    b:SetAllPoints(WeakAuras.regions[aura_env.id].region)\n    b:RegisterForClicks(\"LeftButtonDown\")\n    aura_env.frameData = b\nend\nif aura_env.noCommunication then\n    aura_env.frameData:SetScript(\"OnClick\", function()\n            if debug then print(\"Click Registered - true\") end\n            C_ChatInfo.RegisterAddonMessagePrefix(\"Tirna Ver2\")\n            if debug then \n                print(\"noCommunication sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message, \"WHISPER\", name)) \n            else\n                C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message, \"WHISPER\", name)\n            end\n            WeakAuras.ScanEvents(\"GlowReset\")            \n    end)\nelse\n    aura_env.frameData:SetScript(\"OnClick\", function()\n            if debug then print(\"Click Registered - true\") end\n            C_ChatInfo.RegisterAddonMessagePrefix(\"Tirna Ver2\")\n            if IsPartyLFG() then            \n                if debug then \n                    print(\"LFG sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message, \"INSTANCE_CHAT\")) \n                else\n                    C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message, \"INSTANCE_CHAT\")\n                end\n            elseif UnitInParty(\"player\") then            \n                if debug then \n                    print(\"party sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message, \"PARTY\")) \n                else\n                    C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message, \"PARTY\")\n                end\n            else               \n                if debug then \n                    print(\"solo sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message, \"WHISPER\", name)) \n                else\n                    C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message, \"WHISPER\", name)\n                end\n            end\n            WeakAuras.ScanEvents(\"GlowReset\", true)\n    end)\nend\naura_env.LCG = LibStub(\"LibCustomGlow-1.0\")",
					["do_custom"] = true,
				},
			},
			["uid"] = "gwvcSCXq0)t",
			["texture"] = "Interface\\Addons\\WeakAuras\\PowerAurasMedia\\Auras\\Aura3",
			["alpha"] = 1,
			["zoom"] = 1,
			["auto"] = false,
			["tocversion"] = 90005,
			["id"] = "Reset",
			["semver"] = "2.0.2",
			["frameStrata"] = 1,
			["width"] = 300,
			["url"] = "https://wago.io/hVLym_eLv/67",
			["config"] = {
				["debug"] = false,
				["eLang"] = 1,
				["useEng"] = false,
				["noCommunication"] = false,
				["encounter"] = false,
			},
			["inverse"] = false,
			["authorOptions"] = {
				{
					["type"] = "toggle",
					["key"] = "encounter",
					["default"] = false,
					["name"] = "Show during Boss",
					["useDesc"] = false,
					["width"] = 1,
				}, -- [1]
				{
					["type"] = "toggle",
					["key"] = "noCommunication",
					["default"] = false,
					["name"] = "Disable Cross-Communication with others running the WA",
					["useDesc"] = false,
					["width"] = 2,
				}, -- [2]
				{
					["type"] = "toggle",
					["key"] = "useEng",
					["width"] = 2,
					["name"] = "Use English instead of Client Regional Language",
					["useDesc"] = false,
					["default"] = false,
					["desc"] = "(Overwrites selection below)",
				}, -- [3]
				{
					["type"] = "select",
					["values"] = {
						"Use Client Region Language", -- [1]
						"Dutch", -- [2]
						"Norwegian (Bokmål)", -- [3]
						"Hungarian", -- [4]
					},
					["default"] = 1,
					["name"] = "Language (non-client region Language)",
					["useDesc"] = false,
					["key"] = "eLang",
					["width"] = 1,
				}, -- [4]
				{
					["type"] = "toggle",
					["key"] = "debug",
					["default"] = false,
					["name"] = "|cffff0000Debug testing|r (requires reload for full debug)",
					["useDesc"] = false,
					["width"] = 2,
				}, -- [5]
			},
			["displayIcon"] = "Interface\\AddOns\\WeakAuras\\Media\\Textures\\Square_White_Border.tga",
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["color"] = {
				1, -- [1]
				0.57647058823529, -- [2]
				0.71372549019608, -- [3]
				0.80000001192093, -- [4]
			},
		},
		["09 Seismic Upheaval Group"] = {
			["controlledChildren"] = {
				"07 Seismic Upheaval Stack Alert", -- [1]
				"07 Seismic Upheaval Right", -- [2]
				"07 Seismic Upheaval Left", -- [3]
			},
			["borderBackdrop"] = "Blizzard Tooltip",
			["xOffset"] = 0,
			["preferToUpdate"] = false,
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["borderColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["url"] = "https://wago.io/HcLsyDhV7/2",
			["actions"] = {
				["start"] = {
				},
				["init"] = {
				},
				["finish"] = {
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["debuffType"] = "HELPFUL",
						["type"] = "aura2",
						["spellIds"] = {
						},
						["subeventSuffix"] = "_CAST_START",
						["subeventPrefix"] = "SPELL",
						["names"] = {
						},
						["event"] = "Health",
						["unit"] = "player",
					},
					["untrigger"] = {
					},
				}, -- [1]
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["version"] = 2,
			["subRegions"] = {
			},
			["load"] = {
				["talent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
			},
			["backdropColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0.5, -- [4]
			},
			["scale"] = 1,
			["border"] = false,
			["borderEdge"] = "Square Full White",
			["regionType"] = "group",
			["borderSize"] = 2,
			["borderOffset"] = 4,
			["semver"] = "1.0.1",
			["tocversion"] = 90005,
			["id"] = "09 Seismic Upheaval Group",
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["config"] = {
			},
			["borderInset"] = 1,
			["selfPoint"] = "CENTER",
			["authorOptions"] = {
			},
			["conditions"] = {
			},
			["information"] = {
			},
			["uid"] = "Qrj23qaOeLu",
		},
		["Shaman 2"] = {
			["sparkWidth"] = 10,
			["iconSource"] = -1,
			["authorOptions"] = {
			},
			["preferToUpdate"] = false,
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["sparkRotation"] = 0,
			["url"] = "https://wago.io/ZG34ifiRf/1",
			["backgroundColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				0.46527713537216, -- [4]
			},
			["fontFlags"] = "OUTLINE",
			["icon_color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["selfPoint"] = "CENTER",
			["barColor"] = {
				0, -- [1]
				0.49803921568627, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["desaturate"] = false,
			["sparkOffsetY"] = 0,
			["load"] = {
				["ingroup"] = {
					["multi"] = {
					},
				},
				["use_never"] = false,
				["class"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["level"] = "100",
				["size"] = {
					["single"] = "party",
					["multi"] = {
						["ratedpvp"] = true,
						["arena"] = true,
						["ratedarena"] = true,
						["pvp"] = true,
					},
				},
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_level"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["difficulty"] = {
					["single"] = "challenge",
					["multi"] = {
						["challenge"] = true,
					},
				},
				["zoneIds"] = "",
				["faction"] = {
					["multi"] = {
					},
				},
				["use_size"] = false,
				["use_zone"] = false,
				["race"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
			},
			["useAdjustededMin"] = false,
			["regionType"] = "aurabar",
			["texture"] = "Glamour4",
			["zoom"] = 0.28,
			["spark"] = false,
			["tocversion"] = 80300,
			["alpha"] = 1,
			["sparkColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["displayIcon"] = "Interface\\Icons\\spell_holy_rebuke",
			["stacksPoint"] = "BOTTOMRIGHT",
			["sparkOffsetX"] = 0,
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["sparkRotationMode"] = "AUTO",
			["triggers"] = {
				{
					["trigger"] = {
						["duration"] = "12",
						["spellId"] = "",
						["names"] = {
							"Greater Draenic Intellect Flask", -- [1]
						},
						["inverse"] = true,
						["ownOnly"] = true,
						["genericShowOn"] = "showOnActive",
						["unit"] = "player",
						["debuffType"] = "HELPFUL",
						["use_specific_sourceunit"] = true,
						["name"] = "Flask of the warm sun ",
						["use_spellId"] = false,
						["spellName"] = "Wind Shear",
						["use_destName"] = false,
						["use_cloneId"] = true,
						["custom_hide"] = "timed",
						["use_sourceFlags2"] = true,
						["type"] = "combatlog",
						["unevent"] = "timed",
						["subeventSuffix"] = "_CAST_SUCCESS",
						["sourceName"] = "Draemii",
						["use_sourceName"] = false,
						["event"] = "Combat Log",
						["sourceFlags2"] = "Hostile",
						["use_name"] = true,
						["use_spellName"] = true,
						["spellIds"] = {
						},
						["use_sourceUnit"] = false,
						["use_destunit"] = false,
						["use_destUnit"] = false,
						["sourceUnit"] = "Benzikura",
						["subeventPrefix"] = "SPELL",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
			},
			["backdropInFront"] = false,
			["stickyDuration"] = false,
			["version"] = 1,
			["subRegions"] = {
				{
					["type"] = "subborder",
					["border_anchor"] = "bar",
					["border_size"] = 16,
					["border_color"] = {
						0, -- [1]
						0.54509803921569, -- [2]
						1, -- [3]
						0.5, -- [4]
					},
					["border_visible"] = true,
					["border_edge"] = "1 Pixel",
					["border_offset"] = 2,
				}, -- [1]
				{
					["type"] = "aurabar_bar",
				}, -- [2]
				{
					["text_shadowXOffset"] = 1,
					["text_text"] = "%p",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Vixar",
					["text_text_format_p_time_format"] = 0,
					["text_shadowYOffset"] = -1,
					["text_text_format_p_time_dynamic_threshold"] = 3,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "ICON_CENTER",
					["text_text_format_p_format"] = "timed",
					["text_visible"] = true,
					["text_fontSize"] = 20,
					["anchorXOffset"] = 0,
					["text_text_format_p_time_precision"] = 1,
				}, -- [3]
			},
			["height"] = 30,
			["sparkBlendMode"] = "ADD",
			["useAdjustededMax"] = false,
			["displayStacks"] = "%c",
			["parent"] = "Enemy Interrupt Icons",
			["uid"] = "(LppjezyEh(",
			["borderBackdrop"] = "Blizzard Tooltip",
			["borderInFront"] = false,
			["sparkTexture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
			["icon_side"] = "LEFT",
			["xOffset"] = 0,
			["actions"] = {
				["start"] = {
					["do_message"] = false,
					["message_type"] = "SAY",
				},
				["init"] = {
				},
				["finish"] = {
				},
			},
			["sparkHeight"] = 30,
			["backdropColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0.5, -- [4]
			},
			["stacksContainment"] = "INSIDE",
			["config"] = {
			},
			["semver"] = "1.0.0",
			["width"] = 30,
			["sparkHidden"] = "NEVER",
			["id"] = "Shaman 2",
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["auto"] = true,
			["icon"] = true,
			["inverse"] = false,
			["sparkDesature"] = false,
			["orientation"] = "HORIZONTAL",
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["customTextUpdate"] = "update",
		},
		["07 Dark Recital Stack Up Alert"] = {
			["outline"] = "OUTLINE",
			["color"] = {
				0, -- [1]
				0.87843137254902, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["displayText"] = "STACK UP FOR RECITAL",
			["yOffset"] = 285,
			["anchorPoint"] = "CENTER",
			["displayText_format_p_time_format"] = 0,
			["customTextUpdate"] = "event",
			["url"] = "https://wago.io/seLAtvE5B/1",
			["actions"] = {
				["start"] = {
					["do_custom"] = false,
					["custom"] = "",
					["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\AirHorn.ogg",
					["do_sound"] = false,
				},
				["finish"] = {
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["duration"] = "1",
						["message_operator"] = "find('%s')",
						["use_unit"] = true,
						["remaining"] = "5",
						["debuffType"] = "HELPFUL",
						["type"] = "addons",
						["unevent"] = "auto",
						["use_absorbMode"] = true,
						["event"] = "DBM Timer",
						["remaining_operator"] = "<=",
						["message"] = "Dark Recital",
						["subeventSuffix"] = "_CAST_START",
						["spellIds"] = {
						},
						["use_remaining"] = true,
						["use_message"] = true,
						["unit"] = "player",
						["names"] = {
						},
						["subeventPrefix"] = "SPELL",
					},
					["untrigger"] = {
					},
				}, -- [1]
				{
					["trigger"] = {
						["use_absorbMode"] = true,
						["message_operator"] = "find('%s')",
						["use_unit"] = true,
						["remaining"] = "5",
						["text_operator"] = "find('%s')",
						["debuffType"] = "HELPFUL",
						["use_remaining"] = true,
						["subeventPrefix"] = "SPELL",
						["unevent"] = "auto",
						["subeventSuffix"] = "_CAST_START",
						["text"] = "Dark Recital",
						["event"] = "BigWigs Timer",
						["type"] = "addons",
						["message"] = "Evasive Lunge",
						["use_text"] = true,
						["spellIds"] = {
						},
						["remaining_operator"] = "<=",
						["use_message"] = true,
						["names"] = {
						},
						["unit"] = "player",
						["duration"] = "1",
					},
					["untrigger"] = {
					},
				}, -- [2]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = -10,
			},
			["displayText_format_p_format"] = "timed",
			["internalVersion"] = 45,
			["wordWrap"] = "WordWrap",
			["font"] = "Friz Quadrata TT",
			["version"] = 1,
			["subRegions"] = {
			},
			["load"] = {
				["use_encounterid"] = true,
				["zoneIds"] = "",
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2412",
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["fontSize"] = 60,
			["shadowXOffset"] = 1,
			["selfPoint"] = "BOTTOM",
			["regionType"] = "text",
			["displayText_format_p_time_dynamic_threshold"] = 60,
			["preferToUpdate"] = false,
			["conditions"] = {
			},
			["displayText_format_p_time_precision"] = 1,
			["parent"] = "07 Dark Recital Group",
			["xOffset"] = 0,
			["config"] = {
			},
			["justify"] = "LEFT",
			["tocversion"] = 90002,
			["id"] = "07 Dark Recital Stack Up Alert",
			["shadowYOffset"] = -1,
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["authorOptions"] = {
			},
			["uid"] = "Ag3pNBqkN3T",
			["semver"] = "1.0.0",
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["fixedWidth"] = 200,
			["information"] = {
			},
			["automaticWidth"] = "Auto",
		},
		["06 Shared Suffering - Yellow"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 4,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						1, -- [1]
						0.89803921568627, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.25,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = true,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2406",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "QNq2aTjAgdB",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["useMatch_count"] = true,
						["auranames"] = {
						},
						["groupclone"] = true,
						["matchesShowOn"] = "showOnActive",
						["name_info"] = "aura",
						["unit"] = "group",
						["use_tooltip"] = false,
						["subeventPrefix"] = "SPELL",
						["match_countOperator"] = ">=",
						["match_count"] = "1",
						["buffShowOn"] = "showOnActive",
						["unevent"] = "timed",
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["useExactSpellId"] = true,
						["custom_type"] = "stateupdate",
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["event"] = "Health",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HARMFUL",
						["use_specific_unit"] = false,
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["auraspellids"] = {
							"324983", -- [1]
							"324982", -- [2]
						},
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["duration"] = "1",
						["useGroup_count"] = false,
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["wordWrap"] = "WordWrap",
			["config"] = {
			},
			["text1"] = "%n",
			["text1Containment"] = "INSIDE",
			["width"] = 6.9999866485596,
			["justify"] = "LEFT",
			["semver"] = "1.0.26",
			["zoom"] = 0,
			["id"] = "06 Shared Suffering - Yellow",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["frameStrata"] = 9,
			["anchorFrameType"] = "SCREEN",
			["text1Point"] = "CENTER",
			["text2Font"] = "Friz Quadrata TT",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["Combat Timer"] = {
			["outline"] = "OUTLINE",
			["authorOptions"] = {
				{
					["text"] = "General Settings",
					["type"] = "description",
					["fontSize"] = "large",
					["width"] = 2,
				}, -- [1]
				{
					["type"] = "toggle",
					["key"] = "showMain",
					["width"] = 2,
					["name"] = "Show main combat timer",
					["useDesc"] = true,
					["default"] = true,
					["desc"] = "Toggles the main combat timer, if you don't need that but want the alerts then turn it off. ",
				}, -- [2]
				{
					["type"] = "toggle",
					["key"] = "showProg",
					["width"] = 2,
					["name"] = "Show alert timers",
					["useDesc"] = true,
					["default"] = false,
					["desc"] = "Shows the countdown times on each alert",
				}, -- [3]
				{
					["text"] = "Press |TInterface\\Addons\\WeakAuras\\Media\\Textures\\add.tga:0|t and input values for each alert that you want to create. \n\nI've added a couple of example alerts below. Feel free to delete them and start again, as you need. \n\nSounds can be added in the Conditions tab for any alerts that you want. I've added one in there as an example, feel free to delete it if you don't want it. ",
					["type"] = "description",
					["fontSize"] = "medium",
					["width"] = 2,
				}, -- [4]
				{
					["subOptions"] = {
						{
							["type"] = "number",
							["useDesc"] = true,
							["max"] = 1000,
							["step"] = 1,
							["width"] = 1,
							["min"] = 0,
							["name"] = "Time",
							["default"] = 0,
							["key"] = "time",
							["desc"] = "Number of seconds after which the alert should display",
						}, -- [1]
						{
							["type"] = "number",
							["useDesc"] = true,
							["max"] = 20,
							["step"] = 1,
							["width"] = 1,
							["min"] = 1,
							["name"] = "Duration",
							["default"] = 5,
							["key"] = "duration",
							["desc"] = "Duration for which the alert should display.",
						}, -- [2]
						{
							["type"] = "input",
							["useDesc"] = true,
							["width"] = 2,
							["name"] = "Message",
							["default"] = "",
							["key"] = "text",
							["length"] = 10,
							["desc"] = "Message to displaya on the alert",
							["useLength"] = false,
						}, -- [3]
						{
							["type"] = "color",
							["key"] = "colour",
							["width"] = 1,
							["name"] = "Colour",
							["useDesc"] = true,
							["default"] = {
								1, -- [1]
								1, -- [2]
								1, -- [3]
								1, -- [4]
							},
							["desc"] = "Text colour",
						}, -- [4]
						{
							["type"] = "input",
							["useDesc"] = true,
							["width"] = 1,
							["name"] = "Icon",
							["default"] = "",
							["key"] = "icon",
							["length"] = 10,
							["desc"] = "Must be a valid IconID or texture path. ",
							["useLength"] = false,
						}, -- [5]
					},
					["type"] = "group",
					["useDesc"] = false,
					["width"] = 1,
					["useCollapse"] = false,
					["key"] = "alerts",
					["name"] = "Alerts",
					["limitType"] = "none",
					["groupType"] = "array",
					["collapse"] = false,
					["size"] = 10,
				}, -- [5]
			},
			["displayText"] = "[%c]",
			["customText"] = "function(_,_,prog,_,name)\n    if aura_env.start then \n        local combatTime = GetTime() - aura_env.start\n        return WeakAuras.IsClassic() and GetTimeStringFromSeconds(combatTime, false, true) \n        or SecondsToClock(combatTime, false) \n    end\nend\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["yOffset"] = -555.0003662109375,
			["anchorPoint"] = "CENTER",
			["customTextUpdate"] = "update",
			["url"] = "https://wago.io/CombatTimerAndAlerts/18",
			["actions"] = {
				["start"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["custom"] = "if aura_env.state.type == \"main\" then\n    for _, timer in pairs(aura_env.timers) do\n        timer:Cancel()       \n    end\n    aura_env.timers = {}\nend",
					["do_custom"] = true,
				},
				["init"] = {
					["custom"] = "\n\n",
					["do_custom"] = false,
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "custom",
						["subeventSuffix"] = "_CAST_START",
						["customVariables"] = "{\n    type = {\n        type = \"select\",\n        values = {\n            [\"alert\"] = \"Alert\",\n            [\"main\"] = \"Main\",\n        },\n        display = \"type\",\n    },\n    alert = \"number\",\n}",
						["event"] = "Health",
						["subeventPrefix"] = "SPELL",
						["custom_type"] = "stateupdate",
						["spellIds"] = {
						},
						["custom"] = "function(allstates, event, index, settings)\n    if event == \"START\" then\n        if aura_env.timers then\n            for _, timer in pairs(aura_env.timers) do\n                timer:Cancel()       \n            end\n        end\n        aura_env.start = GetTime()\n        aura_env.timers = {}\n        local aura_env = aura_env\n        for i, settings in ipairs(aura_env.config.alerts) do\n            if settings.time > 0 then\n                local timer = C_Timer.NewTimer(settings.time, function()\n                        WeakAuras.ScanEvents(\"ALERT_START\", i, settings)\n                    end\n                )\n                table.insert(aura_env.timers, timer)\n            end\n        end \n        if aura_env.start and aura_env.config.showMain then\n            local combatTime = GetTime() - aura_env.start\n            allstates[\"maintimer\"] = {\n                show = true,\n                changed = true, \n                type = \"main\",\n                index = 0,\n            }\n        end\n        return true\n    end\n    \nend",
						["events"] = "ALERT_START START",
						["check"] = "event",
						["unit"] = "player",
						["names"] = {
						},
						["debuffType"] = "HELPFUL",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["colorR"] = 1,
					["scalex"] = 1,
					["alphaType"] = "custom",
					["colorA"] = 1,
					["colorG"] = 1,
					["alphaFunc"] = "function(progress, start, delta)\n    if aura_env.state and aura_env.state.type == \"main\" then return 1 end\n    local angle = (progress * 4 * math.pi) - (math.pi / 4)\n    return start + (((math.sin(angle) + 1)/2) * delta)\nend\n\n\n",
					["use_translate"] = true,
					["use_alpha"] = true,
					["type"] = "custom",
					["easeType"] = "none",
					["translateFunc"] = "function(progress, startX, startY, deltaX, deltaY)\n    if aura_env.state and aura_env.state.type == \"main\" then return 1 end\n    return startX + (progress * deltaX), startY + (progress * deltaY)\nend\n\n\n",
					["scaley"] = 1,
					["alpha"] = 0,
					["translateType"] = "custom",
					["y"] = -20,
					["x"] = 0,
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["colorB"] = 1,
					["rotate"] = 0,
					["preset"] = "slidebottom",
					["duration"] = "0.5",
				},
				["main"] = {
					["duration_type"] = "seconds",
					["type"] = "none",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["finish"] = {
					["colorR"] = 1,
					["scalex"] = 1,
					["alphaType"] = "custom",
					["colorB"] = 1,
					["colorG"] = 1,
					["alphaFunc"] = "function(progress, start, delta)\n    if aura_env.state and aura_env.state.type == \"main\" then return 1 end\n    return start + (progress * delta)\nend\n\n\n",
					["use_translate"] = true,
					["use_alpha"] = true,
					["type"] = "custom",
					["easeType"] = "none",
					["translateFunc"] = "function(progress, startX, startY, deltaX, deltaY)\n    if aura_env.state and aura_env.state.type == \"main\" then return 1 end\n    return startX + (progress * deltaX), startY + (progress * deltaY)\nend\n\n\n",
					["scaley"] = 1,
					["alpha"] = 0,
					["y"] = -20,
					["x"] = 0,
					["duration_type"] = "seconds",
					["translateType"] = "custom",
					["easeStrength"] = 3,
					["rotate"] = 0,
					["duration"] = "0.5",
					["colorA"] = 1,
				},
			},
			["font"] = "Friz Quadrata TT",
			["version"] = 18,
			["load"] = {
				["zoneIds"] = "",
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["use_combat"] = true,
				["size"] = {
					["multi"] = {
					},
				},
			},
			["fontSize"] = 24,
			["shadowXOffset"] = 1,
			["regionType"] = "text",
			["wordWrap"] = "WordWrap",
			["preferToUpdate"] = false,
			["fixedWidth"] = 200,
			["automaticWidth"] = "Auto",
			["xOffset"] = 750.831787109375,
			["justify"] = "RIGHT",
			["tocversion"] = 80300,
			["id"] = "Combat Timer",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["config"] = {
				["showMain"] = true,
				["alerts"] = {
					{
						["time"] = 5,
						["text"] = "Test",
						["duration"] = 3,
						["icon"] = "135924",
						["colour"] = {
							1, -- [1]
							1, -- [2]
							1, -- [3]
							1, -- [4]
						},
					}, -- [1]
					{
						["time"] = 12,
						["text"] = "Test2",
						["duration"] = 5,
						["icon"] = "Interface\\TargetingFrame\\UI-RaidTargetingIcon_8",
						["colour"] = {
							1, -- [1]
							0, -- [2]
							0, -- [3]
							1, -- [4]
						},
					}, -- [2]
				},
				["showProg"] = false,
			},
			["uid"] = "1652slsX64w",
			["selfPoint"] = "BOTTOM",
			["semver"] = "1.0.12",
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["value"] = "1",
						["variable"] = "alert",
					},
					["changes"] = {
						{
							["value"] = {
								["sound_type"] = "Play",
								["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\BikeHorn.ogg",
								["sound_channel"] = "Master",
							},
							["property"] = "sound",
						}, -- [1]
					},
				}, -- [1]
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["shadowYOffset"] = -1,
		},
		["HealerCC'ed"] = {
			["iconSource"] = -1,
			["authorOptions"] = {
			},
			["preferToUpdate"] = false,
			["yOffset"] = 190.8245239257813,
			["anchorPoint"] = "CENTER",
			["cooldownSwipe"] = true,
			["customTextUpdate"] = "update",
			["url"] = "https://wago.io/XNg03PyZj/1",
			["icon"] = true,
			["triggers"] = {
				{
					["trigger"] = {
						["auranames"] = {
							"Kidney Shot", -- [1]
							"Cheap Shot", -- [2]
							"Garrote", -- [3]
							"Fear", -- [4]
							"Psychic Horror", -- [5]
							"Mind Bomb", -- [6]
							"Imprison", -- [7]
							"Intimidating Shout", -- [8]
							"Storm Bolt", -- [9]
							"Shockwave", -- [10]
							"Chaos Nova", -- [11]
							"Blind", -- [12]
							"Hex", -- [13]
							"Polymorph", -- [14]
							"Repentance", -- [15]
							"Hammer of Justice", -- [16]
							"Entangling Roots", -- [17]
							"Mighty Bash", -- [18]
							"Maim", -- [19]
							"Freezing Trap", -- [20]
							"Frost Nova", -- [21]
							"Mind Control", -- [22]
							"Sap", -- [23]
							"Seduction", -- [24]
							"Mortal Coil", -- [25]
							"Leg Sweep", -- [26]
							"Fel Eruption", -- [27]
							"Asphyxiate", -- [28]
							"Capacitor Totem", -- [29]
							"Cyclone", -- [30]
							"Dragon's Breath", -- [31]
							"Silence", -- [32]
							"Gouge", -- [33]
							"Psychic Scream", -- [34]
							"Axe Toss", -- [35]
							"Silence", -- [36]
						},
						["group_role"] = {
							["HEALER"] = true,
						},
						["names"] = {
						},
						["ignoreSelf"] = true,
						["match_count"] = "1",
						["debuffType"] = "HARMFUL",
						["showClones"] = true,
						["type"] = "aura2",
						["subeventSuffix"] = "_CAST_START",
						["event"] = "Health",
						["useGroupRole"] = true,
						["unit"] = "group",
						["spellIds"] = {
						},
						["useName"] = true,
						["use_specific_unit"] = true,
						["match_countOperator"] = "<=",
						["buffShowOn"] = "showOnActive",
						["subeventPrefix"] = "SPELL",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["internalVersion"] = 45,
			["keepAspectRatio"] = false,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["version"] = 1,
			["subRegions"] = {
				{
					["text_shadowXOffset"] = 0,
					["text_text_format_s_format"] = "none",
					["text_text"] = "%s",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Friz Quadrata TT",
					["text_shadowYOffset"] = 0,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "INNER_BOTTOMRIGHT",
					["text_fontSize"] = 12,
					["anchorXOffset"] = 0,
					["text_visible"] = true,
				}, -- [1]
				{
					["glowFrequency"] = 0.25,
					["type"] = "subglow",
					["glowXOffset"] = 0,
					["glowType"] = "buttonOverlay",
					["glowLength"] = 10,
					["glowYOffset"] = 0,
					["glowColor"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["useGlowColor"] = false,
					["glow"] = true,
					["glowThickness"] = 4,
					["glowScale"] = 1,
					["glowLines"] = 8,
					["glowBorder"] = false,
				}, -- [2]
			},
			["height"] = 80,
			["load"] = {
				["use_size"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["spec"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["ratedpvp"] = true,
						["arena"] = true,
						["ratedarena"] = true,
						["pvp"] = true,
					},
				},
			},
			["cooldownEdge"] = false,
			["regionType"] = "icon",
			["stickyDuration"] = false,
			["actions"] = {
				["start"] = {
				},
				["init"] = {
				},
				["finish"] = {
				},
			},
			["xOffset"] = -106.908203125,
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["cooldownTextDisabled"] = false,
			["config"] = {
			},
			["zoom"] = 0,
			["auto"] = true,
			["frameStrata"] = 1,
			["id"] = "HealerCC'ed",
			["anchorFrameType"] = "SCREEN",
			["alpha"] = 1,
			["width"] = 80,
			["semver"] = "1.0.0",
			["uid"] = "KQEcXCBrkmH",
			["inverse"] = false,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
			},
			["conditions"] = {
			},
			["cooldown"] = true,
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
		},
		["nocircle_noleaf_nofill"] = {
			["iconSource"] = 0,
			["xOffset"] = 0,
			["preferToUpdate"] = true,
			["customText"] = "function()\n    if aura_env.showText then\n        return aura_env.order\n    else\n        return \"\"\n    end\nend",
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["cooldownSwipe"] = true,
			["cooldownEdge"] = false,
			["icon"] = true,
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "custom",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HELPFUL",
						["event"] = "Health",
						["unit"] = "player",
						["custom_type"] = "status",
						["spellIds"] = {
						},
						["events"] = "ZONE_CHANGED, PLAYER_LOGIN, PLAYER_ENTERING_WORLD",
						["names"] = {
						},
						["check"] = "event",
						["custom"] = "function()\n    WeakAuras.ScanEvents(\"TirnaScitheButtonData\", 7, aura_env.frameData)\n    if aura_env.debugmode then \n        return true \n    end\n    local text = GetMinimapZoneText()\n    if text == aura_env.mistZone then\n        return true\n    else\n        return false\n    end\nend",
						["subeventPrefix"] = "SPELL",
						["custom_hide"] = "custom",
					},
					["untrigger"] = {
					},
				}, -- [1]
				{
					["trigger"] = {
						["debuffType"] = "HELPFUL",
						["type"] = "custom",
						["custom"] = "function(event,eid,...)\n    if event == \"ENCOUNTER_END\" then \n        if eid == 2392 and aura_env.showEncounter == false then \n            return false \n        end\n        return true \n    elseif event == \"ENCOUNTER_START\" then \n        if aura_env.showEncounter == false then \n            return false\n        else\n            return true\n        end\n    end\n    return true\nend",
						["custom_type"] = "status",
						["check"] = "event",
						["customName"] = "function()\n    return \nend",
						["events"] = "ENCOUNTER_START, ENCOUNTER_END, PLAYER_ENTERING_WORLD",
						["unit"] = "player",
					},
					["untrigger"] = {
					},
				}, -- [2]
				{
					["trigger"] = {
						["type"] = "custom",
						["custom_type"] = "status",
						["unit"] = "player",
						["customName"] = "function()\n    return aura_env.order            \nend",
						["events"] = "TirnaScitheButtonOrder",
						["check"] = "event",
						["debuffType"] = "HELPFUL",
						["custom"] = "function(_,button,order)\n    if button == 7 then \n        aura_env.order = order\n        if order == 9 then \n            aura_env.order = \"\"\n        end\n        return true\n    elseif button == 9 then\n        aura_env.order = \"\"\n        return true\n    end\n    return true\nend",
						["custom_hide"] = "timed",
					},
					["untrigger"] = {
					},
				}, -- [3]
				["activeTriggerMode"] = -10,
			},
			["internalVersion"] = 45,
			["keepAspectRatio"] = false,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["desaturate"] = false,
			["version"] = 67,
			["subRegions"] = {
				{
					["glowFrequency"] = 0.25,
					["type"] = "subglow",
					["useGlowColor"] = false,
					["glowType"] = "buttonOverlay",
					["glowLength"] = 10,
					["glowYOffset"] = 0,
					["glowColor"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["glowXOffset"] = 0,
					["glowScale"] = 1,
					["glowThickness"] = 1,
					["glow"] = false,
					["glowLines"] = 8,
					["glowBorder"] = false,
				}, -- [1]
				{
					["text_text_format_p_time_format"] = 0,
					["text_text"] = "%c",
					["text_text_format_p_format"] = "timed",
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Friz Quadrata TT",
					["text_text_format_p_time_precision"] = 1,
					["text_shadowYOffset"] = 0,
					["text_text_format_p_time_dynamic_threshold"] = 60,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "INNER_BOTTOMRIGHT",
					["text_shadowXOffset"] = 0,
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_fontSize"] = 15,
					["anchorXOffset"] = 0,
					["text_visible"] = true,
				}, -- [2]
			},
			["height"] = 80,
			["load"] = {
				["ingroup"] = {
					["single"] = "group",
					["multi"] = {
						["group"] = true,
					},
				},
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["use_encounterid"] = false,
				["difficulty"] = {
					["multi"] = {
						["mythic"] = true,
						["challenge"] = true,
					},
				},
				["use_zone"] = false,
				["zoneIds"] = "1669",
				["use_zoneIds"] = true,
				["zone"] = "",
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["config"] = {
				["encounter"] = false,
				["noCommunication"] = false,
				["debug"] = false,
				["orderText"] = true,
			},
			["actions"] = {
				["start"] = {
				},
				["finish"] = {
				},
				["init"] = {
					["custom"] = "aura_env.debugmode = aura_env.config['debug']\nlocal debug = aura_env.debugmode\naura_env.showEncounter = aura_env.config['encounter']\naura_env.locale=GetLocale()\naura_env.mistZone = \"Mistveil Tangle\"\naura_env.order = \"\"\naura_env.showText = aura_env.config['orderText']\naura_env.noCommunication = aura_env.config['noCommunication']\nif aura_env.locale == \"koKR\" then\n    aura_env.mistZone = \"안개장막 덩굴숲\"\nelseif aura_env.locale == \"frFR\" then\n    aura_env.mistZone = \"Maquis Voile-de-Brume\"\nelseif aura_env.locale == \"deDE\" then\n    aura_env.mistZone = \"Nebelschleierdickicht\"\nelseif aura_env.locale == \"zhCN\" then\n    aura_env.mistZone = \"纱雾迷结\"\nelseif aura_env.locale == \"esES\" then\n    aura_env.mistZone = \"Espesura Velo de Niebla\"\nelseif aura_env.locale == \"zhTW\" then\n    aura_env.mistZone = \"霧紗密林\"\nelseif aura_env.locale == \"esMX\" then\n    aura_env.mistZone = \"Espesura Veloniebla\"\nelseif aura_env.locale == \"ruRU\" then\n    aura_env.mistZone = \"Туманная чащоба\"\nelseif aura_env.locale == \"ptBR\" then\n    aura_env.mistZone = \"Enleio do Véu da Névoa\"\nelseif aura_env.locale == \"itIT\" then\n    aura_env.mistZone = \"Intrico Velofosco\"\nelseif aura_env.locale == \"ptPT\" then\n    aura_env.mistZone = \"Enleio do Véu da Névoa\"        \nend\nlocal values = {1, 0, 1, 7}\nlocal message = string.format(\"%d %d %d %d\", values[1], values[2], values[3], values[4])\nlocal name, _ = UnitName(\"player\")\n\nif not aura_env.frameData then\n    local b = CreateFrame(\"Button\", nil, WeakAuras.regions[aura_env.id].region)\n    b:SetAllPoints(WeakAuras.regions[aura_env.id].region)\n    b:RegisterForClicks(\"LeftButtonDown\", \"RightButtonDown\")\n    aura_env.frameData = b\n    WeakAuras.ScanEvents(\"TirnaScitheButtonData\", 7, b)\nend\nif aura_env.noCommunication then\n    aura_env.frameData:SetScript(\"OnClick\", function(self, button)\n            if debug then print(\"Click Registered - true\") end\n            WeakAuras.ScanEvents(\"GlowUpdate\")\n            C_ChatInfo.RegisterAddonMessagePrefix(\"Tirna Ver2\") \n            if debug then \n                print(\"noCommunication sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"WHISPER\", name)) \n            else\n                C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"WHISPER\", name)\n            end\n            \n    end)\nelse\n    aura_env.frameData:SetScript(\"OnClick\", function(self, button)\n            if debug then print(\"Click Registered - true\") end\n            WeakAuras.ScanEvents(\"GlowUpdate\")\n            C_ChatInfo.RegisterAddonMessagePrefix(\"Tirna Ver2\")\n            if IsPartyLFG() then            \n                if debug then \n                    print(\"LFG sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"INSTANCE_CHAT\")) \n                else\n                    C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"INSTANCE_CHAT\")\n                end\n            elseif UnitInParty(\"player\") then            \n                if debug then \n                    print(\"party sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"PARTY\")) \n                else\n                    C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"PARTY\")\n                end\n            else         \n                if debug then \n                    print(\"solo sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"WHISPER\", name)) \n                else\n                    C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"WHISPER\", name)\n                end\n            end\n    end)\nend",
					["do_custom"] = true,
				},
			},
			["authorOptions"] = {
				{
					["type"] = "toggle",
					["key"] = "orderText",
					["default"] = true,
					["name"] = "Show Order Text",
					["useDesc"] = false,
					["width"] = 1,
				}, -- [1]
				{
					["type"] = "toggle",
					["key"] = "encounter",
					["default"] = false,
					["name"] = "Show during Boss",
					["useDesc"] = false,
					["width"] = 1,
				}, -- [2]
				{
					["type"] = "toggle",
					["key"] = "noCommunication",
					["default"] = false,
					["name"] = "Disable Cross-Communication with others running the WA",
					["useDesc"] = false,
					["width"] = 2,
				}, -- [3]
				{
					["type"] = "toggle",
					["key"] = "debug",
					["default"] = false,
					["name"] = "|cffff0000Debug testing|r (requires reload for full debug)",
					["useDesc"] = false,
					["width"] = 2,
				}, -- [4]
			},
			["regionType"] = "icon",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["cooldown"] = false,
			["conditions"] = {
			},
			["parent"] = "Mists of Tirna Scithe guessing game v2",
			["semver"] = "2.0.2",
			["anchorFrameType"] = "SCREEN",
			["zoom"] = 0,
			["auto"] = false,
			["tocversion"] = 90005,
			["id"] = "nocircle_noleaf_nofill",
			["alpha"] = 1,
			["frameStrata"] = 1,
			["width"] = 80,
			["cooldownTextDisabled"] = false,
			["uid"] = "YKLKHDXFofn",
			["inverse"] = false,
			["selfPoint"] = "CENTER",
			["displayIcon"] = "Interface\\AddOns\\guess\\nocircle_lotus_nofill.tga",
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["url"] = "https://wago.io/hVLym_eLv/67",
		},
		["03 Greater Castigation - White"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 3,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.25,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = true,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2402",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "mSUk5jzZNNN",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["useMatch_count"] = true,
						["auranames"] = {
						},
						["groupclone"] = true,
						["matchesShowOn"] = "showOnActive",
						["name_info"] = "aura",
						["unit"] = "group",
						["use_tooltip"] = false,
						["subeventPrefix"] = "SPELL",
						["match_countOperator"] = ">=",
						["match_count"] = "1",
						["buffShowOn"] = "showOnActive",
						["unevent"] = "timed",
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["useExactSpellId"] = true,
						["custom_type"] = "stateupdate",
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["event"] = "Health",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HARMFUL",
						["use_specific_unit"] = false,
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["auraspellids"] = {
							"328889", -- [1]
							"332871", -- [2]
						},
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["duration"] = "1",
						["useGroup_count"] = false,
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["wordWrap"] = "WordWrap",
			["config"] = {
			},
			["text1"] = "%n",
			["text1Containment"] = "INSIDE",
			["width"] = 6.9999866485596,
			["justify"] = "LEFT",
			["semver"] = "1.0.26",
			["zoom"] = 0,
			["id"] = "03 Greater Castigation - White",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["frameStrata"] = 9,
			["anchorFrameType"] = "SCREEN",
			["text1Point"] = "CENTER",
			["text2Font"] = "Friz Quadrata TT",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["Have Debuff"] = {
			["authorOptions"] = {
			},
			["preferToUpdate"] = false,
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/OEDlN6x_O/1",
			["actions"] = {
				["start"] = {
					["sound"] = "Interface\\Addons\\WeakAuras\\PowerAurasMedia\\Sounds\\phone.ogg",
					["do_sound"] = true,
				},
				["init"] = {
				},
				["finish"] = {
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "combatlog",
						["spellId"] = "331634",
						["subeventSuffix"] = "_CAST_SUCCESS",
						["duration"] = "8",
						["event"] = "Combat Log",
						["unit"] = "player",
						["use_spellId"] = true,
						["spellIds"] = {
						},
						["unevent"] = "timed",
						["names"] = {
						},
						["use_spellName"] = false,
						["subeventPrefix"] = "SPELL",
						["debuffType"] = "HELPFUL",
					},
					["untrigger"] = {
					},
				}, -- [1]
				{
					["trigger"] = {
						["useName"] = false,
						["unit"] = "player",
						["useExactSpellId"] = true,
						["type"] = "aura2",
						["auranames"] = {
							"331637", -- [1]
						},
						["auraspellids"] = {
							"331637", -- [1]
							"331636", -- [2]
						},
						["debuffType"] = "HARMFUL",
					},
					["untrigger"] = {
					},
				}, -- [2]
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["rotation"] = 270,
			["version"] = 1,
			["subRegions"] = {
			},
			["height"] = 144,
			["rotate"] = true,
			["load"] = {
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["use_encounterid"] = true,
				["difficulty"] = {
					["single"] = "mythic",
				},
				["zoneIds"] = "",
				["class"] = {
					["multi"] = {
					},
				},
				["use_difficulty"] = true,
				["encounterid"] = "2412",
				["size"] = {
					["multi"] = {
					},
				},
			},
			["textureWrapMode"] = "CLAMPTOBLACKADDITIVE",
			["mirror"] = false,
			["regionType"] = "texture",
			["blendMode"] = "BLEND",
			["frameStrata"] = 1,
			["texture"] = "Interface\\AddOns\\WeakAuras\\Media\\Textures\\triangle-border.tga",
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["xOffset"] = 100,
			["semver"] = "1.0.0",
			["tocversion"] = 90002,
			["id"] = "Have Debuff",
			["discrete_rotation"] = 0,
			["alpha"] = 1,
			["width"] = 187,
			["config"] = {
			},
			["uid"] = "B0jRHaXo2fL",
			["anchorFrameType"] = "SCREEN",
			["color"] = {
				0, -- [1]
				1, -- [2]
				0.18823529411765, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
			},
			["parent"] = "T27 07 Council of Blood - Dark Recital Arrows",
		},
		["circle_lotus_nofill"] = {
			["iconSource"] = 0,
			["xOffset"] = 0,
			["preferToUpdate"] = true,
			["customText"] = "function()\n    if aura_env.showText then\n        return aura_env.order\n    else\n        return \"\"\n    end\nend",
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["cooldownSwipe"] = true,
			["cooldownEdge"] = false,
			["icon"] = true,
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "custom",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HELPFUL",
						["event"] = "Health",
						["unit"] = "player",
						["custom_type"] = "status",
						["spellIds"] = {
						},
						["events"] = "ZONE_CHANGED, PLAYER_LOGIN, PLAYER_ENTERING_WORLD",
						["names"] = {
						},
						["check"] = "event",
						["custom"] = "function()\n    WeakAuras.ScanEvents(\"TirnaScitheButtonData\", 3, aura_env.frameData)\n    if aura_env.debugmode then \n        return true \n    end\n    local text = GetMinimapZoneText()\n    if text == aura_env.mistZone then\n        return true\n    else\n        return false\n    end\nend\n\n",
						["subeventPrefix"] = "SPELL",
						["custom_hide"] = "custom",
					},
					["untrigger"] = {
					},
				}, -- [1]
				{
					["trigger"] = {
						["debuffType"] = "HELPFUL",
						["type"] = "custom",
						["custom"] = "function(event,eid,...)\n    if event == \"ENCOUNTER_END\" then \n        if eid == 2392 and aura_env.showEncounter == false then \n            return false \n        end\n        return true \n    elseif event == \"ENCOUNTER_START\" then \n        if aura_env.showEncounter == false then \n            return false\n        else\n            return true\n        end\n    end\n    return true\nend\n\n",
						["custom_type"] = "status",
						["check"] = "event",
						["customName"] = "function()\n    return \nend",
						["events"] = "ENCOUNTER_START, ENCOUNTER_END, PLAYER_ENTERING_WORLD",
						["unit"] = "player",
					},
					["untrigger"] = {
					},
				}, -- [2]
				{
					["trigger"] = {
						["type"] = "custom",
						["custom_type"] = "status",
						["unit"] = "player",
						["customName"] = "function()\n    return aura_env.order            \nend",
						["events"] = "TirnaScitheButtonOrder",
						["check"] = "event",
						["debuffType"] = "HELPFUL",
						["custom"] = "function(_,button,order)\n    if button == 3 then \n        aura_env.order = order\n        if order == 9 then \n            aura_env.order = \"\"\n        end\n        return true\n    elseif button == 9 then\n        aura_env.order = \"\"\n        return true\n    end\n    return true\nend\n\n",
						["custom_hide"] = "timed",
					},
					["untrigger"] = {
					},
				}, -- [3]
				["activeTriggerMode"] = -10,
			},
			["internalVersion"] = 45,
			["keepAspectRatio"] = false,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["desaturate"] = false,
			["version"] = 67,
			["subRegions"] = {
				{
					["glowFrequency"] = 0.25,
					["type"] = "subglow",
					["useGlowColor"] = false,
					["glowType"] = "buttonOverlay",
					["glowLength"] = 10,
					["glowYOffset"] = 0,
					["glowColor"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["glowXOffset"] = 0,
					["glowScale"] = 1,
					["glowThickness"] = 1,
					["glow"] = false,
					["glowLines"] = 8,
					["glowBorder"] = false,
				}, -- [1]
				{
					["text_text_format_p_time_format"] = 0,
					["text_text"] = "%c",
					["text_text_format_p_format"] = "timed",
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Friz Quadrata TT",
					["text_text_format_p_time_precision"] = 1,
					["text_shadowYOffset"] = 0,
					["text_text_format_p_time_dynamic_threshold"] = 60,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "INNER_BOTTOMRIGHT",
					["text_shadowXOffset"] = 0,
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_fontSize"] = 15,
					["anchorXOffset"] = 0,
					["text_visible"] = true,
				}, -- [2]
			},
			["height"] = 80,
			["load"] = {
				["ingroup"] = {
					["single"] = "group",
					["multi"] = {
						["group"] = true,
					},
				},
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["use_encounterid"] = false,
				["difficulty"] = {
					["multi"] = {
						["mythic"] = true,
						["challenge"] = true,
					},
				},
				["use_zone"] = false,
				["zoneIds"] = "1669",
				["use_zoneIds"] = true,
				["zone"] = "",
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["config"] = {
				["encounter"] = false,
				["noCommunication"] = false,
				["debug"] = false,
				["orderText"] = true,
			},
			["actions"] = {
				["start"] = {
				},
				["finish"] = {
				},
				["init"] = {
					["custom"] = "aura_env.debugmode = aura_env.config['debug']\nlocal debug = aura_env.debugmode\naura_env.showEncounter = aura_env.config['encounter']\naura_env.locale=GetLocale()\naura_env.mistZone = \"Mistveil Tangle\"\naura_env.order = \"\"\naura_env.showText = aura_env.config['orderText']\naura_env.noCommunication = aura_env.config['noCommunication']\nif aura_env.locale == \"koKR\" then\n    aura_env.mistZone = \"안개장막 덩굴숲\"\nelseif aura_env.locale == \"frFR\" then\n    aura_env.mistZone = \"Maquis Voile-de-Brume\"\nelseif aura_env.locale == \"deDE\" then\n    aura_env.mistZone = \"Nebelschleierdickicht\"\nelseif aura_env.locale == \"zhCN\" then\n    aura_env.mistZone = \"纱雾迷结\"\nelseif aura_env.locale == \"esES\" then\n    aura_env.mistZone = \"Espesura Velo de Niebla\"\nelseif aura_env.locale == \"zhTW\" then\n    aura_env.mistZone = \"霧紗密林\"\nelseif aura_env.locale == \"esMX\" then\n    aura_env.mistZone = \"Espesura Veloniebla\"\nelseif aura_env.locale == \"ruRU\" then\n    aura_env.mistZone = \"Туманная чащоба\"\nelseif aura_env.locale == \"ptBR\" then\n    aura_env.mistZone = \"Enleio do Véu da Névoa\"\nelseif aura_env.locale == \"itIT\" then\n    aura_env.mistZone = \"Intrico Velofosco\"\nelseif aura_env.locale == \"ptPT\" then\n    aura_env.mistZone = \"Enleio do Véu da Névoa\"        \nend\nlocal values = {0, 0, 1, 3}\nlocal message = string.format(\"%d %d %d %d\", values[1], values[2], values[3], values[4])\nlocal name, _ = UnitName(\"player\")\n\nif not aura_env.frameData then\n    local b = CreateFrame(\"Button\", nil, WeakAuras.regions[aura_env.id].region)\n    b:SetAllPoints(WeakAuras.regions[aura_env.id].region)\n    b:RegisterForClicks(\"LeftButtonDown\", \"RightButtonDown\")\n    aura_env.frameData = b\n    WeakAuras.ScanEvents(\"TirnaScitheButtonData\", 3, b)\nend\nif aura_env.noCommunication then\n    aura_env.frameData:SetScript(\"OnClick\", function(self, button)\n            if debug then print(\"Click Registered - true\") end\n            WeakAuras.ScanEvents(\"GlowUpdate\")\n            C_ChatInfo.RegisterAddonMessagePrefix(\"Tirna Ver2\") \n            if debug then \n                print(\"noCommunication sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"WHISPER\", name)) \n            else\n                C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"WHISPER\", name)\n            end\n            \n    end)\nelse\n    aura_env.frameData:SetScript(\"OnClick\", function(self, button)\n            if debug then print(\"Click Registered - true\") end\n            WeakAuras.ScanEvents(\"GlowUpdate\")\n            C_ChatInfo.RegisterAddonMessagePrefix(\"Tirna Ver2\")\n            if IsPartyLFG() then            \n                if debug then \n                    print(\"LFG sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"INSTANCE_CHAT\")) \n                else\n                    C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"INSTANCE_CHAT\")\n                end\n            elseif UnitInParty(\"player\") then            \n                if debug then \n                    print(\"party sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"PARTY\")) \n                else\n                    C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"PARTY\")\n                end\n            else         \n                if debug then \n                    print(\"solo sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"WHISPER\", name)) \n                else\n                    C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"WHISPER\", name)\n                end\n            end\n    end)\nend",
					["do_custom"] = true,
				},
			},
			["authorOptions"] = {
				{
					["type"] = "toggle",
					["key"] = "orderText",
					["default"] = true,
					["name"] = "Show Order Text",
					["useDesc"] = false,
					["width"] = 1,
				}, -- [1]
				{
					["type"] = "toggle",
					["key"] = "encounter",
					["default"] = false,
					["name"] = "Show during Boss",
					["useDesc"] = false,
					["width"] = 1,
				}, -- [2]
				{
					["type"] = "toggle",
					["key"] = "noCommunication",
					["default"] = false,
					["name"] = "Disable Cross-Communication with others running the WA",
					["useDesc"] = false,
					["width"] = 2,
				}, -- [3]
				{
					["type"] = "toggle",
					["key"] = "debug",
					["default"] = false,
					["name"] = "|cffff0000Debug testing|r (requires reload for full debug)",
					["useDesc"] = false,
					["width"] = 2,
				}, -- [4]
			},
			["regionType"] = "icon",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["cooldown"] = false,
			["conditions"] = {
			},
			["parent"] = "Mists of Tirna Scithe guessing game v2",
			["semver"] = "2.0.2",
			["anchorFrameType"] = "SCREEN",
			["zoom"] = 0,
			["auto"] = false,
			["tocversion"] = 90005,
			["id"] = "circle_lotus_nofill",
			["alpha"] = 1,
			["frameStrata"] = 1,
			["width"] = 80,
			["cooldownTextDisabled"] = false,
			["uid"] = "wtYDqz3U(i1",
			["inverse"] = false,
			["selfPoint"] = "CENTER",
			["displayIcon"] = "Interface\\AddOns\\guess\\circle_lotus_nofill.tga",
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["url"] = "https://wago.io/hVLym_eLv/67",
		},
		["10 Impale - Red"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 4,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						1, -- [1]
						0.007843137254902, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.25,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2407",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "gXRA1accee5",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["useMatch_count"] = true,
						["auranames"] = {
						},
						["groupclone"] = true,
						["matchesShowOn"] = "showOnActive",
						["name_info"] = "aura",
						["unit"] = "group",
						["use_tooltip"] = false,
						["subeventPrefix"] = "SPELL",
						["match_countOperator"] = ">=",
						["match_count"] = "1",
						["buffShowOn"] = "showOnActive",
						["unevent"] = "timed",
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["useExactSpellId"] = true,
						["custom_type"] = "stateupdate",
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["event"] = "Health",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HARMFUL",
						["use_specific_unit"] = false,
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["auraspellids"] = {
							"329951", -- [1]
						},
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["duration"] = "1",
						["useGroup_count"] = false,
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["text2Font"] = "Friz Quadrata TT",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text1"] = "%n",
			["text1Point"] = "CENTER",
			["anchorFrameType"] = "SCREEN",
			["semver"] = "1.0.26",
			["justify"] = "LEFT",
			["zoom"] = 0,
			["id"] = "10 Impale - Red",
			["config"] = {
			},
			["frameStrata"] = 9,
			["width"] = 6.9999866485596,
			["text1Containment"] = "INSIDE",
			["wordWrap"] = "WordWrap",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["Maelstrom Bar"] = {
			["overlays"] = {
				[2] = {
					1, -- [1]
					0, -- [2]
					1, -- [3]
					1, -- [4]
				},
			},
			["borderBackdrop"] = "None",
			["xOffset"] = 0,
			["preferToUpdate"] = false,
			["customText"] = "",
			["yOffset"] = -100,
			["anchorPoint"] = "CENTER",
			["spark"] = false,
			["sparkRotation"] = 0,
			["sparkRotationMode"] = "AUTO",
			["url"] = "https://wago.io/EJohkuIKb/38",
			["actions"] = {
				["start"] = {
				},
				["init"] = {
					["custom"] = "local r = aura_env.region\nlocal b = r.bar\nlocal aura_env = aura_env\naura_env.powerCasts = {-- generation, Fortres of the mind\n    [GetSpellInfo(188443)] = {12}, --Chain Lightning (assume 3 targets)\n    [GetSpellInfo(210714)] = {15}, --Icefury\n    [GetSpellInfo(51505)] = {10}, --Lava Burst\n    [GetSpellInfo(188196)] = {8}, --Lightning Bolt\n    [GetSpellInfo(114074)] = {3}, --Lava Beam\n    \n}\naura_env.getPowerFromCast = function()\n    local cast = UnitCastingInfo(\"player\") or UnitChannelInfo(\"player\") or nil\n    if not cast then\n        return 0\n    end\n    local data = aura_env.powerCasts[cast] or nil\n    if not data then\n        return 0\n    end\n    \n    local power = data[1] or 0 \n    \n    return power\nend\n\naura_env.castPower = aura_env.getPowerFromCast()\naura_env.power = UnitPower(\"player\")\naura_env.powerMax = UnitPowerMax(\"player\")\naura_env.powerTextUpdate = function()\n    local a = aura_env\n    local power = UnitPower(\"player\", Enum.PowerType.Maelstrom)\n    local returnString = power\n    local castPower = a.getPowerFromCast()\n    if castPower > 0 then\n        returnString = returnString..\"+\"..castPower\n    end\n    return returnString \nend\nif not r.text3 then\n    local text3 = r.bar:CreateFontString(nil, \"OVERLAY\")\n    r.text3 = text3\n    r.text3:SetJustifyH(\"CENTER\")\n    r.text3:SetJustifyV(\"MIDDLE\")\n    r.text3:SetPoint(\"CENTER\", r.bar, \"CENTER\")\n    r.text3:Show()\n    r.text3:SetShadowOffset(1, -1)\n    r.text3:SetFont(r.text:GetFont())\n    r.text3:SetTextColor(r.text:GetTextColor())    \n    r.text3:SetText(aura_env.powerTextUpdate())\nend\n\nif not b.sparkVF then\n    local sparkVF = b:CreateTexture(nil, \"ARTWORK\")\n    sparkVF:SetDrawLayer(\"ARTWORK\", 4)\n    sparkVF:SetTexture(b:GetTexture())\n    sparkVF:SetColorTexture(1,1,1,1)\n    sparkVF:SetWidth(1);\n    sparkVF:SetHeight(b:GetHeight());\n    sparkVF:ClearAllPoints()\n    sparkVF:Show()\n    b.sparkVF = sparkVF\nend\naura_env.VFAnchor = function()\n    if GetSpecialization() ~= 1 then\n        return b.sparkVF and b.sparkVF:Hide() \n    end\n    b.sparkVF:Show() \n    local r = aura_env.region\n    local bar = aura_env.region.bar\n    local powerMax = aura_env.powerMax\n    local pix = bar:GetWidth() / powerMax\n    local cost = GetSpellPowerCost(8042)\n    local offsetVF = pix * (cost and cost[1].minCost or 0)\n    --print(90/bar:GetWidth())\n    --print(offsetVF)\n    b.sparkVF:SetPoint(\"CENTER\", bar, \"LEFT\", offsetVF, 0)\nend\naura_env.VFAnchor()",
					["do_custom"] = true,
				},
				["finish"] = {
				},
			},
			["fontFlags"] = "OUTLINE",
			["icon_color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["internalVersion"] = 45,
			["sparkWidth"] = 10,
			["selfPoint"] = "CENTER",
			["backdropInFront"] = false,
			["iconSource"] = -1,
			["barColor"] = {
				0, -- [1]
				0.50196078431373, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["desaturate"] = false,
			["customTextUpdate"] = "update",
			["backgroundColor"] = {
				0, -- [1]
				0.18823529411765, -- [2]
				0.37647058823529, -- [3]
				0.65000000596047, -- [4]
			},
			["version"] = 38,
			["subRegions"] = {
				{
					["type"] = "aurabar_bar",
				}, -- [1]
				{
					["text_shadowXOffset"] = 1,
					["text_text"] = " ",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Expressway",
					["text_shadowYOffset"] = -1,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "None",
					["text_anchorPoint"] = "INNER_RIGHT",
					["text_fontSize"] = 18,
					["anchorXOffset"] = 0,
					["text_visible"] = false,
				}, -- [2]
				{
					["text_shadowXOffset"] = 1,
					["text_text"] = " ",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Expressway",
					["text_shadowYOffset"] = -1,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "None",
					["text_anchorPoint"] = "INNER_LEFT",
					["text_fontSize"] = 18,
					["anchorXOffset"] = 0,
					["text_visible"] = true,
				}, -- [3]
				{
					["text_shadowXOffset"] = 1,
					["text_text_format_s_format"] = "none",
					["text_text"] = "%s",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Friz Quadrata TT",
					["text_shadowYOffset"] = -1,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "None",
					["text_anchorPoint"] = "ICON_CENTER",
					["text_fontSize"] = 12,
					["anchorXOffset"] = 0,
					["text_visible"] = false,
				}, -- [4]
				{
					["type"] = "subborder",
					["border_anchor"] = "bar",
					["border_offset"] = 1,
					["border_color"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["border_visible"] = true,
					["border_edge"] = "1 Pixel",
					["border_size"] = 1,
				}, -- [5]
			},
			["height"] = 20,
			["triggers"] = {
				{
					["trigger"] = {
						["use_absorbMode"] = true,
						["genericShowOn"] = "showOnActive",
						["unit"] = "player",
						["use_powertype"] = false,
						["debuffType"] = "HELPFUL",
						["use_unit"] = true,
						["type"] = "custom",
						["unevent"] = "auto",
						["custom_type"] = "status",
						["customOverlay1"] = "function()\n    local a = aura_env\n    return a.power, min(a.power+a.castPower, a.powerMax)\nend\n\n\n\n\n\n\n\n",
						["subeventPrefix"] = "SPELL",
						["customOverlay2"] = "function()\n    local a = aura_env\n    return a.power+a.castPower, min(a.power+a.castPower, a.powerMax)\nend",
						["event"] = "Power",
						["customDuration"] = "function()\n    local a = aura_env\n    --print(a.power, a.powerMax)\n    return a.power, a.powerMax, true\nend\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
						["events"] = "UNIT_SPELLCAST_START, UNIT_SPELLCAST_STOP, UNIT_SPELLCAST_SUCCESS, PLAYER_ENTERING_WORLD, UNIT_POWER_UPDATE, UNIT_POWER_FREQUENT, UNIT_SPELLCAST_CHANNEL_START, UNIT_SPELLCAST_CHANNEL_STOP, UNIT_SPELLCAST_CHANNEL_UPDATE",
						["custom"] = "function(event, unit, ...)\n    local a = aura_env\n    local r = a.region\n    if unit == \"player\" then\n        --[[if \n        event == \"UNIT_SPELLCAST_START\" or \n        event == \"UNIT_SPELLCAST_STOP\" or \n        event == \"UNIT_SPELLCAST_SUCCESS\" \n        then\n            print(...)]]\n        --a.power = UnitPower(\"player\")\n        a.castPower = a.getPowerFromCast()\n        --r.count:SetText(a.powerTextUpdate())\n        --print(a.castPower, \"castPower\")\n        \n        --and (event == \"UNIT_DISPLAYPOWER\" or event == \"PLAYER_ENTERING_WORLD\") then\n        a.power = UnitPower(\"player\", Enum.PowerType.Maelstrom)\n        a.powerMax = UnitPowerMax(\"player\", Enum.PowerType.Maelstrom)\n        r.text3:SetText(aura_env.powerTextUpdate())\n        --print(a.ASPower())\n        --print(a.power)\n        --r.count:SetText(a.powerTextUpdate())\n    end\n    return true\nend",
						["spellIds"] = {
						},
						["check"] = "event",
						["custom_hide"] = "custom",
						["subeventSuffix"] = "_CAST_START",
						["names"] = {
						},
					},
					["untrigger"] = {
					},
				}, -- [1]
				{
					["trigger"] = {
						["type"] = "custom",
						["unevent"] = "auto",
						["use_absorbMode"] = true,
						["event"] = "Health",
						["use_unit"] = true,
						["subeventPrefix"] = "SPELL",
						["events"] = "PLAYER_TALENT_UPDATE, SPELLS_CHANGED",
						["custom"] = "function()\n    aura_env.VFAnchor()\nend\n\n\n\n\n\n\n\n\n",
						["custom_type"] = "status",
						["check"] = "event",
						["unit"] = "player",
						["subeventSuffix"] = "_CAST_START",
						["genericShowOn"] = "showOnActive",
					},
					["untrigger"] = {
					},
				}, -- [2]
				["disjunctive"] = "any",
				["activeTriggerMode"] = -10,
			},
			["load"] = {
				["ingroup"] = {
					["multi"] = {
					},
				},
				["talent"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["spec"] = {
					["single"] = 3,
					["multi"] = {
						true, -- [1]
						true, -- [2]
					},
				},
				["class"] = {
					["single"] = "SHAMAN",
					["multi"] = {
					},
				},
				["difficulty"] = {
					["multi"] = {
					},
				},
				["use_class"] = true,
				["role"] = {
					["multi"] = {
					},
				},
				["talent2"] = {
					["multi"] = {
					},
				},
				["faction"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["use_spec"] = false,
				["race"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["sparkBlendMode"] = "ADD",
			["useAdjustededMax"] = false,
			["sparkColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["uid"] = "CMl852Bt25T",
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
			},
			["anchorFrameType"] = "SCREEN",
			["frameStrata"] = 1,
			["icon"] = false,
			["useAdjustededMin"] = false,
			["regionType"] = "aurabar",
			["borderInFront"] = true,
			["id"] = "Maelstrom Bar",
			["icon_side"] = "RIGHT",
			["backdropColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				0.48000001907349, -- [4]
			},
			["sparkHeight"] = 30,
			["texture"] = "ElvUI Blank",
			["zoom"] = 0,
			["sparkTexture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
			["auto"] = true,
			["semver"] = "1.0.1",
			["sparkHidden"] = "NEVER",
			["stickyDuration"] = false,
			["alpha"] = 1,
			["width"] = 245,
			["sparkOffsetY"] = 0,
			["config"] = {
			},
			["inverse"] = false,
			["sparkOffsetX"] = 0,
			["orientation"] = "HORIZONTAL",
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["authorOptions"] = {
			},
		},
		["Autofollow"] = {
			["controlledChildren"] = {
				"Autofollow Leader", -- [1]
				"Autofollow Follower", -- [2]
			},
			["borderBackdrop"] = "Blizzard Tooltip",
			["authorOptions"] = {
			},
			["preferToUpdate"] = true,
			["yOffset"] = 532.5003051757812,
			["anchorPoint"] = "CENTER",
			["borderColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["url"] = "https://wago.io/UW5MyMEHT/11",
			["actions"] = {
				["start"] = {
				},
				["init"] = {
				},
				["finish"] = {
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["unit"] = "player",
						["type"] = "aura2",
						["spellIds"] = {
						},
						["subeventSuffix"] = "_CAST_START",
						["subeventPrefix"] = "SPELL",
						["debuffType"] = "HELPFUL",
						["event"] = "Health",
						["names"] = {
						},
					},
					["untrigger"] = {
					},
				}, -- [1]
			},
			["internalVersion"] = 45,
			["selfPoint"] = "BOTTOMLEFT",
			["desc"] = "--Honorax@Illidan",
			["version"] = 11,
			["subRegions"] = {
			},
			["load"] = {
				["talent"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["spec"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["backdropColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0.5, -- [4]
			},
			["scale"] = 1,
			["border"] = false,
			["borderEdge"] = "Square Full White",
			["regionType"] = "group",
			["borderSize"] = 2,
			["borderOffset"] = 4,
			["semver"] = "1.0.10",
			["tocversion"] = 90002,
			["id"] = "Autofollow",
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["borderInset"] = 1,
			["config"] = {
			},
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["xOffset"] = -161.6666259765625,
			["conditions"] = {
			},
			["information"] = {
				["groupOffset"] = true,
			},
			["uid"] = "pOW9j20lPNi",
		},
		["Innervate Receive"] = {
			["iconSource"] = 0,
			["xOffset"] = 0,
			["preferToUpdate"] = false,
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["cooldownSwipe"] = true,
			["cooldownEdge"] = false,
			["actions"] = {
				["start"] = {
					["do_loop"] = true,
					["custom"] = "local region = aura_env.region\n\nlocal unit = region.state and region.state.unit\nif not unit then return end\n\nlocal frame = WeakAuras.GetUnitFrame(unit)\nif not frame then return end\n\nregion:SetParent(frame)\nregion:ClearAllPoints()\nregion:SetAllPoints(frame)",
					["do_sound"] = true,
					["sound_repeat"] = 2,
					["do_custom"] = true,
					["sound"] = "Interface\\Addons\\WeakAuras\\PowerAurasMedia\\Sounds\\ESPARK1.ogg",
				},
				["finish"] = {
					["do_custom"] = true,
					["custom"] = "local region = aura_env.region\nregion:ClearAllPoints()",
				},
				["init"] = {
					["do_custom"] = true,
					["custom"] = "aura_env.spellId = 29166 -- Innervate\n\nlocal AceComm = LibStub(\"AceComm-3.0\")\n\nlocal function OnCommReceived(prefix, message, _, sender)\n    if prefix ~= \"TOSH_WA\" then return end\n    local subtype, msg = message:match(\"^(.+):(.+)$\")\n    if subtype ~= \"INNRV\" then return end\n    WeakAuras.ScanEvents(\"TOSH_WA_INNRV_GIVE\", sender)\nend\n\nAceComm:RegisterComm(\"TOSH_WA\", OnCommReceived)",
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["unevent"] = "auto",
						["type"] = "custom",
						["use_absorbMode"] = true,
						["subeventSuffix"] = "_CAST_START",
						["subeventPrefix"] = "SPELL",
						["duration"] = "1",
						["event"] = "Health",
						["names"] = {
						},
						["use_unit"] = true,
						["custom"] = "function(allstates, event, ...) -- TOSH_WA_INNRV_GIVE,CLEU:SPELL_AURA_APPLIED\n    local state = allstates[aura_env.spellId]\n    if not state then\n        state = {\n            changed = true,\n            show = false,\n            progressType = \"timed\",\n            autoHide = true,\n            spellId = aura_env.spellId,\n        }\n        allstates[aura_env.spellId] = state\n    end\n    \n    if event == \"TOSH_WA_INNRV_GIVE\" then\n        if state.show then return end -- already being shown\n        \n        local sender = ...\n        \n        if (\n            aura_env.config.assignment ~= \"\" and\n            sender ~= aura_env.config.assignment\n        ) then return end\n        \n        local cdStart, cdDuration = GetSpellCooldown(aura_env.spellId)\n        local cdEnd = cdStart + cdDuration\n        local now = GetTime()\n        -- if spell is within a second, allow it\n        if cdEnd - now > 1 then return end\n        \n        \n        for unit in WA_IterateGroupMembers() do\n            if UnitName(unit) == sender then\n                state.unit = unit\n                state.changed = true\n                state.show = true\n                state.duration = 5\n                state.expirationTime = now + state.duration\n                break\n            end\n        end\n        \n    elseif event == \"COMBAT_LOG_EVENT_UNFILTERED\" then\n        if not state.show then return end\n        \n        local args = {...}\n        if args[2] ~= \"SPELL_AURA_APPLIED\" or args[12] ~= aura_env.spellId then return end\n        if UnitGUID(state.unit) == args[8] then\n            state.show = false\n            state.changed = true\n        elseif args[4] == myGUID then\n            state.show = false\n            state.changed = true\n        end\n    end\n    \n    return true\nend",
						["spellIds"] = {
						},
						["events"] = "TOSH_WA_INNRV_GIVE,CLEU:SPELL_AURA_APPLIED",
						["check"] = "event",
						["unit"] = "player",
						["custom_type"] = "stateupdate",
						["debuffType"] = "HELPFUL",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["internalVersion"] = 45,
			["keepAspectRatio"] = false,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["desaturate"] = false,
			["subRegions"] = {
				{
					["glowFrequency"] = 0.25,
					["type"] = "subglow",
					["glowXOffset"] = 0,
					["glowType"] = "ACShine",
					["glowLength"] = 10,
					["glowYOffset"] = 0,
					["glowColor"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["useGlowColor"] = true,
					["glow"] = true,
					["glowThickness"] = 1,
					["glowScale"] = 1,
					["glowLines"] = 8,
					["glowBorder"] = false,
				}, -- [1]
			},
			["height"] = 48,
			["load"] = {
				["zoneIds"] = "",
				["class_and_spec"] = {
					["multi"] = {
						[102] = true,
						[105] = true,
					},
				},
				["talent"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["single"] = "PRIEST",
					["multi"] = {
						["PRIEST"] = true,
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["use_class_and_spec"] = false,
				["size"] = {
					["multi"] = {
					},
				},
			},
			["regionType"] = "icon",
			["displayIcon"] = "136048",
			["information"] = {
			},
			["url"] = "",
			["icon"] = true,
			["selfPoint"] = "CENTER",
			["zoom"] = 0,
			["cooldownTextDisabled"] = false,
			["uid"] = "MpP)zIHaoUL",
			["tocversion"] = 90002,
			["id"] = "Innervate Receive",
			["width"] = 76,
			["alpha"] = 1,
			["anchorFrameType"] = "UNITFRAME",
			["frameStrata"] = 1,
			["config"] = {
				["assignment"] = "",
			},
			["inverse"] = false,
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0, -- [4]
			},
			["conditions"] = {
			},
			["cooldown"] = false,
			["authorOptions"] = {
				{
					["type"] = "input",
					["useDesc"] = true,
					["width"] = 2,
					["desc"] = "Put the name of your assigned PI target. If set, will ignore other requestors. If empty, will show any requests.",
					["key"] = "assignment",
					["multiline"] = false,
					["default"] = "",
					["length"] = 10,
					["name"] = "Assignment",
					["useLength"] = false,
				}, -- [1]
			},
		},
		["Paladin 2"] = {
			["sparkWidth"] = 10,
			["iconSource"] = -1,
			["authorOptions"] = {
			},
			["preferToUpdate"] = false,
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["sparkRotation"] = 0,
			["url"] = "https://wago.io/ZG34ifiRf/1",
			["actions"] = {
				["start"] = {
					["do_message"] = false,
					["message_type"] = "SAY",
				},
				["init"] = {
				},
				["finish"] = {
				},
			},
			["fontFlags"] = "OUTLINE",
			["icon_color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["selfPoint"] = "CENTER",
			["barColor"] = {
				1, -- [1]
				0.49803921568627, -- [2]
				0.84705882352941, -- [3]
				1, -- [4]
			},
			["desaturate"] = false,
			["sparkOffsetY"] = 0,
			["load"] = {
				["ingroup"] = {
					["multi"] = {
					},
				},
				["use_never"] = false,
				["class"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["level"] = "100",
				["size"] = {
					["single"] = "party",
					["multi"] = {
						["ratedpvp"] = true,
						["arena"] = true,
						["ratedarena"] = true,
						["pvp"] = true,
					},
				},
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_level"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["difficulty"] = {
					["single"] = "challenge",
					["multi"] = {
						["challenge"] = true,
					},
				},
				["zoneIds"] = "",
				["faction"] = {
					["multi"] = {
					},
				},
				["use_size"] = false,
				["race"] = {
					["multi"] = {
					},
				},
				["use_zone"] = false,
				["pvptalent"] = {
					["multi"] = {
					},
				},
			},
			["useAdjustededMin"] = false,
			["regionType"] = "aurabar",
			["texture"] = "Glamour4",
			["sparkTexture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
			["spark"] = false,
			["tocversion"] = 80300,
			["alpha"] = 1,
			["sparkColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["displayIcon"] = "Interface\\Icons\\spell_holy_rebuke",
			["stacksPoint"] = "BOTTOMRIGHT",
			["borderBackdrop"] = "Blizzard Tooltip",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customTextUpdate"] = "update",
			["triggers"] = {
				{
					["trigger"] = {
						["duration"] = "15",
						["spellId"] = "",
						["subeventPrefix"] = "SPELL",
						["spellIds"] = {
						},
						["ownOnly"] = true,
						["genericShowOn"] = "showOnActive",
						["unit"] = "player",
						["debuffType"] = "HELPFUL",
						["use_specific_sourceunit"] = true,
						["inverse"] = true,
						["use_spellId"] = false,
						["type"] = "combatlog",
						["use_destName"] = false,
						["use_cloneId"] = true,
						["spellName"] = "Rebuke",
						["use_sourceFlags2"] = true,
						["use_sourceName"] = false,
						["unevent"] = "timed",
						["subeventSuffix"] = "_CAST_SUCCESS",
						["custom_hide"] = "timed",
						["sourceName"] = "Draemii",
						["event"] = "Combat Log",
						["sourceFlags2"] = "Hostile",
						["use_name"] = true,
						["use_spellName"] = true,
						["name"] = "Flask of the warm sun ",
						["use_sourceUnit"] = false,
						["use_destunit"] = false,
						["use_destUnit"] = false,
						["sourceUnit"] = "Benzikura",
						["names"] = {
							"Greater Draenic Intellect Flask", -- [1]
						},
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
			},
			["backdropInFront"] = false,
			["stickyDuration"] = false,
			["version"] = 1,
			["subRegions"] = {
				{
					["type"] = "subborder",
					["border_anchor"] = "bar",
					["border_size"] = 16,
					["border_color"] = {
						1, -- [1]
						0.4156862745098, -- [2]
						0.93333333333333, -- [3]
						0.5, -- [4]
					},
					["border_visible"] = true,
					["border_edge"] = "1 Pixel",
					["border_offset"] = 2,
				}, -- [1]
				{
					["type"] = "aurabar_bar",
				}, -- [2]
				{
					["text_shadowXOffset"] = 1,
					["text_text"] = "%p",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Vixar",
					["text_text_format_p_time_format"] = 0,
					["text_shadowYOffset"] = -1,
					["text_text_format_p_time_dynamic_threshold"] = 3,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "ICON_CENTER",
					["text_text_format_p_format"] = "timed",
					["text_visible"] = true,
					["text_fontSize"] = 20,
					["anchorXOffset"] = 0,
					["text_text_format_p_time_precision"] = 1,
				}, -- [3]
			},
			["height"] = 30,
			["sparkBlendMode"] = "ADD",
			["useAdjustededMax"] = false,
			["displayStacks"] = "%c",
			["auto"] = true,
			["backgroundColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				0.45673036575317, -- [4]
			},
			["uid"] = "AXt2379)sES",
			["borderInFront"] = false,
			["sparkOffsetX"] = 0,
			["icon_side"] = "LEFT",
			["xOffset"] = 0,
			["zoom"] = 0.28,
			["sparkHeight"] = 30,
			["icon"] = true,
			["stacksContainment"] = "INSIDE",
			["parent"] = "Enemy Interrupt Icons",
			["semver"] = "1.0.0",
			["width"] = 30,
			["sparkHidden"] = "NEVER",
			["config"] = {
			},
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["id"] = "Paladin 2",
			["backdropColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0.5, -- [4]
			},
			["inverse"] = false,
			["sparkDesature"] = false,
			["orientation"] = "HORIZONTAL",
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["sparkRotationMode"] = "AUTO",
		},
		["DisableAutoAdd"] = {
			["outline"] = "OUTLINE",
			["authorOptions"] = {
			},
			["displayText"] = "",
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["displayText_format_p_time_format"] = 0,
			["customTextUpdate"] = "update",
			["automaticWidth"] = "Auto",
			["actions"] = {
				["start"] = {
				},
				["init"] = {
					["custom"] = "-- Don't animate\nIconIntroTracker:UnregisterEvent('SPELL_PUSHED_TO_ACTIONBAR')\nIconIntroTracker.RegisterEvent = function() end",
					["do_custom"] = true,
				},
				["finish"] = {
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "custom",
						["unevent"] = "timed",
						["custom_hide"] = "timed",
						["duration"] = "1",
						["event"] = "Chat Message",
						["names"] = {
						},
						["spellIds"] = {
						},
						["custom"] = "function(event, spellID, slotIndex, slotPos)\n    if not InCombatLockdown() then\n        ClearCursor()\n        PickupAction(slotIndex)\n        ClearCursor()\n    end\nend",
						["events"] = "SPELL_PUSHED_TO_ACTIONBAR",
						["subeventSuffix"] = "_CAST_START",
						["subeventPrefix"] = "SPELL",
						["unit"] = "player",
						["custom_type"] = "event",
						["debuffType"] = "HELPFUL",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["displayText_format_p_format"] = "timed",
			["internalVersion"] = 45,
			["selfPoint"] = "BOTTOM",
			["font"] = "Friz Quadrata TT",
			["version"] = 1,
			["load"] = {
				["talent"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["spec"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["fontSize"] = 12,
			["shadowXOffset"] = 1,
			["regionType"] = "text",
			["preferToUpdate"] = false,
			["wordWrap"] = "WordWrap",
			["displayText_format_p_time_precision"] = 1,
			["conditions"] = {
			},
			["shadowYOffset"] = -1,
			["url"] = "https://wago.io/UQFNKr05D/1",
			["semver"] = "1.0.0",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["id"] = "DisableAutoAdd",
			["uid"] = "jF9)I)f3R5T",
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
			},
			["config"] = {
			},
			["xOffset"] = 0,
			["justify"] = "LEFT",
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["fixedWidth"] = 200,
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["displayText_format_p_time_dynamic_threshold"] = 60,
		},
		["04 Recombobulation"] = {
			["iconSource"] = -1,
			["authorOptions"] = {
			},
			["preferToUpdate"] = false,
			["yOffset"] = -2,
			["anchorPoint"] = "CENTER",
			["cooldownSwipe"] = true,
			["url"] = "https://wago.io/slraid1/132",
			["icon"] = true,
			["triggers"] = {
				{
					["trigger"] = {
						["spellId"] = "327902",
						["auranames"] = {
							"328324", -- [1]
						},
						["use_absorbMode"] = true,
						["unit"] = "player",
						["destUnit"] = "player",
						["debuffType"] = "HARMFUL",
						["useName"] = true,
						["unevent"] = "timed",
						["fetchTooltip"] = false,
						["event"] = "Combat Log",
						["use_unit"] = true,
						["subeventSuffix"] = "_AURA_APPLIED",
						["use_spellId"] = true,
						["spellIds"] = {
						},
						["type"] = "aura2",
						["subeventPrefix"] = "SPELL",
						["use_destUnit"] = true,
						["duration"] = "12",
						["names"] = {
						},
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["internalVersion"] = 45,
			["keepAspectRatio"] = false,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["colorR"] = 0.5843137254902,
					["scalex"] = 1,
					["colorA"] = 1,
					["colorG"] = 0.5843137254902,
					["type"] = "none",
					["easeType"] = "none",
					["scaley"] = 1,
					["alpha"] = 0,
					["use_color"] = false,
					["y"] = 0,
					["x"] = 0,
					["colorType"] = "custom",
					["rotate"] = 0,
					["colorFunc"] = "function(progress, r1, g1, b1, a1, r2, g2, b2, a2)\n    local angle = (progress * 4 * math.pi) - (math.pi / 2)\n    local newProgress = ((math.sin(angle) + 1)/2);\n    return WeakAuras.GetHSVTransition(newProgress, r1, g1, b1, a1, r2, g2, b2, a2)\nend",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["colorB"] = 0.5843137254902,
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["desaturate"] = false,
			["version"] = 132,
			["subRegions"] = {
				{
					["text_shadowXOffset"] = 0,
					["text_text_format_s_format"] = "none",
					["text_text"] = "Can't Port",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_text_format_tooltip1_format"] = "none",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Expressway",
					["text_shadowYOffset"] = 0,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "OUTER_BOTTOM",
					["text_fontSize"] = 16,
					["anchorXOffset"] = 0,
					["text_visible"] = true,
				}, -- [1]
				{
					["text_text_format_p_time_precision"] = 1,
					["text_text"] = "%p",
					["text_text_format_p_format"] = "timed",
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Expressway",
					["text_text_format_p_time_format"] = 0,
					["text_shadowYOffset"] = 0,
					["text_text_format_p_time_dynamic_threshold"] = 0,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "THICKOUTLINE",
					["text_anchorPoint"] = "CENTER",
					["text_visible"] = true,
					["text_shadowXOffset"] = 0,
					["text_fontSize"] = 25,
					["anchorXOffset"] = 0,
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
				}, -- [2]
				{
					["text_shadowXOffset"] = 0,
					["text_text_format_s_format"] = "none",
					["text_text"] = "%s",
					["text_text_format_p_format"] = "timed",
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["text_text_format_p_time_format"] = 0,
					["type"] = "subtext",
					["text_anchorXOffset"] = 6,
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Arial Narrow",
					["text_text_format_p_time_dynamic_threshold"] = 60,
					["text_anchorYOffset"] = -6,
					["text_visible"] = false,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "INNER_BOTTOMRIGHT",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_text_format_p_time_precision"] = 1,
					["text_fontSize"] = 25,
					["anchorXOffset"] = 0,
					["text_shadowYOffset"] = 0,
				}, -- [3]
			},
			["height"] = 80,
			["load"] = {
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2405",
				["use_encounterid"] = true,
				["role"] = {
					["single"] = "TANK",
					["multi"] = {
						["TANK"] = true,
					},
				},
				["zoneIds"] = "",
				["spec"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["cooldownTextDisabled"] = true,
			["xOffset"] = 81.00054931640625,
			["regionType"] = "icon",
			["cooldownEdge"] = false,
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["cooldown"] = true,
			["conditions"] = {
			},
			["config"] = {
			},
			["frameStrata"] = 1,
			["zoom"] = 0.3,
			["auto"] = true,
			["tocversion"] = 90002,
			["id"] = "04 Recombobulation",
			["anchorFrameType"] = "SCREEN",
			["alpha"] = 1,
			["width"] = 80,
			["semver"] = "1.0.131",
			["uid"] = "M92D7FBhyYp",
			["inverse"] = false,
			["selfPoint"] = "CENTER",
			["displayIcon"] = "3512762",
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["actions"] = {
				["start"] = {
					["sound"] = " custom",
					["do_sound"] = false,
				},
				["finish"] = {
				},
				["init"] = {
				},
			},
		},
		["07 Dark Recital Left"] = {
			["iconSource"] = -1,
			["xOffset"] = 0,
			["preferToUpdate"] = false,
			["yOffset"] = 200,
			["anchorPoint"] = "CENTER",
			["cooldownSwipe"] = true,
			["cooldownEdge"] = false,
			["icon"] = true,
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "combatlog",
						["spellId"] = "331636",
						["subeventSuffix"] = "_AURA_APPLIED",
						["duration"] = "8.5",
						["event"] = "Combat Log",
						["subeventPrefix"] = "SPELL",
						["destUnit"] = "player",
						["use_spellId"] = true,
						["spellIds"] = {
						},
						["unevent"] = "timed",
						["names"] = {
						},
						["use_destUnit"] = true,
						["unit"] = "player",
						["debuffType"] = "HARMFUL",
					},
					["untrigger"] = {
					},
				}, -- [1]
				{
					["trigger"] = {
						["type"] = "combatlog",
						["spellId"] = "331637",
						["subeventSuffix"] = "_AURA_APPLIED",
						["duration"] = "8.5",
						["event"] = "Combat Log",
						["subeventPrefix"] = "SPELL",
						["destUnit"] = "player",
						["use_spellId"] = true,
						["spellIds"] = {
						},
						["unevent"] = "timed",
						["names"] = {
						},
						["use_destUnit"] = true,
						["unit"] = "player",
						["debuffType"] = "HARMFUL",
					},
					["untrigger"] = {
					},
				}, -- [2]
				{
					["trigger"] = {
						["useMatch_count"] = true,
						["auraspellids"] = {
							"331636", -- [1]
							"331637", -- [2]
						},
						["unit"] = "group",
						["useIgnoreName"] = false,
						["type"] = "aura2",
						["match_count"] = "1",
						["useExactSpellId"] = true,
						["match_countOperator"] = ">=",
						["debuffType"] = "HARMFUL",
					},
					["untrigger"] = {
					},
				}, -- [3]
				["disjunctive"] = "custom",
				["customTriggerLogic"] = "function(t)\n    return t[3] and not t[1] and not t[2]\nend",
				["activeTriggerMode"] = -10,
			},
			["internalVersion"] = 45,
			["keepAspectRatio"] = false,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["desaturate"] = false,
			["discrete_rotation"] = 0,
			["version"] = 1,
			["subRegions"] = {
			},
			["height"] = 100,
			["rotate"] = true,
			["load"] = {
				["use_encounterid"] = true,
				["zoneIds"] = "",
				["talent"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2412",
				["class"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["textureWrapMode"] = "CLAMPTOBLACKADDITIVE",
			["uid"] = "Jdmdar0fjz)",
			["selfPoint"] = "CENTER",
			["parent"] = "07 Dark Recital Group",
			["mirror"] = false,
			["color"] = {
				1, -- [1]
				0, -- [2]
				0.031372549019608, -- [3]
				1, -- [4]
			},
			["regionType"] = "texture",
			["cooldown"] = false,
			["blendMode"] = "BLEND",
			["url"] = "https://wago.io/seLAtvE5B/1",
			["actions"] = {
				["start"] = {
					["do_custom"] = true,
					["custom"] = "if DBM then\n    aura_env.region.texture:SetTexture(\"Interface\\\\Addons\\\\DBM-Core\\\\textures\\\\arrows\\\\arrow.blp\", false)\n    aura_env.region.texture:SetTexCoord(0, 1/9, 3/12-1/50, 4/12-1/50)\nend",
					["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\Left.ogg",
					["do_sound"] = true,
				},
				["finish"] = {
				},
				["init"] = {
				},
			},
			["texture"] = "Interface\\AddOns\\WeakAuras\\Media\\Textures\\targeting-mark.tga",
			["frameStrata"] = 1,
			["zoom"] = 0,
			["semver"] = "1.0.0",
			["tocversion"] = 90002,
			["id"] = "07 Dark Recital Left",
			["width"] = 100,
			["alpha"] = 1,
			["anchorFrameType"] = "SCREEN",
			["cooldownTextDisabled"] = false,
			["config"] = {
			},
			["inverse"] = false,
			["rotation"] = 270,
			["conditions"] = {
			},
			["information"] = {
			},
			["authorOptions"] = {
			},
		},
		["Rogue 2"] = {
			["sparkWidth"] = 10,
			["iconSource"] = -1,
			["authorOptions"] = {
			},
			["preferToUpdate"] = false,
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["sparkRotation"] = 0,
			["url"] = "https://wago.io/ZG34ifiRf/1",
			["backgroundColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				0.46527713537216, -- [4]
			},
			["fontFlags"] = "OUTLINE",
			["icon_color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["selfPoint"] = "CENTER",
			["barColor"] = {
				1, -- [1]
				1, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["desaturate"] = false,
			["sparkOffsetY"] = 0,
			["load"] = {
				["ingroup"] = {
					["multi"] = {
					},
				},
				["use_never"] = false,
				["class"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["level"] = "100",
				["size"] = {
					["single"] = "party",
					["multi"] = {
						["ratedpvp"] = true,
						["arena"] = true,
						["ratedarena"] = true,
						["pvp"] = true,
					},
				},
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_level"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["difficulty"] = {
					["single"] = "challenge",
					["multi"] = {
						["challenge"] = true,
					},
				},
				["zoneIds"] = "",
				["faction"] = {
					["multi"] = {
					},
				},
				["use_size"] = false,
				["use_zone"] = false,
				["race"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
			},
			["useAdjustededMin"] = false,
			["regionType"] = "aurabar",
			["texture"] = "Glamour4",
			["sparkTexture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
			["auto"] = true,
			["tocversion"] = 80300,
			["alpha"] = 1,
			["uid"] = "58fsb4websn",
			["displayIcon"] = "Interface\\Icons\\spell_holy_rebuke",
			["stacksPoint"] = "BOTTOMRIGHT",
			["borderBackdrop"] = "Blizzard Tooltip",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customTextUpdate"] = "update",
			["triggers"] = {
				{
					["trigger"] = {
						["duration"] = "15",
						["spellId"] = "",
						["subeventPrefix"] = "SPELL",
						["spellIds"] = {
						},
						["ownOnly"] = true,
						["genericShowOn"] = "showOnActive",
						["unit"] = "player",
						["debuffType"] = "HELPFUL",
						["use_specific_sourceunit"] = true,
						["inverse"] = true,
						["use_spellId"] = false,
						["type"] = "combatlog",
						["use_destName"] = false,
						["use_cloneId"] = true,
						["spellName"] = "Kick",
						["use_sourceFlags2"] = true,
						["use_sourceName"] = false,
						["unevent"] = "timed",
						["subeventSuffix"] = "_CAST_SUCCESS",
						["custom_hide"] = "timed",
						["sourceName"] = "Draemii",
						["event"] = "Combat Log",
						["sourceFlags2"] = "Hostile",
						["use_name"] = true,
						["use_spellName"] = true,
						["name"] = "Flask of the warm sun ",
						["use_sourceUnit"] = false,
						["use_destunit"] = false,
						["use_destUnit"] = false,
						["sourceUnit"] = "Benzikura",
						["names"] = {
							"Greater Draenic Intellect Flask", -- [1]
						},
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
			},
			["backdropInFront"] = false,
			["stickyDuration"] = false,
			["version"] = 1,
			["subRegions"] = {
				{
					["type"] = "subborder",
					["border_anchor"] = "bar",
					["border_size"] = 16,
					["border_color"] = {
						1, -- [1]
						0.9921568627451, -- [2]
						0, -- [3]
						0.5, -- [4]
					},
					["border_visible"] = true,
					["border_edge"] = "1 Pixel",
					["border_offset"] = 2,
				}, -- [1]
				{
					["type"] = "aurabar_bar",
				}, -- [2]
				{
					["text_shadowXOffset"] = 1,
					["text_text"] = "%p",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Vixar",
					["text_text_format_p_time_format"] = 0,
					["text_shadowYOffset"] = -1,
					["text_text_format_p_time_dynamic_threshold"] = 3,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "ICON_CENTER",
					["text_text_format_p_format"] = "timed",
					["text_visible"] = true,
					["text_fontSize"] = 20,
					["anchorXOffset"] = 0,
					["text_text_format_p_time_precision"] = 1,
				}, -- [3]
			},
			["height"] = 30,
			["sparkBlendMode"] = "ADD",
			["useAdjustededMax"] = false,
			["displayStacks"] = "%c",
			["sparkColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["spark"] = false,
			["icon"] = true,
			["borderInFront"] = false,
			["sparkOffsetX"] = 0,
			["icon_side"] = "LEFT",
			["xOffset"] = 0,
			["zoom"] = 0.28,
			["sparkHeight"] = 30,
			["actions"] = {
				["start"] = {
					["do_message"] = false,
					["message_type"] = "SAY",
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
				},
			},
			["stacksContainment"] = "INSIDE",
			["parent"] = "Enemy Interrupt Icons",
			["semver"] = "1.0.0",
			["width"] = 30,
			["id"] = "Rogue 2",
			["config"] = {
			},
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["sparkHidden"] = "NEVER",
			["backdropColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0.5, -- [4]
			},
			["inverse"] = false,
			["sparkDesature"] = false,
			["orientation"] = "HORIZONTAL",
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["sparkRotationMode"] = "AUTO",
		},
		["02 Petrifying Howl - White"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 4,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_frequency"] = 0.25,
					["custom"] = "",
					["glow_border"] = true,
					["glow_lines"] = 12,
					["glow_type"] = "Pixel",
					["use_glow_color"] = true,
					["do_custom"] = false,
					["do_glow"] = true,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = true,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2418",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "m689yMCiUWp",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["useMatch_count"] = true,
						["auranames"] = {
						},
						["groupclone"] = true,
						["matchesShowOn"] = "showOnActive",
						["name_info"] = "aura",
						["unit"] = "group",
						["use_tooltip"] = false,
						["subeventPrefix"] = "SPELL",
						["match_countOperator"] = ">=",
						["match_count"] = "1",
						["buffShowOn"] = "showOnActive",
						["unevent"] = "timed",
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["useExactSpellId"] = true,
						["custom_type"] = "stateupdate",
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["event"] = "Health",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HARMFUL",
						["use_specific_unit"] = false,
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["auraspellids"] = {
							"334852", -- [1]
						},
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["duration"] = "1",
						["useGroup_count"] = false,
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["wordWrap"] = "WordWrap",
			["config"] = {
			},
			["text1"] = "%n",
			["text1Containment"] = "INSIDE",
			["width"] = 6.9999866485596,
			["justify"] = "LEFT",
			["semver"] = "1.0.26",
			["zoom"] = 0,
			["id"] = "02 Petrifying Howl - White",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["frameStrata"] = 9,
			["anchorFrameType"] = "SCREEN",
			["text1Point"] = "CENTER",
			["text2Font"] = "Friz Quadrata TT",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["Chain Distance 2"] = {
			["outline"] = "OUTLINE",
			["iconSource"] = -1,
			["authorOptions"] = {
			},
			["displayText"] = "%c",
			["customText"] = "function()\n    if (aura_env.state and aura_env.state.casterName) then\n        local partner = aura_env.state.casterName\n        local myRole = UnitGroupRolesAssigned(\"player\")\n        local theirRole = UnitGroupRolesAssigned(partner)\n        local chainRole \n        if (myRole == \"HEALER\" and theirRole ~= \"HEALER\") then\n            chainRole = \"YOU ARE ALPHA (FRONT/RIGHT)\"\n        elseif (theirRole == \"HEALER\" and myRole ~= \"HEALER\") then\n            chainRole = \"YOU ARE BETA (BACK/LEFT)\"\n            \n        elseif (aura_env.playerName > aura_env.state.casterName) then\n            chainRole = \"YOU ARE ALPHA (FRONT/RIGHT)\"\n        else\n            chainRole = \"YOU ARE BETA (BACK/LEFT)\"\n        end\n        \n        local icon = \"\"\n        if theirRole ~= \"NONE\" then\n            local x1,x2,y1,y2 = GetTexCoordsForRole(theirRole)\n            x1,x2,y1,y2 = x1*256, x2*256, y1*256, y2*256\n            icon = \"|TInterface\\\\LFGFRAME\\\\UI-LFG-ICON-ROLES:0:0:0:0:256:256:\" .. x1 .. \":\" .. x2 .. \":\" .. y1 .. \":\" .. y2 .. \"|t\"\n        end\n        \n        if aura_env.RangeCheck(partner, 6) then\n            aura_env.LCG.ButtonGlow_Start(aura_env.region, {0,1,0,1})\n        elseif aura_env.RangeCheck(partner, 8) then\n            aura_env.LCG.ButtonGlow_Start(aura_env.region, {1,1,0,1})\n        else\n            aura_env.LCG.ButtonGlow_Start(aura_env.region, {1,0,0,1}) \n        end\n        \n        return icon .. WA_ClassColorName(partner) .. \": \" .. aura_env.RangeCheck(partner) .. \"|n\" .. chainRole\n    end\nend\n\n\n",
			["shadowYOffset"] = -1,
			["displayText_format_p_time_dynamic"] = false,
			["cooldownSwipe"] = true,
			["customTextUpdate"] = "update",
			["automaticWidth"] = "Auto",
			["actions"] = {
				["start"] = {
					["do_custom"] = false,
					["custom"] = "aura_env.pairs = {}",
				},
				["finish"] = {
					["hide_all_glows"] = true,
					["do_custom"] = true,
					["custom"] = "local LCG = LibStub(\"LibCustomGlow-1.0\")\nLCG.ButtonGlow_Stop(aura_env.region)",
				},
				["init"] = {
					["do_custom"] = true,
					["custom"] = "aura_env.EnemyTargets = {\n    keys = {\n        [5] = 37727, -- Ruby Acorn \n        [6] = 63427, -- Worgsaw \n        [7] = 61323, -- Ruby Seeds\n        [8] = 34368, -- Attuned Crystal Cores \n        [10] = 32321, -- Sparrowhawk Net \n        [15] = 33069, -- Sturdy Rope\n        [20] = 10645, -- Gnomish Death Ray\n        [25] = 24268, -- Netherweave Net\n        [30] = 835, -- Large Rope Net\n        [35] = 24269, -- Heavy Netherweave Net\n        [38] = 140786, -- Ley Spider Eggs\n        [40] = 28767, -- The Decapitator\n        [45] = 23836, -- Goblin Rocket Launcher\n        [50] = 116139, -- Haunting Memento \n        [60] = 32825, -- Soul Cannon \n        [70] = 41265, -- Eyesore Blaster \n        [80] = 35278, -- Reinforced Net \n        [90] = 133925, -- Fel Lash\n        [100] = 33119 -- Malister's Frost Wand\n    },    \n    {0, 1}, -- 0\n    {5, 37727}, -- Ruby Acorn \n    {6, 63427}, -- Worgsaw \n    {7, 61323}, -- Ruby Seeds\n    {8, 34368}, -- Attuned Crystal Cores \n    {10, 32321}, -- Sparrowhawk Net \n    {15, 33069}, -- Sturdy Rope\n    {20, 10645}, -- Gnomish Death Ray\n    {25, 24268}, -- Netherweave Net\n    {30, 835}, -- Large Rope Net\n    {35, 24269}, -- Heavy Netherweave Net\n    {38, 140786}, -- Ley Spider Eggs\n    {40, 28767}, -- The Decapitator\n    {45, 23836}, -- Goblin Rocket Launcher\n    {50, 116139}, -- Haunting Memento \n    {60, 32825}, -- Soul Cannon \n    {70, 41265}, -- Eyesore Blaster \n    {80, 35278}, -- Reinforced Net \n    {90, 133925}, -- Fel Lash\n    {100, 33119}, -- Malister's Frost Wand\n}\naura_env.FriendlyTargets = {\n    keys = {\n        [5] = 37727, -- Ruby Acorn\n        [6] = 63427, -- Worgsaw\n        [7] = 61323, -- Ruby Seeds\n        [8] = 34368, -- Attuned Crystal Cores\n        [10] = 32321, -- Sparrowhawk Net\n        [15] = 1251, -- Linen Bandage\n        [20] = 21519, -- Mistletoe\n        [25] = 31463, -- Zezzak's Shard\n        [30] = 1180, -- Scroll of Stamina\n        [35] = 18904, -- Zorbin's Ultra-Shrinker\n        [38] = 140786, -- Ley Spider Eggs\n        [40] = 34471, -- Vial of the Sunwell\n        [45] = 32698, --Wrangling Rope\n        [50] = 116139, -- Haunting Memento\n        [60] = 32825, -- Soul Cannon\n        [70] = 41265, -- Eyesore Blaster\n        [80] = 35278, -- Reinforced Net\n        [90] = 133925, -- Fel Lash\n        [100] = 41058, -- Hyldnir Harpoon\n        [150] = 6954, -- Flaming Spears\n        [200] = 75208 -- Rancher's Lariat\n    },\n    {0, 1}, -- 0\n    {5, 37727}, -- Ruby Acorn\n    {6, 63427}, -- Worgsaw\n    {7, 61323}, -- Ruby Seeds\n    {8, 34368}, -- Attuned Crystal Cores\n    {10, 32321}, -- Sparrowhawk Net\n    {15, 1251}, -- Linen Bandage\n    {20, 21519}, -- Mistletoe\n    {25, 31463}, -- Zezzak's Shard\n    {30, 1180}, -- Scroll of Stamina\n    {35, 18904}, -- Zorbin's Ultra-Shrinker\n    {38, 140786}, -- Ley Spider Eggs\n    {40, 34471}, -- Vial of the Sunwell\n    {45, 32698}, --Wrangling Rope\n    {50, 116139}, -- Haunting Memento\n    {60, 32825}, -- Soul Cannon\n    {70, 41265}, -- Eyesore Blaster\n    {80, 35278}, -- Reinforced Net\n    {90, 133925}, -- Fel Lash\n    {100, 41058}, -- Hyldnir Harpoon\n    {150, 6954}, -- Flaming Spears\n    {200, 75208} -- Rancher's Lariat\n}\n\n--main range check function\naura_env.RangeCheck = function(unit, range)\n    if not UnitExists(unit) then return end\n    local t = {}\n    if UnitCanAttack(\"player\", unit) then\n        t = aura_env.EnemyTargets\n    else\n        t = aura_env.FriendlyTargets\n    end\n    \n    if range then\n        if not t.keys[range] then return end\n        if IsItemInRange(t.keys[range], unit) then\n            return true\n        else\n            return false\n        end\n    else\n        for i=1, #t do\n            if IsItemInRange(t[i][2], unit) then\n                return t[i-1][1] .. \"-\" .. t[i][1]\n            end\n        end\n    end\nend\n\n--count amount of units in X range (enemy targets by nameplates only)\naura_env.inRange = function(inRange, isEnemy, spellid, missing)\n    if not inRange then return \"no range selected\" end\n    local count = 0\n    local aura = true\n    if isEnemy then\n        for i=1, 40 do\n            if aura_env.RangeCheck(\"nameplate\"..i, inRange) then\n                count = count + 1\n            end\n        end\n    else\n        for unit in aura_env.GroupMembers() do\n            if spellid then if missing then aura = not UnitAura(unit, GetSpellInfo(spellid)) else aura = UnitAura(unit, GetSpellInfo(spellid)) end end\n            if aura_env.RangeCheck(unit, inRange) and not UnitIsDeadOrGhost(unit) and WeakAuras.myGUID~=UnitGUID(unit) and aura then\n                count = count + 1\n            end\n        end\n    end\n    return count\nend\n\n--show names of group units in X range\naura_env.inRangeNames = function(inRange, spellid, missing)\n    if not inRange then return \"no range selected\" end\n    local names = \"\"\n    local aura = true\n    for unit in aura_env.GroupMembers() do\n        if spellid then if missing then aura = not UnitAura(unit, GetSpellInfo(spellid)) else aura = UnitAura(unit, GetSpellInfo(spellid)) end end\n        if aura_env.RangeCheck(unit, inRange) and not UnitIsDeadOrGhost(unit) and WeakAuras.myGUID~=UnitGUID(unit) and aura then\n            local name = UnitName(unit)\n            local _, class = UnitClass(unit)\n            local colour = select(4, GetClassColor(class))\n            name = WrapTextInColorCode(name, colour)\n            names = names .. name .. \"|n\"\n        end\n    end\n    return names\nend\n\naura_env.LCG = LibStub(\"LibCustomGlow-1.0\")\n\naura_env.playerName = UnitName(\"player\")\n\n\n\n\n\n\n\n\n\n\n\n",
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["useName"] = true,
						["custom_type"] = "event",
						["debuffType"] = "HARMFUL",
						["duration"] = "56",
						["event"] = "Health",
						["subeventPrefix"] = "SPELL",
						["subeventSuffix"] = "_CAST_START",
						["type"] = "aura2",
						["spellIds"] = {
						},
						["auranames"] = {
							"Chain Link", -- [1]
						},
						["names"] = {
						},
						["events"] = "CLEU:SPELL_AURA_APPLIED",
						["unit"] = "player",
						["custom_hide"] = "timed",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["displayText_format_p_format"] = "timed",
			["internalVersion"] = 45,
			["keepAspectRatio"] = false,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["desaturate"] = false,
			["font"] = "Oswald",
			["subRegions"] = {
				{
					["text_text_format_p_time_format"] = 0,
					["text_text"] = "%c",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Oswald",
					["text_shadowXOffset"] = 0,
					["text_shadowYOffset"] = 0,
					["text_text_format_p_time_dynamic_threshold"] = 60,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "OUTER_BOTTOM",
					["text_text_format_p_format"] = "timed",
					["text_text_format_p_time_precision"] = 1,
					["text_fontSize"] = 20,
					["anchorXOffset"] = 0,
					["text_visible"] = true,
				}, -- [1]
			},
			["height"] = 94.00000762939453,
			["load"] = {
				["use_encounterid"] = true,
				["zoneIds"] = "",
				["talent"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2399",
				["size"] = {
					["multi"] = {
					},
				},
			},
			["alpha"] = 1,
			["fontSize"] = 12,
			["preferToUpdate"] = false,
			["cooldown"] = false,
			["shadowXOffset"] = 1,
			["displayIcon"] = 463560,
			["fixedWidth"] = 200,
			["icon"] = true,
			["regionType"] = "icon",
			["anchorPoint"] = "CENTER",
			["config"] = {
			},
			["yOffset"] = 126.4129638671875,
			["width"] = 92.99978637695312,
			["displayText_format_p_time_precision"] = 1,
			["wordWrap"] = "WordWrap",
			["zoom"] = 0,
			["cooldownTextDisabled"] = false,
			["justify"] = "LEFT",
			["tocversion"] = 90005,
			["id"] = "Chain Distance 2",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["xOffset"] = 352.5001220703125,
			["uid"] = "EpRuRG8ilUa",
			["inverse"] = false,
			["cooldownEdge"] = false,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
			},
			["selfPoint"] = "BOTTOM",
		},
		["Hunter 2"] = {
			["sparkWidth"] = 10,
			["iconSource"] = -1,
			["authorOptions"] = {
			},
			["preferToUpdate"] = false,
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["sparkRotation"] = 0,
			["url"] = "https://wago.io/ZG34ifiRf/1",
			["actions"] = {
				["start"] = {
					["do_message"] = false,
					["message_type"] = "SAY",
				},
				["init"] = {
				},
				["finish"] = {
				},
			},
			["fontFlags"] = "OUTLINE",
			["icon_color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["selfPoint"] = "CENTER",
			["barColor"] = {
				0.58823529411765, -- [1]
				0.77647058823529, -- [2]
				0.32549019607843, -- [3]
				1, -- [4]
			},
			["desaturate"] = false,
			["sparkOffsetY"] = 0,
			["load"] = {
				["ingroup"] = {
					["multi"] = {
					},
				},
				["use_never"] = false,
				["class"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["level"] = "100",
				["size"] = {
					["single"] = "party",
					["multi"] = {
						["ratedpvp"] = true,
						["arena"] = true,
						["ratedarena"] = true,
						["pvp"] = true,
					},
				},
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_level"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["difficulty"] = {
					["single"] = "challenge",
					["multi"] = {
						["challenge"] = true,
					},
				},
				["zoneIds"] = "",
				["faction"] = {
					["multi"] = {
					},
				},
				["use_size"] = false,
				["use_zone"] = false,
				["race"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
			},
			["useAdjustededMin"] = false,
			["regionType"] = "aurabar",
			["texture"] = "Glamour4",
			["sparkTexture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
			["auto"] = true,
			["tocversion"] = 80300,
			["alpha"] = 1,
			["config"] = {
			},
			["displayIcon"] = "Interface\\Icons\\spell_holy_rebuke",
			["stacksPoint"] = "BOTTOMRIGHT",
			["sparkOffsetX"] = 0,
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customTextUpdate"] = "update",
			["triggers"] = {
				{
					["trigger"] = {
						["duration"] = "24",
						["spellId"] = "",
						["names"] = {
							"Greater Draenic Intellect Flask", -- [1]
						},
						["spellIds"] = {
						},
						["ownOnly"] = true,
						["genericShowOn"] = "showOnActive",
						["subeventPrefix"] = "SPELL",
						["custom_hide"] = "timed",
						["use_specific_sourceunit"] = true,
						["name"] = "Flask of the warm sun ",
						["use_spellName"] = true,
						["type"] = "combatlog",
						["use_destName"] = false,
						["use_cloneId"] = true,
						["spellName"] = "Counter Shot",
						["sourceFlags2"] = "Hostile",
						["sourceName"] = "",
						["unevent"] = "timed",
						["subeventSuffix"] = "_CAST_SUCCESS",
						["debuffType"] = "HELPFUL",
						["use_sourceName"] = false,
						["event"] = "Combat Log",
						["use_sourceFlags2"] = true,
						["use_name"] = true,
						["use_spellId"] = false,
						["inverse"] = true,
						["use_sourceUnit"] = false,
						["use_destunit"] = false,
						["use_destUnit"] = false,
						["sourceUnit"] = "Benzikura",
						["unit"] = "player",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
			},
			["backdropInFront"] = false,
			["stickyDuration"] = false,
			["version"] = 1,
			["subRegions"] = {
				{
					["type"] = "subborder",
					["border_anchor"] = "bar",
					["border_size"] = 16,
					["border_color"] = {
						0.59607843137255, -- [1]
						0.78039215686275, -- [2]
						0.40392156862745, -- [3]
						0.5, -- [4]
					},
					["border_visible"] = true,
					["border_edge"] = "1 Pixel",
					["border_offset"] = 2,
				}, -- [1]
				{
					["type"] = "aurabar_bar",
				}, -- [2]
				{
					["text_shadowXOffset"] = 1,
					["text_text"] = "%p",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Vixar",
					["text_text_format_p_time_format"] = 0,
					["text_shadowYOffset"] = -1,
					["text_text_format_p_time_dynamic_threshold"] = 3,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "ICON_CENTER",
					["text_text_format_p_format"] = "timed",
					["text_visible"] = true,
					["text_fontSize"] = 20,
					["anchorXOffset"] = 0,
					["text_text_format_p_time_precision"] = 1,
				}, -- [3]
			},
			["height"] = 30,
			["sparkBlendMode"] = "ADD",
			["useAdjustededMax"] = false,
			["displayStacks"] = "%c",
			["sparkColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["icon"] = true,
			["uid"] = "0r1UZteQ6py",
			["borderInFront"] = false,
			["borderBackdrop"] = "Blizzard Tooltip",
			["icon_side"] = "LEFT",
			["xOffset"] = 0,
			["zoom"] = 0.28,
			["sparkHeight"] = 30,
			["backgroundColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				0.43108940124512, -- [4]
			},
			["stacksContainment"] = "INSIDE",
			["parent"] = "Enemy Interrupt Icons",
			["semver"] = "1.0.0",
			["width"] = 30,
			["id"] = "Hunter 2",
			["spark"] = false,
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["sparkHidden"] = "NEVER",
			["backdropColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0.5, -- [4]
			},
			["inverse"] = false,
			["sparkDesature"] = false,
			["orientation"] = "HORIZONTAL",
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["sparkRotationMode"] = "AUTO",
		},
		["01 Exsanguinated High Stacks - Black"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 3,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.2,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = true,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2398",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "ibWR0uHZ2Ds",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["custom_type"] = "stateupdate",
						["useMatch_count"] = true,
						["useStacks"] = true,
						["useGroup_count"] = false,
						["groupclone"] = true,
						["matchesShowOn"] = "showOnActive",
						["name_info"] = "aura",
						["subeventPrefix"] = "SPELL",
						["useExactSpellId"] = true,
						["stacks"] = "7",
						["debuffType"] = "HARMFUL",
						["subeventSuffix"] = "_CAST_START",
						["match_count"] = "1",
						["stacksOperator"] = ">=",
						["auraspellids"] = {
							"328897", -- [1]
						},
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["match_countOperator"] = ">=",
						["unit"] = "group",
						["duration"] = "1",
						["event"] = "Health",
						["use_specific_unit"] = false,
						["unevent"] = "timed",
						["use_tooltip"] = false,
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["buffShowOn"] = "showOnActive",
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["auranames"] = {
						},
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["wordWrap"] = "WordWrap",
			["config"] = {
			},
			["text1"] = "%n",
			["text1Containment"] = "INSIDE",
			["width"] = 6.9999866485596,
			["justify"] = "LEFT",
			["semver"] = "1.0.26",
			["zoom"] = 0,
			["id"] = "01 Exsanguinated High Stacks - Black",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["frameStrata"] = 9,
			["anchorFrameType"] = "SCREEN",
			["text1Point"] = "CENTER",
			["text2Font"] = "Friz Quadrata TT",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["03 Crimson Torment - Red"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 3,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						1, -- [1]
						0, -- [2]
						0.035294117647059, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.25,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = true,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2402",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "Ja1h(YlOyzP",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["useMatch_count"] = true,
						["auranames"] = {
						},
						["groupclone"] = true,
						["matchesShowOn"] = "showOnActive",
						["name_info"] = "aura",
						["unit"] = "group",
						["use_tooltip"] = false,
						["subeventPrefix"] = "SPELL",
						["match_countOperator"] = ">=",
						["match_count"] = "1",
						["buffShowOn"] = "showOnActive",
						["unevent"] = "timed",
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["useExactSpellId"] = true,
						["custom_type"] = "stateupdate",
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["event"] = "Health",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HARMFUL",
						["use_specific_unit"] = false,
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["auraspellids"] = {
							"326586", -- [1]
						},
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["duration"] = "1",
						["useGroup_count"] = false,
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["wordWrap"] = "WordWrap",
			["config"] = {
			},
			["text1"] = "%n",
			["text1Containment"] = "INSIDE",
			["width"] = 6.9999866485596,
			["justify"] = "LEFT",
			["semver"] = "1.0.26",
			["zoom"] = 0,
			["id"] = "03 Crimson Torment - Red",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["frameStrata"] = 9,
			["anchorFrameType"] = "SCREEN",
			["text1Point"] = "CENTER",
			["text2Font"] = "Friz Quadrata TT",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["nocircle_leaf_fill"] = {
			["iconSource"] = 0,
			["xOffset"] = 0,
			["preferToUpdate"] = true,
			["customText"] = "function()\n    if aura_env.showText then\n        return aura_env.order\n    else\n        return \"\"\n    end\nend",
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["cooldownSwipe"] = true,
			["cooldownEdge"] = false,
			["icon"] = true,
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "custom",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HELPFUL",
						["event"] = "Health",
						["unit"] = "player",
						["custom_type"] = "status",
						["spellIds"] = {
						},
						["events"] = "ZONE_CHANGED, PLAYER_LOGIN, PLAYER_ENTERING_WORLD",
						["names"] = {
						},
						["check"] = "event",
						["custom"] = "function()\n    WeakAuras.ScanEvents(\"TirnaScitheButtonData\", 4, aura_env.frameData)\n    if aura_env.debugmode then \n        return true \n    end\n    local text = GetMinimapZoneText()\n    if text == aura_env.mistZone then\n        return true\n    else\n        return false\n    end\nend\n\n",
						["subeventPrefix"] = "SPELL",
						["custom_hide"] = "custom",
					},
					["untrigger"] = {
					},
				}, -- [1]
				{
					["trigger"] = {
						["debuffType"] = "HELPFUL",
						["type"] = "custom",
						["custom"] = "function(event,eid,...)\n    if event == \"ENCOUNTER_END\" then \n        if eid == 2392 and aura_env.showEncounter == false then \n            return false \n        end\n        return true \n    elseif event == \"ENCOUNTER_START\" then \n        if aura_env.showEncounter == false then \n            return false\n        else\n            return true\n        end\n    end\n    return true\nend\n\n",
						["custom_type"] = "status",
						["check"] = "event",
						["customName"] = "function()\n    return \nend",
						["events"] = "ENCOUNTER_START, ENCOUNTER_END, PLAYER_ENTERING_WORLD",
						["unit"] = "player",
					},
					["untrigger"] = {
					},
				}, -- [2]
				{
					["trigger"] = {
						["type"] = "custom",
						["custom_type"] = "status",
						["unit"] = "player",
						["customName"] = "function()\n    return aura_env.order            \nend",
						["events"] = "TirnaScitheButtonOrder",
						["check"] = "event",
						["debuffType"] = "HELPFUL",
						["custom"] = "function(_,button,order)\n    if button == 4 then \n        aura_env.order = order\n        if order == 9 then \n            aura_env.order = \"\"\n        end\n        return true\n    elseif button == 9 then\n        aura_env.order = \"\"\n        return true\n    end\n    return true\nend\n\n",
						["custom_hide"] = "timed",
					},
					["untrigger"] = {
					},
				}, -- [3]
				["activeTriggerMode"] = -10,
			},
			["internalVersion"] = 45,
			["keepAspectRatio"] = false,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["desaturate"] = false,
			["version"] = 67,
			["subRegions"] = {
				{
					["glowFrequency"] = 0.25,
					["type"] = "subglow",
					["useGlowColor"] = false,
					["glowType"] = "buttonOverlay",
					["glowLength"] = 10,
					["glowYOffset"] = 0,
					["glowColor"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["glowXOffset"] = 0,
					["glowScale"] = 1,
					["glowThickness"] = 1,
					["glow"] = false,
					["glowLines"] = 8,
					["glowBorder"] = false,
				}, -- [1]
				{
					["text_text_format_p_time_format"] = 0,
					["text_text"] = "%c",
					["text_text_format_p_format"] = "timed",
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Friz Quadrata TT",
					["text_text_format_p_time_precision"] = 1,
					["text_shadowYOffset"] = 0,
					["text_text_format_p_time_dynamic_threshold"] = 60,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "INNER_BOTTOMRIGHT",
					["text_shadowXOffset"] = 0,
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_fontSize"] = 15,
					["anchorXOffset"] = 0,
					["text_visible"] = true,
				}, -- [2]
			},
			["height"] = 80,
			["load"] = {
				["ingroup"] = {
					["single"] = "group",
					["multi"] = {
						["group"] = true,
					},
				},
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["use_encounterid"] = false,
				["difficulty"] = {
					["multi"] = {
						["mythic"] = true,
						["challenge"] = true,
					},
				},
				["use_zone"] = false,
				["zoneIds"] = "1669",
				["use_zoneIds"] = true,
				["zone"] = "",
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["config"] = {
				["encounter"] = false,
				["noCommunication"] = false,
				["debug"] = false,
				["orderText"] = true,
			},
			["actions"] = {
				["start"] = {
				},
				["finish"] = {
				},
				["init"] = {
					["custom"] = "aura_env.debugmode = aura_env.config['debug']\nlocal debug = aura_env.debugmode\naura_env.showEncounter = aura_env.config['encounter']\naura_env.locale=GetLocale()\naura_env.mistZone = \"Mistveil Tangle\"\naura_env.order = \"\"\naura_env.showText = aura_env.config['orderText']\naura_env.noCommunication = aura_env.config['noCommunication']\nif aura_env.locale == \"koKR\" then\n    aura_env.mistZone = \"안개장막 덩굴숲\"\nelseif aura_env.locale == \"frFR\" then\n    aura_env.mistZone = \"Maquis Voile-de-Brume\"\nelseif aura_env.locale == \"deDE\" then\n    aura_env.mistZone = \"Nebelschleierdickicht\"\nelseif aura_env.locale == \"zhCN\" then\n    aura_env.mistZone = \"纱雾迷结\"\nelseif aura_env.locale == \"esES\" then\n    aura_env.mistZone = \"Espesura Velo de Niebla\"\nelseif aura_env.locale == \"zhTW\" then\n    aura_env.mistZone = \"霧紗密林\"\nelseif aura_env.locale == \"esMX\" then\n    aura_env.mistZone = \"Espesura Veloniebla\"\nelseif aura_env.locale == \"ruRU\" then\n    aura_env.mistZone = \"Туманная чащоба\"\nelseif aura_env.locale == \"ptBR\" then\n    aura_env.mistZone = \"Enleio do Véu da Névoa\"\nelseif aura_env.locale == \"itIT\" then\n    aura_env.mistZone = \"Intrico Velofosco\"\nelseif aura_env.locale == \"ptPT\" then\n    aura_env.mistZone = \"Enleio do Véu da Névoa\"        \nend\nlocal values = {1, 1, 0, 4}\nlocal message = string.format(\"%d %d %d %d\", values[1], values[2], values[3], values[4])\nlocal name, _ = UnitName(\"player\")\n\nif not aura_env.frameData then\n    local b = CreateFrame(\"Button\", nil, WeakAuras.regions[aura_env.id].region)\n    b:SetAllPoints(WeakAuras.regions[aura_env.id].region)\n    b:RegisterForClicks(\"LeftButtonDown\", \"RightButtonDown\")\n    aura_env.frameData = b\n    WeakAuras.ScanEvents(\"TirnaScitheButtonData\", 4, b)\nend\nif aura_env.noCommunication then\n    aura_env.frameData:SetScript(\"OnClick\", function(self, button)\n            if debug then print(\"Click Registered - true\") end\n            WeakAuras.ScanEvents(\"GlowUpdate\")\n            C_ChatInfo.RegisterAddonMessagePrefix(\"Tirna Ver2\") \n            if debug then \n                print(\"noCommunication sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"WHISPER\", name)) \n            else\n                C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"WHISPER\", name)\n            end\n            \n    end)\nelse\n    aura_env.frameData:SetScript(\"OnClick\", function(self, button)\n            if debug then print(\"Click Registered - true\") end\n            WeakAuras.ScanEvents(\"GlowUpdate\")\n            C_ChatInfo.RegisterAddonMessagePrefix(\"Tirna Ver2\")\n            if IsPartyLFG() then            \n                if debug then \n                    print(\"LFG sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"INSTANCE_CHAT\")) \n                else\n                    C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"INSTANCE_CHAT\")\n                end\n            elseif UnitInParty(\"player\") then            \n                if debug then \n                    print(\"party sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"PARTY\")) \n                else\n                    C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"PARTY\")\n                end\n            else         \n                if debug then \n                    print(\"solo sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"WHISPER\", name)) \n                else\n                    C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"WHISPER\", name)\n                end\n            end\n    end)\nend",
					["do_custom"] = true,
				},
			},
			["authorOptions"] = {
				{
					["type"] = "toggle",
					["key"] = "orderText",
					["default"] = true,
					["name"] = "Show Order Text",
					["useDesc"] = false,
					["width"] = 1,
				}, -- [1]
				{
					["type"] = "toggle",
					["key"] = "encounter",
					["default"] = false,
					["name"] = "Show during Boss",
					["useDesc"] = false,
					["width"] = 1,
				}, -- [2]
				{
					["type"] = "toggle",
					["key"] = "noCommunication",
					["default"] = false,
					["name"] = "Disable Cross-Communication with others running the WA",
					["useDesc"] = false,
					["width"] = 2,
				}, -- [3]
				{
					["type"] = "toggle",
					["key"] = "debug",
					["default"] = false,
					["name"] = "|cffff0000Debug testing|r (requires reload for full debug)",
					["useDesc"] = false,
					["width"] = 2,
				}, -- [4]
			},
			["regionType"] = "icon",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["cooldown"] = false,
			["conditions"] = {
			},
			["parent"] = "Mists of Tirna Scithe guessing game v2",
			["semver"] = "2.0.2",
			["anchorFrameType"] = "SCREEN",
			["zoom"] = 0,
			["auto"] = false,
			["tocversion"] = 90005,
			["id"] = "nocircle_leaf_fill",
			["alpha"] = 1,
			["frameStrata"] = 1,
			["width"] = 80,
			["cooldownTextDisabled"] = false,
			["uid"] = "Wzw0aT)U2sT",
			["inverse"] = false,
			["selfPoint"] = "CENTER",
			["displayIcon"] = "Interface\\AddOns\\guess\\nocircle_leaf_fill.tga",
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["url"] = "https://wago.io/hVLym_eLv/67",
		},
		["05 Gluttonous Miasma - Black"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 3,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.25,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = true,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2383",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "r50IsE)mCZJ",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["useMatch_count"] = true,
						["auranames"] = {
						},
						["groupclone"] = true,
						["matchesShowOn"] = "showOnActive",
						["name_info"] = "aura",
						["unit"] = "group",
						["use_tooltip"] = false,
						["subeventPrefix"] = "SPELL",
						["match_countOperator"] = ">=",
						["match_count"] = "1",
						["buffShowOn"] = "showOnActive",
						["unevent"] = "timed",
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["useExactSpellId"] = true,
						["custom_type"] = "stateupdate",
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["event"] = "Health",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HARMFUL",
						["use_specific_unit"] = false,
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["auraspellids"] = {
							"329298", -- [1]
						},
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["duration"] = "1",
						["useGroup_count"] = false,
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["wordWrap"] = "WordWrap",
			["config"] = {
			},
			["text1"] = "%n",
			["text1Containment"] = "INSIDE",
			["width"] = 6.9999866485596,
			["justify"] = "LEFT",
			["semver"] = "1.0.26",
			["zoom"] = 0,
			["id"] = "05 Gluttonous Miasma - Black",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["frameStrata"] = 9,
			["anchorFrameType"] = "SCREEN",
			["text1Point"] = "CENTER",
			["text2Font"] = "Friz Quadrata TT",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["Hunter Survival 2"] = {
			["sparkWidth"] = 10,
			["iconSource"] = -1,
			["authorOptions"] = {
			},
			["preferToUpdate"] = false,
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["sparkRotation"] = 0,
			["url"] = "https://wago.io/ZG34ifiRf/1",
			["backgroundColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				0.43108940124512, -- [4]
			},
			["fontFlags"] = "OUTLINE",
			["icon_color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["selfPoint"] = "CENTER",
			["barColor"] = {
				0.58823529411765, -- [1]
				0.77647058823529, -- [2]
				0.32549019607843, -- [3]
				1, -- [4]
			},
			["desaturate"] = false,
			["sparkOffsetY"] = 0,
			["load"] = {
				["ingroup"] = {
					["multi"] = {
					},
				},
				["use_never"] = false,
				["class"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["level"] = "100",
				["size"] = {
					["single"] = "party",
					["multi"] = {
						["ratedpvp"] = true,
						["arena"] = true,
						["ratedarena"] = true,
						["pvp"] = true,
					},
				},
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_level"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["difficulty"] = {
					["single"] = "challenge",
					["multi"] = {
						["challenge"] = true,
					},
				},
				["zoneIds"] = "",
				["faction"] = {
					["multi"] = {
					},
				},
				["use_size"] = false,
				["race"] = {
					["multi"] = {
					},
				},
				["use_zone"] = false,
				["pvptalent"] = {
					["multi"] = {
					},
				},
			},
			["useAdjustededMin"] = false,
			["regionType"] = "aurabar",
			["texture"] = "Glamour4",
			["zoom"] = 0.28,
			["auto"] = true,
			["tocversion"] = 80300,
			["alpha"] = 1,
			["sparkColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["displayIcon"] = "Interface\\Icons\\spell_holy_rebuke",
			["stacksPoint"] = "BOTTOMRIGHT",
			["sparkOffsetX"] = 0,
			["parent"] = "Enemy Interrupt Icons",
			["sparkRotationMode"] = "AUTO",
			["triggers"] = {
				{
					["trigger"] = {
						["duration"] = "15",
						["spellId"] = "",
						["subeventPrefix"] = "SPELL",
						["sourceName"] = "",
						["ownOnly"] = true,
						["genericShowOn"] = "showOnActive",
						["names"] = {
							"Greater Draenic Intellect Flask", -- [1]
						},
						["debuffType"] = "HELPFUL",
						["use_specific_sourceunit"] = true,
						["spellIds"] = {
						},
						["use_spellId"] = false,
						["use_sourceName"] = false,
						["use_destName"] = false,
						["use_cloneId"] = true,
						["custom_hide"] = "timed",
						["use_sourceFlags2"] = true,
						["type"] = "combatlog",
						["unevent"] = "timed",
						["subeventSuffix"] = "_CAST_SUCCESS",
						["inverse"] = true,
						["spellName"] = "Muzzle",
						["event"] = "Combat Log",
						["sourceFlags2"] = "Hostile",
						["use_name"] = true,
						["use_spellName"] = true,
						["name"] = "Flask of the warm sun ",
						["use_sourceUnit"] = false,
						["use_destunit"] = false,
						["use_destUnit"] = false,
						["sourceUnit"] = "Benzikura",
						["unit"] = "player",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
			},
			["backdropInFront"] = false,
			["stickyDuration"] = false,
			["version"] = 1,
			["subRegions"] = {
				{
					["type"] = "subborder",
					["border_anchor"] = "bar",
					["border_size"] = 16,
					["border_color"] = {
						0.63137254901961, -- [1]
						0.78823529411765, -- [2]
						0.41960784313725, -- [3]
						0.5, -- [4]
					},
					["border_visible"] = true,
					["border_edge"] = "1 Pixel",
					["border_offset"] = 2,
				}, -- [1]
				{
					["type"] = "aurabar_bar",
				}, -- [2]
				{
					["text_shadowXOffset"] = 1,
					["text_text"] = "%p",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Vixar",
					["text_text_format_p_time_format"] = 0,
					["text_shadowYOffset"] = -1,
					["text_text_format_p_time_dynamic_threshold"] = 3,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "ICON_CENTER",
					["text_text_format_p_format"] = "timed",
					["text_visible"] = true,
					["text_fontSize"] = 20,
					["anchorXOffset"] = 0,
					["text_text_format_p_time_precision"] = 1,
				}, -- [3]
			},
			["height"] = 30,
			["sparkBlendMode"] = "ADD",
			["useAdjustededMax"] = false,
			["displayStacks"] = "%c",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["uid"] = "A)J8OZYmhmz",
			["config"] = {
			},
			["borderInFront"] = false,
			["borderBackdrop"] = "Blizzard Tooltip",
			["icon_side"] = "LEFT",
			["xOffset"] = 0,
			["spark"] = false,
			["sparkHeight"] = 30,
			["icon"] = true,
			["stacksContainment"] = "INSIDE",
			["actions"] = {
				["start"] = {
					["do_message"] = false,
					["message_type"] = "SAY",
				},
				["init"] = {
				},
				["finish"] = {
				},
			},
			["semver"] = "1.0.0",
			["anchorFrameType"] = "SCREEN",
			["sparkHidden"] = "NEVER",
			["sparkTexture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
			["frameStrata"] = 1,
			["width"] = 30,
			["id"] = "Hunter Survival 2",
			["backdropColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0.5, -- [4]
			},
			["inverse"] = false,
			["sparkDesature"] = false,
			["orientation"] = "HORIZONTAL",
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["customTextUpdate"] = "update",
		},
		["Dont Have Debuff"] = {
			["authorOptions"] = {
			},
			["preferToUpdate"] = false,
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/OEDlN6x_O/1",
			["actions"] = {
				["start"] = {
					["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\Left.ogg",
					["do_sound"] = true,
				},
				["init"] = {
				},
				["finish"] = {
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "combatlog",
						["spellId"] = "331634",
						["subeventSuffix"] = "_CAST_SUCCESS",
						["duration"] = "8",
						["event"] = "Combat Log",
						["unit"] = "player",
						["use_spellId"] = true,
						["spellIds"] = {
						},
						["unevent"] = "timed",
						["names"] = {
						},
						["use_spellName"] = false,
						["subeventPrefix"] = "SPELL",
						["debuffType"] = "HELPFUL",
					},
					["untrigger"] = {
					},
				}, -- [1]
				{
					["trigger"] = {
						["debuffType"] = "HARMFUL",
						["useName"] = false,
						["useExactSpellId"] = true,
						["auranames"] = {
							"331637", -- [1]
						},
						["auraspellids"] = {
							"331637", -- [1]
							"331636", -- [2]
						},
						["matchesShowOn"] = "showOnMissing",
						["type"] = "aura2",
						["unit"] = "player",
					},
					["untrigger"] = {
					},
				}, -- [2]
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["rotation"] = 90,
			["version"] = 1,
			["subRegions"] = {
			},
			["height"] = 144,
			["rotate"] = true,
			["load"] = {
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["use_encounterid"] = true,
				["difficulty"] = {
					["single"] = "mythic",
				},
				["zoneIds"] = "",
				["class"] = {
					["multi"] = {
					},
				},
				["use_difficulty"] = true,
				["encounterid"] = "2412",
				["size"] = {
					["multi"] = {
					},
				},
			},
			["textureWrapMode"] = "CLAMPTOBLACKADDITIVE",
			["mirror"] = false,
			["regionType"] = "texture",
			["blendMode"] = "BLEND",
			["frameStrata"] = 1,
			["texture"] = "Interface\\AddOns\\WeakAuras\\Media\\Textures\\triangle-border.tga",
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["xOffset"] = -100,
			["semver"] = "1.0.0",
			["tocversion"] = 90002,
			["id"] = "Dont Have Debuff",
			["discrete_rotation"] = 0,
			["alpha"] = 1,
			["width"] = 187,
			["config"] = {
			},
			["uid"] = "os(kka0DVoS",
			["anchorFrameType"] = "SCREEN",
			["color"] = {
				1, -- [1]
				0, -- [2]
				0.023529411764706, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
			},
			["parent"] = "T27 07 Council of Blood - Dark Recital Arrows",
		},
		["T27 07 Council of Blood - Dark Recital Arrows"] = {
			["controlledChildren"] = {
				"Have Debuff", -- [1]
				"Dont Have Debuff", -- [2]
			},
			["borderBackdrop"] = "Blizzard Tooltip",
			["xOffset"] = 0,
			["preferToUpdate"] = false,
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["borderColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["url"] = "https://wago.io/OEDlN6x_O/1",
			["actions"] = {
				["start"] = {
				},
				["init"] = {
				},
				["finish"] = {
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["debuffType"] = "HELPFUL",
						["type"] = "aura2",
						["spellIds"] = {
						},
						["subeventSuffix"] = "_CAST_START",
						["subeventPrefix"] = "SPELL",
						["names"] = {
						},
						["event"] = "Health",
						["unit"] = "player",
					},
					["untrigger"] = {
					},
				}, -- [1]
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["version"] = 1,
			["subRegions"] = {
			},
			["load"] = {
				["talent"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["spec"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["backdropColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0.5, -- [4]
			},
			["scale"] = 1,
			["border"] = false,
			["borderEdge"] = "Square Full White",
			["regionType"] = "group",
			["borderSize"] = 2,
			["borderOffset"] = 4,
			["semver"] = "1.0.0",
			["tocversion"] = 90002,
			["id"] = "T27 07 Council of Blood - Dark Recital Arrows",
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["selfPoint"] = "CENTER",
			["uid"] = "Ej4li0iXItK",
			["borderInset"] = 1,
			["config"] = {
			},
			["conditions"] = {
			},
			["information"] = {
			},
			["authorOptions"] = {
			},
		},
		["Demon_Hunter 2"] = {
			["sparkWidth"] = 10,
			["iconSource"] = -1,
			["authorOptions"] = {
			},
			["preferToUpdate"] = false,
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["sparkRotation"] = 0,
			["url"] = "https://wago.io/ZG34ifiRf/1",
			["backgroundColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				0.45673036575317, -- [4]
			},
			["fontFlags"] = "OUTLINE",
			["icon_color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["selfPoint"] = "CENTER",
			["barColor"] = {
				0.73333333333333, -- [1]
				0.07843137254902, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["desaturate"] = false,
			["sparkOffsetY"] = 0,
			["load"] = {
				["ingroup"] = {
					["multi"] = {
					},
				},
				["use_never"] = false,
				["class"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["level"] = "100",
				["size"] = {
					["single"] = "party",
					["multi"] = {
						["ratedpvp"] = true,
						["arena"] = true,
						["ratedarena"] = true,
						["pvp"] = true,
					},
				},
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_level"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["difficulty"] = {
					["single"] = "challenge",
					["multi"] = {
						["challenge"] = true,
					},
				},
				["zoneIds"] = "",
				["faction"] = {
					["multi"] = {
					},
				},
				["use_size"] = false,
				["use_zone"] = false,
				["race"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
			},
			["useAdjustededMin"] = false,
			["regionType"] = "aurabar",
			["texture"] = "Glamour4",
			["sparkTexture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
			["auto"] = true,
			["tocversion"] = 80300,
			["alpha"] = 1,
			["sparkColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["displayIcon"] = "Interface\\Icons\\spell_holy_rebuke",
			["stacksPoint"] = "BOTTOMRIGHT",
			["borderBackdrop"] = "Blizzard Tooltip",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customTextUpdate"] = "update",
			["triggers"] = {
				{
					["trigger"] = {
						["duration"] = "15",
						["spellId"] = "",
						["subeventPrefix"] = "SPELL",
						["use_sourceName"] = false,
						["ownOnly"] = true,
						["genericShowOn"] = "showOnActive",
						["names"] = {
							"Greater Draenic Intellect Flask", -- [1]
						},
						["debuffType"] = "HELPFUL",
						["use_sourceUnit"] = false,
						["spellIds"] = {
						},
						["use_spellId"] = false,
						["type"] = "combatlog",
						["use_destName"] = false,
						["use_cloneId"] = true,
						["spellName"] = "Disrupt",
						["use_sourceFlags2"] = true,
						["sourceName"] = "",
						["unevent"] = "timed",
						["subeventSuffix"] = "_CAST_SUCCESS",
						["inverse"] = true,
						["custom_hide"] = "timed",
						["event"] = "Combat Log",
						["sourceFlags2"] = "Hostile",
						["use_name"] = true,
						["use_spellName"] = true,
						["name"] = "Flask of the warm sun ",
						["use_specific_sourceunit"] = true,
						["use_destunit"] = false,
						["use_destUnit"] = false,
						["sourceUnit"] = "Benzikura",
						["unit"] = "player",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
			},
			["backdropInFront"] = false,
			["stickyDuration"] = false,
			["version"] = 1,
			["subRegions"] = {
				{
					["type"] = "subborder",
					["border_anchor"] = "bar",
					["border_size"] = 16,
					["border_color"] = {
						0.71764705882353, -- [1]
						0.058823529411765, -- [2]
						1, -- [3]
						0.5, -- [4]
					},
					["border_visible"] = true,
					["border_edge"] = "1 Pixel",
					["border_offset"] = 2,
				}, -- [1]
				{
					["type"] = "aurabar_bar",
				}, -- [2]
				{
					["text_shadowXOffset"] = 1,
					["text_text"] = "%p",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Vixar",
					["text_text_format_p_time_format"] = 0,
					["text_shadowYOffset"] = -1,
					["text_text_format_p_time_dynamic_threshold"] = 3,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "ICON_CENTER",
					["text_text_format_p_format"] = "timed",
					["text_visible"] = true,
					["text_fontSize"] = 20,
					["anchorXOffset"] = 0,
					["text_text_format_p_time_precision"] = 1,
				}, -- [3]
			},
			["height"] = 30,
			["sparkBlendMode"] = "ADD",
			["useAdjustededMax"] = false,
			["displayStacks"] = "%c",
			["uid"] = "EUcp)0ibBQL",
			["zoom"] = 0.28,
			["actions"] = {
				["start"] = {
					["do_message"] = false,
					["message_type"] = "SAY",
				},
				["init"] = {
				},
				["finish"] = {
				},
			},
			["borderInFront"] = false,
			["sparkOffsetX"] = 0,
			["icon_side"] = "LEFT",
			["xOffset"] = 0,
			["spark"] = false,
			["sparkHeight"] = 30,
			["icon"] = true,
			["stacksContainment"] = "INSIDE",
			["config"] = {
			},
			["semver"] = "1.0.0",
			["width"] = 30,
			["sparkHidden"] = "NEVER",
			["parent"] = "Enemy Interrupt Icons",
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["id"] = "Demon_Hunter 2",
			["backdropColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0.5, -- [4]
			},
			["inverse"] = false,
			["sparkDesature"] = false,
			["orientation"] = "HORIZONTAL",
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["sparkRotationMode"] = "AUTO",
		},
		["Monk 2"] = {
			["sparkWidth"] = 10,
			["iconSource"] = -1,
			["authorOptions"] = {
			},
			["preferToUpdate"] = false,
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["sparkRotation"] = 0,
			["url"] = "https://wago.io/ZG34ifiRf/1",
			["actions"] = {
				["start"] = {
					["do_message"] = false,
					["message_type"] = "SAY",
				},
				["init"] = {
				},
				["finish"] = {
				},
			},
			["fontFlags"] = "OUTLINE",
			["icon_color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["selfPoint"] = "CENTER",
			["barColor"] = {
				0, -- [1]
				1, -- [2]
				0.6078431372549, -- [3]
				1, -- [4]
			},
			["desaturate"] = false,
			["sparkOffsetY"] = 0,
			["load"] = {
				["ingroup"] = {
					["multi"] = {
					},
				},
				["use_never"] = false,
				["class"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["level"] = "100",
				["size"] = {
					["single"] = "party",
					["multi"] = {
						["ratedpvp"] = true,
						["arena"] = true,
						["ratedarena"] = true,
						["pvp"] = true,
					},
				},
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_level"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["difficulty"] = {
					["single"] = "challenge",
					["multi"] = {
						["challenge"] = true,
					},
				},
				["zoneIds"] = "",
				["faction"] = {
					["multi"] = {
					},
				},
				["use_size"] = false,
				["race"] = {
					["multi"] = {
					},
				},
				["use_zone"] = false,
				["pvptalent"] = {
					["multi"] = {
					},
				},
			},
			["useAdjustededMin"] = false,
			["regionType"] = "aurabar",
			["texture"] = "Glamour4",
			["sparkTexture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
			["spark"] = false,
			["tocversion"] = 80300,
			["alpha"] = 1,
			["sparkColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["displayIcon"] = "Interface\\Icons\\spell_holy_rebuke",
			["stacksPoint"] = "BOTTOMRIGHT",
			["borderBackdrop"] = "Blizzard Tooltip",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customTextUpdate"] = "update",
			["triggers"] = {
				{
					["trigger"] = {
						["ownOnly"] = true,
						["spellId"] = "",
						["names"] = {
							"Greater Draenic Intellect Flask", -- [1]
						},
						["inverse"] = true,
						["duration"] = "15",
						["genericShowOn"] = "showOnActive",
						["unit"] = "player",
						["debuffType"] = "HELPFUL",
						["use_specific_sourceunit"] = true,
						["name"] = "Flask of the warm sun ",
						["use_spellId"] = false,
						["spellName"] = "Spear Hand Strike",
						["use_destName"] = false,
						["use_cloneId"] = true,
						["custom_hide"] = "timed",
						["use_sourceFlags2"] = true,
						["type"] = "combatlog",
						["unevent"] = "timed",
						["subeventSuffix"] = "_CAST_SUCCESS",
						["sourceName"] = "Draemii",
						["use_sourceName"] = false,
						["event"] = "Combat Log",
						["sourceFlags2"] = "Hostile",
						["use_name"] = true,
						["use_spellName"] = true,
						["spellIds"] = {
						},
						["use_sourceUnit"] = false,
						["use_destunit"] = false,
						["use_destUnit"] = false,
						["sourceUnit"] = "Benzikura",
						["subeventPrefix"] = "SPELL",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
			},
			["backdropInFront"] = false,
			["stickyDuration"] = false,
			["version"] = 1,
			["subRegions"] = {
				{
					["type"] = "subborder",
					["border_anchor"] = "bar",
					["border_size"] = 16,
					["border_color"] = {
						0, -- [1]
						1, -- [2]
						0.64705882352941, -- [3]
						0.5, -- [4]
					},
					["border_visible"] = true,
					["border_edge"] = "1 Pixel",
					["border_offset"] = 2,
				}, -- [1]
				{
					["type"] = "aurabar_bar",
				}, -- [2]
				{
					["text_shadowXOffset"] = 1,
					["text_text"] = "%p",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Vixar",
					["text_text_format_p_time_format"] = 0,
					["text_shadowYOffset"] = -1,
					["text_text_format_p_time_dynamic_threshold"] = 3,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "ICON_CENTER",
					["text_text_format_p_format"] = "timed",
					["text_visible"] = true,
					["text_fontSize"] = 20,
					["anchorXOffset"] = 0,
					["text_text_format_p_time_precision"] = 1,
				}, -- [3]
			},
			["height"] = 30,
			["sparkBlendMode"] = "ADD",
			["useAdjustededMax"] = false,
			["displayStacks"] = "%c",
			["auto"] = true,
			["backgroundColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				0.48237186670303, -- [4]
			},
			["uid"] = "QzKgstcOk)g",
			["borderInFront"] = false,
			["sparkOffsetX"] = 0,
			["icon_side"] = "LEFT",
			["xOffset"] = 0,
			["zoom"] = 0.28,
			["sparkHeight"] = 30,
			["icon"] = true,
			["stacksContainment"] = "INSIDE",
			["parent"] = "Enemy Interrupt Icons",
			["semver"] = "1.0.0",
			["width"] = 30,
			["sparkHidden"] = "NEVER",
			["config"] = {
			},
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["id"] = "Monk 2",
			["backdropColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0.5, -- [4]
			},
			["inverse"] = false,
			["sparkDesature"] = false,
			["orientation"] = "HORIZONTAL",
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["sparkRotationMode"] = "AUTO",
		},
		["07 Seismic Upheaval Stack Alert"] = {
			["outline"] = "OUTLINE",
			["xOffset"] = 0,
			["displayText"] = "STACK FOR SEISMIC",
			["yOffset"] = 285,
			["anchorPoint"] = "CENTER",
			["displayText_format_p_time_format"] = 0,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["actions"] = {
				["start"] = {
					["do_custom"] = false,
					["custom"] = "",
					["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\AirHorn.ogg",
					["do_sound"] = false,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["remaining_operator"] = "<=",
						["message_operator"] = "find('%s')",
						["unit"] = "player",
						["remaining"] = "5",
						["debuffType"] = "HELPFUL",
						["type"] = "addons",
						["unevent"] = "auto",
						["names"] = {
						},
						["event"] = "DBM Timer",
						["use_unit"] = true,
						["message"] = "Seismic Upheaval",
						["subeventPrefix"] = "SPELL",
						["spellIds"] = {
						},
						["use_remaining"] = true,
						["use_message"] = true,
						["use_absorbMode"] = true,
						["subeventSuffix"] = "_CAST_START",
						["duration"] = "1",
					},
					["untrigger"] = {
					},
				}, -- [1]
				{
					["trigger"] = {
						["use_absorbMode"] = true,
						["message_operator"] = "find('%s')",
						["unit"] = "player",
						["remaining"] = "5",
						["text_operator"] = "find('%s')",
						["debuffType"] = "HELPFUL",
						["use_remaining"] = true,
						["names"] = {
						},
						["unevent"] = "auto",
						["subeventPrefix"] = "SPELL",
						["text"] = "Seismic Upheaval",
						["use_text"] = true,
						["duration"] = "1",
						["message"] = "Evasive Lunge",
						["subeventSuffix"] = "_CAST_START",
						["spellIds"] = {
						},
						["remaining_operator"] = "<=",
						["use_message"] = true,
						["type"] = "addons",
						["event"] = "BigWigs Timer",
						["use_unit"] = true,
					},
					["untrigger"] = {
					},
				}, -- [2]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = -10,
			},
			["displayText_format_p_format"] = "timed",
			["internalVersion"] = 45,
			["selfPoint"] = "BOTTOM",
			["font"] = "Friz Quadrata TT",
			["version"] = 2,
			["subRegions"] = {
			},
			["load"] = {
				["size"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2417",
				["zoneIds"] = "",
				["use_encounterid"] = true,
			},
			["fontSize"] = 60,
			["shadowXOffset"] = 1,
			["parent"] = "09 Seismic Upheaval Group",
			["regionType"] = "text",
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["preferToUpdate"] = false,
			["fixedWidth"] = 200,
			["displayText_format_p_time_precision"] = 1,
			["displayText_format_p_time_dynamic_threshold"] = 60,
			["authorOptions"] = {
			},
			["uid"] = "4UZTv(86AnT",
			["semver"] = "1.0.1",
			["tocversion"] = 90005,
			["id"] = "07 Seismic Upheaval Stack Alert",
			["shadowYOffset"] = -1,
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["justify"] = "LEFT",
			["config"] = {
			},
			["color"] = {
				0, -- [1]
				0.87843137254902, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["wordWrap"] = "WordWrap",
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
			},
			["url"] = "https://wago.io/HcLsyDhV7/2",
		},
		["Chain Link Personal Assignment"] = {
			["iconSource"] = -1,
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["preferToUpdate"] = false,
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["cooldownSwipe"] = true,
			["cooldownEdge"] = false,
			["icon"] = true,
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "custom",
						["use_debuffClass"] = false,
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HELPFUL",
						["event"] = "Health",
						["unit"] = "player",
						["custom_type"] = "stateupdate",
						["spellIds"] = {
						},
						["events"] = "CLEU:SPELL_AURA_APPLIED, CHAT_MSG_ADDON, UNIT_SPELLCAST_SUCCEEDED:boss1, ENCOUNTER_START",
						["names"] = {
						},
						["check"] = "event",
						["custom"] = "function(allstates, event, ...)\n    if event == \"COMBAT_LOG_EVENT_UNFILTERED\" then\n        local timestamp, subEvent, hideCaster, sourceGUID, sourceName, sourceFlags, sourceRaidFlags, destGUID, destName, destFlags, destRaidFlags, spellID, _,_, auraType = ...\n        if subEvent == \"SPELL_AURA_APPLIED\"\n        and spellID == 335293 then\n            local config = aura_env.config\n            local maxAmount = config.amountTeams == 2\n            if (config.ignoreTanks\n                and UnitGroupRolesAssigned(destName) ~= \"TANK\")\n            or not config.ignoreTanks then\n                if (config.rangesOnly \n                    and aura_env.subRole[destName] == \"Range\"\n                    and aura_env.subRole[sourceName] == \"Range\"\n                or not config.rangesOnly) then\n                    aura_env.counter = aura_env.counter + 1\n                    if not maxAmount and aura_env.counter <= 4 \n                    or maxAmount and aura_env.counter <= 8 then\n                        if (not aura_env.myCounter and (destName == WeakAuras.me\n                        or sourceName == WeakAuras.me)) then\n                            aura_env.myCounter = maxAmount and math.floor((aura_env.counter+1)/2) or aura_env.counter\n                            allstates[aura_env.myCounter] = {\n                                show = true,\n                                changed = true,\n                                mark = ICON_LIST[aura_env.myCounter]..\"16|t\",\n                                partner = destName ~= WeakAuras.me and destName or sourceName ~= WeakAuras.me and sourceName,\n                                assignment = destName ~= WeakAuras.me and \"Front\" or \"Back\",\n                                num = maxAmount and (aura_env.counter % 2 == 0 and 2 or 1) or \"\",\n                            }\n                            return true\n                        end\n                    end\n                end\n            end\n        end\n        --[[if subEvent == \"SPELL_AURA_REMOVED\"\n        and spellID == 335293 then\n            local state = allstates[aura_env.myCounter]\n            if state then\n                if (state.partner == sourceName or state.partner == destName) then\n                    state.show = false\n                    state.changed = true\n                    aura_env.myCounter = nil\n                    return true\n                end\n            end\n        end ]]\n    elseif event == \"CHAT_MSG_ADDON\" then\n        local prefix, text, channel, sender = ...\n        if sender then\n            local unit = Ambiguate(sender, 'short')\n            if prefix == \"CAUSESE_SPECINFO\" then\n                aura_env.subRole[unit] = text\n            end    \n        end\n    elseif event == \"ENCOUNTER_START\" then\n        aura_env.counter = 0\n        aura_env.myCounter = nil\n    elseif event == \"UNIT_SPELLCAST_SUCCEEDED\" then\n        local _, _, spellID = ...\n        if spellID == 335300 then\n            aura_env.counter = 0\n            aura_env.myCounter = nil\n            for k,v in pairs(allstates) do\n                v.show = false\n                v.changed = true\n                return true\n            end\n        end\n    end\nend",
						["subeventPrefix"] = "SPELL",
						["custom_hide"] = "timed",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["internalVersion"] = 45,
			["keepAspectRatio"] = false,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["version"] = 5,
			["subRegions"] = {
				{
					["text_shadowXOffset"] = 0,
					["text_text_format_s_format"] = "none",
					["text_text"] = "%mark",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["text_text_format_mark_format"] = "none",
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Expressway",
					["text_shadowYOffset"] = 0,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "CENTER",
					["text_fontSize"] = 54,
					["anchorXOffset"] = 0,
					["text_visible"] = true,
				}, -- [1]
				{
					["text_text_format_partner_abbreviate_max"] = 8,
					["text_text"] = "%partner",
					["text_text_format_p_format"] = "timed",
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["text_text_format_p_time_format"] = 0,
					["text_text_format_partner_abbreviate"] = false,
					["text_justify"] = "CENTER",
					["text_text_format_partner_format"] = "Unit",
					["text_text_format_p_time_dynamic_threshold"] = 60,
					["anchorYOffset"] = 0,
					["text_shadowXOffset"] = 0,
					["type"] = "subtext",
					["text_text_format_p_time_precision"] = 1,
					["text_text_format_partner_color"] = "class",
					["text_font"] = "Expressway",
					["text_text_format_partner_realm_name"] = "never",
					["text_shadowYOffset"] = 0,
					["text_visible"] = true,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "OUTER_BOTTOM",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_fontSize"] = 12,
					["anchorXOffset"] = 0,
					["rotateText"] = "NONE",
				}, -- [2]
				{
					["text_text_format_p_time_precision"] = 1,
					["text_text"] = "%assignment%num",
					["text_text_format_partner_abbreviate_max"] = 8,
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_text_format_p_time_dynamic_threshold"] = 60,
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["text_text_format_p_time_format"] = 0,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["text_text_format_partner_format"] = "Unit",
					["text_text_format_p_format"] = "timed",
					["text_text_format_assignment_format"] = "none",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["type"] = "subtext",
					["text_fontType"] = "OUTLINE",
					["text_text_format_partner_color"] = "class",
					["text_font"] = "Expressway",
					["text_text_format_partner_realm_name"] = "never",
					["text_shadowYOffset"] = 0,
					["text_shadowXOffset"] = 0,
					["text_wordWrap"] = "WordWrap",
					["text_visible"] = true,
					["text_anchorPoint"] = "CENTER",
					["rotateText"] = "NONE",
					["text_text_format_partner_abbreviate"] = false,
					["text_fontSize"] = 12,
					["anchorXOffset"] = 0,
					["text_text_format_num_format"] = "none",
				}, -- [3]
			},
			["height"] = 64,
			["load"] = {
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["use_encounterid"] = true,
				["difficulty"] = {
					["single"] = "mythic",
					["multi"] = {
						["mythic"] = true,
					},
				},
				["role"] = {
				},
				["zoneIds"] = "",
				["class"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2399",
				["size"] = {
					["multi"] = {
					},
				},
			},
			["cooldownTextDisabled"] = true,
			["actions"] = {
				["start"] = {
				},
				["finish"] = {
				},
				["init"] = {
					["custom"] = "aura_env.counter = 0\naura_env.subRole = {}\naura_env.myCounter = nil\n\n--convert specID to subRole\naura_env.convert = {\n    [250] = \"Melee\",\n    [251] = \"Melee\",\n    [252] = \"Melee\",\n    [577] = \"Melee\",\n    [581] = \"Melee\",\n    [102] = \"Range\",\n    [103] = \"Melee\",\n    [104] = \"Melee\",\n    [105] = \"Range\",\n    [253] = \"Range\",\n    [254] = \"Range\",\n    [255] = \"Melee\",\n    [62] = \"Range\",\n    [63] = \"Range\",\n    [64] = \"Range\",\n    [268] = \"Melee\",\n    [270] = aura_env.config.forceRange and \"Range\" or \"Melee\",\n    [269] = \"Melee\",\n    [65] = aura_env.config.forceRange and \"Range\" or \"Melee\",\n    [66] = \"Melee\",\n    [70] = \"Melee\",\n    [256] = \"Range\",\n    [257] = \"Range\",\n    [258] = \"Range\",\n    [259] = \"Melee\",\n    [260] = \"Melee\",\n    [261] = \"Melee\",\n    [262] = \"Range\",\n    [263] = \"Melee\",\n    [264] = \"Range\",\n    [265] = \"Range\",\n    [266] = \"Range\",\n    [267] = \"Range\",\n    [71] = \"Melee\",\n    [72] = \"Melee\",\n    [73] = \"Melee\",\n}\n\nlocal specID = GetSpecializationInfo(GetSpecialization())\nC_ChatInfo.RegisterAddonMessagePrefix(\"CAUSESE_SPECINFO\")\nif specID then\n    local aura_env = aura_env\n    C_Timer.After(1, function() C_ChatInfo.SendAddonMessage(\"CAUSESE_SPECINFO\", aura_env.convert[specID], \"RAID\") end)\nend",
					["do_custom"] = true,
				},
			},
			["regionType"] = "icon",
			["authorOptions"] = {
				{
					["type"] = "range",
					["useDesc"] = false,
					["max"] = 2,
					["step"] = 1,
					["width"] = 2,
					["min"] = 1,
					["key"] = "amountTeams",
					["default"] = 2,
					["name"] = "amount of teams per mark",
				}, -- [1]
				{
					["type"] = "toggle",
					["key"] = "rangesOnly",
					["default"] = true,
					["name"] = "Assign only if both chained players are ranges",
					["useDesc"] = false,
					["width"] = 2,
				}, -- [2]
				{
					["type"] = "space",
					["variableWidth"] = true,
					["height"] = 1,
					["useHeight"] = false,
					["width"] = 0.15,
				}, -- [3]
				{
					["type"] = "toggle",
					["key"] = "forceRange",
					["default"] = true,
					["name"] = "and classify Holy Pal / MW Monk as Range",
					["useDesc"] = false,
					["width"] = 1.85,
				}, -- [4]
				{
					["type"] = "toggle",
					["key"] = "ignoreTanks",
					["default"] = true,
					["name"] = "Ignore Tanks",
					["useDesc"] = false,
					["width"] = 1,
				}, -- [5]
			},
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["cooldown"] = true,
			["conditions"] = {
			},
			["config"] = {
				["amountTeams"] = 2,
				["ignoreTanks"] = true,
				["forceRange"] = true,
				["rangesOnly"] = true,
			},
			["frameStrata"] = 1,
			["zoom"] = 0,
			["semver"] = "1.0.4",
			["tocversion"] = 90002,
			["id"] = "Chain Link Personal Assignment",
			["anchorFrameType"] = "SCREEN",
			["alpha"] = 1,
			["width"] = 64,
			["auto"] = true,
			["uid"] = "wySqRz9mqD6",
			["inverse"] = false,
			["url"] = "https://wago.io/gjR8EBMKE/5",
			["displayIcon"] = "",
			["information"] = {
			},
			["xOffset"] = 0,
		},
		["Druid_Skull_Bash 2"] = {
			["sparkWidth"] = 10,
			["iconSource"] = -1,
			["authorOptions"] = {
			},
			["preferToUpdate"] = false,
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["sparkRotation"] = 0,
			["url"] = "https://wago.io/ZG34ifiRf/1",
			["backgroundColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				0.4909183382988, -- [4]
			},
			["fontFlags"] = "OUTLINE",
			["icon_color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["selfPoint"] = "CENTER",
			["barColor"] = {
				1, -- [1]
				0.49803921568627, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["desaturate"] = false,
			["sparkOffsetY"] = 0,
			["load"] = {
				["ingroup"] = {
					["multi"] = {
					},
				},
				["use_never"] = false,
				["class"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["level"] = "100",
				["size"] = {
					["single"] = "party",
					["multi"] = {
						["ratedpvp"] = true,
						["arena"] = true,
						["ratedarena"] = true,
						["pvp"] = true,
					},
				},
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_level"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["difficulty"] = {
					["single"] = "challenge",
					["multi"] = {
						["challenge"] = true,
					},
				},
				["zoneIds"] = "",
				["faction"] = {
					["multi"] = {
					},
				},
				["use_size"] = false,
				["race"] = {
					["multi"] = {
					},
				},
				["use_zone"] = false,
				["pvptalent"] = {
					["multi"] = {
					},
				},
			},
			["useAdjustededMin"] = false,
			["regionType"] = "aurabar",
			["texture"] = "Glamour4",
			["sparkTexture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
			["auto"] = true,
			["tocversion"] = 80300,
			["alpha"] = 1,
			["sparkColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["displayIcon"] = "Interface\\Icons\\spell_holy_rebuke",
			["stacksPoint"] = "BOTTOMRIGHT",
			["borderBackdrop"] = "Blizzard Tooltip",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customTextUpdate"] = "update",
			["triggers"] = {
				{
					["trigger"] = {
						["ownOnly"] = true,
						["spellId"] = "",
						["subeventPrefix"] = "SPELL",
						["spellIds"] = {
						},
						["duration"] = "15",
						["genericShowOn"] = "showOnActive",
						["names"] = {
							"Greater Draenic Intellect Flask", -- [1]
						},
						["custom_hide"] = "timed",
						["use_sourceUnit"] = false,
						["name"] = "Flask of the warm sun ",
						["use_spellName"] = true,
						["spellName"] = "Skull Bash",
						["use_destName"] = false,
						["use_cloneId"] = true,
						["debuffType"] = "HELPFUL",
						["use_sourceFlags2"] = true,
						["type"] = "combatlog",
						["unevent"] = "timed",
						["subeventSuffix"] = "_CAST_SUCCESS",
						["sourceName"] = "",
						["use_sourceName"] = false,
						["event"] = "Combat Log",
						["sourceFlags2"] = "Hostile",
						["use_name"] = true,
						["use_spellId"] = false,
						["inverse"] = true,
						["use_specific_sourceunit"] = true,
						["use_destunit"] = false,
						["use_destUnit"] = false,
						["sourceUnit"] = "Benzikura",
						["unit"] = "player",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
			},
			["backdropInFront"] = false,
			["stickyDuration"] = false,
			["version"] = 1,
			["subRegions"] = {
				{
					["type"] = "subborder",
					["border_anchor"] = "bar",
					["border_size"] = 16,
					["border_color"] = {
						1, -- [1]
						0.5843137254902, -- [2]
						0, -- [3]
						0.5, -- [4]
					},
					["border_visible"] = true,
					["border_edge"] = "1 Pixel",
					["border_offset"] = 2,
				}, -- [1]
				{
					["type"] = "aurabar_bar",
				}, -- [2]
				{
					["text_shadowXOffset"] = 1,
					["text_text"] = "%p",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Vixar",
					["text_text_format_p_time_format"] = 0,
					["text_shadowYOffset"] = -1,
					["text_text_format_p_time_dynamic_threshold"] = 3,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "ICON_CENTER",
					["text_text_format_p_format"] = "timed",
					["text_visible"] = true,
					["text_fontSize"] = 20,
					["anchorXOffset"] = 0,
					["text_text_format_p_time_precision"] = 1,
				}, -- [3]
			},
			["height"] = 30,
			["sparkBlendMode"] = "ADD",
			["useAdjustededMax"] = false,
			["displayStacks"] = "%c",
			["uid"] = "VaBFXuKRRgQ",
			["zoom"] = 0.28,
			["actions"] = {
				["start"] = {
					["do_message"] = false,
					["message_type"] = "SAY",
				},
				["init"] = {
				},
				["finish"] = {
				},
			},
			["borderInFront"] = false,
			["sparkOffsetX"] = 0,
			["icon_side"] = "LEFT",
			["xOffset"] = 0,
			["spark"] = false,
			["sparkHeight"] = 30,
			["icon"] = true,
			["stacksContainment"] = "INSIDE",
			["config"] = {
			},
			["semver"] = "1.0.0",
			["width"] = 30,
			["sparkHidden"] = "NEVER",
			["parent"] = "Enemy Interrupt Icons",
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["id"] = "Druid_Skull_Bash 2",
			["backdropColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0.5, -- [4]
			},
			["inverse"] = false,
			["sparkDesature"] = false,
			["orientation"] = "HORIZONTAL",
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["sparkRotationMode"] = "AUTO",
		},
		["Death_Knight 2"] = {
			["sparkWidth"] = 10,
			["iconSource"] = -1,
			["xOffset"] = 0,
			["preferToUpdate"] = false,
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["sparkRotation"] = 0,
			["url"] = "https://wago.io/ZG34ifiRf/1",
			["backgroundColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				0.45673036575317, -- [4]
			},
			["fontFlags"] = "OUTLINE",
			["icon_color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["selfPoint"] = "CENTER",
			["barColor"] = {
				0.77647058823529, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["desaturate"] = false,
			["sparkOffsetY"] = 0,
			["load"] = {
				["ingroup"] = {
					["multi"] = {
					},
				},
				["use_never"] = false,
				["class"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["level"] = "100",
				["size"] = {
					["single"] = "party",
					["multi"] = {
						["ratedpvp"] = true,
						["arena"] = true,
						["ratedarena"] = true,
						["pvp"] = true,
					},
				},
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_level"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["difficulty"] = {
					["single"] = "challenge",
					["multi"] = {
						["challenge"] = true,
					},
				},
				["zoneIds"] = "",
				["faction"] = {
					["multi"] = {
					},
				},
				["use_size"] = false,
				["race"] = {
					["multi"] = {
					},
				},
				["use_zone"] = false,
				["pvptalent"] = {
					["multi"] = {
					},
				},
			},
			["useAdjustededMin"] = false,
			["regionType"] = "aurabar",
			["texture"] = "Glamour4",
			["zoom"] = 0.28,
			["auto"] = true,
			["tocversion"] = 80300,
			["alpha"] = 1,
			["uid"] = "kISKYXz7sfZ",
			["displayIcon"] = "Interface\\Icons\\spell_holy_rebuke",
			["stacksPoint"] = "BOTTOMRIGHT",
			["borderBackdrop"] = "Blizzard Tooltip",
			["parent"] = "Enemy Interrupt Icons",
			["customTextUpdate"] = "update",
			["triggers"] = {
				{
					["trigger"] = {
						["duration"] = "15",
						["spellId"] = "",
						["names"] = {
							"Greater Draenic Intellect Flask", -- [1]
						},
						["spellIds"] = {
						},
						["ownOnly"] = true,
						["genericShowOn"] = "showOnActive",
						["unit"] = "player",
						["debuffType"] = "HELPFUL",
						["use_sourceUnit"] = false,
						["name"] = "Flask of the warm sun ",
						["use_spellName"] = true,
						["type"] = "combatlog",
						["use_destName"] = false,
						["use_cloneId"] = true,
						["spellName"] = "Mind Freeze",
						["use_sourceFlags2"] = true,
						["sourceName"] = "",
						["unevent"] = "timed",
						["subeventSuffix"] = "_CAST_SUCCESS",
						["use_sourceName"] = false,
						["custom_hide"] = "timed",
						["event"] = "Combat Log",
						["sourceFlags2"] = "Hostile",
						["use_name"] = true,
						["use_spellId"] = false,
						["inverse"] = true,
						["use_specific_sourceunit"] = true,
						["use_destunit"] = false,
						["use_destUnit"] = false,
						["sourceUnit"] = "Benzikura",
						["subeventPrefix"] = "SPELL",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
			},
			["backdropInFront"] = false,
			["stickyDuration"] = false,
			["version"] = 1,
			["subRegions"] = {
				{
					["type"] = "subborder",
					["border_anchor"] = "bar",
					["border_size"] = 16,
					["border_color"] = {
						0.78823529411765, -- [1]
						0, -- [2]
						0, -- [3]
						0.52409601211548, -- [4]
					},
					["border_visible"] = true,
					["border_edge"] = "1 Pixel",
					["border_offset"] = 2,
				}, -- [1]
				{
					["type"] = "aurabar_bar",
				}, -- [2]
				{
					["text_shadowXOffset"] = 1,
					["text_text"] = "%p",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Vixar",
					["text_text_format_p_time_format"] = 0,
					["text_shadowYOffset"] = -1,
					["text_text_format_p_time_dynamic_threshold"] = 3,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "ICON_CENTER",
					["text_text_format_p_format"] = "timed",
					["text_visible"] = true,
					["text_fontSize"] = 20,
					["anchorXOffset"] = 0,
					["text_text_format_p_time_precision"] = 1,
				}, -- [3]
			},
			["height"] = 30,
			["sparkBlendMode"] = "ADD",
			["useAdjustededMax"] = false,
			["displayStacks"] = "%c",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["sparkColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["sparkOffsetX"] = 0,
			["borderInFront"] = false,
			["spark"] = false,
			["icon_side"] = "LEFT",
			["authorOptions"] = {
			},
			["actions"] = {
				["start"] = {
					["do_message"] = false,
					["message_type"] = "SAY",
				},
				["init"] = {
				},
				["finish"] = {
				},
			},
			["sparkHeight"] = 30,
			["backdropColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0.5, -- [4]
			},
			["stacksContainment"] = "INSIDE",
			["config"] = {
			},
			["semver"] = "1.0.0",
			["width"] = 30,
			["sparkHidden"] = "NEVER",
			["id"] = "Death_Knight 2",
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["sparkTexture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
			["icon"] = true,
			["inverse"] = false,
			["sparkDesature"] = false,
			["orientation"] = "HORIZONTAL",
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["sparkRotationMode"] = "AUTO",
		},
		["04 Fixate"] = {
			["iconSource"] = 0,
			["authorOptions"] = {
			},
			["preferToUpdate"] = false,
			["yOffset"] = 336.666748046875,
			["anchorPoint"] = "CENTER",
			["cooldownSwipe"] = true,
			["url"] = "https://wago.io/slraid1/132",
			["icon"] = true,
			["triggers"] = {
				{
					["trigger"] = {
						["spellId"] = "327902",
						["auranames"] = {
							"327902", -- [1]
						},
						["use_absorbMode"] = true,
						["unit"] = "player",
						["destUnit"] = "player",
						["debuffType"] = "HARMFUL",
						["useName"] = true,
						["unevent"] = "timed",
						["fetchTooltip"] = false,
						["event"] = "Combat Log",
						["use_unit"] = true,
						["subeventSuffix"] = "_AURA_APPLIED",
						["use_spellId"] = true,
						["spellIds"] = {
						},
						["type"] = "aura2",
						["subeventPrefix"] = "SPELL",
						["use_destUnit"] = true,
						["duration"] = "12",
						["names"] = {
						},
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["internalVersion"] = 45,
			["keepAspectRatio"] = false,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["colorR"] = 0.5843137254902,
					["scalex"] = 1,
					["colorA"] = 1,
					["colorG"] = 0.5843137254902,
					["type"] = "none",
					["easeType"] = "none",
					["scaley"] = 1,
					["alpha"] = 0,
					["use_color"] = false,
					["y"] = 0,
					["x"] = 0,
					["colorType"] = "custom",
					["rotate"] = 0,
					["colorFunc"] = "",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["colorB"] = 0.5843137254902,
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["desaturate"] = false,
			["version"] = 132,
			["subRegions"] = {
				{
					["text_shadowXOffset"] = 0,
					["text_text_format_s_format"] = "none",
					["text_text"] = "Fixate",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_text_format_tooltip1_format"] = "none",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Expressway",
					["text_shadowYOffset"] = 0,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "OUTER_BOTTOM",
					["text_fontSize"] = 16,
					["anchorXOffset"] = 0,
					["text_visible"] = true,
				}, -- [1]
				{
					["text_text_format_p_time_precision"] = 1,
					["text_text"] = "%p",
					["text_text_format_p_format"] = "timed",
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Expressway",
					["text_text_format_p_time_format"] = 0,
					["text_shadowYOffset"] = 0,
					["text_text_format_p_time_dynamic_threshold"] = 0,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "THICKOUTLINE",
					["text_anchorPoint"] = "CENTER",
					["text_visible"] = true,
					["text_shadowXOffset"] = 0,
					["text_fontSize"] = 25,
					["anchorXOffset"] = 0,
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
				}, -- [2]
				{
					["text_shadowXOffset"] = 0,
					["text_text_format_s_format"] = "none",
					["text_text"] = "%s",
					["text_text_format_p_format"] = "timed",
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["text_text_format_p_time_format"] = 0,
					["type"] = "subtext",
					["text_anchorXOffset"] = 6,
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Arial Narrow",
					["text_text_format_p_time_dynamic_threshold"] = 60,
					["text_anchorYOffset"] = -6,
					["text_visible"] = false,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "INNER_BOTTOMRIGHT",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_text_format_p_time_precision"] = 1,
					["text_fontSize"] = 25,
					["anchorXOffset"] = 0,
					["text_shadowYOffset"] = 0,
				}, -- [3]
			},
			["height"] = 80,
			["load"] = {
				["use_encounterid"] = true,
				["zoneIds"] = "",
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2405",
				["size"] = {
					["multi"] = {
					},
				},
			},
			["cooldownTextDisabled"] = true,
			["xOffset"] = -8.3336181640625,
			["regionType"] = "icon",
			["cooldownEdge"] = false,
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["cooldown"] = true,
			["conditions"] = {
			},
			["config"] = {
			},
			["frameStrata"] = 1,
			["zoom"] = 0.3,
			["auto"] = false,
			["tocversion"] = 90002,
			["id"] = "04 Fixate",
			["anchorFrameType"] = "SCREEN",
			["alpha"] = 1,
			["width"] = 80,
			["semver"] = "1.0.131",
			["uid"] = "hA90DEIkTIC",
			["inverse"] = false,
			["selfPoint"] = "CENTER",
			["displayIcon"] = "841380",
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["actions"] = {
				["start"] = {
					["sound"] = "Interface\\Addons\\SharedMedia_Causese\\sound\\Fixate.ogg",
					["do_sound"] = true,
				},
				["finish"] = {
				},
				["init"] = {
				},
			},
		},
		["01 Horrified - Red"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 4,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						1, -- [1]
						0.015686274509804, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.25,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = true,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2398",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "jcIEdfv1A8B",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["useMatch_count"] = true,
						["auranames"] = {
						},
						["groupclone"] = true,
						["matchesShowOn"] = "showOnActive",
						["name_info"] = "aura",
						["unit"] = "group",
						["use_tooltip"] = false,
						["subeventPrefix"] = "SPELL",
						["match_countOperator"] = ">=",
						["match_count"] = "1",
						["buffShowOn"] = "showOnActive",
						["unevent"] = "timed",
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["useExactSpellId"] = true,
						["custom_type"] = "stateupdate",
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["event"] = "Health",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HARMFUL",
						["use_specific_unit"] = false,
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["auraspellids"] = {
							"343024", -- [1]
						},
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["duration"] = "1",
						["useGroup_count"] = false,
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["wordWrap"] = "WordWrap",
			["config"] = {
			},
			["text1"] = "%n",
			["text1Containment"] = "INSIDE",
			["width"] = 6.9999866485596,
			["justify"] = "LEFT",
			["semver"] = "1.0.26",
			["zoom"] = 0,
			["id"] = "01 Horrified - Red",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["frameStrata"] = 9,
			["anchorFrameType"] = "SCREEN",
			["text1Point"] = "CENTER",
			["text2Font"] = "Friz Quadrata TT",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["Volatile Miasma (duration)"] = {
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["displayText"] = "Duration: %p",
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["displayText_format_p_time_format"] = 0,
			["customTextUpdate"] = "event",
			["url"] = "https://wago.io/PescorusMiasma/9",
			["actions"] = {
				["start"] = {
				},
				["init"] = {
				},
				["finish"] = {
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["useMatch_count"] = true,
						["useUnitName"] = false,
						["match_countOperator"] = ">",
						["type"] = "aura2",
						["names"] = {
						},
						["event"] = "Health",
						["unit"] = "group",
						["match_count"] = "0",
						["subeventSuffix"] = "_CAST_START",
						["spellIds"] = {
						},
						["useExactSpellId"] = true,
						["combineMode"] = "showHighest",
						["subeventPrefix"] = "SPELL",
						["auraspellids"] = {
							"329298", -- [1]
						},
						["debuffType"] = "HARMFUL",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["displayText_format_p_format"] = "timed",
			["internalVersion"] = 45,
			["selfPoint"] = "BOTTOM",
			["font"] = "PT Sans Narrow",
			["version"] = 9,
			["subRegions"] = {
			},
			["load"] = {
				["size"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2383",
				["class"] = {
					["multi"] = {
					},
				},
				["use_encounterid"] = true,
			},
			["fontSize"] = 20,
			["shadowXOffset"] = 1,
			["parent"] = "Miasma soak group",
			["regionType"] = "text",
			["displayText_format_p_time_dynamic_threshold"] = 3,
			["wordWrap"] = "WordWrap",
			["fixedWidth"] = 200,
			["displayText_format_p_time_precision"] = 1,
			["preferToUpdate"] = false,
			["automaticWidth"] = "Auto",
			["uid"] = "rGe9)3q9C0(",
			["justify"] = "LEFT",
			["tocversion"] = 90002,
			["id"] = "Volatile Miasma (duration)",
			["semver"] = "1.0.8",
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["authorOptions"] = {
			},
			["config"] = {
			},
			["xOffset"] = 0,
			["shadowYOffset"] = -1,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
			},
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
		},
		["Combat Res - text"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["xOffset"] = 0,
			["displayText"] = "CR: %s / %p",
			["yOffset"] = 0,
			["anchorPoint"] = "BOTTOM",
			["displayText_format_p_time_format"] = 0,
			["url"] = "https://wago.io/ggt8EanjY/1",
			["icon"] = true,
			["text2Font"] = "Friz Quadrata TT",
			["keepAspectRatio"] = false,
			["selfPoint"] = "TOP",
			["text1Containment"] = "INSIDE",
			["glowColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["use_size"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["spec"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["scenario"] = true,
						["ten"] = true,
						["twentyfive"] = true,
						["fortyman"] = true,
						["ratedpvp"] = true,
						["party"] = true,
						["flexible"] = true,
						["twenty"] = true,
						["pvp"] = true,
					},
				},
			},
			["glowType"] = "buttonOverlay",
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["zoom"] = 0,
			["auto"] = true,
			["text2Enabled"] = true,
			["uid"] = "8z6mXGUgHnQ",
			["fixedWidth"] = 200,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["shadowYOffset"] = -1,
			["cooldownSwipe"] = true,
			["customTextUpdate"] = "update",
			["cooldownEdge"] = false,
			["triggers"] = {
				{
					["trigger"] = {
						["duration"] = "1",
						["names"] = {
						},
						["customIcon"] = "function() return GetSpellTexture(20484) end",
						["debuffType"] = "HELPFUL",
						["use_unit"] = true,
						["type"] = "custom",
						["subeventPrefix"] = "SPELL",
						["custom_type"] = "status",
						["unit"] = "player",
						["events"] = "SPELL_COOLDOWN_READY SPELL_COOLDOWN_STARTED SPELL_COOLDOWN_CHANGED",
						["event"] = "Health",
						["customStacks"] = "function()\n    local c, m = GetSpellCharges(20484)\n    return c\nend",
						["customDuration"] = "function() \n    local _, _, s, d = GetSpellCharges(20484)\n    return d, d+s \nend",
						["customName"] = "function() return GetSpellInfo(20484) end",
						["spellIds"] = {
						},
						["unevent"] = "auto",
						["check"] = "event",
						["custom"] = "function(e, id) \n    return id == 20484 and GetSpellCharges(20484) \nend",
						["subeventSuffix"] = "_CAST_START",
						["use_absorbMode"] = true,
					},
					["untrigger"] = {
						["custom"] = "function() \n    return not GetSpellCharges(20484) \nend",
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["displayText_format_p_format"] = "timed",
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["duration_type"] = "seconds",
					["type"] = "none",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["main"] = {
					["duration_type"] = "seconds",
					["type"] = "none",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["finish"] = {
					["duration_type"] = "seconds",
					["type"] = "none",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 1,
			["height"] = 64,
			["displayText_format_p_time_dynamic_threshold"] = 60,
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["displayText_format_s_format"] = "none",
			["preferToUpdate"] = false,
			["fontSize"] = 20,
			["text2Containment"] = "INSIDE",
			["text1Enabled"] = true,
			["text1Font"] = "Friz Quadrata TT",
			["cooldownTextDisabled"] = false,
			["text2"] = "%s / %p",
			["anchorFrameFrame"] = "WeakAuras:Combat Timer",
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["width"] = 64,
			["text1Point"] = "BOTTOMRIGHT",
			["actions"] = {
				["start"] = {
				},
				["finish"] = {
				},
				["init"] = {
					["custom"] = "WeakAuras.WatchSpellCooldown(20484)",
					["do_custom"] = true,
				},
			},
			["desaturate"] = false,
			["displayText_format_p_time_precision"] = 1,
			["text1"] = "%n",
			["semver"] = "1.0.0",
			["config"] = {
			},
			["justify"] = "LEFT",
			["automaticWidth"] = "Auto",
			["id"] = "Combat Res - text",
			["useglowColor"] = false,
			["frameStrata"] = 1,
			["anchorFrameType"] = "SELECTFRAME",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["alpha"] = 1,
			["inverse"] = false,
			["wordWrap"] = "WordWrap",
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["cooldown"] = true,
			["authorOptions"] = {
			},
		},
		["Stagger [bar]"] = {
			["sparkWidth"] = 10,
			["sparkOffsetX"] = 0,
			["frameStrata"] = 1,
			["authorOptions"] = {
				{
					["type"] = "select",
					["values"] = {
						"Shorten (12.3K)", -- [1]
						"Large (12,123)", -- [2]
						"None", -- [3]
					},
					["default"] = 1,
					["name"] = "|cFF00FFFF[Tick] damage format|r",
					["useDesc"] = false,
					["key"] = "tick",
					["width"] = 1,
				}, -- [1]
				{
					["type"] = "select",
					["values"] = {
						"Shorten (12.3K)", -- [1]
						"Large (12,123)", -- [2]
						"None", -- [3]
					},
					["default"] = 1,
					["name"] = "|cFF00FFFF[Stagger left] format|r",
					["useDesc"] = false,
					["key"] = "left",
					["width"] = 1,
				}, -- [2]
			},
			["preferToUpdate"] = false,
			["customText"] = "function()\n    local c = aura_env.config\n    if not WeakAuras.IsOptionsOpen() and aura_env.state then\n        local healthTotal = UnitHealthMax(\"player\")\n        local tick, left = select(16, WA_GetUnitAura(\"player\", aura_env.state.spellId, \"HARMFUL|PLAYER\"))\n        if tick then \n            local percent = 100 / (healthTotal / tick)\n            \n            local r, g, b = 0, 1, 0\n            if aura_env.state.spellId == 124274 then\n                r, g, b = 1, 1, 0\n            elseif aura_env.state.spellId == 124273 then\n                r, g ,b = 1, 0, 0\n            end\n            \n            aura_env.region:SetDurationInfo(left, healthTotal, true)\n            aura_env.region:Color(r, g, b, 1)\n            \n            return aura_env.formatNumber(tick, c.tick), aura_env.formatNumber(left, c.left), string.format(\"%.1f\", percent)\n        end\n    else \n        aura_env.region:Color(0, 1, 0, 1)\n        aura_env.region:SetDurationInfo(2, 6, true)\n        return aura_env.formatNumber(1912, c.tick), aura_env.formatNumber(32123, c.left), string.format(\"%.1f\", 0.912312)\n    end\nend\n\n\n\n\n",
			["yOffset"] = -147.4996948242188,
			["anchorPoint"] = "CENTER",
			["iconSource"] = -1,
			["cooldownSwipe"] = true,
			["sparkRotation"] = 0,
			["sparkRotationMode"] = "AUTO",
			["cooldownEdge"] = false,
			["icon"] = true,
			["triggers"] = {
				{
					["trigger"] = {
						["spellId"] = "124275",
						["unit"] = "player",
						["debuffType"] = "HARMFUL",
						["type"] = "aura2",
						["custom_hide"] = "timed",
						["auraspellids"] = {
							"124275", -- [1]
							"124274", -- [2]
							"124273", -- [3]
						},
						["subeventSuffix"] = "_CAST_START",
						["custom_type"] = "status",
						["event"] = "Health",
						["buffShowOn"] = "showOnActive",
						["name"] = "Light Stagger",
						["use_spellId"] = true,
						["spellIds"] = {
							124275, -- [1]
						},
						["useExactSpellId"] = true,
						["check"] = "update",
						["names"] = {
							"Light Stagger", -- [1]
						},
						["subeventPrefix"] = "SPELL",
						["fullscan"] = true,
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = -10,
			},
			["icon_color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["internalVersion"] = 45,
			["keepAspectRatio"] = false,
			["selfPoint"] = "CENTER",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["sparkColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["barColor"] = {
				1, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["desaturate"] = false,
			["customTextUpdate"] = "event",
			["url"] = "https://wago.io/StaggerGroup/11",
			["version"] = 11,
			["subRegions"] = {
				{
					["type"] = "aurabar_bar",
				}, -- [1]
				{
					["text_shadowXOffset"] = 0,
					["text_text"] = "%c1 [%c3%%]",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_text_format_c1_big_number_format"] = "AbbreviateLargeNumbers",
					["text_fixedWidth"] = 64,
					["text_text_format_c1_gcd_channel"] = false,
					["text_text_format_c1_gcd_cast"] = false,
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_text_format_c1_gcd_hide_zero"] = false,
					["text_shadowYOffset"] = 0,
					["text_text_format_c1_gcd_gcd"] = true,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_fontSize"] = 16,
					["anchorXOffset"] = 0,
					["text_text_format_c1_color"] = true,
					["text_text_format_c1_abbreviate"] = false,
					["text_automaticWidth"] = "Auto",
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["text_text_format_c1_realm_name"] = "never",
					["text_font"] = "000 PFSquareSansPro-Medium",
					["type"] = "subtext",
					["text_selfPoint"] = "AUTO",
					["text_text_format_c1_decimal_precision"] = 1,
					["text_text_format_c1_format"] = "none",
					["text_text_format_c3_format"] = "none",
					["text_text_format_c1_round_type"] = "floor",
					["text_anchorPoint"] = "INNER_LEFT",
					["text_text_format_c1_time_precision"] = 1,
					["rotateText"] = "NONE",
					["text_text_format_c1_abbreviate_max"] = 8,
					["text_visible"] = true,
				}, -- [2]
				{
					["text_text_format_n_format"] = "none",
					["text_text_format_c2_big_number_format"] = "AbbreviateLargeNumbers",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["text_text_format_c2_decimal_precision"] = 1,
					["type"] = "subtext",
					["text_anchorXOffset"] = 0,
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "000 PFSquareSansPro-Medium",
					["text_shadowYOffset"] = 0,
					["text_shadowXOffset"] = 0,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "INNER_RIGHT",
					["text_text"] = "%c2",
					["text_text_format_c2_format"] = "none",
					["text_fontSize"] = 16,
					["anchorXOffset"] = 0,
					["text_visible"] = true,
				}, -- [3]
				{
					["glowFrequency"] = 0.25,
					["type"] = "subglow",
					["useGlowColor"] = true,
					["glowType"] = "Pixel",
					["glowThickness"] = 2,
					["glowYOffset"] = 0,
					["glowColor"] = {
						0, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["glowXOffset"] = 0,
					["glowScale"] = 1,
					["glow"] = false,
					["glow_anchor"] = "bar",
					["glowLength"] = 10,
					["glowLines"] = 14,
					["glowBorder"] = true,
				}, -- [4]
			},
			["height"] = 30,
			["actions"] = {
				["start"] = {
				},
				["finish"] = {
				},
				["init"] = {
					["custom"] = "aura_env.formatNumber = function(value, method)\n    if not value or type(value) ~= \"number\" then return end\n    \n    if method == 1 then\n        if value > 999999999 then\n            value = string.format(\"%.2fB\", value/1000000000)\n        elseif value > 999999 then\n            value = string.format(\"%.2fM\", value/1000000)   \n        elseif value > 99999 then\n            value = string.format(\"%.0fK\", value/1000)\n        elseif value > 999 then\n            value = string.format(\"%.1fK\", value/1000)\n        elseif value <= 0 then\n            value = 0\n        end \n    elseif method == 2 then\n        if value > 999999999 then\n            value = (\"%d,%03d,%03d,%03d\"):format(floor(value/1000000000), floor(value/1000000)%1000, floor(value/1000)%1000, value%1000)\n        elseif value > 999999 then\n            value = (\"%d,%03d,%03d\"):format(floor(value/1000000), floor(value/1000)%1000, value%1000)\n        elseif value > 999 then\n            value = (\"%d,%03d\"):format(floor(value/1000), value%1000)\n        elseif value <= 0 then\n            value = 0\n        end\n    elseif method == 3 or not method then\n        return value\n    end\n    \n    return value\nend\n\n\n",
					["do_custom"] = true,
				},
			},
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["class"] = {
					["single"] = "MONK",
					["multi"] = {
					},
				},
				["spec"] = {
					["single"] = 1,
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["use_class"] = true,
				["race"] = {
					["multi"] = {
					},
				},
				["difficulty"] = {
					["multi"] = {
					},
				},
				["faction"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["use_spec"] = true,
				["ingroup"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["sparkBlendMode"] = "ADD",
			["useAdjustededMax"] = false,
			["useTooltip"] = false,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["colorR"] = 1,
					["scalex"] = 1,
					["colorA"] = 1,
					["colorG"] = 1,
					["type"] = "none",
					["easeType"] = "none",
					["scaley"] = 1,
					["use_color"] = true,
					["alpha"] = 0,
					["duration_type"] = "seconds",
					["y"] = 0,
					["x"] = 0,
					["rotate"] = 0,
					["duration"] = "1",
					["colorFunc"] = "\n\n",
					["easeStrength"] = 3,
					["colorType"] = "custom",
					["colorB"] = 1,
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["xOffset"] = 29.165771484375,
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["stickyDuration"] = false,
			["sparkOffsetY"] = 0,
			["useAdjustededMin"] = false,
			["regionType"] = "aurabar",
			["backgroundColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				0.5, -- [4]
			},
			["conditions"] = {
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["value"] = "124273",
						["variable"] = "spellId",
					},
					["changes"] = {
						{
							["value"] = true,
							["property"] = "sub.4.glow",
						}, -- [1]
					},
				}, -- [1]
			},
			["icon_side"] = "LEFT",
			["auto"] = true,
			["config"] = {
				["left"] = 1,
				["tick"] = 1,
			},
			["sparkHeight"] = 30,
			["texture"] = "Smooth v2",
			["semver"] = "1.0.3",
			["zoom"] = 0,
			["spark"] = false,
			["tocversion"] = 80300,
			["id"] = "Stagger [bar]",
			["anchorFrameType"] = "SCREEN",
			["alpha"] = 1,
			["width"] = 220,
			["cooldownTextDisabled"] = false,
			["uid"] = "zg2eAqvq4JR",
			["inverse"] = false,
			["sparkHidden"] = "NEVER",
			["orientation"] = "HORIZONTAL",
			["displayIcon"] = 463282,
			["cooldown"] = false,
			["sparkTexture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
		},
		["09 Ravenous Feast - Pink"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 4,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						1, -- [1]
						0, -- [2]
						0.83921568627451, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.25,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = true,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2417",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "O51Zf3uElpl",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["useMatch_count"] = true,
						["auranames"] = {
						},
						["groupclone"] = true,
						["matchesShowOn"] = "showOnActive",
						["name_info"] = "aura",
						["unit"] = "group",
						["use_tooltip"] = false,
						["subeventPrefix"] = "SPELL",
						["match_countOperator"] = ">=",
						["match_count"] = "1",
						["buffShowOn"] = "showOnActive",
						["unevent"] = "timed",
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["useExactSpellId"] = true,
						["custom_type"] = "stateupdate",
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["event"] = "Health",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HARMFUL",
						["use_specific_unit"] = false,
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["auraspellids"] = {
							"342735", -- [1]
						},
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["duration"] = "1",
						["useGroup_count"] = false,
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["text2Font"] = "Friz Quadrata TT",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text1"] = "%n",
			["text1Point"] = "CENTER",
			["anchorFrameType"] = "SCREEN",
			["semver"] = "1.0.26",
			["justify"] = "LEFT",
			["zoom"] = 0,
			["id"] = "09 Ravenous Feast - Pink",
			["config"] = {
			},
			["frameStrata"] = 9,
			["width"] = 6.9999866485596,
			["text1Containment"] = "INSIDE",
			["wordWrap"] = "WordWrap",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["Backup Alert"] = {
			["sparkWidth"] = 10,
			["iconSource"] = -1,
			["xOffset"] = 0,
			["displayText"] = "%c",
			["yOffset"] = 200,
			["anchorPoint"] = "CENTER",
			["sparkRotation"] = 0,
			["url"] = "https://wago.io/kJFQFy0V8/3",
			["actions"] = {
				["start"] = {
					["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\AirHorn.ogg",
					["do_sound"] = false,
				},
				["finish"] = {
				},
				["init"] = {
					["custom"] = "aura_env.icons= { \n    [1]  = \n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_1:12\\124t\",\n    [2] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_2:12\\124t\", \n    [3] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_3:12\\124t\", \n    [4] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_4:12\\124t\", \n    [5] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_5:12\\124t\", \n    [6] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_6:12\\124t\",   \n    [7] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_7:12\\124t\",\n    [8] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_8:12\\124t\",\n    [9] =\n    \" Right of \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_1:12\\124t\",\n    [10] =\n    \" 10\",\n    [11] =\n    \" 11\",\n    [12] =\n    \" 12\",\n    [13] =\n    \" 13\",\n    [14] =\n    \" 14\",\n    [15] =\n    \" 15\",\n}",
					["do_custom"] = true,
				},
			},
			["icon_color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["selfPoint"] = "BOTTOM",
			["barColor"] = {
				1, -- [1]
				0, -- [2]
				0.007843137254902, -- [3]
				1, -- [4]
			},
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["sparkOffsetY"] = 0,
			["load"] = {
				["use_encounterid"] = true,
				["zoneIds"] = "",
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2417",
				["spec"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["shadowXOffset"] = 1,
			["useAdjustededMin"] = false,
			["regionType"] = "aurabar",
			["texture"] = "Details Flat",
			["sparkTexture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
			["spark"] = false,
			["tocversion"] = 90002,
			["alpha"] = 1,
			["sparkColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["fixedWidth"] = 200,
			["outline"] = "OUTLINE",
			["sparkOffsetX"] = 0,
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "function()\n    aura_env.mark = aura_env.mark or \"\"\n    if aura_env.state and aura_env.state.marker then\n        return \"BACKUP:\"..aura_env.icons[aura_env.state.marker]\n    end\nend",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["debuffType"] = "HELPFUL",
						["type"] = "custom",
						["subeventSuffix"] = "_CAST_START",
						["custom_type"] = "stateupdate",
						["names"] = {
						},
						["duration"] = "5.2",
						["event"] = "Health",
						["subeventPrefix"] = "SPELL",
						["dynamicDuration"] = false,
						["custom"] = "function(states, event, mark)\n    if mark then \n        states [\"\"] = {\n            show = true,\n            progressType = \"timed\",\n            duration = 5.3,\n            expirationTime = GetTime()+5.3,\n            autoHide = true,\n            changed = true,\n            marker = mark\n        }\n        return true\n    end\nend",
						["events"] = "RELOE_BACKUP_ALERT",
						["spellIds"] = {
						},
						["check"] = "event",
						["unit"] = "player",
						["customVariables"] = "{\n    marker = \"number\"\n    }\n    \n    ",
						["custom_hide"] = "timed",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["displayText_format_p_format"] = "timed",
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["version"] = 3,
			["subRegions"] = {
				{
					["type"] = "aurabar_bar",
				}, -- [1]
				{
					["text_text_format_n_format"] = "none",
					["text_text"] = "%c",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_anchorXOffset"] = 0,
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "ExpressWay",
					["text_shadowYOffset"] = -1,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "None",
					["text_anchorPoint"] = "INNER_TOP",
					["text_anchorYOffset"] = 40,
					["text_shadowXOffset"] = 1,
					["text_fontSize"] = 40,
					["anchorXOffset"] = 0,
					["text_visible"] = true,
				}, -- [2]
				{
					["text_text_format_p_time_precision"] = 1,
					["text_text"] = "%p",
					["text_text_format_p_format"] = "Number",
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["text_text_format_p_decimal_precision"] = 1,
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Friz Quadrata TT",
					["text_shadowYOffset"] = -1,
					["text_shadowXOffset"] = 1,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "None",
					["text_anchorPoint"] = "INNER_RIGHT",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_text_format_n_format"] = "none",
					["text_fontSize"] = 20,
					["anchorXOffset"] = 0,
					["text_visible"] = true,
				}, -- [3]
				{
					["glowFrequency"] = 0.25,
					["type"] = "subglow",
					["useGlowColor"] = false,
					["glowType"] = "Pixel",
					["glowThickness"] = 5,
					["glowYOffset"] = 0,
					["glowColor"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["glowXOffset"] = 0,
					["glowScale"] = 1,
					["glow"] = true,
					["glow_anchor"] = "bar",
					["glowLength"] = 10,
					["glowLines"] = 14,
					["glowBorder"] = false,
				}, -- [4]
				{
					["type"] = "subborder",
					["border_anchor"] = "bar",
					["border_size"] = 2,
					["border_color"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["border_visible"] = true,
					["border_edge"] = "Square Full White",
					["border_offset"] = 0,
				}, -- [5]
			},
			["height"] = 40,
			["sparkBlendMode"] = "ADD",
			["useAdjustededMax"] = false,
			["fontSize"] = 60,
			["preferToUpdate"] = false,
			["parent"] = "09 Generals Soak Alerts",
			["uid"] = "vRiLgq3H3BI",
			["orientation"] = "HORIZONTAL",
			["displayText_format_p_time_dynamic"] = false,
			["icon_side"] = "RIGHT",
			["config"] = {
			},
			["displayText_format_p_time_precision"] = 1,
			["sparkHeight"] = 30,
			["sparkRotationMode"] = "AUTO",
			["anchorFrameType"] = "SCREEN",
			["zoom"] = 0,
			["semver"] = "1.0.2",
			["justify"] = "LEFT",
			["id"] = "Backup Alert",
			["sparkHidden"] = "NEVER",
			["frameStrata"] = 1,
			["width"] = 350,
			["backgroundColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				0.5, -- [4]
			},
			["icon"] = false,
			["inverse"] = false,
			["authorOptions"] = {
			},
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["value"] = "1",
						["variable"] = "marker",
					},
					["changes"] = {
						{
							["value"] = {
								0.97254901960784, -- [1]
								1, -- [2]
								0, -- [3]
								1, -- [4]
							},
							["property"] = "barColor",
						}, -- [1]
						{
							["value"] = {
								["sound_type"] = "Play",
								["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\Star.ogg",
								["sound_channel"] = "Master",
							},
							["property"] = "sound",
						}, -- [2]
					},
				}, -- [1]
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["value"] = "2",
						["variable"] = "marker",
					},
					["changes"] = {
						{
							["value"] = {
								1, -- [1]
								0.53725490196078, -- [2]
								0, -- [3]
								1, -- [4]
							},
							["property"] = "barColor",
						}, -- [1]
						{
							["value"] = {
								["sound_type"] = "Play",
								["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\Circle.ogg",
								["sound_channel"] = "Master",
							},
							["property"] = "sound",
						}, -- [2]
					},
				}, -- [2]
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["value"] = "3",
						["variable"] = "marker",
					},
					["changes"] = {
						{
							["value"] = {
								0.56862745098039, -- [1]
								0, -- [2]
								1, -- [3]
								1, -- [4]
							},
							["property"] = "barColor",
						}, -- [1]
						{
							["value"] = {
								["sound_type"] = "Play",
								["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\Diamond.ogg",
								["sound_channel"] = "Master",
							},
							["property"] = "sound",
						}, -- [2]
					},
				}, -- [3]
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["value"] = "4",
						["variable"] = "marker",
					},
					["changes"] = {
						{
							["value"] = {
								0.12549019607843, -- [1]
								1, -- [2]
								0, -- [3]
								1, -- [4]
							},
							["property"] = "barColor",
						}, -- [1]
						{
							["value"] = {
								["sound_type"] = "Play",
								["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\Triangle.ogg",
								["sound_channel"] = "Master",
							},
							["property"] = "sound",
						}, -- [2]
					},
				}, -- [4]
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["value"] = "5",
						["variable"] = "marker",
					},
					["changes"] = {
						{
							["value"] = {
								0.52156862745098, -- [1]
								1, -- [2]
								0.95686274509804, -- [3]
								1, -- [4]
							},
							["property"] = "barColor",
						}, -- [1]
						{
							["value"] = {
								["sound_type"] = "Play",
								["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\Moon.ogg",
								["sound_channel"] = "Master",
							},
							["property"] = "sound",
						}, -- [2]
					},
				}, -- [5]
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["value"] = "6",
						["variable"] = "marker",
					},
					["changes"] = {
						{
							["value"] = {
								0, -- [1]
								0.40392156862745, -- [2]
								1, -- [3]
								1, -- [4]
							},
							["property"] = "barColor",
						}, -- [1]
						{
							["value"] = {
								["sound_type"] = "Play",
								["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\Square.ogg",
								["sound_channel"] = "Master",
							},
							["property"] = "sound",
						}, -- [2]
					},
				}, -- [6]
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["value"] = "7",
						["variable"] = "marker",
					},
					["changes"] = {
						{
							["value"] = {
								1, -- [1]
								0.066666666666667, -- [2]
								0, -- [3]
								1, -- [4]
							},
							["property"] = "barColor",
						}, -- [1]
						{
							["value"] = {
								["sound_type"] = "Play",
								["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\Cross.ogg",
								["sound_channel"] = "Master",
							},
							["property"] = "sound",
						}, -- [2]
					},
				}, -- [7]
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["value"] = "8",
						["variable"] = "marker",
					},
					["changes"] = {
						{
							["property"] = "barColor",
						}, -- [1]
						{
							["value"] = {
								["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\Skull.ogg",
								["sound_type"] = "Play",
							},
							["property"] = "sound",
						}, -- [2]
					},
				}, -- [8]
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["value"] = "9",
						["variable"] = "marker",
					},
					["changes"] = {
						{
							["value"] = {
								0.95686274509804, -- [1]
								1, -- [2]
								0, -- [3]
								1, -- [4]
							},
							["property"] = "barColor",
						}, -- [1]
						{
							["value"] = {
								["sound_type"] = "Play",
								["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\Star.ogg",
								["sound_channel"] = "Master",
							},
							["property"] = "sound",
						}, -- [2]
					},
				}, -- [9]
			},
			["information"] = {
			},
			["wordWrap"] = "WordWrap",
		},
		["09 Generals Orbs & Mana List"] = {
			["arcLength"] = 360,
			["controlledChildren"] = {
				"09 Generals Mana Bar", -- [1]
				"09 Generals Orb", -- [2]
			},
			["borderBackdrop"] = "Blizzard Tooltip",
			["authorOptions"] = {
			},
			["preferToUpdate"] = true,
			["groupIcon"] = "3614362",
			["anchorPoint"] = "CENTER",
			["fullCircle"] = true,
			["space"] = 2,
			["url"] = "https://wago.io/cUe1XBhBn/7",
			["actions"] = {
				["start"] = {
				},
				["finish"] = {
				},
				["init"] = {
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["unit"] = "player",
						["type"] = "aura2",
						["spellIds"] = {
						},
						["subeventSuffix"] = "_CAST_START",
						["subeventPrefix"] = "SPELL",
						["names"] = {
						},
						["event"] = "Health",
						["debuffType"] = "HELPFUL",
					},
					["untrigger"] = {
					},
				}, -- [1]
			},
			["columnSpace"] = 1,
			["radius"] = 200,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["align"] = "LEFT",
			["stagger"] = 0,
			["version"] = 7,
			["subRegions"] = {
			},
			["grow"] = "DOWN",
			["borderInset"] = 1,
			["load"] = {
				["talent"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["spec"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["selfPoint"] = "TOPLEFT",
			["backdropColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0.5, -- [4]
			},
			["uid"] = "82rpBaNUPZJ",
			["animate"] = false,
			["rowSpace"] = 1,
			["scale"] = 1,
			["rotation"] = 0,
			["border"] = false,
			["borderEdge"] = "Square Full White",
			["regionType"] = "dynamicgroup",
			["borderSize"] = 2,
			["sort"] = "none",
			["internalVersion"] = 45,
			["gridType"] = "RD",
			["constantFactor"] = "RADIUS",
			["xOffset"] = -646.3334350585938,
			["borderOffset"] = 4,
			["semver"] = "1.0.6",
			["tocversion"] = 90002,
			["id"] = "09 Generals Orbs & Mana List",
			["gridWidth"] = 5,
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["limit"] = 5,
			["config"] = {
			},
			["yOffset"] = 354.1673583984375,
			["useLimit"] = false,
			["conditions"] = {
			},
			["information"] = {
			},
			["borderColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
		},
		["04 Displacement Cypher - Green"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 2,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						0.03921568627451, -- [1]
						1, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.25,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = true,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2405",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "dYZgzCmYKhm",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["useMatch_count"] = true,
						["auranames"] = {
						},
						["groupclone"] = true,
						["matchesShowOn"] = "showOnActive",
						["name_info"] = "aura",
						["unit"] = "group",
						["use_tooltip"] = false,
						["subeventPrefix"] = "SPELL",
						["match_countOperator"] = ">=",
						["match_count"] = "1",
						["buffShowOn"] = "showOnActive",
						["unevent"] = "timed",
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["useExactSpellId"] = true,
						["custom_type"] = "stateupdate",
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["event"] = "Health",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HARMFUL",
						["use_specific_unit"] = false,
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["auraspellids"] = {
							"328468", -- [1]
							"328448", -- [2]
						},
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["duration"] = "1",
						["useGroup_count"] = false,
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["wordWrap"] = "WordWrap",
			["config"] = {
			},
			["text1"] = "%n",
			["text1Containment"] = "INSIDE",
			["width"] = 6.9999866485596,
			["justify"] = "LEFT",
			["semver"] = "1.0.26",
			["zoom"] = 0,
			["id"] = "04 Displacement Cypher - Green",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["frameStrata"] = 9,
			["anchorFrameType"] = "SCREEN",
			["text1Point"] = "CENTER",
			["text2Font"] = "Friz Quadrata TT",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["02 Vicious Wound - Red"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 3,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						1, -- [1]
						0.066666666666667, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.25,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = true,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2418",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "BgvMhBheSsI",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["useMatch_count"] = true,
						["auranames"] = {
						},
						["groupclone"] = true,
						["matchesShowOn"] = "showOnActive",
						["name_info"] = "aura",
						["unit"] = "group",
						["use_tooltip"] = false,
						["subeventPrefix"] = "SPELL",
						["match_countOperator"] = ">=",
						["match_count"] = "1",
						["buffShowOn"] = "showOnActive",
						["unevent"] = "timed",
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["useExactSpellId"] = true,
						["custom_type"] = "stateupdate",
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["event"] = "Health",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HARMFUL",
						["use_specific_unit"] = false,
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["auraspellids"] = {
							"334960", -- [1]
						},
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["duration"] = "1",
						["useGroup_count"] = false,
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["wordWrap"] = "WordWrap",
			["config"] = {
			},
			["text1"] = "%n",
			["text1Containment"] = "INSIDE",
			["width"] = 6.9999866485596,
			["justify"] = "LEFT",
			["semver"] = "1.0.26",
			["zoom"] = 0,
			["id"] = "02 Vicious Wound - Red",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["frameStrata"] = 9,
			["anchorFrameType"] = "SCREEN",
			["text1Point"] = "CENTER",
			["text2Font"] = "Friz Quadrata TT",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["09 Generals Mana Bar"] = {
			["sparkWidth"] = 10,
			["sparkOffsetX"] = 0,
			["xOffset"] = 0,
			["preferToUpdate"] = true,
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["sparkRotation"] = 0,
			["sparkRotationMode"] = "AUTO",
			["url"] = "https://wago.io/cUe1XBhBn/7",
			["actions"] = {
				["start"] = {
				},
				["finish"] = {
				},
				["init"] = {
					["custom"] = "aura_env.shields = {\n    [329636] = true,\n    [329808] = true\n}\n\naura_env.orb1 = 342659\naura_env.orb2 = 342700\naura_env.orbd1 = 342655\naura_env.orbd2 = 342698\naura_env.debug = \"HARMFUL\"\n\nif aura_env.config[\"debug\"] then\n    aura_env.shield = 774\n    aura_env.orb1 = 8936\n    aura_env.orb2 = 48438\n    aura_env.orbd1 = 24858\n    aura_env.orbd2 = 22812\n    aura_env.debug = \"HELPFUL\"\nend\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
					["do_custom"] = true,
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "custom",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HELPFUL",
						["custom_type"] = "stateupdate",
						["event"] = "Health",
						["subeventPrefix"] = "SPELL",
						["names"] = {
						},
						["spellIds"] = {
						},
						["custom"] = "function(states, event, timestamp, subevent, hideCaster, sourceGUID, sourceName, sourceFlags, sourceRaidFlags, destGUID, destName, destFlags, destRaidFlags, spellID, ...)\n    \n    if subevent == \"SPELL_AURA_APPLIED\" then\n        if aura_env.shields[spellID] then\n            states[\"\"] = {\n                progressType = \"static\",\n                value = 0,\n                total = 10, \n                show = true,\n                changed = true,\n                additionalProgress = {\n                    {\n                        direction = \"forward\",\n                        width = 0,\n                        offset = 0,\n                        \n                    },\n                },\n            }\n            return true\n        else\n            local v = (spellID == aura_env.orbd1 and 1) or (spellID == aura_env.orbd2 and 3) or 0\n            if v > 0 and states[\"\"] then\n                states[\"\"].additionalProgress[1].width = states[\"\"].additionalProgress[1].width+v\n                return true\n            end\n        end\n    elseif subevent == \"SPELL_AURA_REMOVED\" then\n        if spellID == aura_env.orbd1 or spellID == aura_env.orbd2 then\n            local v = 0\n            for unit in WA_IterateGroupMembers() do\n                local a = (WA_GetUnitAura(unit, aura_env.orbd1, aura_env.debug) and 1) or (WA_GetUnitAura(unit, aura_env.orbd2, aura_env.debug) and 3) or 0\n                v = v+a\n            end\n            if states[\"\"] then\n                states[\"\"].additionalProgress[1].width = v\n                states[\"\"].changed = true\n                return true\n            end\n            \n        elseif aura_env.shields[spellID] then\n            aura_env.debuffed = {}\n            if states[\"\"] then\n                states[\"\"].show = false\n                states[\"\"].changed = true\n                return true\n            end\n            \n        end\n    elseif subevent == \"SPELL_ENERGIZE\" then\n        local v = (spellID == aura_env.orb1 and 1) or (spellID == aura_env.orb2 and 3) or 0\n        if v > 0 and states[\"\"] then\n            states[\"\"].value = states[\"\"].value+v\n            states[\"\"].changed = true\n            return true\n        end\n    end\n    \nend\n\n\n\n\n\n\n--[[\n\nlocal d = (v == 1 and aura_env.orbd1) or (v == 3 and aura_env.orbd2)\nfor a, b in pairs(aura_env.debuffed) do\n    if b and b[2] == v and not WA_GetUnitAura(a, d, aura_env.debug) then\n        states[\"\"].additionalProgress[1].width = states[\"\"].additionalProgress[1].width-v\n        aura_env.debuffed[a] = nil\n    end\nend]]\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
						["events"] = "ENCOUNTER_START CLEU:SPELL_AURA_APPLIED:SPELL_ENERGIZE:SPELL_AURA_REMOVED",
						["check"] = "event",
						["unit"] = "player",
						["customVariables"] = "{\n    additionalProgress = 1\n    }\n    \n    ",
						["custom_hide"] = "timed",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["icon_color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["internalVersion"] = 45,
			["selfPoint"] = "CENTER",
			["barColor"] = {
				0, -- [1]
				1, -- [2]
				0.9921568627451, -- [3]
				1, -- [4]
			},
			["desaturate"] = false,
			["frameStrata"] = 1,
			["version"] = 7,
			["subRegions"] = {
				{
					["type"] = "aurabar_bar",
				}, -- [1]
				{
					["text_text_format_p_time_precision"] = 1,
					["text_text"] = "%value",
					["text_text_format_p_format"] = "timed",
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["text_text_format_value_format"] = "none",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Friz Quadrata TT",
					["text_text_format_p_time_format"] = 0,
					["text_shadowYOffset"] = -1,
					["text_text_format_p_time_dynamic_threshold"] = 60,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "None",
					["text_anchorPoint"] = "INNER_LEFT",
					["text_visible"] = true,
					["text_shadowXOffset"] = 1,
					["text_fontSize"] = 20,
					["anchorXOffset"] = 0,
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
				}, -- [2]
				{
					["tick_blend_mode"] = "ADD",
					["tick_rotation"] = 0,
					["tick_xOffset"] = 0,
					["tick_desaturate"] = false,
					["automatic_length"] = false,
					["tick_thickness"] = 5,
					["type"] = "subtick",
					["use_texture"] = false,
					["tick_placement_mode"] = "AtValue",
					["tick_yOffset"] = 0,
					["tick_visible"] = true,
					["tick_placement"] = "1",
					["tick_texture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
					["tick_length"] = 34,
					["tick_mirror"] = false,
					["tick_color"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
				}, -- [3]
				{
					["tick_blend_mode"] = "ADD",
					["tick_rotation"] = 0,
					["tick_xOffset"] = 0,
					["tick_desaturate"] = false,
					["automatic_length"] = false,
					["tick_thickness"] = 5,
					["type"] = "subtick",
					["use_texture"] = false,
					["tick_placement_mode"] = "AtValue",
					["tick_yOffset"] = 0,
					["tick_visible"] = true,
					["tick_placement"] = "2",
					["tick_texture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
					["tick_length"] = 34,
					["tick_mirror"] = false,
					["tick_color"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
				}, -- [4]
				{
					["tick_blend_mode"] = "ADD",
					["tick_rotation"] = 0,
					["tick_xOffset"] = 0,
					["tick_desaturate"] = false,
					["automatic_length"] = false,
					["tick_thickness"] = 5,
					["type"] = "subtick",
					["use_texture"] = false,
					["tick_placement_mode"] = "AtValue",
					["tick_yOffset"] = 0,
					["tick_visible"] = true,
					["tick_placement"] = "3",
					["tick_texture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
					["tick_length"] = 34,
					["tick_mirror"] = false,
					["tick_color"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
				}, -- [5]
				{
					["tick_blend_mode"] = "ADD",
					["tick_rotation"] = 0,
					["tick_xOffset"] = 0,
					["tick_desaturate"] = false,
					["automatic_length"] = false,
					["tick_thickness"] = 5,
					["type"] = "subtick",
					["use_texture"] = false,
					["tick_placement_mode"] = "AtValue",
					["tick_yOffset"] = 0,
					["tick_visible"] = true,
					["tick_placement"] = "4",
					["tick_texture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
					["tick_length"] = 34,
					["tick_mirror"] = false,
					["tick_color"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
				}, -- [6]
				{
					["tick_blend_mode"] = "ADD",
					["tick_rotation"] = 0,
					["tick_xOffset"] = 0,
					["tick_desaturate"] = false,
					["automatic_length"] = false,
					["tick_thickness"] = 5,
					["type"] = "subtick",
					["use_texture"] = false,
					["tick_placement_mode"] = "AtValue",
					["tick_yOffset"] = 0,
					["tick_visible"] = true,
					["tick_placement"] = "5",
					["tick_texture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
					["tick_length"] = 34,
					["tick_mirror"] = false,
					["tick_color"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
				}, -- [7]
				{
					["tick_blend_mode"] = "ADD",
					["tick_rotation"] = 0,
					["tick_xOffset"] = 0,
					["tick_desaturate"] = false,
					["automatic_length"] = false,
					["tick_thickness"] = 5,
					["type"] = "subtick",
					["use_texture"] = false,
					["tick_placement_mode"] = "AtValue",
					["tick_yOffset"] = 0,
					["tick_visible"] = true,
					["tick_placement"] = "6",
					["tick_texture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
					["tick_length"] = 34,
					["tick_mirror"] = false,
					["tick_color"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
				}, -- [8]
				{
					["tick_blend_mode"] = "ADD",
					["tick_rotation"] = 0,
					["tick_xOffset"] = 0,
					["tick_desaturate"] = false,
					["automatic_length"] = false,
					["tick_thickness"] = 5,
					["type"] = "subtick",
					["use_texture"] = false,
					["tick_placement_mode"] = "AtValue",
					["tick_yOffset"] = 0,
					["tick_visible"] = true,
					["tick_placement"] = "7",
					["tick_texture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
					["tick_length"] = 34,
					["tick_mirror"] = false,
					["tick_color"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
				}, -- [9]
				{
					["tick_blend_mode"] = "ADD",
					["tick_rotation"] = 0,
					["tick_xOffset"] = 0,
					["tick_desaturate"] = false,
					["automatic_length"] = false,
					["tick_thickness"] = 5,
					["type"] = "subtick",
					["use_texture"] = false,
					["tick_placement_mode"] = "AtValue",
					["tick_yOffset"] = 0,
					["tick_visible"] = true,
					["tick_placement"] = "8",
					["tick_texture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
					["tick_length"] = 34,
					["tick_mirror"] = false,
					["tick_color"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
				}, -- [10]
				{
					["tick_blend_mode"] = "ADD",
					["tick_rotation"] = 0,
					["tick_xOffset"] = 0,
					["tick_desaturate"] = false,
					["automatic_length"] = false,
					["tick_thickness"] = 5,
					["type"] = "subtick",
					["use_texture"] = false,
					["tick_placement_mode"] = "AtValue",
					["tick_yOffset"] = 0,
					["tick_visible"] = true,
					["tick_placement"] = "9",
					["tick_texture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
					["tick_length"] = 34,
					["tick_mirror"] = false,
					["tick_color"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
				}, -- [11]
				{
					["type"] = "subborder",
					["border_anchor"] = "bar",
					["border_size"] = 2,
					["border_color"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["border_visible"] = true,
					["border_edge"] = "Square Full White",
					["border_offset"] = 0,
				}, -- [12]
			},
			["height"] = 35,
			["load"] = {
				["use_encounterid"] = true,
				["zoneIds"] = "",
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2417",
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["sparkBlendMode"] = "ADD",
			["useAdjustededMax"] = false,
			["parent"] = "09 Generals Orbs & Mana List",
			["sparkOffsetY"] = 0,
			["overlays"] = {
				{
					1, -- [1]
					0.56078431372549, -- [2]
					0, -- [3]
					1, -- [4]
				}, -- [1]
			},
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["icon"] = false,
			["config"] = {
				["debug"] = false,
			},
			["useAdjustededMin"] = false,
			["regionType"] = "aurabar",
			["uid"] = "MSBdRtFLxKx",
			["zoom"] = 0,
			["icon_side"] = "RIGHT",
			["anchorFrameType"] = "SCREEN",
			["sparkHeight"] = 30,
			["texture"] = "Details Flat",
			["id"] = "09 Generals Mana Bar",
			["sparkTexture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
			["semver"] = "1.0.6",
			["tocversion"] = 90002,
			["sparkHidden"] = "NEVER",
			["spark"] = false,
			["alpha"] = 1,
			["width"] = 300,
			["backgroundColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				0.5, -- [4]
			},
			["sparkColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["inverse"] = false,
			["iconSource"] = -1,
			["orientation"] = "HORIZONTAL",
			["conditions"] = {
			},
			["information"] = {
			},
			["authorOptions"] = {
				{
					["type"] = "toggle",
					["key"] = "debug",
					["default"] = false,
					["name"] = "Debug Mode",
					["useDesc"] = false,
					["width"] = 1,
				}, -- [1]
			},
		},
		["Harder Daddy"] = {
			["outline"] = "OUTLINE",
			["iconSource"] = -1,
			["authorOptions"] = {
			},
			["preferToUpdate"] = false,
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["justify"] = "LEFT",
			["cooldownSwipe"] = true,
			["selfPoint"] = "BOTTOM",
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["actions"] = {
				["start"] = {
					["message"] = "shut it you masochistic freaks",
					["do_sound"] = false,
					["message_type"] = "SAY",
					["sound"] = " custom",
					["do_message"] = false,
				},
				["finish"] = {
					["message"] = "shut it you masochistic freaks",
					["do_message"] = true,
					["message_type"] = "SAY",
				},
				["init"] = {
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["spellId"] = "326851",
						["duration"] = ".5",
						["unit"] = "player",
						["use_sourceFlags"] = false,
						["debuffType"] = "HELPFUL",
						["type"] = "combatlog",
						["unevent"] = "timed",
						["event"] = "Combat Log",
						["custom_hide"] = "timed",
						["subeventSuffix"] = "_CAST_SUCCESS",
						["use_spellId"] = true,
						["events"] = "COMBAT_LOG_EVENT_UNFILTERED",
						["custom_type"] = "event",
						["check"] = "event",
						["subeventPrefix"] = "SPELL",
						["spellIds"] = {
						},
						["names"] = {
						},
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "all",
				["activeTriggerMode"] = -10,
			},
			["internalVersion"] = 45,
			["keepAspectRatio"] = false,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["xOffset"] = 0,
			["cooldown"] = false,
			["conditions"] = {
			},
			["desaturate"] = false,
			["discrete_rotation"] = 0,
			["font"] = "Friz Quadrata TT",
			["version"] = 2,
			["subRegions"] = {
			},
			["height"] = 11.999991416931,
			["rotate"] = true,
			["load"] = {
				["ingroup"] = {
					["single"] = "raid",
					["multi"] = {
						["raid"] = true,
					},
				},
				["talent"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["difficulty"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["faction"] = {
					["multi"] = {
					},
				},
				["use_ingroup"] = true,
				["spec"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["icon"] = true,
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["fontSize"] = 12,
			["cooldownEdge"] = false,
			["config"] = {
			},
			["shadowXOffset"] = 1,
			["displayIcon"] = 3670321,
			["mirror"] = false,
			["anchorFrameType"] = "SCREEN",
			["regionType"] = "text",
			["alpha"] = 1,
			["blendMode"] = "BLEND",
			["rotation"] = 0,
			["wordWrap"] = "WordWrap",
			["texture"] = "326851",
			["zoom"] = 0,
			["cooldownTextDisabled"] = false,
			["semver"] = "1.0.1",
			["tocversion"] = 90005,
			["id"] = "Harder Daddy",
			["url"] = "https://wago.io/CwJ6vGxws/2",
			["frameStrata"] = 1,
			["width"] = 6.9999866485596,
			["textureWrapMode"] = "CLAMPTOBLACKADDITIVE",
			["uid"] = ")hGlY)yV2ET",
			["inverse"] = false,
			["displayText"] = "",
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["fixedWidth"] = 200,
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["shadowYOffset"] = -1,
		},
		["Glow Handler"] = {
			["sparkWidth"] = 10,
			["iconSource"] = -1,
			["xOffset"] = 0,
			["displayText"] = " ",
			["yOffset"] = 200,
			["anchorPoint"] = "CENTER",
			["sparkRotation"] = 0,
			["url"] = "https://wago.io/kJFQFy0V8/3",
			["actions"] = {
				["start"] = {
					["glow_color"] = {
						0.96862745098039, -- [1]
						1, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["glow_thickness"] = 3,
					["glow_action"] = "show",
					["do_glow"] = true,
					["glow_lines"] = 12,
					["do_sound"] = false,
					["glow_type"] = "Pixel",
					["use_glow_color"] = true,
					["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\AirHorn.ogg",
					["glow_frame_type"] = "UNITFRAME",
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["do_glow"] = true,
					["glow_action"] = "hide",
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
			},
			["icon_color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["selfPoint"] = "BOTTOM",
			["barColor"] = {
				1, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["sparkOffsetY"] = 0,
			["load"] = {
				["use_encounterid"] = true,
				["zoneIds"] = "",
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2417",
				["spec"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["shadowXOffset"] = 1,
			["useAdjustededMin"] = false,
			["regionType"] = "text",
			["texture"] = "Details Flat",
			["sparkTexture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
			["spark"] = false,
			["tocversion"] = 90002,
			["alpha"] = 1,
			["sparkColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["fixedWidth"] = 200,
			["outline"] = "OUTLINE",
			["sparkOffsetX"] = 0,
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["debuffType"] = "HELPFUL",
						["type"] = "custom",
						["subeventSuffix"] = "_CAST_START",
						["custom_type"] = "stateupdate",
						["names"] = {
						},
						["duration"] = "5.2",
						["event"] = "Health",
						["subeventPrefix"] = "SPELL",
						["dynamicDuration"] = false,
						["custom"] = "function(states, event, player)\n    if player then \n        states[player] = {\n            show = true,\n            progressType = \"timed\",\n            duration = 5.3,\n            expirationTime = GetTime()+5.3,\n            autoHide = true,\n            changed = true,\n            unit = player,\n        }\n        return true\n    end\nend",
						["events"] = "RELOE_GLOW_ALERT",
						["spellIds"] = {
						},
						["check"] = "event",
						["unit"] = "player",
						["customVariables"] = "",
						["custom_hide"] = "timed",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["displayText_format_p_format"] = "timed",
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["version"] = 3,
			["subRegions"] = {
			},
			["height"] = 40,
			["sparkBlendMode"] = "ADD",
			["useAdjustededMax"] = false,
			["fontSize"] = 60,
			["preferToUpdate"] = false,
			["parent"] = "09 Generals Soak Alerts",
			["displayText_format_p_time_dynamic_threshold"] = 60,
			["orientation"] = "HORIZONTAL",
			["uid"] = "iOv3Ln78GNx",
			["config"] = {
			},
			["icon_side"] = "RIGHT",
			["displayText_format_p_time_format"] = 0,
			["displayText_format_p_time_precision"] = 1,
			["sparkHeight"] = 30,
			["anchorFrameType"] = "SCREEN",
			["zoom"] = 0,
			["backgroundColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				0.5, -- [4]
			},
			["justify"] = "LEFT",
			["id"] = "Glow Handler",
			["sparkHidden"] = "NEVER",
			["semver"] = "1.0.2",
			["frameStrata"] = 1,
			["width"] = 350,
			["icon"] = false,
			["sparkRotationMode"] = "AUTO",
			["inverse"] = false,
			["authorOptions"] = {
			},
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
			},
			["wordWrap"] = "WordWrap",
		},
		["Tosh Nathria"] = {
			["grow"] = "DOWN",
			["controlledChildren"] = {
				"Infuser's Boon", -- [1]
			},
			["borderBackdrop"] = "Blizzard Tooltip",
			["xOffset"] = -289.9993896484375,
			["yOffset"] = 55.000244140625,
			["anchorPoint"] = "CENTER",
			["borderColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["space"] = 2,
			["actions"] = {
				["start"] = {
				},
				["init"] = {
				},
				["finish"] = {
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["names"] = {
						},
						["type"] = "aura2",
						["spellIds"] = {
						},
						["subeventSuffix"] = "_CAST_START",
						["unit"] = "player",
						["subeventPrefix"] = "SPELL",
						["event"] = "Health",
						["debuffType"] = "HELPFUL",
					},
					["untrigger"] = {
					},
				}, -- [1]
			},
			["columnSpace"] = 1,
			["radius"] = 200,
			["selfPoint"] = "TOP",
			["align"] = "CENTER",
			["stagger"] = 0,
			["subRegions"] = {
			},
			["load"] = {
				["talent"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["class"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["backdropColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0.5, -- [4]
			},
			["arcLength"] = 360,
			["animate"] = false,
			["authorOptions"] = {
			},
			["scale"] = 1,
			["internalVersion"] = 45,
			["border"] = false,
			["borderEdge"] = "Square Full White",
			["regionType"] = "dynamicgroup",
			["borderSize"] = 2,
			["sort"] = "none",
			["rowSpace"] = 1,
			["fullCircle"] = true,
			["constantFactor"] = "RADIUS",
			["rotation"] = 0,
			["borderOffset"] = 4,
			["borderInset"] = 1,
			["limit"] = 5,
			["id"] = "Tosh Nathria",
			["frameStrata"] = 1,
			["gridWidth"] = 5,
			["anchorFrameType"] = "SCREEN",
			["config"] = {
			},
			["uid"] = "rjId4yJ753X",
			["useLimit"] = false,
			["gridType"] = "RD",
			["conditions"] = {
			},
			["information"] = {
			},
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
		},
		["Power Infusion Send"] = {
			["outline"] = "OUTLINE",
			["iconSource"] = -1,
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["displayText_format_p_time_dynamic_threshold"] = 60,
			["shadowYOffset"] = -1,
			["anchorPoint"] = "CENTER",
			["cooldownSwipe"] = true,
			["displayText_format_p_time_format"] = 0,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["icon"] = true,
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "custom",
						["subeventSuffix"] = "_CAST_START",
						["duration"] = "1",
						["event"] = "Health",
						["subeventPrefix"] = "SPELL",
						["custom_hide"] = "timed",
						["spellIds"] = {
						},
						["events"] = "TOSH_WA_PI_ASK",
						["custom"] = "function(event, ...)\n    if event == \"TOSH_WA_PI_ASK\" then\n        aura_env:Request()\n    end\nend\n",
						["custom_type"] = "event",
						["names"] = {
						},
						["unit"] = "player",
						["debuffType"] = "HELPFUL",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["displayText_format_p_format"] = "timed",
			["internalVersion"] = 45,
			["keepAspectRatio"] = false,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["subRegions"] = {
			},
			["height"] = 64,
			["load"] = {
				["talent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
			},
			["width"] = 64,
			["fontSize"] = 12,
			["preferToUpdate"] = false,
			["shadowXOffset"] = 1,
			["cooldown"] = false,
			["fixedWidth"] = 200,
			["wordWrap"] = "WordWrap",
			["regionType"] = "text",
			["actions"] = {
				["start"] = {
				},
				["init"] = {
					["do_custom"] = true,
					["custom"] = "local AceComm = LibStub(\"AceComm-3.0\")\n\nfunction aura_env:Request()\n    local dist = \"PARTY\"\n    if IsInRaid() then\n        dist = \"RAID\"\n    elseif IsInGroup() == \"LE_PARTY_CATEGORY_INSTANCE\" then\n        dist = \"INSTANCE_CHAT\"\n    end\n    AceComm:SendCommMessage(\"TOSH_WA\", \"PI:me\", dist)\nend\n",
				},
				["finish"] = {
				},
			},
			["cooldownEdge"] = false,
			["config"] = {
			},
			["xOffset"] = 0,
			["displayText_format_p_time_precision"] = 1,
			["frameStrata"] = 1,
			["displayText"] = "",
			["cooldownTextDisabled"] = false,
			["justify"] = "LEFT",
			["tocversion"] = 90005,
			["id"] = "Power Infusion Send",
			["zoom"] = 0,
			["alpha"] = 1,
			["anchorFrameType"] = "SCREEN",
			["authorOptions"] = {
				{
					["type"] = "description",
					["fontSize"] = "large",
					["text"] = "To use, add to a macro (or create a new one):\n\n/script WeakAuras.ScanEvents(\"TOSH_WA_PI_ASK\")\n",
					["width"] = 2,
				}, -- [1]
			},
			["uid"] = "i5LvrJDtwzE",
			["inverse"] = false,
			["yOffset"] = 0,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
			},
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
		},
		["circle_leaf_nofill"] = {
			["iconSource"] = 0,
			["xOffset"] = 0,
			["preferToUpdate"] = true,
			["customText"] = "function()\n    if aura_env.showText then\n        return aura_env.order\n    else\n        return \"\"\n    end\nend",
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["cooldownSwipe"] = true,
			["cooldownEdge"] = false,
			["icon"] = true,
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "custom",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HELPFUL",
						["event"] = "Health",
						["unit"] = "player",
						["custom_type"] = "status",
						["spellIds"] = {
						},
						["events"] = "ZONE_CHANGED, PLAYER_LOGIN, PLAYER_ENTERING_WORLD",
						["names"] = {
						},
						["check"] = "event",
						["custom"] = "function()\n    WeakAuras.ScanEvents(\"TirnaScitheButtonData\", 1, aura_env.frameData)\n    if aura_env.debugmode then \n        return true \n    end\n    local text = GetMinimapZoneText()\n    if text == aura_env.mistZone then\n        return true\n    else\n        return false\n    end\nend",
						["subeventPrefix"] = "SPELL",
						["custom_hide"] = "custom",
					},
					["untrigger"] = {
					},
				}, -- [1]
				{
					["trigger"] = {
						["debuffType"] = "HELPFUL",
						["type"] = "custom",
						["custom"] = "function(event,eid,...)\n    if event == \"ENCOUNTER_END\" then \n        if eid == 2392 and aura_env.showEncounter == false then \n            return false \n        end\n        return true \n    elseif event == \"ENCOUNTER_START\" then \n        if aura_env.showEncounter == false then \n            return false\n        else\n            return true\n        end\n    end\n    return true\nend\n\n",
						["custom_type"] = "status",
						["check"] = "event",
						["customName"] = "function()\n    return \nend",
						["events"] = "ENCOUNTER_START, ENCOUNTER_END, PLAYER_ENTERING_WORLD",
						["unit"] = "player",
					},
					["untrigger"] = {
					},
				}, -- [2]
				{
					["trigger"] = {
						["type"] = "custom",
						["custom_type"] = "status",
						["unit"] = "player",
						["customName"] = "function()\n    return aura_env.order            \nend",
						["events"] = "TirnaScitheButtonOrder",
						["check"] = "event",
						["debuffType"] = "HELPFUL",
						["custom"] = "function(_,button,order)\n    if button == 1 then \n        aura_env.order = order\n        if order == 9 then \n            aura_env.order = \"\"\n        end\n        return true\n    elseif button == 9 then\n        aura_env.order = \"\"\n        return true\n    end\n    return true\nend\n\n",
						["custom_hide"] = "timed",
					},
					["untrigger"] = {
					},
				}, -- [3]
				["activeTriggerMode"] = -10,
			},
			["internalVersion"] = 45,
			["keepAspectRatio"] = false,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["desaturate"] = false,
			["version"] = 67,
			["subRegions"] = {
				{
					["glowFrequency"] = 0.25,
					["type"] = "subglow",
					["useGlowColor"] = false,
					["glowType"] = "buttonOverlay",
					["glowLength"] = 10,
					["glowYOffset"] = 0,
					["glowColor"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["glowXOffset"] = 0,
					["glowScale"] = 1,
					["glowThickness"] = 1,
					["glow"] = false,
					["glowLines"] = 8,
					["glowBorder"] = false,
				}, -- [1]
				{
					["text_text_format_p_time_format"] = 0,
					["text_text"] = "%c",
					["text_text_format_p_format"] = "timed",
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Friz Quadrata TT",
					["text_text_format_p_time_precision"] = 1,
					["text_shadowYOffset"] = 0,
					["text_text_format_p_time_dynamic_threshold"] = 60,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "INNER_BOTTOMRIGHT",
					["text_shadowXOffset"] = 0,
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_fontSize"] = 15,
					["anchorXOffset"] = 0,
					["text_visible"] = true,
				}, -- [2]
			},
			["height"] = 80,
			["load"] = {
				["ingroup"] = {
					["single"] = "group",
					["multi"] = {
						["group"] = true,
					},
				},
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["use_encounterid"] = false,
				["difficulty"] = {
					["multi"] = {
						["mythic"] = true,
						["challenge"] = true,
					},
				},
				["use_zone"] = false,
				["zoneIds"] = "1669",
				["use_zoneIds"] = true,
				["zone"] = "",
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["config"] = {
				["encounter"] = false,
				["noCommunication"] = false,
				["debug"] = false,
				["orderText"] = true,
			},
			["actions"] = {
				["start"] = {
				},
				["finish"] = {
				},
				["init"] = {
					["custom"] = "aura_env.debugmode = aura_env.config['debug']\nlocal debug = aura_env.debugmode\naura_env.showEncounter = aura_env.config['encounter']\naura_env.locale=GetLocale()\naura_env.mistZone = \"Mistveil Tangle\"\naura_env.order = \"\"\naura_env.showText = aura_env.config['orderText']\naura_env.noCommunication = aura_env.config['noCommunication']\nif aura_env.locale == \"koKR\" then\n    aura_env.mistZone = \"안개장막 덩굴숲\"\nelseif aura_env.locale == \"frFR\" then\n    aura_env.mistZone = \"Maquis Voile-de-Brume\"\nelseif aura_env.locale == \"deDE\" then\n    aura_env.mistZone = \"Nebelschleierdickicht\"\nelseif aura_env.locale == \"zhCN\" then\n    aura_env.mistZone = \"纱雾迷结\"\nelseif aura_env.locale == \"esES\" then\n    aura_env.mistZone = \"Espesura Velo de Niebla\"\nelseif aura_env.locale == \"zhTW\" then\n    aura_env.mistZone = \"霧紗密林\"\nelseif aura_env.locale == \"esMX\" then\n    aura_env.mistZone = \"Espesura Veloniebla\"\nelseif aura_env.locale == \"ruRU\" then\n    aura_env.mistZone = \"Туманная чащоба\"\nelseif aura_env.locale == \"ptBR\" then\n    aura_env.mistZone = \"Enleio do Véu da Névoa\"\nelseif aura_env.locale == \"itIT\" then\n    aura_env.mistZone = \"Intrico Velofosco\"\nelseif aura_env.locale == \"ptPT\" then\n    aura_env.mistZone = \"Enleio do Véu da Névoa\"        \nend\nlocal values = {0, 1, 1, 1}\nlocal message = string.format(\"%d %d %d %d\", values[1], values[2], values[3], values[4])\nlocal name, _ = UnitName(\"player\")\n\nif not aura_env.frameData then\n    local b = CreateFrame(\"Button\", nil, WeakAuras.regions[aura_env.id].region)\n    b:SetAllPoints(WeakAuras.regions[aura_env.id].region)\n    b:RegisterForClicks(\"LeftButtonDown\", \"RightButtonDown\")\n    aura_env.frameData = b\n    WeakAuras.ScanEvents(\"TirnaScitheButtonData\", 1, b)\nend\nif aura_env.noCommunication then\n    aura_env.frameData:SetScript(\"OnClick\", function(self, button)\n            if debug then print(\"Click Registered - true\") end\n            WeakAuras.ScanEvents(\"GlowUpdate\")\n            C_ChatInfo.RegisterAddonMessagePrefix(\"Tirna Ver2\") \n            if debug then \n                print(\"noCommunication sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"WHISPER\", name)) \n            else\n                C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"WHISPER\", name)\n            end\n            \n    end)\nelse\n    aura_env.frameData:SetScript(\"OnClick\", function(self, button)\n            if debug then print(\"Click Registered - true\") end\n            WeakAuras.ScanEvents(\"GlowUpdate\")\n            C_ChatInfo.RegisterAddonMessagePrefix(\"Tirna Ver2\")\n            if IsPartyLFG() then            \n                if debug then \n                    print(\"LFG sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"INSTANCE_CHAT\")) \n                else\n                    C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"INSTANCE_CHAT\")\n                end\n            elseif UnitInParty(\"player\") then            \n                if debug then \n                    print(\"party sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"PARTY\")) \n                else\n                    C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"PARTY\")\n                end\n            else         \n                if debug then \n                    print(\"solo sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"WHISPER\", name)) \n                else\n                    C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"WHISPER\", name)\n                end\n            end\n    end)\nend",
					["do_custom"] = true,
				},
			},
			["authorOptions"] = {
				{
					["type"] = "toggle",
					["key"] = "orderText",
					["default"] = true,
					["name"] = "Show Order Text",
					["useDesc"] = false,
					["width"] = 1,
				}, -- [1]
				{
					["type"] = "toggle",
					["key"] = "encounter",
					["default"] = false,
					["name"] = "Show during Boss",
					["useDesc"] = false,
					["width"] = 1,
				}, -- [2]
				{
					["type"] = "toggle",
					["key"] = "noCommunication",
					["default"] = false,
					["name"] = "Disable Cross-Communication with others running the WA",
					["useDesc"] = false,
					["width"] = 2,
				}, -- [3]
				{
					["type"] = "toggle",
					["key"] = "debug",
					["default"] = false,
					["name"] = "|cffff0000Debug testing|r (requires reload for full debug)",
					["useDesc"] = false,
					["width"] = 2,
				}, -- [4]
			},
			["regionType"] = "icon",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["cooldown"] = false,
			["conditions"] = {
			},
			["parent"] = "Mists of Tirna Scithe guessing game v2",
			["semver"] = "2.0.2",
			["anchorFrameType"] = "SCREEN",
			["zoom"] = 0,
			["auto"] = false,
			["tocversion"] = 90005,
			["id"] = "circle_leaf_nofill",
			["alpha"] = 1,
			["frameStrata"] = 1,
			["width"] = 80,
			["cooldownTextDisabled"] = false,
			["uid"] = "jO6Q2OafN)M",
			["inverse"] = false,
			["selfPoint"] = "CENTER",
			["displayIcon"] = "Interface\\AddOns\\guess\\circle_leaf_nofill.tga",
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["url"] = "https://wago.io/hVLym_eLv/67",
		},
		["Impale Positions"] = {
			["grow"] = "CUSTOM",
			["controlledChildren"] = {
				"Position", -- [1]
				"Mirror Text", -- [2]
			},
			["borderBackdrop"] = "Blizzard Tooltip",
			["xOffset"] = 0,
			["preferToUpdate"] = false,
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["borderColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["space"] = 2,
			["url"] = "https://wago.io/sFnb6lvjO/5",
			["actions"] = {
				["start"] = {
				},
				["finish"] = {
				},
				["init"] = {
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["unit"] = "player",
						["type"] = "aura2",
						["spellIds"] = {
						},
						["subeventSuffix"] = "_CAST_START",
						["subeventPrefix"] = "SPELL",
						["names"] = {
						},
						["event"] = "Health",
						["debuffType"] = "HELPFUL",
					},
					["untrigger"] = {
					},
				}, -- [1]
			},
			["columnSpace"] = 1,
			["radius"] = 200,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["align"] = "CENTER",
			["rotation"] = 0,
			["version"] = 5,
			["subRegions"] = {
			},
			["borderInset"] = 1,
			["useLimit"] = false,
			["load"] = {
				["talent"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["class"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["internalVersion"] = 45,
			["backdropColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0.5, -- [4]
			},
			["arcLength"] = 360,
			["animate"] = false,
			["customGrow"] = "function(newPos, activeReg)\n    for i, region in ipairs(activeReg) do\n        if (region.region.header) then\n            newPos[i] = {\n                0,\n                150\n            }\n        elseif (region.region.state) then\n            local pos = region.region.state.position\n            local x,y = 0, 0\n            if (pos == 1) then\n                x = -200\n                y = -75\n            elseif (pos == 2) then\n                x = -200\n                y = 75\n            elseif (pos == 3) then\n                x = 200\n                y = 75\n            elseif (pos == 4) then\n                x = 200\n                y = -75\n            end\n            \n            newPos[i] = {x,y}\n        end\n    end\nend",
			["scale"] = 1,
			["rowSpace"] = 1,
			["border"] = false,
			["borderEdge"] = "Square Full White",
			["regionType"] = "dynamicgroup",
			["borderSize"] = 2,
			["limit"] = 5,
			["authorOptions"] = {
			},
			["fullCircle"] = true,
			["constantFactor"] = "RADIUS",
			["sort"] = "none",
			["borderOffset"] = 4,
			["semver"] = "1.0.4",
			["tocversion"] = 90005,
			["id"] = "Impale Positions",
			["gridWidth"] = 5,
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["gridType"] = "RD",
			["uid"] = "uHuvwogd1WJ",
			["selfPoint"] = "CENTER",
			["config"] = {
			},
			["conditions"] = {
			},
			["information"] = {
			},
			["stagger"] = 0,
		},
		["nocircle_leaf_nofill"] = {
			["iconSource"] = 0,
			["xOffset"] = 0,
			["preferToUpdate"] = true,
			["customText"] = "function()\n    if aura_env.showText then\n        return aura_env.order\n    else\n        return \"\"\n    end\nend",
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["cooldownSwipe"] = true,
			["cooldownEdge"] = false,
			["icon"] = true,
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "custom",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HELPFUL",
						["event"] = "Health",
						["unit"] = "player",
						["custom_type"] = "status",
						["spellIds"] = {
						},
						["events"] = "ZONE_CHANGED, PLAYER_LOGIN, PLAYER_ENTERING_WORLD",
						["names"] = {
						},
						["check"] = "event",
						["custom"] = "function()\n    WeakAuras.ScanEvents(\"TirnaScitheButtonData\", 5, aura_env.frameData)\n    if aura_env.debugmode then \n        return true \n    end\n    local text = GetMinimapZoneText()\n    if text == aura_env.mistZone then\n        return true\n    else\n        return false\n    end\nend\n\n",
						["subeventPrefix"] = "SPELL",
						["custom_hide"] = "custom",
					},
					["untrigger"] = {
					},
				}, -- [1]
				{
					["trigger"] = {
						["debuffType"] = "HELPFUL",
						["type"] = "custom",
						["custom"] = "function(event,eid,...)\n    if event == \"ENCOUNTER_END\" then \n        if eid == 2392 and aura_env.showEncounter == false then \n            return false \n        end\n        return true \n    elseif event == \"ENCOUNTER_START\" then \n        if aura_env.showEncounter == false then \n            return false\n        else\n            return true\n        end\n    end\n    return true\nend\n\n",
						["custom_type"] = "status",
						["check"] = "event",
						["customName"] = "function()\n    return \nend",
						["events"] = "ENCOUNTER_START, ENCOUNTER_END, PLAYER_ENTERING_WORLD",
						["unit"] = "player",
					},
					["untrigger"] = {
					},
				}, -- [2]
				{
					["trigger"] = {
						["type"] = "custom",
						["custom_type"] = "status",
						["unit"] = "player",
						["customName"] = "function()\n    return aura_env.order            \nend",
						["events"] = "TirnaScitheButtonOrder",
						["check"] = "event",
						["debuffType"] = "HELPFUL",
						["custom"] = "function(_,button,order)\n    if button == 5 then \n        aura_env.order = order\n        if order == 9 then \n            aura_env.order = \"\"\n        end\n        return true\n    elseif button == 9 then\n        aura_env.order = \"\"\n        return true\n    end\n    return true\nend\n\n",
						["custom_hide"] = "timed",
					},
					["untrigger"] = {
					},
				}, -- [3]
				["activeTriggerMode"] = -10,
			},
			["internalVersion"] = 45,
			["keepAspectRatio"] = false,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["desaturate"] = false,
			["version"] = 67,
			["subRegions"] = {
				{
					["glowFrequency"] = 0.25,
					["type"] = "subglow",
					["useGlowColor"] = false,
					["glowType"] = "buttonOverlay",
					["glowLength"] = 10,
					["glowYOffset"] = 0,
					["glowColor"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["glowXOffset"] = 0,
					["glowScale"] = 1,
					["glowThickness"] = 1,
					["glow"] = false,
					["glowLines"] = 8,
					["glowBorder"] = false,
				}, -- [1]
				{
					["text_text_format_p_time_format"] = 0,
					["text_text"] = "%c",
					["text_text_format_p_format"] = "timed",
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Friz Quadrata TT",
					["text_text_format_p_time_precision"] = 1,
					["text_shadowYOffset"] = 0,
					["text_text_format_p_time_dynamic_threshold"] = 60,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "INNER_BOTTOMRIGHT",
					["text_shadowXOffset"] = 0,
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_fontSize"] = 15,
					["anchorXOffset"] = 0,
					["text_visible"] = true,
				}, -- [2]
			},
			["height"] = 80,
			["load"] = {
				["ingroup"] = {
					["single"] = "group",
					["multi"] = {
						["group"] = true,
					},
				},
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["use_encounterid"] = false,
				["difficulty"] = {
					["multi"] = {
						["mythic"] = true,
						["challenge"] = true,
					},
				},
				["use_zone"] = false,
				["zoneIds"] = "1669",
				["use_zoneIds"] = true,
				["zone"] = "",
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["config"] = {
				["encounter"] = false,
				["noCommunication"] = false,
				["debug"] = false,
				["orderText"] = true,
			},
			["actions"] = {
				["start"] = {
				},
				["finish"] = {
				},
				["init"] = {
					["custom"] = "aura_env.debugmode = aura_env.config['debug']\nlocal debug = aura_env.debugmode\naura_env.showEncounter = aura_env.config['encounter']\naura_env.locale=GetLocale()\naura_env.mistZone = \"Mistveil Tangle\"\naura_env.order = \"\"\naura_env.showText = aura_env.config['orderText']\naura_env.noCommunication = aura_env.config['noCommunication']\nif aura_env.locale == \"koKR\" then\n    aura_env.mistZone = \"안개장막 덩굴숲\"\nelseif aura_env.locale == \"frFR\" then\n    aura_env.mistZone = \"Maquis Voile-de-Brume\"\nelseif aura_env.locale == \"deDE\" then\n    aura_env.mistZone = \"Nebelschleierdickicht\"\nelseif aura_env.locale == \"zhCN\" then\n    aura_env.mistZone = \"纱雾迷结\"\nelseif aura_env.locale == \"esES\" then\n    aura_env.mistZone = \"Espesura Velo de Niebla\"\nelseif aura_env.locale == \"zhTW\" then\n    aura_env.mistZone = \"霧紗密林\"\nelseif aura_env.locale == \"esMX\" then\n    aura_env.mistZone = \"Espesura Veloniebla\"\nelseif aura_env.locale == \"ruRU\" then\n    aura_env.mistZone = \"Туманная чащоба\"\nelseif aura_env.locale == \"ptBR\" then\n    aura_env.mistZone = \"Enleio do Véu da Névoa\"\nelseif aura_env.locale == \"itIT\" then\n    aura_env.mistZone = \"Intrico Velofosco\"\nelseif aura_env.locale == \"ptPT\" then\n    aura_env.mistZone = \"Enleio do Véu da Névoa\"        \nend\nlocal values = {1, 1, 1, 5}\nlocal message = string.format(\"%d %d %d %d\", values[1], values[2], values[3], values[4])\nlocal name, _ = UnitName(\"player\")\n\nif not aura_env.frameData then\n    local b = CreateFrame(\"Button\", nil, WeakAuras.regions[aura_env.id].region)\n    b:SetAllPoints(WeakAuras.regions[aura_env.id].region)\n    b:RegisterForClicks(\"LeftButtonDown\", \"RightButtonDown\")\n    aura_env.frameData = b\n    WeakAuras.ScanEvents(\"TirnaScitheButtonData\", 5, b)\nend\nif aura_env.noCommunication then\n    aura_env.frameData:SetScript(\"OnClick\", function(self, button)\n            if debug then print(\"Click Registered - true\") end\n            WeakAuras.ScanEvents(\"GlowUpdate\")\n            C_ChatInfo.RegisterAddonMessagePrefix(\"Tirna Ver2\") \n            if debug then \n                print(\"noCommunication sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"WHISPER\", name)) \n            else\n                C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"WHISPER\", name)\n            end\n            \n    end)\nelse\n    aura_env.frameData:SetScript(\"OnClick\", function(self, button)\n            if debug then print(\"Click Registered - true\") end\n            WeakAuras.ScanEvents(\"GlowUpdate\")\n            C_ChatInfo.RegisterAddonMessagePrefix(\"Tirna Ver2\")\n            if IsPartyLFG() then            \n                if debug then \n                    print(\"LFG sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"INSTANCE_CHAT\")) \n                else\n                    C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"INSTANCE_CHAT\")\n                end\n            elseif UnitInParty(\"player\") then            \n                if debug then \n                    print(\"party sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"PARTY\")) \n                else\n                    C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"PARTY\")\n                end\n            else         \n                if debug then \n                    print(\"solo sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"WHISPER\", name)) \n                else\n                    C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"WHISPER\", name)\n                end\n            end\n    end)\nend",
					["do_custom"] = true,
				},
			},
			["authorOptions"] = {
				{
					["type"] = "toggle",
					["key"] = "orderText",
					["default"] = true,
					["name"] = "Show Order Text",
					["useDesc"] = false,
					["width"] = 1,
				}, -- [1]
				{
					["type"] = "toggle",
					["key"] = "encounter",
					["default"] = false,
					["name"] = "Show during Boss",
					["useDesc"] = false,
					["width"] = 1,
				}, -- [2]
				{
					["type"] = "toggle",
					["key"] = "noCommunication",
					["default"] = false,
					["name"] = "Disable Cross-Communication with others running the WA",
					["useDesc"] = false,
					["width"] = 2,
				}, -- [3]
				{
					["type"] = "toggle",
					["key"] = "debug",
					["default"] = false,
					["name"] = "|cffff0000Debug testing|r (requires reload for full debug)",
					["useDesc"] = false,
					["width"] = 2,
				}, -- [4]
			},
			["regionType"] = "icon",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["cooldown"] = false,
			["conditions"] = {
			},
			["parent"] = "Mists of Tirna Scithe guessing game v2",
			["semver"] = "2.0.2",
			["anchorFrameType"] = "SCREEN",
			["zoom"] = 0,
			["auto"] = false,
			["tocversion"] = 90005,
			["id"] = "nocircle_leaf_nofill",
			["alpha"] = 1,
			["frameStrata"] = 1,
			["width"] = 80,
			["cooldownTextDisabled"] = false,
			["uid"] = "15JH7fjnk6w",
			["inverse"] = false,
			["selfPoint"] = "CENTER",
			["displayIcon"] = "Interface\\AddOns\\guess\\nocircle_leaf_nofill.tga",
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["url"] = "https://wago.io/hVLym_eLv/67",
		},
		["Mists of Tirna Scithe guessing game v2"] = {
			["grow"] = "GRID",
			["controlledChildren"] = {
				"circle_leaf_fill", -- [1]
				"circle_leaf_nofill", -- [2]
				"circle_lotus_fill", -- [3]
				"circle_lotus_nofill", -- [4]
				"nocircle_leaf_fill", -- [5]
				"nocircle_leaf_nofill", -- [6]
				"nocircle_noleaf_fill", -- [7]
				"nocircle_noleaf_nofill", -- [8]
				"Reset", -- [9]
				"Solution Display", -- [10]
			},
			["borderBackdrop"] = "Blizzard Tooltip",
			["xOffset"] = -1075.491430035845,
			["preferToUpdate"] = true,
			["yOffset"] = -310.9803443701997,
			["gridType"] = "RD",
			["borderColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["space"] = 2,
			["url"] = "https://wago.io/hVLym_eLv/67",
			["actions"] = {
				["start"] = {
				},
				["init"] = {
				},
				["finish"] = {
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["debuffType"] = "HELPFUL",
						["type"] = "aura2",
						["spellIds"] = {
						},
						["subeventSuffix"] = "_CAST_START",
						["subeventPrefix"] = "SPELL",
						["names"] = {
						},
						["event"] = "Health",
						["unit"] = "player",
					},
					["untrigger"] = {
					},
				}, -- [1]
			},
			["columnSpace"] = 2,
			["internalVersion"] = 45,
			["useLimit"] = false,
			["align"] = "CENTER",
			["desc"] = "Created by Saxayone#2791 EU Sylvanas",
			["stagger"] = 0,
			["version"] = 67,
			["subRegions"] = {
			},
			["uid"] = "YmFqaIu8E9g",
			["fullCircle"] = true,
			["load"] = {
				["talent"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["spec"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["radius"] = 200,
			["backdropColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0.5, -- [4]
			},
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["animate"] = false,
			["customGrow"] = "function(newPositions, activeRegions)\n    local newheight = 2\n    local offset = 0\n    for i = 1, 4 do\n        newPositions[i] = {\n            activeRegions[i][\"dimensions\"].width*(2.5-i),\n            0\n        }\n    end\n    for i = 5, 8 do\n        newPositions[i] = {\n            activeRegions[i][\"dimensions\"].width*(2.5-i+4),\n            activeRegions[i][\"dimensions\"].height*-1\n        }\n    end\n    \n    newPositions[9] = {\n        0,\n        (activeRegions[1][\"dimensions\"].height*-1.5)-(activeRegions[9][\"dimensions\"].height*0.5)\n    }\n    newPositions[10] = {\n        0,\n        (activeRegions[1][\"dimensions\"].height*-1.5)-(activeRegions[9][\"dimensions\"].height*1.5)\n    }\n    newPositions[11] = {\n        0,\n        (activeRegions[1][\"dimensions\"].height*-1.5)-(activeRegions[9][\"dimensions\"].height*1.5)-(activeRegions[10][\"dimensions\"].height*1.5)\n    }\n    \nend",
			["scale"] = 0.85,
			["authorOptions"] = {
			},
			["border"] = false,
			["borderEdge"] = "Square Full White",
			["regionType"] = "dynamicgroup",
			["borderSize"] = 2,
			["sort"] = "none",
			["anchorPoint"] = "TOP",
			["rotation"] = 0,
			["constantFactor"] = "RADIUS",
			["selfPoint"] = "TOPLEFT",
			["borderOffset"] = 4,
			["semver"] = "2.0.2",
			["tocversion"] = 90002,
			["id"] = "Mists of Tirna Scithe guessing game v2",
			["frameStrata"] = 1,
			["gridWidth"] = 4,
			["anchorFrameType"] = "SCREEN",
			["limit"] = 5,
			["borderInset"] = 1,
			["rowSpace"] = 2,
			["config"] = {
			},
			["conditions"] = {
			},
			["information"] = {
			},
			["arcLength"] = 360,
		},
		["Essence Sap"] = {
			["overlays"] = {
				{
					0.30980392156863, -- [1]
					0.91372549019608, -- [2]
					1, -- [3]
					1, -- [4]
				}, -- [1]
			},
			["sparkOffsetX"] = 0,
			["authorOptions"] = {
			},
			["preferToUpdate"] = false,
			["customText"] = "function()\n    if WeakAuras.IsOptionsOpen() then\n        return WA_ClassColorName(UnitName(\"player\"))\n    end\n    \n    if aura_env and aura_env.state and aura_env.state.name then\n        return WA_ClassColorName(Ambiguate(aura_env.state.name, \"short\"))\n    end\nend",
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["sparkRotation"] = 0,
			["sparkRotationMode"] = "AUTO",
			["url"] = "https://wago.io/PescorusMiasma/9",
			["icon"] = true,
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "custom",
						["subeventSuffix"] = "_CAST_START",
						["customVariables"] = "{\n    expirationTime = true,\n    stacks = \"number\",\n    dead = \"bool\"\n}",
						["event"] = "Health",
						["unit"] = "player",
						["subeventPrefix"] = "SPELL",
						["spellIds"] = {
						},
						["events"] = "ENCOUNTER_START, CLEU:SPELL_AURA_APPLIED:SPELL_AURA_APPLIED_DOSE:SPELL_AURA_REMOVED:UNIT_DIED, ACCEPT_RES",
						["names"] = {
						},
						["check"] = "event",
						["custom"] = "function(s, event, ...)\n    if event == \"ENCOUNTER_START\" then\n        aura_env.soakers = {}\n        \n        if IsAddOnLoaded(\"ExRT\")  and _G.VExRT.Note.Text1 then\n            local text = _G.VExRT.Note.Text1\n            \n            for line in text:gmatch('[^\\r\\n]+') do\n                local chunks = {}\n                for substring in line:gmatch(\"%S+\") do table.insert(chunks, substring) end\n                \n                if chunks[1] == \"Miasma\" and line:lower():match(UnitName(\"player\"):lower()) then\n                    for unit in WA_IterateGroupMembers() do\n                        local name, realm = UnitName(unit) \n                        \n                        if line:lower():match(name:lower()) then\n                            if realm then name = name .. \"-\" .. realm end -- Use Name-Realm to match CLEU destName\n                            aura_env.soakers[name] = true \n                            \n                            s[name] = {\n                                show = true,\n                                changed = true,\n                                progressType = \"timed\",\n                                duration = 0,\n                                expirationTime = 0,\n                                stacks = 0,\n                                name = name,\n                                dead = false,\n                                autoHide = false\n                            }\n                        end\n                    end\n                end\n            end\n        end\n        \n        return true\n    elseif event == \"COMBAT_LOG_EVENT_UNFILTERED\" and ... then\n        local _, subEvent, _, _, _, _, _, _, destName, _, _, spellId = ...\n        if not aura_env.soakers[destName] then return false end\n        \n        if subEvent == \"SPELL_AURA_APPLIED\" then\n            if spellId == 329298 then -- Miasma applied\n                if s[destName] then\n                    s[destName].show = false\n                    s[destName].dead = false\n                    s[destName].changed = true\n                end\n            elseif spellId == 334755 then -- Essence Sap applied\n                if s[destName] then\n                    s[destName].duration = 20\n                    s[destName].expirationTime = GetTime() + 20\n                    s[destName].stacks = 1\n                    s[destName].dead = false\n                    s[destName].changed = true\n                end\n            end\n        elseif subEvent == \"SPELL_AURA_APPLIED_DOSE\" then\n            if spellId == 334755 then -- Essence Sap stack applied\n                if s[destName] then\n                    s[destName].duration = 20\n                    s[destName].expirationTime = GetTime() + 20\n                    s[destName].stacks = s[destName].stacks + 1\n                    s[destName].dead = false\n                    s[destName].changed = true\n                end\n            end\n        elseif subEvent == \"SPELL_AURA_REMOVED\" then\n            if spellId == 329298 then -- Miasma removed\n                local _, _, count, _, _, expirationTime = WA_GetUnitDebuff(destName, 334755)\n                \n                s[destName] = {\n                    show = true,\n                    changed = true,\n                    progressType = \"timed\",\n                    duration = count and 20 or 0,\n                    expirationTime = expirationTime or 0,\n                    stacks = count or 0,\n                    name = destName,\n                    autoHide = false\n                }\n            elseif spellId == 334755 then -- Essence Sap removed\n                if s[destName] then\n                    s[destName].duration = 0\n                    s[destName].expirationTime = 0\n                    s[destName].stacks = 0\n                    s[destName].changed = true\n                end\n            elseif spellId == 160029 then -- Resurrecting\n                -- Just to be sure that the debuff faded because the unit accepted the res (and not canceled)\n                C_Timer.After(0.1, function() WeakAuras.ScanEvents(\"ACCEPT_RES\") end)\n            end\n        elseif subEvent == \"UNIT_DIED\" then\n            s[destName] = {\n                show = true,\n                changed = true,\n                progressType = \"timed\",\n                duration = 0,\n                expirationTime = 0,\n                stacks = 0,\n                name = destName,\n                dead = true,\n                autoHide = false\n            }\n        end\n        \n        return true\n    elseif event == \"ACCEPT_RES\" then\n        for name, state in pairs(s) do\n            if UnitExists(name) then\n                state.dead = UnitIsDeadOrGhost(name)\n                state.changed = true\n            end\n        end\n        \n        return true\n    end\n    \n    return false\nend\n\n\n",
						["custom_type"] = "stateupdate",
						["debuffType"] = "HELPFUL",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["icon_color"] = {
				0.52941176470588, -- [1]
				0.52941176470588, -- [2]
				0.52941176470588, -- [3]
				1, -- [4]
			},
			["internalVersion"] = 45,
			["selfPoint"] = "CENTER",
			["sparkHidden"] = "NEVER",
			["barColor"] = {
				0.22745098039216, -- [1]
				0.78823529411765, -- [2]
				0.69019607843137, -- [3]
				1, -- [4]
			},
			["desaturate"] = false,
			["parent"] = "Miasma soak group",
			["sparkOffsetY"] = 0,
			["subRegions"] = {
				{
					["type"] = "aurabar_bar",
				}, -- [1]
				{
					["text_text_format_p_time_precision"] = 1,
					["text_text"] = "%c1",
					["text_text_format_p_format"] = "timed",
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["text_text_format_mark_format"] = "none",
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["text_text_format_p_time_format"] = 0,
					["text_text_format_p_time_dynamic_threshold"] = 60,
					["type"] = "subtext",
					["text_anchorXOffset"] = 2,
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "PT Sans Narrow",
					["text_shadowXOffset"] = 1,
					["text_shadowYOffset"] = -1,
					["text_text_format_c2_format"] = "none",
					["text_wordWrap"] = "WordWrap",
					["text_visible"] = true,
					["text_text_format_c1_format"] = "none",
					["text_anchorPoint"] = "INNER_LEFT",
					["text_fontType"] = "None",
					["text_fontSize"] = 16,
					["anchorXOffset"] = 0,
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
				}, -- [2]
				{
					["text_text_format_p_time_precision"] = 1,
					["text_text_format_p_big_number_format"] = "AbbreviateNumbers",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["text_text_format_p_time_format"] = 0,
					["type"] = "subtext",
					["text_text_format_p_time_dynamic_threshold"] = 3,
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "PT Sans Narrow",
					["text_text_format_n_format"] = "none",
					["text_shadowYOffset"] = -1,
					["text_shadowXOffset"] = 1,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "None",
					["text_anchorPoint"] = "INNER_RIGHT",
					["text_visible"] = true,
					["text_text"] = "%p",
					["text_fontSize"] = 16,
					["anchorXOffset"] = 0,
					["text_text_format_p_format"] = "timed",
				}, -- [3]
				{
					["text_text_format_p_time_precision"] = 1,
					["text_text_format_s_format"] = "none",
					["text_text_format_p_big_number_format"] = "AbbreviateNumbers",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["text_text_format_stacks_format"] = "none",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "PT Sans Narrow",
					["text_text_format_p_format"] = "BigNumber",
					["text_shadowYOffset"] = -1,
					["text_fontType"] = "None",
					["text_wordWrap"] = "WordWrap",
					["text_visible"] = true,
					["text_anchorPoint"] = "ICON_CENTER",
					["text_text"] = "%s",
					["text_text_format_n_format"] = "none",
					["text_fontSize"] = 16,
					["anchorXOffset"] = 0,
					["text_shadowXOffset"] = 1,
				}, -- [4]
				{
					["type"] = "subborder",
					["border_anchor"] = "bar",
					["border_offset"] = 1,
					["border_color"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["border_visible"] = true,
					["border_edge"] = "Square Full White",
					["border_size"] = 1,
				}, -- [5]
				{
					["type"] = "subborder",
					["border_anchor"] = "icon",
					["border_offset"] = 1,
					["border_color"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["border_visible"] = true,
					["border_edge"] = "Square Full White",
					["border_size"] = 1,
				}, -- [6]
			},
			["height"] = 24,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["load"] = {
				["difficulty"] = {
				},
				["use_encounterid"] = true,
				["zoneIds"] = "",
				["talent"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2383",
				["class"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["sparkBlendMode"] = "ADD",
			["useAdjustededMax"] = false,
			["displayIcon"] = "136158",
			["backgroundColor"] = {
				0.03921568627451, -- [1]
				0.13725490196078, -- [2]
				0.12156862745098, -- [3]
				1, -- [4]
			},
			["version"] = 9,
			["iconSource"] = 0,
			["sparkColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["uid"] = "XhV6xndYAGp",
			["useAdjustededMin"] = false,
			["regionType"] = "aurabar",
			["width"] = 240,
			["alpha"] = 1,
			["icon_side"] = "LEFT",
			["sparkHeight"] = 30,
			["overlayclip"] = false,
			["texture"] = "Smooth v2",
			["spark"] = false,
			["zoom"] = 0.3,
			["semver"] = "1.0.8",
			["tocversion"] = 90002,
			["id"] = "Essence Sap",
			["sparkTexture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["sparkWidth"] = 10,
			["config"] = {
			},
			["inverse"] = false,
			["xOffset"] = 0,
			["orientation"] = "HORIZONTAL",
			["conditions"] = {
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = ">",
						["variable"] = "stacks",
						["value"] = "10",
					},
					["changes"] = {
						{
							["value"] = {
								1, -- [1]
								0, -- [2]
								0, -- [3]
								1, -- [4]
							},
							["property"] = "sub.4.text_color",
						}, -- [1]
					},
				}, -- [1]
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["variable"] = "stacks",
						["value"] = "0",
					},
					["changes"] = {
						{
							["value"] = true,
							["property"] = "desaturate",
						}, -- [1]
						{
							["value"] = {
								0.2156862745098, -- [1]
								0.2156862745098, -- [2]
								0.2156862745098, -- [3]
								1, -- [4]
							},
							["property"] = "backgroundColor",
						}, -- [2]
					},
				}, -- [2]
				{
					["check"] = {
						["trigger"] = 1,
						["variable"] = "dead",
						["value"] = 1,
					},
					["changes"] = {
						{
							["value"] = 0.3,
							["property"] = "alpha",
						}, -- [1]
					},
				}, -- [3]
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = ">",
						["variable"] = "expirationTime",
						["value"] = "18.4",
					},
					["changes"] = {
						{
							["value"] = {
								1, -- [1]
								0.52156862745098, -- [2]
								0.88627450980392, -- [3]
								1, -- [4]
							},
							["property"] = "barColor",
						}, -- [1]
						{
							["value"] = {
								0.15294117647059, -- [1]
								0.082352941176471, -- [2]
								0.13725490196078, -- [3]
								1, -- [4]
							},
							["property"] = "backgroundColor",
						}, -- [2]
					},
				}, -- [4]
			},
			["information"] = {
			},
			["actions"] = {
				["start"] = {
				},
				["init"] = {
					["custom"] = "\n\n",
					["do_custom"] = false,
				},
				["finish"] = {
				},
			},
		},
		["09 Generals Soak Alerts"] = {
			["controlledChildren"] = {
				"Cast Targeted Alert", -- [1]
				"Soak Alert", -- [2]
				"Backup Alert", -- [3]
				"Glow Handler", -- [4]
				"No Backup Warning", -- [5]
			},
			["borderBackdrop"] = "Blizzard Tooltip",
			["authorOptions"] = {
			},
			["preferToUpdate"] = false,
			["yOffset"] = 142.00024414063,
			["anchorPoint"] = "CENTER",
			["borderColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["url"] = "https://wago.io/kJFQFy0V8/3",
			["actions"] = {
				["start"] = {
				},
				["finish"] = {
				},
				["init"] = {
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["unit"] = "player",
						["type"] = "aura2",
						["spellIds"] = {
						},
						["subeventSuffix"] = "_CAST_START",
						["subeventPrefix"] = "SPELL",
						["names"] = {
						},
						["event"] = "Health",
						["debuffType"] = "HELPFUL",
					},
					["untrigger"] = {
					},
				}, -- [1]
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["version"] = 3,
			["subRegions"] = {
			},
			["load"] = {
				["talent"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["spec"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["backdropColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0.5, -- [4]
			},
			["scale"] = 1,
			["border"] = false,
			["borderEdge"] = "Square Full White",
			["regionType"] = "group",
			["borderSize"] = 2,
			["borderOffset"] = 4,
			["semver"] = "1.0.2",
			["tocversion"] = 90002,
			["id"] = "09 Generals Soak Alerts",
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["xOffset"] = 0,
			["borderInset"] = 1,
			["config"] = {
			},
			["selfPoint"] = "CENTER",
			["conditions"] = {
			},
			["information"] = {
			},
			["uid"] = "ooLb9W9epIa",
		},
		["04 Glyph of Destruction - Yellow"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 3,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						0.98039215686275, -- [1]
						1, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.25,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = true,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2405",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "gdRn48EVZj1",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["useMatch_count"] = true,
						["auranames"] = {
						},
						["groupclone"] = true,
						["matchesShowOn"] = "showOnActive",
						["name_info"] = "aura",
						["unit"] = "group",
						["use_tooltip"] = false,
						["subeventPrefix"] = "SPELL",
						["match_countOperator"] = ">=",
						["match_count"] = "1",
						["buffShowOn"] = "showOnActive",
						["unevent"] = "timed",
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["useExactSpellId"] = true,
						["custom_type"] = "stateupdate",
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["event"] = "Health",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HARMFUL",
						["use_specific_unit"] = false,
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["auraspellids"] = {
							"325236", -- [1]
						},
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["duration"] = "1",
						["useGroup_count"] = false,
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["wordWrap"] = "WordWrap",
			["config"] = {
			},
			["text1"] = "%n",
			["text1Containment"] = "INSIDE",
			["width"] = 6.9999866485596,
			["justify"] = "LEFT",
			["semver"] = "1.0.26",
			["zoom"] = 0,
			["id"] = "04 Glyph of Destruction - Yellow",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["frameStrata"] = 9,
			["anchorFrameType"] = "SCREEN",
			["text1Point"] = "CENTER",
			["text2Font"] = "Friz Quadrata TT",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["Council of Blood - Dreadbolt Volley Cast 2"] = {
			["sparkWidth"] = 10,
			["sparkOffsetX"] = 0,
			["authorOptions"] = {
			},
			["preferToUpdate"] = false,
			["yOffset"] = 238.33312988281,
			["anchorPoint"] = "CENTER",
			["sparkRotation"] = 0,
			["sparkRotationMode"] = "AUTO",
			["url"] = "https://wago.io/VqAXD3338/1",
			["actions"] = {
				["start"] = {
					["do_sound"] = false,
				},
				["finish"] = {
				},
				["init"] = {
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["spellId"] = "328789",
						["duration"] = "3",
						["use_spell"] = true,
						["subeventPrefix"] = "SPELL",
						["use_unit"] = true,
						["names"] = {
						},
						["use_specific_unit"] = true,
						["spellName"] = "Dreadbolt Volley",
						["auraspellids"] = {
							"329834", -- [1]
						},
						["spell"] = "Dreadbolt Volley",
						["useExactSpellId"] = true,
						["unevent"] = "auto",
						["subeventSuffix"] = "_CAST_START",
						["use_spellName"] = true,
						["event"] = "Cast",
						["type"] = "unit",
						["realSpellName"] = "Dreadbolt Volley",
						["use_spellId"] = false,
						["spellIds"] = {
						},
						["use_genericShowOn"] = true,
						["genericShowOn"] = "showOnCooldown",
						["debuffType"] = "HELPFUL",
						["use_track"] = true,
						["unit"] = "boss2",
					},
					["untrigger"] = {
						["use_specific_unit"] = true,
						["unit"] = "boss2",
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["icon_color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["internalVersion"] = 45,
			["selfPoint"] = "CENTER",
			["barColor"] = {
				0.5529411764705883, -- [1]
				0.09019607843137255, -- [2]
				0.09803921568627451, -- [3]
				1, -- [4]
			},
			["desaturate"] = false,
			["version"] = 1,
			["subRegions"] = {
				{
					["type"] = "aurabar_bar",
				}, -- [1]
				{
					["text_text_format_n_format"] = "none",
					["text_text"] = "Dreadbolt Volley",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Accidental Presidency",
					["text_shadowYOffset"] = -1,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "None",
					["text_anchorPoint"] = "INNER_RIGHT",
					["text_shadowXOffset"] = 1,
					["text_fontSize"] = 25,
					["anchorXOffset"] = 0,
					["text_visible"] = true,
				}, -- [2]
				{
					["text_text_format_p_time_precision"] = 1,
					["text_text_format_s_format"] = "none",
					["text_text"] = "%p",
					["text_text_format_p_format"] = "timed",
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Accidental Presidency",
					["text_text_format_p_time_format"] = 0,
					["text_shadowYOffset"] = -1,
					["text_text_format_p_time_dynamic_threshold"] = 60,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "None",
					["text_anchorPoint"] = "INNER_LEFT",
					["text_visible"] = true,
					["text_shadowXOffset"] = 1,
					["text_fontSize"] = 25,
					["anchorXOffset"] = 0,
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
				}, -- [3]
				{
					["type"] = "subborder",
					["border_anchor"] = "bar",
					["border_size"] = 2,
					["border_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["border_visible"] = true,
					["border_edge"] = "Square Full White",
					["border_offset"] = 0,
				}, -- [4]
			},
			["height"] = 43,
			["anchorFrameType"] = "SCREEN",
			["load"] = {
				["use_encounterid"] = true,
				["zoneIds"] = "",
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2412",
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["sparkBlendMode"] = "ADD",
			["useAdjustededMax"] = false,
			["icon"] = false,
			["sparkOffsetY"] = 0,
			["backgroundColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				0.5, -- [4]
			},
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["iconSource"] = -1,
			["useAdjustededMin"] = false,
			["regionType"] = "aurabar",
			["config"] = {
			},
			["uid"] = "C7nMd4K(q7w",
			["icon_side"] = "RIGHT",
			["sparkHidden"] = "NEVER",
			["sparkHeight"] = 30,
			["texture"] = "Solid",
			["alpha"] = 1,
			["zoom"] = 0,
			["semver"] = "1.0.0",
			["tocversion"] = 90002,
			["id"] = "Council of Blood - Dreadbolt Volley Cast 2",
			["spark"] = false,
			["frameStrata"] = 1,
			["width"] = 360,
			["sparkTexture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
			["sparkColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["inverse"] = false,
			["xOffset"] = -457.50018310547,
			["orientation"] = "HORIZONTAL",
			["conditions"] = {
			},
			["information"] = {
			},
			["parent"] = "Council of Blood - Dreadbolt Volley - Cast Bar",
		},
		["09 Generals Orb"] = {
			["sparkWidth"] = 10,
			["sparkOffsetX"] = 0,
			["xOffset"] = 0,
			["preferToUpdate"] = true,
			["customText"] = "function()\n    if aura_env.state and aura_env.state.unit then\n        return WA_ClassColorName(aura_env.state.unit)\n    end\nend",
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["spark"] = false,
			["cooldownSwipe"] = true,
			["sparkRotation"] = 0,
			["sparkRotationMode"] = "AUTO",
			["url"] = "https://wago.io/cUe1XBhBn/7",
			["icon"] = false,
			["triggers"] = {
				{
					["trigger"] = {
						["useMatch_count"] = true,
						["spellId"] = "328731",
						["duration"] = "20",
						["names"] = {
						},
						["unit"] = "group",
						["match_count"] = "1",
						["subeventSuffix"] = "_CAST_SUCCESS",
						["use_cloneId"] = true,
						["debuffType"] = "HARMFUL",
						["showClones"] = true,
						["type"] = "custom",
						["useExactSpellId"] = true,
						["custom_type"] = "stateupdate",
						["custom_hide"] = "timed",
						["unevent"] = "timed",
						["event"] = "Combat Log",
						["spellIds"] = {
						},
						["auraspellids"] = {
							"342698", -- [1]
							"342655", -- [2]
						},
						["use_spellId"] = true,
						["custom"] = "function(states, event, timestamp, subevent, hideCaster, sourceGUID, sourceName, sourceFlags, sourceRaidFlags, destGUID, destName, destFlags, destRaidFlags, spellID, ...)\n    if subevent == \"SPELL_AURA_APPLIED\" then\n        if aura_env.spells[spellID] then\n            states[destName..spellID] = {\n                show = true,\n                changed = true,\n                progressType = \"timed\",\n                duration = 20,\n                expirationTime = GetTime()+20,\n                index = GetTime()+20,\n                autoHide = true,\n                unit = destName,\n                glow = aura_env.spells[spellID],\n                wicked = WA_GetUnitAura(destName, aura_env.wicked, aura_env.debug),\n                me = UnitName(\"player\") == destName,\n            }\n            return true\n        elseif spellID == aura_env.wicked then\n            local update = false\n            for _, s in pairs(states) do\n                if s.unit == destName then\n                    s.changed = true\n                    s.wicked = true\n                    update = true\n                end \n            end\n            return update\n        end\n        \n    elseif subevent == \"SPELL_AURA_REMOVED\" then\n        if aura_env.spells[spellID] then\n            if states[destName..spellID] then\n                states[destName..spellID].show = false\n                states[destName..spellID].changed = true    \n                return true\n            end\n        elseif spellID == aura_env.wicked then\n            local update = false\n            for _, s in pairs(states) do\n                if s.unit == destName then\n                    s.changed = true\n                    s.wicked = false\n                    update = true\n                end \n            end\n            return update\n        end\n    end\nend",
						["events"] = "CLEU:SPELL_AURA_APPLIED:SPELL_AURA_REMOVED",
						["check"] = "event",
						["match_countOperator"] = ">=",
						["subeventPrefix"] = "SPELL",
						["customVariables"] = "{\n    glow = \"number\",\n    wicked = \"bool\",\n    me = \"bool\"\n}",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["icon_color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["internalVersion"] = 45,
			["keepAspectRatio"] = false,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["iconSource"] = -1,
			["barColor"] = {
				1, -- [1]
				0.30588235294118, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["desaturate"] = false,
			["information"] = {
			},
			["conditions"] = {
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = "==",
						["value"] = 1,
						["variable"] = "me",
					},
					["changes"] = {
						{
							["value"] = {
								0, -- [1]
								1, -- [2]
								0.94117647058824, -- [3]
								1, -- [4]
							},
							["property"] = "barColor",
						}, -- [1]
					},
				}, -- [1]
				{
					["check"] = {
						["trigger"] = 1,
						["op"] = ">=",
						["value"] = "30",
						["variable"] = "glow",
					},
					["changes"] = {
						{
							["value"] = true,
							["property"] = "sub.4.glow",
						}, -- [1]
					},
				}, -- [2]
				{
					["check"] = {
						["trigger"] = 1,
						["variable"] = "wicked",
						["value"] = 1,
					},
					["changes"] = {
						{
							["value"] = {
								0.36078431372549, -- [1]
								0.36078431372549, -- [2]
								0.36078431372549, -- [3]
								1, -- [4]
							},
							["property"] = "barColor",
						}, -- [1]
					},
				}, -- [3]
			},
			["version"] = 7,
			["subRegions"] = {
				{
					["type"] = "aurabar_bar",
				}, -- [1]
				{
					["text_text_format_p_time_precision"] = 1,
					["text_text_format_s_format"] = "none",
					["text_text"] = "%p",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["text_text_format_p_decimal_precision"] = 0,
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Expressway",
					["text_shadowYOffset"] = 0,
					["text_shadowXOffset"] = 0,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "INNER_RIGHT",
					["text_text_format_p_format"] = "Number",
					["text_visible"] = true,
					["text_fontSize"] = 18,
					["anchorXOffset"] = 0,
					["text_text_format_p_round_type"] = "floor",
				}, -- [2]
				{
					["text_text_format_n_format"] = "none",
					["text_text"] = "%c",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Expressway",
					["text_shadowYOffset"] = -1,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "INNER_LEFT",
					["text_shadowXOffset"] = 1,
					["text_fontSize"] = 12,
					["anchorXOffset"] = 0,
					["text_visible"] = true,
				}, -- [3]
				{
					["glowFrequency"] = 0.25,
					["type"] = "subglow",
					["useGlowColor"] = false,
					["glowType"] = "Pixel",
					["glowThickness"] = 5,
					["glowYOffset"] = 0,
					["glowColor"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["glowXOffset"] = 0,
					["glowScale"] = 1,
					["glow"] = false,
					["glow_anchor"] = "bar",
					["glowLength"] = 10,
					["glowLines"] = 8,
					["glowBorder"] = false,
				}, -- [4]
				{
					["type"] = "subborder",
					["border_anchor"] = "bar",
					["border_size"] = 2,
					["border_color"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["border_visible"] = true,
					["border_edge"] = "Square Full White",
					["border_offset"] = 0,
				}, -- [5]
			},
			["height"] = 40,
			["parent"] = "09 Generals Orbs & Mana List",
			["load"] = {
				["use_encounterid"] = true,
				["zoneIds"] = "",
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2417",
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["sparkBlendMode"] = "ADD",
			["useAdjustededMax"] = false,
			["selfPoint"] = "CENTER",
			["cooldownEdge"] = false,
			["config"] = {
				["debug"] = false,
			},
			["actions"] = {
				["start"] = {
				},
				["finish"] = {
				},
				["init"] = {
					["custom"] = "aura_env.spells = {\n    [342655] = 10,\n    [342698] = 30,\n}\naura_env.wicked = 333377\n\nif aura_env.config[\"debug\"] then\n    aura_env.spells[774] = 10\n    aura_env.spells[8936] = 30\n    aura_env.wicked = 24858\nend\n\n\n",
					["do_custom"] = true,
				},
			},
			["anchorFrameType"] = "SCREEN",
			["alpha"] = 1,
			["useAdjustededMin"] = false,
			["regionType"] = "aurabar",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["id"] = "09 Generals Orb",
			["icon_side"] = "RIGHT",
			["sparkTexture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
			["sparkHeight"] = 30,
			["texture"] = "Details Flat",
			["cooldownTextDisabled"] = false,
			["zoom"] = 0.3,
			["semver"] = "1.0.6",
			["tocversion"] = 90002,
			["sparkHidden"] = "NEVER",
			["uid"] = "dT4RkOKDSOO",
			["frameStrata"] = 1,
			["width"] = 200,
			["sparkOffsetY"] = 0,
			["sparkColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["inverse"] = false,
			["authorOptions"] = {
				{
					["type"] = "toggle",
					["key"] = "debug",
					["default"] = false,
					["name"] = "Debug Mode",
					["useDesc"] = false,
					["width"] = 1,
				}, -- [1]
			},
			["orientation"] = "HORIZONTAL",
			["displayIcon"] = 628267,
			["cooldown"] = true,
			["backgroundColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				0.5, -- [4]
			},
		},
		["Great Vault on Weekly Reward frame"] = {
			["outline"] = "OUTLINE",
			["xOffset"] = 0,
			["displayText_format_p_time_dynamic_threshold"] = 60,
			["shadowYOffset"] = -1,
			["anchorPoint"] = "CENTER",
			["displayText_format_p_time_format"] = 0,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["actions"] = {
				["start"] = {
				},
				["init"] = {
					["do_custom"] = false,
				},
				["finish"] = {
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "custom",
						["subeventSuffix"] = "_CAST_START",
						["event"] = "Health",
						["unit"] = "player",
						["custom_hide"] = "timed",
						["spellIds"] = {
						},
						["events"] = "GLOBAL_MOUSE_UP",
						["subeventPrefix"] = "SPELL",
						["names"] = {
						},
						["custom_type"] = "event",
						["custom"] = "function(event)\n    if PVPQueueFrame then\n        aura_env.RatedPvP = PVPQueueFrame.HonorInset.RatedPanel.WeeklyChest\n        aura_env.CasualPvP = PVPQueueFrame.HonorInset.CasualPanel.WeeklyChest\n        end\n    if ChallengesFrame then\n        aura_env.ChallengeKey = ChallengesFrame.WeeklyInfo.Child.WeeklyChest\n    end\n    LoadAddOn('Blizzard_WeeklyRewards')\n    local GreatVaultFrame = \"WeeklyRewardsFrame\" -- frame name to reference when adding to UISpecialFrames\n    local RPvP = aura_env.RatedPvP\n    local CPvP = aura_env.CasualPvP\n    local CKey = aura_env.ChallengeKey\n    if event == \"GLOBAL_MOUSE_UP\" then\n        if not WeeklyRewardsFrame:IsVisible() and\n        ((RPvP and RPvP:IsVisible() and RPvP:IsMouseOver()) or\n            (CPvP and CPvP:IsVisible() and CPvP:IsMouseOver()) or\n            (CKey and CKey:IsVisible() and CKey:IsMouseOver())) then\n            WeeklyRewardsFrame:Show()\n            tinsert(UISpecialFrames, GreatVaultFrame) -- UISpecialFrames | see more at https://wow.gamepedia.com/Make_frames_closable_with_the_Escape_key\n        elseif WeeklyRewardsFrame:IsVisible() and\n        ((RPvP and RPvP:IsVisible() and RPvP:IsMouseOver()) or\n            (CPvP and CPvP:IsVisible() and CPvP:IsMouseOver()) or\n            (CKey and CKey:IsVisible() and CKey:IsMouseOver())) then\n            WeeklyRewardsFrame:Hide()\n        end\n    end\nend",
						["debuffType"] = "HELPFUL",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["displayText_format_p_format"] = "timed",
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["font"] = "Friz Quadrata TT",
			["version"] = 10,
			["subRegions"] = {
			},
			["load"] = {
				["size"] = {
					["multi"] = {
					},
				},
				["level_operator"] = ">=",
				["use_level"] = true,
				["talent"] = {
					["multi"] = {
					},
				},
				["level"] = "60",
				["class"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
			},
			["fontSize"] = 12,
			["shadowXOffset"] = 1,
			["regionType"] = "text",
			["fixedWidth"] = 200,
			["preferToUpdate"] = false,
			["url"] = "https://wago.io/8VoPnmm5Z/10",
			["displayText_format_p_time_precision"] = 1,
			["selfPoint"] = "BOTTOM",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["displayText"] = "",
			["justify"] = "LEFT",
			["tocversion"] = 90005,
			["id"] = "Great Vault on Weekly Reward frame",
			["uid"] = "eXtQuDwDOsa",
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["authorOptions"] = {
			},
			["config"] = {
			},
			["semver"] = "1.0.9",
			["yOffset"] = 0,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
			},
			["wordWrap"] = "WordWrap",
		},
		["Sound when targeted by a boss"] = {
			["text2Point"] = "CENTER",
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -40,
			["anchorPoint"] = "TOP",
			["url"] = "https://wago.io/BFADungeonTargetedSpells/68",
			["icon"] = true,
			["text1Enabled"] = true,
			["keepAspectRatio"] = false,
			["wordWrap"] = "WordWrap",
			["desaturate"] = false,
			["glowColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["ingroup"] = {
					["multi"] = {
					},
				},
				["use_never"] = false,
				["class"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
						["HEALER"] = true,
						["DAMAGER"] = true,
					},
				},
				["size"] = {
					["multi"] = {
						["scenario"] = true,
						["ten"] = true,
						["twentyfive"] = true,
						["fortyman"] = true,
						["ratedpvp"] = true,
						["flexible"] = true,
						["ratedarena"] = true,
						["pvp"] = true,
						["arena"] = true,
						["party"] = true,
						["none"] = true,
					},
				},
				["use_size"] = false,
				["affixes"] = {
					["multi"] = {
					},
				},
				["talent"] = {
					["multi"] = {
					},
				},
				["use_role"] = false,
				["spec"] = {
					["multi"] = {
					},
				},
				["difficulty"] = {
					["multi"] = {
						["challenge"] = true,
						["heroic"] = true,
						["lfr"] = true,
						["timewalking"] = true,
						["none"] = true,
					},
				},
				["zoneIds"] = "",
				["talent3"] = {
					["multi"] = {
					},
				},
				["faction"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_encounter"] = true,
				["race"] = {
					["multi"] = {
					},
				},
			},
			["glowType"] = "buttonOverlay",
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["cooldownTextDisabled"] = false,
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["config"] = {
			},
			["displayIcon"] = 134427,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n-- Do not remove this comment, it is part of this trigger: Sound when a boss target you",
			["shadowYOffset"] = -1,
			["cooldownSwipe"] = true,
			["customTextUpdate"] = "update",
			["automaticWidth"] = "Auto",
			["useglowColor"] = false,
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeType"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["preset"] = "spiral",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 68,
			["height"] = 102.63238525391,
			["preferToUpdate"] = true,
			["text2Font"] = "Friz Quadrata TT",
			["alpha"] = 1,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["cooldownEdge"] = false,
			["glow"] = false,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["anchorFrameFrame"] = "UIParent",
			["uid"] = "VCMkmT9LnOU",
			["width"] = 100.44454956055,
			["xOffset"] = 0,
			["triggers"] = {
				{
					["trigger"] = {
						["use_status"] = true,
						["duration"] = "0.1",
						["genericShowOn"] = "showOnActive",
						["names"] = {
						},
						["status"] = 2,
						["debuffType"] = "HELPFUL",
						["type"] = "custom",
						["unit"] = "player",
						["custom_type"] = "stateupdate",
						["custom_hide"] = "custom",
						["unevent"] = "auto",
						["threatUnit"] = "target",
						["event"] = "Threat Situation",
						["use_threatUnit"] = true,
						["spellIds"] = {
						},
						["custom"] = "function(states, event, unit)\n    if unit == \"boss1\"\n    or unit == \"boss2\"\n    or unit == \"boss3\"\n    or unit == \"boss4\"\n    or unit == \"boss5\"\n    then\n        if UnitIsUnit(\"player\", unit..\"target\") then\n            states[\"\"] = {\n                show = true,\n                changed = true,\n            }\n        else\n            local state = states[\"\"]\n            if state then\n                state.show = false\n                state.changed = true \n            end\n        end\n    end\n    return true\nend",
						["events"] = "UNIT_TARGET:boss",
						["check"] = "event",
						["subeventPrefix"] = "SPELL",
						["use_unit"] = true,
						["subeventSuffix"] = "_CAST_START",
					},
					["untrigger"] = {
						["threatUnit"] = "target",
						["custom"] = "function(event, unit)\n    if aura_env.unit and aura_env.unit == unit then\n        return not UnitIsUnit(\"player\", unit..\"target\")\n    end    \nend",
					},
				}, -- [1]
				["activeTriggerMode"] = 1,
			},
			["fixedWidth"] = 200,
			["text1"] = " ",
			["anchorFrameParent"] = false,
			["justify"] = "LEFT",
			["text1Point"] = "BOTTOMRIGHT",
			["semver"] = "3.0.6",
			["zoom"] = 0,
			["id"] = "Sound when targeted by a boss",
			["text2"] = "%p",
			["frameStrata"] = 1,
			["anchorFrameType"] = "SELECTFRAME",
			["actions"] = {
				["start"] = {
					["message"] = "Saw Dingo is forever.",
					["do_sound"] = true,
					["message_type"] = "SAY",
					["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\RobotBlip.ogg",
					["do_message"] = false,
				},
				["finish"] = {
				},
				["init"] = {
					["custom"] = "\n-- Do not remove this comment, it is part of this trigger: Sound when a boss target you",
				},
			},
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["inverse"] = false,
			["selfPoint"] = "TOP",
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["parent"] = "Dungeon - Targeted Spells",
		},
		["09 Volatile Anima Infusion - Green"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 2,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						0, -- [1]
						1, -- [2]
						0.031372549019608, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.25,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = true,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2417",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "rG8vXbeYv(p",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["useMatch_count"] = true,
						["auranames"] = {
						},
						["groupclone"] = true,
						["matchesShowOn"] = "showOnActive",
						["name_info"] = "aura",
						["unit"] = "group",
						["use_tooltip"] = false,
						["subeventPrefix"] = "SPELL",
						["match_countOperator"] = ">=",
						["match_count"] = "1",
						["buffShowOn"] = "showOnActive",
						["unevent"] = "timed",
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["useExactSpellId"] = true,
						["custom_type"] = "stateupdate",
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["event"] = "Health",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HARMFUL",
						["use_specific_unit"] = false,
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["auraspellids"] = {
							"342655", -- [1]
							"342698", -- [2]
						},
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["duration"] = "1",
						["useGroup_count"] = false,
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["text2Font"] = "Friz Quadrata TT",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text1"] = "%n",
			["text1Point"] = "CENTER",
			["anchorFrameType"] = "SCREEN",
			["semver"] = "1.0.26",
			["justify"] = "LEFT",
			["zoom"] = 0,
			["id"] = "09 Volatile Anima Infusion - Green",
			["config"] = {
			},
			["frameStrata"] = 9,
			["width"] = 6.9999866485596,
			["text1Containment"] = "INSIDE",
			["wordWrap"] = "WordWrap",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["Council of Blood - Dreadbolt Volley - Cast Bar"] = {
			["controlledChildren"] = {
				"Council of Blood - Dreadbolt Volley Cast 2", -- [1]
				"Council of Blood - Dreadbolt Volley Cast 3", -- [2]
				"Cast 1", -- [3]
			},
			["borderBackdrop"] = "Blizzard Tooltip",
			["authorOptions"] = {
			},
			["preferToUpdate"] = false,
			["yOffset"] = -22.49761962890625,
			["anchorPoint"] = "CENTER",
			["borderColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["url"] = "https://wago.io/VqAXD3338/1",
			["actions"] = {
				["start"] = {
				},
				["finish"] = {
				},
				["init"] = {
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["debuffType"] = "HELPFUL",
						["type"] = "aura2",
						["spellIds"] = {
						},
						["subeventSuffix"] = "_CAST_START",
						["subeventPrefix"] = "SPELL",
						["unit"] = "player",
						["event"] = "Health",
						["names"] = {
						},
					},
					["untrigger"] = {
					},
				}, -- [1]
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["version"] = 1,
			["subRegions"] = {
			},
			["load"] = {
				["talent"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["class"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["backdropColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0.5, -- [4]
			},
			["scale"] = 1,
			["border"] = false,
			["borderEdge"] = "Square Full White",
			["regionType"] = "group",
			["borderSize"] = 2,
			["borderOffset"] = 4,
			["semver"] = "1.0.0",
			["tocversion"] = 90002,
			["id"] = "Council of Blood - Dreadbolt Volley - Cast Bar",
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["xOffset"] = 430.9681396484375,
			["uid"] = "NcW08kLVNGf",
			["config"] = {
			},
			["borderInset"] = 1,
			["conditions"] = {
			},
			["information"] = {
			},
			["selfPoint"] = "CENTER",
		},
		["Gluttonous Miasma Chat Announcer"] = {
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["displayText"] = "%c",
			["customText"] = "function()\n    if not WeakAuras.IsOptionsOpen() then\n        if aura_env.state and (not aura_env.lastSay or aura_env.lastSay <= GetTime() - aura_env.config.specifiedInterval)  then\n            aura_env.lastSay = GetTime()\n            local currentHP = Round((UnitHealth(\"player\")/UnitHealthMax(\"player\"))*100)\n            local mark = GetRaidTargetIndex(\"player\")\n            if mark and mark ~= 9 and currentHP <= aura_env.config.specifiedHP then\n                SendChatMessage(\"{rt\"..mark..\"} \"..currentHP..\" {rt\"..mark..\"}\", \"YELL\")\n            elseif not mark and currentHP <= aura_env.config.specifiedHP then\n                SendChatMessage(currentHP, \"YELL\")\n            end\n        end\n    end\nend",
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["displayText_format_p_time_format"] = 0,
			["customTextUpdate"] = "update",
			["url"] = "https://wago.io/PXM28wJfq/5",
			["actions"] = {
				["start"] = {
				},
				["init"] = {
				},
				["finish"] = {
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["useName"] = true,
						["subeventSuffix"] = "_CAST_START",
						["event"] = "Health",
						["unit"] = "player",
						["names"] = {
						},
						["spellIds"] = {
						},
						["type"] = "aura2",
						["useExactSpellId"] = false,
						["subeventPrefix"] = "SPELL",
						["auranames"] = {
							"329298", -- [1]
						},
						["debuffType"] = "HARMFUL",
					},
					["untrigger"] = {
					},
				}, -- [1]
				{
					["trigger"] = {
						["auranames"] = {
							"329298", -- [1]
						},
						["duration"] = "4",
						["message_operator"] = "find('%s')",
						["unit"] = "player",
						["subeventPrefix"] = "SPELL",
						["use_track"] = true,
						["names"] = {
						},
						["use_genericShowOn"] = true,
						["custom_hide"] = "timed",
						["debuffType"] = "HARMFUL",
						["useExactSpellId"] = false,
						["useName"] = true,
						["subeventSuffix"] = "_CAST_START",
						["custom_type"] = "event",
						["message"] = "spell:334064",
						["type"] = "event",
						["event"] = "Chat Message",
						["unevent"] = "timed",
						["realSpellName"] = 0,
						["use_spellName"] = true,
						["spellIds"] = {
						},
						["spellName"] = 0,
						["use_message"] = true,
						["genericShowOn"] = "showOnCooldown",
						["use_messageType"] = true,
						["messageType"] = "CHAT_MSG_RAID_BOSS_WHISPER",
					},
					["untrigger"] = {
					},
				}, -- [2]
				["disjunctive"] = "custom",
				["customTriggerLogic"] = "function(t)\n    return aura_env.config.hasVolatile and t[1] and not t[2]\n    or not aura_env.config.hasVolatile and t[1]\nend",
				["activeTriggerMode"] = -10,
			},
			["displayText_format_p_format"] = "timed",
			["internalVersion"] = 45,
			["selfPoint"] = "BOTTOM",
			["font"] = "Friz Quadrata TT",
			["version"] = 5,
			["subRegions"] = {
			},
			["load"] = {
				["size"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["talent"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2383",
				["use_encounterid"] = true,
			},
			["fontSize"] = 12,
			["shadowXOffset"] = 1,
			["regionType"] = "text",
			["fixedWidth"] = 200,
			["displayText_format_p_time_dynamic_threshold"] = 60,
			["preferToUpdate"] = false,
			["displayText_format_p_time_precision"] = 1,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["authorOptions"] = {
				{
					["type"] = "description",
					["text"] = "Announce from",
					["fontSize"] = "medium",
					["width"] = 0.5,
				}, -- [1]
				{
					["type"] = "number",
					["useDesc"] = false,
					["max"] = 100,
					["step"] = 1,
					["width"] = 0.3,
					["min"] = 0,
					["key"] = "specifiedHP",
					["name"] = "%Health",
					["default"] = 100,
				}, -- [2]
				{
					["type"] = "space",
					["variableWidth"] = true,
					["height"] = 1,
					["width"] = 1.2,
					["useHeight"] = false,
				}, -- [3]
				{
					["type"] = "description",
					["text"] = "Spam every",
					["fontSize"] = "medium",
					["width"] = 0.5,
				}, -- [4]
				{
					["type"] = "number",
					["useDesc"] = false,
					["max"] = 10,
					["step"] = 0.1,
					["width"] = 0.3,
					["min"] = 0.1,
					["key"] = "specifiedInterval",
					["name"] = "seconds",
					["default"] = 2,
				}, -- [5]
				{
					["type"] = "toggle",
					["key"] = "markOnly",
					["default"] = true,
					["useDesc"] = false,
					["name"] = "Spam only if marked",
					["width"] = 2,
				}, -- [6]
				{
					["type"] = "toggle",
					["key"] = "hasVolatile",
					["default"] = false,
					["useDesc"] = false,
					["name"] = "Stop spam during Volatile Ejection",
					["width"] = 2,
				}, -- [7]
			},
			["justify"] = "LEFT",
			["semver"] = "1.0.4",
			["tocversion"] = 90002,
			["id"] = "Gluttonous Miasma Chat Announcer",
			["uid"] = "SFs8PBp00pf",
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["xOffset"] = 0,
			["config"] = {
				["markOnly"] = true,
				["specifiedHP"] = 100,
				["specifiedInterval"] = 2,
				["hasVolatile"] = false,
			},
			["shadowYOffset"] = -1,
			["automaticWidth"] = "Auto",
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["wordWrap"] = "WordWrap",
		},
		["02 Jagged Claws High Stacks - Orange"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 3,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						1, -- [1]
						0.49411764705882, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.25,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = true,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2418",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "t(HNhotIrsu",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["custom_type"] = "stateupdate",
						["useMatch_count"] = true,
						["useStacks"] = true,
						["useGroup_count"] = false,
						["groupclone"] = true,
						["matchesShowOn"] = "showOnActive",
						["name_info"] = "aura",
						["subeventPrefix"] = "SPELL",
						["useExactSpellId"] = true,
						["stacks"] = "4",
						["debuffType"] = "HARMFUL",
						["subeventSuffix"] = "_CAST_START",
						["match_count"] = "1",
						["stacksOperator"] = ">=",
						["auraspellids"] = {
							"334971", -- [1]
						},
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["match_countOperator"] = ">=",
						["unit"] = "group",
						["duration"] = "1",
						["event"] = "Health",
						["use_specific_unit"] = false,
						["unevent"] = "timed",
						["use_tooltip"] = false,
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["buffShowOn"] = "showOnActive",
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["auranames"] = {
						},
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["wordWrap"] = "WordWrap",
			["config"] = {
			},
			["text1"] = "%n",
			["text1Containment"] = "INSIDE",
			["width"] = 6.9999866485596,
			["justify"] = "LEFT",
			["semver"] = "1.0.26",
			["zoom"] = 0,
			["id"] = "02 Jagged Claws High Stacks - Orange",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["frameStrata"] = 9,
			["anchorFrameType"] = "SCREEN",
			["text1Point"] = "CENTER",
			["text2Font"] = "Friz Quadrata TT",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["Mage 2"] = {
			["sparkWidth"] = 10,
			["iconSource"] = -1,
			["authorOptions"] = {
			},
			["preferToUpdate"] = false,
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["sparkRotation"] = 0,
			["url"] = "https://wago.io/ZG34ifiRf/1",
			["backgroundColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				0.4738245010376, -- [4]
			},
			["fontFlags"] = "OUTLINE",
			["icon_color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["selfPoint"] = "CENTER",
			["barColor"] = {
				0, -- [1]
				0.90196078431373, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["desaturate"] = false,
			["sparkOffsetY"] = 0,
			["load"] = {
				["ingroup"] = {
					["multi"] = {
					},
				},
				["use_never"] = false,
				["class"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["level"] = "100",
				["size"] = {
					["single"] = "party",
					["multi"] = {
						["ratedpvp"] = true,
						["arena"] = true,
						["ratedarena"] = true,
						["pvp"] = true,
					},
				},
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_level"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["difficulty"] = {
					["single"] = "challenge",
					["multi"] = {
						["challenge"] = true,
					},
				},
				["zoneIds"] = "",
				["faction"] = {
					["multi"] = {
					},
				},
				["use_size"] = false,
				["use_zone"] = false,
				["race"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
			},
			["useAdjustededMin"] = false,
			["regionType"] = "aurabar",
			["texture"] = "Glamour4",
			["sparkTexture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
			["auto"] = true,
			["tocversion"] = 80300,
			["alpha"] = 1,
			["sparkColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["displayIcon"] = "Interface\\Icons\\spell_holy_rebuke",
			["stacksPoint"] = "BOTTOMRIGHT",
			["sparkOffsetX"] = 0,
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["sparkRotationMode"] = "AUTO",
			["triggers"] = {
				{
					["trigger"] = {
						["duration"] = "24",
						["spellId"] = "",
						["subeventPrefix"] = "SPELL",
						["spellIds"] = {
						},
						["ownOnly"] = true,
						["genericShowOn"] = "showOnActive",
						["unit"] = "player",
						["debuffType"] = "HELPFUL",
						["use_specific_sourceunit"] = true,
						["inverse"] = true,
						["use_spellId"] = false,
						["type"] = "combatlog",
						["use_destName"] = false,
						["use_cloneId"] = true,
						["spellName"] = "Counterspell",
						["use_sourceFlags2"] = true,
						["use_sourceName"] = false,
						["unevent"] = "timed",
						["subeventSuffix"] = "_CAST_SUCCESS",
						["custom_hide"] = "timed",
						["sourceName"] = "Draemii",
						["event"] = "Combat Log",
						["sourceFlags2"] = "Hostile",
						["use_name"] = true,
						["use_spellName"] = true,
						["name"] = "Flask of the warm sun ",
						["use_sourceUnit"] = false,
						["use_destunit"] = false,
						["use_destUnit"] = false,
						["sourceUnit"] = "Benzikura",
						["names"] = {
							"Greater Draenic Intellect Flask", -- [1]
						},
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
			},
			["backdropInFront"] = false,
			["stickyDuration"] = false,
			["version"] = 1,
			["subRegions"] = {
				{
					["type"] = "subborder",
					["border_anchor"] = "bar",
					["border_size"] = 16,
					["border_color"] = {
						0, -- [1]
						0.87058823529412, -- [2]
						1, -- [3]
						0.5, -- [4]
					},
					["border_visible"] = true,
					["border_edge"] = "1 Pixel",
					["border_offset"] = 2,
				}, -- [1]
				{
					["type"] = "aurabar_bar",
				}, -- [2]
				{
					["text_shadowXOffset"] = 1,
					["text_text"] = "%p",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Vixar",
					["text_text_format_p_time_format"] = 0,
					["text_shadowYOffset"] = -1,
					["text_text_format_p_time_dynamic_threshold"] = 3,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "ICON_CENTER",
					["text_text_format_p_format"] = "timed",
					["text_visible"] = true,
					["text_fontSize"] = 20,
					["anchorXOffset"] = 0,
					["text_text_format_p_time_precision"] = 1,
				}, -- [3]
			},
			["height"] = 30,
			["sparkBlendMode"] = "ADD",
			["backdropColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0.5, -- [4]
			},
			["displayStacks"] = "%c",
			["spark"] = false,
			["config"] = {
			},
			["uid"] = "WFI6XQud82i",
			["borderInFront"] = false,
			["borderBackdrop"] = "Blizzard Tooltip",
			["icon_side"] = "LEFT",
			["xOffset"] = 0,
			["zoom"] = 0.28,
			["sparkHeight"] = 30,
			["actions"] = {
				["start"] = {
					["do_message"] = false,
					["message_type"] = "SAY",
				},
				["init"] = {
				},
				["finish"] = {
				},
			},
			["stacksContainment"] = "INSIDE",
			["parent"] = "Enemy Interrupt Icons",
			["semver"] = "1.0.0",
			["width"] = 30,
			["id"] = "Mage 2",
			["icon"] = true,
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["sparkHidden"] = "NEVER",
			["useAdjustededMax"] = false,
			["inverse"] = false,
			["sparkDesature"] = false,
			["orientation"] = "HORIZONTAL",
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["customTextUpdate"] = "update",
		},
		["09 Rended Heart - Red"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 4,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						1, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.25,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = true,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2417",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "WQax4p(hryJ",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["useMatch_count"] = true,
						["auranames"] = {
						},
						["groupclone"] = true,
						["matchesShowOn"] = "showOnActive",
						["name_info"] = "aura",
						["unit"] = "group",
						["use_tooltip"] = false,
						["subeventPrefix"] = "SPELL",
						["match_countOperator"] = ">=",
						["match_count"] = "1",
						["buffShowOn"] = "showOnActive",
						["unevent"] = "timed",
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["useExactSpellId"] = true,
						["custom_type"] = "stateupdate",
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["event"] = "Health",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HARMFUL",
						["use_specific_unit"] = false,
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["auraspellids"] = {
							"334771", -- [1]
						},
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["duration"] = "1",
						["useGroup_count"] = false,
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["text2Font"] = "Friz Quadrata TT",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text1"] = "%n",
			["text1Point"] = "CENTER",
			["anchorFrameType"] = "SCREEN",
			["semver"] = "1.0.26",
			["justify"] = "LEFT",
			["zoom"] = 0,
			["id"] = "09 Rended Heart - Red",
			["config"] = {
			},
			["frameStrata"] = 9,
			["width"] = 6.9999866485596,
			["text1Containment"] = "INSIDE",
			["wordWrap"] = "WordWrap",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["07 Dark Recital Right"] = {
			["iconSource"] = -1,
			["xOffset"] = 0,
			["preferToUpdate"] = false,
			["yOffset"] = 200,
			["anchorPoint"] = "CENTER",
			["cooldownSwipe"] = true,
			["cooldownEdge"] = false,
			["icon"] = true,
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "combatlog",
						["spellId"] = "331636",
						["subeventSuffix"] = "_AURA_APPLIED",
						["duration"] = "8.5",
						["event"] = "Combat Log",
						["subeventPrefix"] = "SPELL",
						["destUnit"] = "player",
						["use_spellId"] = true,
						["spellIds"] = {
						},
						["unevent"] = "timed",
						["names"] = {
						},
						["use_destUnit"] = true,
						["unit"] = "player",
						["debuffType"] = "HARMFUL",
					},
					["untrigger"] = {
					},
				}, -- [1]
				{
					["trigger"] = {
						["type"] = "combatlog",
						["spellId"] = "331637",
						["subeventSuffix"] = "_AURA_APPLIED",
						["duration"] = "8.5",
						["event"] = "Combat Log",
						["subeventPrefix"] = "SPELL",
						["destUnit"] = "player",
						["use_spellId"] = true,
						["spellIds"] = {
						},
						["unevent"] = "timed",
						["names"] = {
						},
						["use_destUnit"] = true,
						["unit"] = "player",
						["debuffType"] = "HARMFUL",
					},
					["untrigger"] = {
					},
				}, -- [2]
				["disjunctive"] = "any",
				["activeTriggerMode"] = -10,
			},
			["internalVersion"] = 45,
			["keepAspectRatio"] = false,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["desaturate"] = false,
			["discrete_rotation"] = 90,
			["version"] = 1,
			["subRegions"] = {
			},
			["height"] = 100,
			["rotate"] = true,
			["load"] = {
				["use_encounterid"] = true,
				["zoneIds"] = "",
				["talent"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2412",
				["class"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["textureWrapMode"] = "CLAMPTOBLACKADDITIVE",
			["uid"] = "5HuSQCD107i",
			["selfPoint"] = "CENTER",
			["parent"] = "07 Dark Recital Group",
			["mirror"] = false,
			["color"] = {
				0, -- [1]
				1, -- [2]
				0.26666666666667, -- [3]
				1, -- [4]
			},
			["regionType"] = "texture",
			["cooldown"] = false,
			["blendMode"] = "BLEND",
			["url"] = "https://wago.io/seLAtvE5B/1",
			["actions"] = {
				["start"] = {
					["do_custom"] = true,
					["custom"] = "if DBM then\n    aura_env.region.texture:SetTexture(\"Interface\\\\Addons\\\\DBM-Core\\\\textures\\\\arrows\\\\arrow.blp\", false)\n    aura_env.region.texture:SetTexCoord(0, 1/9, 9/12-1/50, 10/12-1/50)\nend",
					["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\Right.ogg",
					["do_sound"] = true,
				},
				["finish"] = {
				},
				["init"] = {
				},
			},
			["texture"] = "Interface\\AddOns\\WeakAuras\\Media\\Textures\\targeting-mark.tga",
			["frameStrata"] = 1,
			["zoom"] = 0,
			["semver"] = "1.0.0",
			["tocversion"] = 90002,
			["id"] = "07 Dark Recital Right",
			["width"] = 100,
			["alpha"] = 1,
			["anchorFrameType"] = "SCREEN",
			["cooldownTextDisabled"] = false,
			["config"] = {
			},
			["inverse"] = false,
			["rotation"] = 90,
			["conditions"] = {
			},
			["information"] = {
			},
			["authorOptions"] = {
			},
		},
		["06 Change of Heart - Orange"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 2,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						1, -- [1]
						0.41960784313725, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.25,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = true,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2406",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "mbss74ZKVaf",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["useMatch_count"] = true,
						["auranames"] = {
						},
						["groupclone"] = true,
						["matchesShowOn"] = "showOnActive",
						["name_info"] = "aura",
						["unit"] = "group",
						["use_tooltip"] = false,
						["subeventPrefix"] = "SPELL",
						["match_countOperator"] = ">=",
						["match_count"] = "1",
						["buffShowOn"] = "showOnActive",
						["unevent"] = "timed",
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["useExactSpellId"] = true,
						["custom_type"] = "stateupdate",
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["event"] = "Health",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HARMFUL",
						["use_specific_unit"] = false,
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["auraspellids"] = {
							"340452", -- [1]
						},
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["duration"] = "1",
						["useGroup_count"] = false,
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["wordWrap"] = "WordWrap",
			["config"] = {
			},
			["text1"] = "%n",
			["text1Containment"] = "INSIDE",
			["width"] = 6.9999866485596,
			["justify"] = "LEFT",
			["semver"] = "1.0.26",
			["zoom"] = 0,
			["id"] = "06 Change of Heart - Orange",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["frameStrata"] = 9,
			["anchorFrameType"] = "SCREEN",
			["text1Point"] = "CENTER",
			["text2Font"] = "Friz Quadrata TT",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["02 Vicious Lunge - Green"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 2,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						0, -- [1]
						1, -- [2]
						0.015686274509804, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.25,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = true,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2418",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "Yz1PXd2bslF",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["useMatch_count"] = true,
						["auranames"] = {
						},
						["groupclone"] = true,
						["matchesShowOn"] = "showOnActive",
						["name_info"] = "aura",
						["unit"] = "group",
						["use_tooltip"] = false,
						["subeventPrefix"] = "SPELL",
						["match_countOperator"] = ">=",
						["match_count"] = "1",
						["buffShowOn"] = "showOnActive",
						["unevent"] = "timed",
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["useExactSpellId"] = true,
						["custom_type"] = "stateupdate",
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["event"] = "Health",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HARMFUL",
						["use_specific_unit"] = false,
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["auraspellids"] = {
							"334945", -- [1]
						},
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["duration"] = "1",
						["useGroup_count"] = false,
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["wordWrap"] = "WordWrap",
			["config"] = {
			},
			["text1"] = "%n",
			["text1Containment"] = "INSIDE",
			["width"] = 6.9999866485596,
			["justify"] = "LEFT",
			["semver"] = "1.0.26",
			["zoom"] = 0,
			["id"] = "02 Vicious Lunge - Green",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["frameStrata"] = 9,
			["anchorFrameType"] = "SCREEN",
			["text1Point"] = "CENTER",
			["text2Font"] = "Friz Quadrata TT",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["01 Echolocation - White"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 4,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						0.98039215686275, -- [1]
						1, -- [2]
						0.91764705882353, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.25,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = true,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2398",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "gp3bwZU8WfR",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["useMatch_count"] = true,
						["auranames"] = {
						},
						["groupclone"] = true,
						["matchesShowOn"] = "showOnActive",
						["name_info"] = "aura",
						["unit"] = "group",
						["use_tooltip"] = false,
						["subeventPrefix"] = "SPELL",
						["match_countOperator"] = ">=",
						["match_count"] = "1",
						["buffShowOn"] = "showOnActive",
						["unevent"] = "timed",
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["useExactSpellId"] = true,
						["custom_type"] = "stateupdate",
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["event"] = "Health",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HARMFUL",
						["use_specific_unit"] = false,
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["auraspellids"] = {
							"342077", -- [1]
						},
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["duration"] = "1",
						["useGroup_count"] = false,
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["wordWrap"] = "WordWrap",
			["config"] = {
			},
			["text1"] = "%n",
			["text1Containment"] = "INSIDE",
			["width"] = 6.9999866485596,
			["justify"] = "LEFT",
			["semver"] = "1.0.26",
			["zoom"] = 0,
			["id"] = "01 Echolocation - White",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["frameStrata"] = 9,
			["anchorFrameType"] = "SCREEN",
			["text1Point"] = "CENTER",
			["text2Font"] = "Friz Quadrata TT",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["10 Night Hunter - Green"] = {
			["glow"] = false,
			["text1FontSize"] = 12,
			["authorOptions"] = {
			},
			["displayText"] = " ",
			["yOffset"] = -36.000030517578,
			["anchorPoint"] = "CENTER",
			["url"] = "https://wago.io/NathriaGlows/27",
			["actions"] = {
				["start"] = {
					["glow_frame_type"] = "UNITFRAME",
					["glow_thickness"] = 2,
					["glow_action"] = "show",
					["glow_scale"] = 2,
					["glow_color"] = {
						0.019607843137255, -- [1]
						1, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["do_glow"] = true,
					["glow_border"] = true,
					["custom"] = "",
					["glow_type"] = "Pixel",
					["glow_frequency"] = 0.25,
					["use_glow_color"] = true,
					["do_custom"] = false,
					["glow_lines"] = 12,
				},
				["init"] = {
					["custom"] = "",
					["do_custom"] = false,
				},
				["finish"] = {
					["glow_frame_type"] = "UNITFRAME",
					["hide_all_glows"] = true,
					["custom"] = "",
					["glow_action"] = "hide",
					["do_custom"] = false,
					["do_glow"] = false,
				},
			},
			["text1Enabled"] = true,
			["selfPoint"] = "CENTER",
			["desaturate"] = false,
			["font"] = "Friz Quadrata TT",
			["text2FontFlags"] = "OUTLINE",
			["load"] = {
				["talent2"] = {
					["multi"] = {
					},
				},
				["use_never"] = true,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["zoneIds"] = "",
				["use_encounterid"] = true,
				["difficulty"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2407",
				["faction"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
						["twenty"] = true,
						["flexible"] = true,
					},
				},
			},
			["shadowXOffset"] = 1,
			["text1FontFlags"] = "OUTLINE",
			["regionType"] = "text",
			["text2FontSize"] = 24,
			["text2"] = "%p",
			["auto"] = false,
			["tocversion"] = 90002,
			["text2Enabled"] = false,
			["uid"] = "bGfwWHIdTlH",
			["displayIcon"] = 841384,
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["customText"] = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
			["shadowYOffset"] = -1,
			["customTextUpdate"] = "event",
			["automaticWidth"] = "Auto",
			["triggers"] = {
				{
					["trigger"] = {
						["useMatch_count"] = true,
						["auranames"] = {
						},
						["groupclone"] = true,
						["matchesShowOn"] = "showOnActive",
						["name_info"] = "aura",
						["unit"] = "group",
						["use_tooltip"] = false,
						["subeventPrefix"] = "SPELL",
						["match_countOperator"] = ">=",
						["match_count"] = "1",
						["buffShowOn"] = "showOnActive",
						["unevent"] = "timed",
						["custom_hide"] = "timed",
						["showClones"] = true,
						["type"] = "aura2",
						["use_debuffClass"] = false,
						["useExactSpellId"] = true,
						["custom_type"] = "stateupdate",
						["names"] = {
							"Putrid Paroxysm", -- [1]
							"Absorbed in Darkness", -- [2]
							"Gestate", -- [3]
							"Roiling Deceit", -- [4]
							"Imminent Ruin", -- [5]
							"Unleashed Shadow", -- [6]
							"Deathwish", -- [7]
							"Torment", -- [8]
						},
						["event"] = "Health",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HARMFUL",
						["use_specific_unit"] = false,
						["spellIds"] = {
							262314, -- [1]
							274387, -- [2]
							265212, -- [3]
							265360, -- [4]
							272536, -- [5]
							276672, -- [6]
							274271, -- [7]
							267427, -- [8]
						},
						["auraspellids"] = {
							"327796", -- [1]
						},
						["check"] = "update",
						["combineMatches"] = "showLowest",
						["duration"] = "1",
						["useGroup_count"] = false,
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "any",
				["customTriggerLogic"] = "",
				["activeTriggerMode"] = 1,
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["easeStrength"] = 3,
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["stickyDuration"] = false,
			["version"] = 27,
			["height"] = 11.99995803833,
			["fontSize"] = 12,
			["text2Containment"] = "INSIDE",
			["text1Font"] = "Friz Quadrata TT",
			["fixedWidth"] = 200,
			["text2Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text2Point"] = "CENTER",
			["parent"] = "Castle Nathria Glows",
			["xOffset"] = -542.00009155273,
			["cooldownTextDisabled"] = false,
			["text2Font"] = "Friz Quadrata TT",
			["text1Color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["text1"] = "%n",
			["text1Point"] = "CENTER",
			["anchorFrameType"] = "SCREEN",
			["semver"] = "1.0.26",
			["justify"] = "LEFT",
			["zoom"] = 0,
			["id"] = "10 Night Hunter - Green",
			["config"] = {
			},
			["frameStrata"] = 9,
			["width"] = 6.9999866485596,
			["text1Containment"] = "INSIDE",
			["wordWrap"] = "WordWrap",
			["inverse"] = false,
			["icon"] = true,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["conditions"] = {
			},
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["preferToUpdate"] = false,
		},
		["Cast 1"] = {
			["sparkWidth"] = 10,
			["iconSource"] = -1,
			["authorOptions"] = {
			},
			["preferToUpdate"] = false,
			["yOffset"] = 238.33312988281,
			["anchorPoint"] = "CENTER",
			["sparkRotation"] = 0,
			["sparkRotationMode"] = "AUTO",
			["actions"] = {
				["start"] = {
					["do_sound"] = false,
				},
				["finish"] = {
				},
				["init"] = {
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["spellId"] = "328789",
						["use_genericShowOn"] = true,
						["genericShowOn"] = "showOnCooldown",
						["names"] = {
						},
						["subeventPrefix"] = "SPELL",
						["use_unit"] = true,
						["unit"] = "boss1",
						["spellName"] = "Dreadbolt Volley",
						["useExactSpellId"] = true,
						["spell"] = "Dreadbolt Volley",
						["unevent"] = "auto",
						["auraspellids"] = {
							"329834", -- [1]
						},
						["subeventSuffix"] = "_CAST_START",
						["use_spellName"] = true,
						["event"] = "Cast",
						["type"] = "unit",
						["realSpellName"] = "Dreadbolt Volley",
						["use_spellId"] = false,
						["spellIds"] = {
						},
						["duration"] = "3",
						["use_spell"] = true,
						["debuffType"] = "HELPFUL",
						["use_track"] = true,
						["use_specific_unit"] = true,
					},
					["untrigger"] = {
						["use_specific_unit"] = true,
						["unit"] = "boss1",
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["icon_color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["internalVersion"] = 45,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["barColor"] = {
				0.5529411764705883, -- [1]
				0.09019607843137255, -- [2]
				0.09803921568627451, -- [3]
				1, -- [4]
			},
			["desaturate"] = false,
			["sparkOffsetY"] = 0,
			["subRegions"] = {
				{
					["type"] = "aurabar_bar",
				}, -- [1]
				{
					["text_text_format_n_format"] = "none",
					["text_text"] = "Dreadbolt Volley",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Accidental Presidency",
					["text_shadowYOffset"] = -1,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "None",
					["text_anchorPoint"] = "INNER_RIGHT",
					["text_shadowXOffset"] = 1,
					["text_fontSize"] = 25,
					["anchorXOffset"] = 0,
					["text_visible"] = true,
				}, -- [2]
				{
					["text_text_format_p_time_precision"] = 1,
					["text_text_format_s_format"] = "none",
					["text_text"] = "%p",
					["text_text_format_p_format"] = "timed",
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Accidental Presidency",
					["text_text_format_p_time_format"] = 0,
					["text_shadowYOffset"] = -1,
					["text_text_format_p_time_dynamic_threshold"] = 60,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "None",
					["text_anchorPoint"] = "INNER_LEFT",
					["text_visible"] = true,
					["text_shadowXOffset"] = 1,
					["text_fontSize"] = 25,
					["anchorXOffset"] = 0,
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
				}, -- [3]
				{
					["border_offset"] = 0,
					["border_anchor"] = "bar",
					["type"] = "subborder",
					["border_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["border_visible"] = true,
					["border_edge"] = "Square Full White",
					["border_size"] = 2,
				}, -- [4]
			},
			["height"] = 43,
			["load"] = {
				["use_encounterid"] = true,
				["zoneIds"] = "",
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2412",
				["size"] = {
					["multi"] = {
					},
				},
			},
			["sparkBlendMode"] = "ADD",
			["useAdjustededMax"] = false,
			["sparkColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["selfPoint"] = "CENTER",
			["backgroundColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				0.5, -- [4]
			},
			["icon"] = false,
			["useAdjustededMin"] = false,
			["regionType"] = "aurabar",
			["xOffset"] = -457.50018310547,
			["config"] = {
			},
			["icon_side"] = "RIGHT",
			["zoom"] = 0,
			["sparkHeight"] = 30,
			["texture"] = "Solid",
			["alpha"] = 1,
			["sparkTexture"] = "Interface\\CastingBar\\UI-CastingBar-Spark",
			["spark"] = false,
			["tocversion"] = 90002,
			["id"] = "Cast 1",
			["anchorFrameType"] = "SCREEN",
			["frameStrata"] = 1,
			["width"] = 360,
			["sparkHidden"] = "NEVER",
			["uid"] = "aHMzcVv7aTA",
			["inverse"] = false,
			["sparkOffsetX"] = 0,
			["orientation"] = "HORIZONTAL",
			["conditions"] = {
			},
			["information"] = {
			},
			["parent"] = "Council of Blood - Dreadbolt Volley - Cast Bar",
		},
		["circle_lotus_fill"] = {
			["iconSource"] = 0,
			["xOffset"] = 0,
			["preferToUpdate"] = true,
			["customText"] = "function()\n    if aura_env.showText then\n        return aura_env.order\n    else\n        return \"\"\n    end\nend",
			["yOffset"] = 0,
			["anchorPoint"] = "CENTER",
			["cooldownSwipe"] = true,
			["cooldownEdge"] = false,
			["icon"] = true,
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "custom",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HELPFUL",
						["event"] = "Health",
						["unit"] = "player",
						["custom_type"] = "status",
						["spellIds"] = {
						},
						["events"] = "ZONE_CHANGED, PLAYER_LOGIN, PLAYER_ENTERING_WORLD",
						["names"] = {
						},
						["check"] = "event",
						["custom"] = "function()\n    WeakAuras.ScanEvents(\"TirnaScitheButtonData\", 2, aura_env.frameData)\n    if aura_env.debugmode then \n        return true \n    end\n    local text = GetMinimapZoneText()\n    if text == aura_env.mistZone then\n        return true\n    else\n        return false\n    end\nend\n\n",
						["subeventPrefix"] = "SPELL",
						["custom_hide"] = "custom",
					},
					["untrigger"] = {
					},
				}, -- [1]
				{
					["trigger"] = {
						["debuffType"] = "HELPFUL",
						["type"] = "custom",
						["custom"] = "function(event,eid,...)\n    if event == \"ENCOUNTER_END\" then \n        if eid == 2392 and aura_env.showEncounter == false then \n            return false \n        end\n        return true \n    elseif event == \"ENCOUNTER_START\" then \n        if aura_env.showEncounter == false then \n            return false\n        else\n            return true\n        end\n    end\n    return true\nend\n\n",
						["custom_type"] = "status",
						["check"] = "event",
						["customName"] = "function()\n    return \nend",
						["events"] = "ENCOUNTER_START, ENCOUNTER_END, PLAYER_ENTERING_WORLD",
						["unit"] = "player",
					},
					["untrigger"] = {
					},
				}, -- [2]
				{
					["trigger"] = {
						["type"] = "custom",
						["custom_type"] = "status",
						["unit"] = "player",
						["customName"] = "function()\n    return aura_env.order            \nend",
						["events"] = "TirnaScitheButtonOrder",
						["check"] = "event",
						["debuffType"] = "HELPFUL",
						["custom"] = "function(_,button,order)\n    if button == 2 then \n        aura_env.order = order\n        if order == 9 then \n            aura_env.order = \"\"\n        end\n        return true\n    elseif button == 9 then\n        aura_env.order = \"\"\n        return true\n    end\n    return true\nend\n\n",
						["custom_hide"] = "timed",
					},
					["untrigger"] = {
					},
				}, -- [3]
				["activeTriggerMode"] = -10,
			},
			["internalVersion"] = 45,
			["keepAspectRatio"] = false,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["desaturate"] = false,
			["version"] = 67,
			["subRegions"] = {
				{
					["glowFrequency"] = 0.25,
					["type"] = "subglow",
					["useGlowColor"] = false,
					["glowType"] = "buttonOverlay",
					["glowLength"] = 10,
					["glowYOffset"] = 0,
					["glowColor"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["glowXOffset"] = 0,
					["glowScale"] = 1,
					["glowThickness"] = 1,
					["glow"] = false,
					["glowLines"] = 8,
					["glowBorder"] = false,
				}, -- [1]
				{
					["text_text_format_p_time_format"] = 0,
					["text_text"] = "%c",
					["text_text_format_p_format"] = "timed",
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Friz Quadrata TT",
					["text_text_format_p_time_precision"] = 1,
					["text_shadowYOffset"] = 0,
					["text_text_format_p_time_dynamic_threshold"] = 60,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "OUTLINE",
					["text_anchorPoint"] = "INNER_BOTTOMRIGHT",
					["text_shadowXOffset"] = 0,
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_fontSize"] = 15,
					["anchorXOffset"] = 0,
					["text_visible"] = true,
				}, -- [2]
			},
			["height"] = 80,
			["load"] = {
				["ingroup"] = {
					["single"] = "group",
					["multi"] = {
						["group"] = true,
					},
				},
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["use_encounterid"] = false,
				["difficulty"] = {
					["multi"] = {
						["mythic"] = true,
						["challenge"] = true,
					},
				},
				["use_zone"] = false,
				["zoneIds"] = "1669",
				["use_zoneIds"] = true,
				["zone"] = "",
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["config"] = {
				["encounter"] = false,
				["noCommunication"] = false,
				["debug"] = false,
				["orderText"] = true,
			},
			["actions"] = {
				["start"] = {
				},
				["finish"] = {
				},
				["init"] = {
					["custom"] = "aura_env.debugmode = aura_env.config['debug']\nlocal debug = aura_env.debugmode\naura_env.showEncounter = aura_env.config['encounter']\naura_env.locale=GetLocale()\naura_env.mistZone = \"Mistveil Tangle\"\naura_env.order = \"\"\naura_env.showText = aura_env.config['orderText']\naura_env.noCommunication = aura_env.config['noCommunication']\nif aura_env.locale == \"koKR\" then\n    aura_env.mistZone = \"안개장막 덩굴숲\"\nelseif aura_env.locale == \"frFR\" then\n    aura_env.mistZone = \"Maquis Voile-de-Brume\"\nelseif aura_env.locale == \"deDE\" then\n    aura_env.mistZone = \"Nebelschleierdickicht\"\nelseif aura_env.locale == \"zhCN\" then\n    aura_env.mistZone = \"纱雾迷结\"\nelseif aura_env.locale == \"esES\" then\n    aura_env.mistZone = \"Espesura Velo de Niebla\"\nelseif aura_env.locale == \"zhTW\" then\n    aura_env.mistZone = \"霧紗密林\"\nelseif aura_env.locale == \"esMX\" then\n    aura_env.mistZone = \"Espesura Veloniebla\"\nelseif aura_env.locale == \"ruRU\" then\n    aura_env.mistZone = \"Туманная чащоба\"\nelseif aura_env.locale == \"ptBR\" then\n    aura_env.mistZone = \"Enleio do Véu da Névoa\"\nelseif aura_env.locale == \"itIT\" then\n    aura_env.mistZone = \"Intrico Velofosco\"\nelseif aura_env.locale == \"ptPT\" then\n    aura_env.mistZone = \"Enleio do Véu da Névoa\"        \nend\nlocal values = {0, 0, 0, 2}\nlocal message = string.format(\"%d %d %d %d\", values[1], values[2], values[3], values[4])\nlocal name, _ = UnitName(\"player\")\n\nif not aura_env.frameData then\n    local b = CreateFrame(\"Button\", nil, WeakAuras.regions[aura_env.id].region)\n    b:SetAllPoints(WeakAuras.regions[aura_env.id].region)\n    b:RegisterForClicks(\"LeftButtonDown\", \"RightButtonDown\")\n    aura_env.frameData = b\n    WeakAuras.ScanEvents(\"TirnaScitheButtonData\", 2, b)\nend\nif aura_env.noCommunication then\n    aura_env.frameData:SetScript(\"OnClick\", function(self, button)\n            if debug then print(\"Click Registered - true\") end\n            WeakAuras.ScanEvents(\"GlowUpdate\")\n            C_ChatInfo.RegisterAddonMessagePrefix(\"Tirna Ver2\") \n            if debug then \n                print(\"noCommunication sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"WHISPER\", name)) \n            else\n                C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"WHISPER\", name)\n            end\n            \n    end)\nelse\n    aura_env.frameData:SetScript(\"OnClick\", function(self, button)\n            if debug then print(\"Click Registered - true\") end\n            WeakAuras.ScanEvents(\"GlowUpdate\")\n            C_ChatInfo.RegisterAddonMessagePrefix(\"Tirna Ver2\")\n            if IsPartyLFG() then            \n                if debug then \n                    print(\"LFG sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"INSTANCE_CHAT\")) \n                else\n                    C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"INSTANCE_CHAT\")\n                end\n            elseif UnitInParty(\"player\") then            \n                if debug then \n                    print(\"party sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"PARTY\")) \n                else\n                    C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"PARTY\")\n                end\n            else         \n                if debug then \n                    print(\"solo sent - \", C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"WHISPER\", name)) \n                else\n                    C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", message .. \" \" .. button, \"WHISPER\", name)\n                end\n            end\n    end)\nend",
					["do_custom"] = true,
				},
			},
			["authorOptions"] = {
				{
					["type"] = "toggle",
					["key"] = "orderText",
					["default"] = true,
					["name"] = "Show Order Text",
					["useDesc"] = false,
					["width"] = 1,
				}, -- [1]
				{
					["type"] = "toggle",
					["key"] = "encounter",
					["default"] = false,
					["name"] = "Show during Boss",
					["useDesc"] = false,
					["width"] = 1,
				}, -- [2]
				{
					["type"] = "toggle",
					["key"] = "noCommunication",
					["default"] = false,
					["name"] = "Disable Cross-Communication with others running the WA",
					["useDesc"] = false,
					["width"] = 2,
				}, -- [3]
				{
					["type"] = "toggle",
					["key"] = "debug",
					["default"] = false,
					["name"] = "|cffff0000Debug testing|r (requires reload for full debug)",
					["useDesc"] = false,
					["width"] = 2,
				}, -- [4]
			},
			["regionType"] = "icon",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["cooldown"] = false,
			["conditions"] = {
			},
			["parent"] = "Mists of Tirna Scithe guessing game v2",
			["semver"] = "2.0.2",
			["anchorFrameType"] = "SCREEN",
			["zoom"] = 0,
			["auto"] = false,
			["tocversion"] = 90005,
			["id"] = "circle_lotus_fill",
			["alpha"] = 1,
			["frameStrata"] = 1,
			["width"] = 80,
			["cooldownTextDisabled"] = false,
			["uid"] = "6(1qKruXkc6",
			["inverse"] = false,
			["selfPoint"] = "CENTER",
			["displayIcon"] = "Interface\\AddOns\\guess\\circle_lotus_fill.tga",
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["url"] = "https://wago.io/hVLym_eLv/67",
		},
		["Quaking Bar"] = {
			["sparkWidth"] = 3,
			["borderBackdrop"] = "None",
			["useAdjustededMax"] = false,
			["xOffset"] = 0,
			["preferToUpdate"] = false,
			["customText"] = "function()\n    if WeakAuras.IsOptionsOpen() then\n        return \"20\"\n    else\n        if aura_env.state and aura_env.state.expirationTime then\n            local remaining = aura_env.state.expirationTime - GetTime()\n            \n            return floor(remaining % 20)\n        end\n    end\nend",
			["yOffset"] = 380,
			["anchorPoint"] = "CENTER",
			["sparkOffsetX"] = 0,
			["iconSource"] = 0,
			["sparkRotation"] = 0,
			["customTextUpdate"] = "update",
			["url"] = "https://wago.io/B1H06AUNM/3",
			["actions"] = {
				["start"] = {
					["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\Blast.ogg",
					["do_custom"] = false,
					["do_sound"] = true,
				},
				["init"] = {
					["custom"] = "-- Made by Domestos#3678 @Discord\nlocal bar = WeakAuras.regions[aura_env.id].region.bar\n\nlocal function CreateSpark()\n    local spark = bar:CreateTexture(nil, \"ARTWORK\")\n    spark:SetDrawLayer(\"ARTWORK\", 4)\n    spark:SetTexture(bar.spark:GetTexture())\n    spark:SetBlendMode(bar.spark:GetBlendMode())\n    spark:SetWidth(bar.spark:GetWidth())\n    spark:SetHeight(bar.spark:GetHeight())\n    \n    return spark\nend\n\nlocal function DrawSpark(offset, spark)\n    spark:SetPoint(\"CENTER\", bar, \"RIGHT\", offset * -1, 0)\n    spark:Show()\nend\n\n-- create sparks\nif bar.spark20 then bar.spark20:Hide() end\nif bar.spark40 then bar.spark40:Hide() end\nif bar.spark60 then bar.spark60:Hide() end\nbar.spark20 = CreateSpark()\nbar.spark40 = CreateSpark()\nbar.spark60 = CreateSpark()\n\n-- place sparks on bar\nlocal sizePerUnit = bar:GetWidth() / 60\nDrawSpark(sizePerUnit * 20, bar.spark20)\nDrawSpark(sizePerUnit * 40, bar.spark40)\nDrawSpark(sizePerUnit * 60, bar.spark60)",
					["do_custom"] = true,
				},
				["finish"] = {
				},
			},
			["fontFlags"] = "OUTLINE",
			["icon_color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["internalVersion"] = 45,
			["authorOptions"] = {
			},
			["selfPoint"] = "CENTER",
			["backdropInFront"] = false,
			["id"] = "Quaking Bar",
			["barColor"] = {
				0.63529411764706, -- [1]
				0.34117647058824, -- [2]
				0.18823529411765, -- [3]
				1, -- [4]
			},
			["desaturate"] = false,
			["color"] = {
			},
			["sparkRotationMode"] = "AUTO",
			["sparkOffsetY"] = 0,
			["subRegions"] = {
				{
					["type"] = "subborder",
					["border_anchor"] = "bar",
					["border_offset"] = 2,
					["border_color"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["border_visible"] = true,
					["border_edge"] = "1 Pixel",
					["border_size"] = 2,
				}, -- [1]
				{
					["type"] = "aurabar_bar",
				}, -- [2]
				{
					["text_shadowXOffset"] = 1,
					["text_text"] = "%c",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Expressway",
					["text_shadowYOffset"] = -1,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "None",
					["text_anchorPoint"] = "INNER_RIGHT",
					["text_fontSize"] = 28,
					["anchorXOffset"] = 0,
					["text_visible"] = true,
				}, -- [3]
				{
					["text_shadowXOffset"] = 1,
					["text_text"] = " ",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Expressway",
					["text_shadowYOffset"] = -1,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "None",
					["text_anchorPoint"] = "INNER_LEFT",
					["text_fontSize"] = 25,
					["anchorXOffset"] = 0,
					["text_visible"] = false,
				}, -- [4]
				{
					["text_shadowXOffset"] = 1,
					["text_text_format_s_format"] = "none",
					["text_text"] = "%s",
					["text_shadowColor"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["text_selfPoint"] = "AUTO",
					["text_automaticWidth"] = "Auto",
					["text_fixedWidth"] = 64,
					["anchorYOffset"] = 0,
					["text_justify"] = "CENTER",
					["rotateText"] = "NONE",
					["type"] = "subtext",
					["text_color"] = {
						1, -- [1]
						1, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["text_font"] = "Friz Quadrata TT",
					["text_shadowYOffset"] = -1,
					["text_wordWrap"] = "WordWrap",
					["text_fontType"] = "None",
					["text_anchorPoint"] = "ICON_CENTER",
					["text_fontSize"] = 12,
					["anchorXOffset"] = 0,
					["text_visible"] = false,
				}, -- [5]
			},
			["height"] = 30,
			["icon"] = true,
			["load"] = {
				["ingroup"] = {
					["multi"] = {
					},
				},
				["use_never"] = false,
				["class"] = {
					["multi"] = {
					},
				},
				["race"] = {
					["multi"] = {
					},
				},
				["use_difficulty"] = true,
				["size"] = {
					["single"] = "party",
					["multi"] = {
						["party"] = true,
					},
				},
				["use_size"] = true,
				["affixes"] = {
					["single"] = 14,
				},
				["talent"] = {
					["multi"] = {
					},
				},
				["use_affixes"] = true,
				["difficulty"] = {
					["single"] = "challenge",
					["multi"] = {
						["challenge"] = true,
					},
				},
				["zoneIds"] = "",
				["faction"] = {
					["multi"] = {
					},
				},
				["talent2"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["role"] = {
					["multi"] = {
					},
				},
				["pvptalent"] = {
					["multi"] = {
					},
				},
			},
			["sparkBlendMode"] = "ADD",
			["backdropColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				0, -- [4]
			},
			["triggers"] = {
				{
					["trigger"] = {
						["itemName"] = 0,
						["spellId"] = "240447",
						["use_absorbMode"] = true,
						["genericShowOn"] = "showOnActive",
						["subeventPrefix"] = "SPELL",
						["destUnit"] = "player",
						["use_genericShowOn"] = true,
						["duration"] = "60",
						["names"] = {
						},
						["unit"] = "player",
						["debuffType"] = "HELPFUL",
						["use_itemName"] = true,
						["type"] = "combatlog",
						["spellIds"] = {
						},
						["subeventSuffix"] = "_AURA_APPLIED",
						["use_showOn"] = true,
						["use_spellName"] = false,
						["event"] = "Combat Log",
						["unevent"] = "timed",
						["realSpellName"] = 0,
						["use_spellId"] = true,
						["name"] = "Quake",
						["auraType"] = "DEBUFF",
						["spellName"] = 0,
						["use_destUnit"] = true,
						["use_unit"] = true,
						["use_auraType"] = true,
					},
					["untrigger"] = {
					},
				}, -- [1]
				["disjunctive"] = "all",
				["activeTriggerMode"] = -10,
			},
			["animation"] = {
				["start"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["main"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
				["finish"] = {
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeStrength"] = 3,
					["easeType"] = "none",
				},
			},
			["conditions"] = {
			},
			["stickyDuration"] = false,
			["backgroundColor"] = {
				0.086274509803922, -- [1]
				0.086274509803922, -- [2]
				0.086274509803922, -- [3]
				1, -- [4]
			},
			["smoothProgress"] = false,
			["useAdjustededMin"] = false,
			["regionType"] = "aurabar",
			["borderInFront"] = false,
			["version"] = 3,
			["icon_side"] = "LEFT",
			["config"] = {
			},
			["spark"] = false,
			["sparkHeight"] = 30,
			["texture"] = "ElvUI Norm",
			["width"] = 400,
			["sparkTexture"] = "Interface\\AddOns\\WeakAuras\\Media\\Textures\\Square_White",
			["semver"] = "1.0.0",
			["zoom"] = 0.3,
			["sparkHidden"] = "NEVER",
			["auto"] = false,
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["alpha"] = 1,
			["sparkColor"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["inverse"] = false,
			["sparkDesature"] = false,
			["orientation"] = "HORIZONTAL",
			["displayIcon"] = 136025,
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["uid"] = "v0WIvsY8rNA",
		},
		["Solution Display"] = {
			["outline"] = "OUTLINE",
			["xOffset"] = -290,
			["displayText"] = "%c",
			["customText"] = "function()\n    return aura_env.solution .. \"\\n\" .. aura_env.lastSolve\nend",
			["yOffset"] = -31,
			["anchorPoint"] = "CENTER",
			["displayText_format_p_time_format"] = 0,
			["customTextUpdate"] = "event",
			["url"] = "https://wago.io/hVLym_eLv/67",
			["actions"] = {
				["start"] = {
				},
				["finish"] = {
				},
				["init"] = {
					["custom"] = "aura_env.debugmode = aura_env.config['debug']\naura_env.version = WeakAuras.GetData(aura_env.id).version\naura_env.versionCheck = 0\naura_env.maxVersion = 0\naura_env.versionPersonal = aura_env.config['versionPersonal']\naura_env.showEncounter = aura_env.config['encounter']\naura_env.selectionColour = aura_env.config['selection']\naura_env.answerColour = aura_env.config['answer']\naura_env.potColour = aura_env.config['potentialSol']\naura_env.announceClicked = aura_env.config['announceClicked']\naura_env.announceSolution = aura_env.config['announceSolution']\naura_env.lotusFlower = aura_env.config['lotusFlower']\naura_env.announceStyle = aura_env.config['announceStyle']\naura_env.announce = aura_env.config['announcer']\naura_env.matchFound = false\naura_env.noCommunication = aura_env.config['noCommunication']\naura_env.useEng = aura_env.config['useEng']\naura_env.eLang = aura_env.config['eLang']\naura_env.lastSolvePH = \"\"\naura_env.announcer = false\naura_env.encounter = false\naura_env.edgeCase = false\naura_env.fiveFour = aura_env.config['fiveFour']\naura_env.specializationName = {\n    [250] = \"TANK\",\n    [581] = \"TANK\",\n    [104] = \"TANK\",\n    [268] = \"TANK\",\n    [66] = \"TANK\",\n    [73] = \"TANK\",\n    [105] = \"HEALER\", \n    [270] = \"HEALER\", \n    [65] = \"HEALER\", \n    [256] = \"HEALER\", \n    [257] = \"HEALER\", \n    [264] = \"HEALER\", \n}\nfunction aura_env.announcement(which,i,count,mark,experiment,cout,sout,fout)\n    aura_env.announcer = false\n    local lead = UnitIsGroupLeader(\"player\")\n    aura_env.playerInfo = aura_env.specializationName[GetSpecializationInfo(GetSpecialization())]\n    if lead and aura_env.announce[1] == true then\n        aura_env.announcer = true\n    elseif aura_env.playerInfo == \"TANK\" and aura_env.announce[2] == true then\n        aura_env.announcer = true\n    elseif aura_env.playerInfo == \"HEALER\" and aura_env.announce[3] == true then\n        aura_env.announcer = true\n    elseif not IsPartyLFG() and not UnitInParty(\"player\") then\n        aura_env.announcer = true\n    elseif aura_env.announce[4] == true then\n        aura_env.announcer = true\n    end\n    if which ~= 1 and aura_env.announcer == true then\n        local cirSay, shapeSay, fillSay, msg\n        if (tonumber(aura_env.logged_images[i][1]) == 0) then\n        cirSay = aura_env.circleText else cirSay = aura_env.nocircleText end\n        if (tonumber(aura_env.logged_images[i][2]) == 0) then\n        shapeSay = aura_env.lotusText else shapeSay = aura_env.leafText end\n        if (tonumber(aura_env.logged_images[i][3]) == 0) then\n        fillSay = aura_env.filledText else fillSay = aura_env.emptyText end\n        if experiment then\n            if cout == 0 then\n                cirSay = aura_env.circleText\n            else \n                cirSay = aura_env.nocircleText\n            end\n            if sout == 0 then\n                shapeSay = aura_env.lotusText            \n            else \n                shapeSay = aura_env.leafText\n            end\n            if fout == 0 then\n                fillSay = aura_env.filledText\n            else \n                fillSay = aura_env.emptyText\n            end\n        end\n        local channel = aura_env.announceChannel(which)\n        if count == true then \n            msg = i ..\": \" .. cirSay .. \" + \" .. shapeSay .. \" + \" .. fillSay \n        else\n            msg = aura_env.solutionText .. \": \"\n            if aura_env.announceStyle == 2 then\n                msg = msg .. cirSay .. \" + \" .. shapeSay .. \" + \" .. fillSay  .. \" > \"\n            end\n            if aura_env.announceStyle == 3 then\n                msg = msg .. cirSay .. \" + \" .. shapeSay .. \" + \" .. fillSay\n            else\n                if mark ==1 then\n                    msg = msg .. cirSay\n                    aura_env.solution = aura_env.solutionText ..\": \" .. cirSay\n                    aura_env.lastSolvePH  = aura_env.lastSolveText .. \": \" .. cirSay\n                elseif mark == 2 then \n                    msg = msg .. shapeSay\n                    aura_env.solution = aura_env.solutionText ..\": \" .. shapeSay\n                    aura_env.lastSolvePH  = aura_env.lastSolveText .. \": \" .. shapeSay\n                else \n                    msg = msg .. fillSay\n                    aura_env.solution = aura_env.solutionText ..\": \" .. fillSay\n                    aura_env.lastSolvePH  = aura_env.lastSolveText .. \": \" .. fillSay\n                end\n            end\n        end\n        SendChatMessage(msg,channel);\n    end\nend\nfunction aura_env.announceChannel(out)\n    if out == 2 then\n        return \"SAY\"\n    elseif out == 3 then\n        return \"YELL\"\n    elseif out == 4 then\n        if IsPartyLFG() then\n            return \"INSTANCE_CHAT\"\n        elseif UnitInParty(\"player\") then\n            return \"PARTY\"\n        else\n            return \"SAY\"\n        end\n    end\nend\naura_env.locale=GetLocale()\naura_env.mistZone = \"Mistveil Tangle\"\naura_env.solution = \"No submissions yet\"\naura_env.solutionText = \"Solution\"\naura_env.nocircleText =\"NO CIRCLE\"\naura_env.circleText = \"CIRCLE\"\naura_env.leafText = \"LEAF\"\naura_env.lotusText = \"LOTUS\"\nif aura_env.lotusFlower == 2 then aura_env.lotusText = \"FLOWER\" end\naura_env.emptyText = \"EMPTY\"\naura_env.filledText = \"FILLED\"\naura_env.noSolveText = \"No solution found\"\naura_env.sigSentText = \"Sigil sent by\"\naura_env.totalText = \"Total provided\"\naura_env.dupeText = \"Duplicated input\"\naura_env.noSubText = \"No submissions yet\"\naura_env.lastSolve = \"\"\naura_env.lastSolveText = \"Previous Solution\"\naura_env.resetText = \"Reset\"\naura_env.mistcaller = \"Mistcaller\"\n\nif aura_env.locale == \"koKR\" then\n    aura_env.mistZone = \"안개장막 덩굴숲\"\n    aura_env.solution = \"입력 없음\"\n    aura_env.solutionText = \"정답\"\n    aura_env.nocircleText =\"테두리 없음\"\n    aura_env.circleText = \"테두리 있음\"\n    aura_env.leafText = \"나뭇잎\"\n    aura_env.lotusText = \"꽃\"\n    aura_env.emptyText = \"색칠 안 됨\"\n    aura_env.filledText = \"색칠됨\"\n    aura_env.noSolveText = \"답을 찾을 수 없음\"\n    aura_env.sigSentText = \"정보 받음 \"\n    aura_env.totalText = \"받은 정보\"\n    aura_env.dupeText = \"중복 입력\"\n    aura_env.noSubText = \"입력 없음\"\n    aura_env.lastSolveText = \"이전 정답\"\n    aura_env.resetText = \"초기화\"\n    aura_env.mistcaller = \"미스트콜러\"\nelseif aura_env.locale == \"frFR\" then\n    aura_env.mistZone = \"Maquis Voile-de-Brume\"\n    aura_env.solution = \"Pas d'entrée actuelle\"\n    aura_env.solutionText = \"Solution\"\n    aura_env.nocircleText =\"NO CERCLE\"\n    aura_env.circleText = \"CERCLE\"\n    aura_env.leafText = \"FEUILLE\"\n    aura_env.lotusText = \"LOTUS\"\n    aura_env.emptyText = \"VIDE\"\n    aura_env.filledText = \"REMPLI\"\n    aura_env.noSolveText = \"Pas de solution trouvée\"\n    aura_env.sigSentText = \"Sigil envoyé par\"\n    aura_env.totalText = \"Total Fourni\"\n    aura_env.dupeText = \"Saise dupliquée\"\n    aura_env.noSubText = \"Pas d'entrée actuelle\"\n    aura_env.lastSolveText = \"Solution précédente\"\n    aura_env.resetText = \"Reset\"\n    aura_env.mistcaller = \"Mandebrume\"\nelseif aura_env.locale == \"deDE\" then\n    aura_env.mistZone = \"Nebelschleierdickicht\"\n    aura_env.solution = \"Noch keine Eingaben\"\n    aura_env.solutionText = \"Lösung\"\n    aura_env.nocircleText = \"KEIN KREIS\"\n    aura_env.circleText = \"KREIS\"\n    aura_env.leafText = \"BLATT\"\n    aura_env.lotusText = \"LOTUS\"\n    aura_env.emptyText = \"NICHT AUSGEFÜLLT\"\n    aura_env.filledText = \"AUSGEFÜLLT\"\n    aura_env.noSolveText = \"Keine Lösung gefunden\"\n    aura_env.sigSentText = \"Siegel gesendet von\"\n    aura_env.totalText = \"Insgesamt bereitgestellt\"\n    aura_env.dupeText = \"Doppelt eingegeben\"\n    aura_env.noSubText = \"Noch keine Eingaben\"\n    aura_env.lastSolveText = \"Vorherige Lösung\"\n    aura_env.resetText = \"Zurücksetzen\"\n    aura_env.mistcaller = \"Nebelruferin\"\nelseif aura_env.locale == \"zhCN\" then\n    aura_env.mistZone = \"纱雾迷结\"\n    aura_env.solution = \"尚未标记\"\n    aura_env.solutionText = \"答案\"\n    aura_env.nocircleText = \"无圈\"\n    aura_env.circleText = \"有圈\"\n    aura_env.leafText = \"叶\"\n    aura_env.lotusText = \"花\"\n    aura_env.emptyText = \"空心\"\n    aura_env.filledText = \"实心\"\n    aura_env.noSolveText = \"未找到答案\"\n    aura_env.sigSentText = \"标记来自于\"\n    aura_env.totalText = \"已标记数\"\n    aura_env.dupeText = \"标记重复\"\n    aura_env.noSubText = \"尚未标记\"\n    aura_env.lastSolveText = \"最终答案\"\n    aura_env.resetText = \"重置\"\n    aura_env.mistcaller = \"唤雾者\"\nelseif aura_env.locale == \"esES\" then\n    aura_env.mistZone = \"Espesura Velo de Niebla\"\n    aura_env.solution = \"Aún no hay entradas\"\n    aura_env.solutionText = \"Solución\"\n    aura_env.nocircleText =\"SIN CIRCULO\"\n    aura_env.circleText = \"CIRCULO\"\n    aura_env.leafText = \"HOJA\"\n    aura_env.lotusText = \"LOTO\"\n    aura_env.emptyText = \"VACIO\"\n    aura_env.filledText = \"RELLENO\"\n    aura_env.noSolveText = \"Solución no encontrada\"\n    aura_env.sigSentText = \"Marca enviada por\"\n    aura_env.totalText = \"Total proporcionado\"\n    aura_env.dupeText = \"Entrada Duplicada\"\n    aura_env.noSubText = \"Aún no hay entradas\"\n    aura_env.lastSolveText = \"Solución anterior\"\n    aura_env.resetText = \"Resetear\"\n    aura_env.mistcaller = \"Clamaneblina\"\nelseif aura_env.locale == \"zhTW\" then\n    aura_env.mistZone = \"霧紗密林\"\n    aura_env.solution = \"尚未標記\"\n    aura_env.solutionText = \"答案\"\n    aura_env.nocircleText =\"沒圈\"\n    aura_env.circleText = \"有圈\"\n    aura_env.leafText = \"葉子\"\n    aura_env.lotusText = \"花\"\n    aura_env.emptyText = \"空心\"\n    aura_env.filledText = \"實心\"\n    aura_env.noSolveText = \"沒有答案\"\n    aura_env.sigSentText = \"圖標製作為\"\n    aura_env.totalText = \"已標記圖標\"\n    aura_env.dupeText = \"標記重複了\"\n    aura_env.noSubText = \"尚未標記\"\n    aura_env.lastSolveText = \"上一個答案\"\n    aura_env.resetText = \"重置\"\n    aura_env.mistcaller = \"唤雾者\"\nelseif aura_env.locale == \"esMX\" then\n    aura_env.mistZone = \"Espesura Veloniebla\"\n    aura_env.solution = \"Aun no hay entradas\"\n    aura_env.solutionText = \"Solucion\"\n    aura_env.nocircleText =\"Sin circulo\"\n    aura_env.circleText = \"Circulo\"\n    aura_env.leafText = \"Hoja\"\n    aura_env.lotusText = \"Loto\"\n    aura_env.emptyText = \"Vacio\"\n    aura_env.filledText = \"Lleno\"\n    aura_env.noSolveText = \"No se encuentra solucion\"\n    aura_env.sigSentText = \"Sigilo enviado por\"\n    aura_env.totalText = \"Total provisto\"\n    aura_env.dupeText = \"Entrada duplicada\"\n    aura_env.noSubText = \"Aun no hay entradas\"\n    aura_env.lastSolveText = \"Solucion anterior\"\n    aura_env.resetText = \"Reiniciar\"\n    aura_env.mistcaller = \"Clamaneblina\"\nelseif aura_env.locale == \"ruRU\" then\n    aura_env.mistZone = \"Туманная чащоба\"\n    aura_env.solution = \"Ничего не введено\"\n    aura_env.solutionText = \"Решение\"\n    aura_env.nocircleText = \"БЕЗ КРУГА\"\n    aura_env.circleText = \"КРУГ\"\n    aura_env.leafText = \"ЛИСТ\"\n    aura_env.lotusText = \"ЛОТОС\"\n    aura_env.emptyText = \"ПУСТОЙ\"\n    aura_env.filledText = \"ЦВЕТНОЙ\"\n    aura_env.noSolveText = \"Нет решения\"\n    aura_env.sigSentText = \"Отправлено\"\n    aura_env.totalText = \"Всего выбрано\"\n    aura_env.dupeText = \"Дублирующиеся данные\"\n    aura_env.noSubText = \"Ничего не введено\"\n    aura_env.lastSolveText = \"Предыдущее решение\"\n    aura_env.resetText = \"Сброс\"\n    aura_env.mistcaller = \"Призывательница Туманов\"\nelseif aura_env.locale == \"ptBR\" then\n    aura_env.mistZone = \"Enleio do Véu da Névoa\"\n    aura_env.solution = \"Sem submições\"\n    aura_env.solutionText = \"Solução\"\n    aura_env.nocircleText = \"Sem Circulo\"\n    aura_env.circleText = \"Circulo\"\n    aura_env.leafText = \"Folha\"\n    aura_env.lotusText = \"Flor\"\n    aura_env.emptyText = \"Vazio\"\n    aura_env.filledText = \"Cheio\"\n    aura_env.noSolveText = \"Solução não encontrada\"\n    aura_env.sigSentText = \"Marca enviada por\"\n    aura_env.totalText = \"Total provido\"\n    aura_env.dupeText = \"Entrada Duplicada\"\n    aura_env.noSubText = \"Sem submições\"\n    aura_env.lastSolveText = \"Solução anterior\"\n    aura_env.resetText = \"Resetar\"\n    aura_env.mistcaller = \"Chamabruma\"\nelseif aura_env.locale == \"itIT\" then\n    aura_env.mistZone = \"Intrico Velofosco\"\n    aura_env.solution = \"Nessuna immissione\"\n    aura_env.solutionText = \"Soluzione\"\n    aura_env.nocircleText =\"NON CERCHIATO\"\n    aura_env.circleText = \"CERCHIATO\"\n    aura_env.leafText = \"FOGLIA\"\n    aura_env.lotusText = \"FIORE\"\n    aura_env.emptyText = \"VUOTO\"\n    aura_env.filledText = \"PIENO\"\n    aura_env.noSolveText = \"Nessuna soluzione trovata\"\n    aura_env.sigSentText = \"Sigillo inviato da\"\n    aura_env.totalText = \"Totale provveduto\"\n    aura_env.dupeText = \"Doppia immissione\"\n    aura_env.noSubText = \"Nessuna immissione\"\n    aura_env.lastSolveText = \"Soluzione precedente\"\n    aura_env.resetText = \"Reset\" \n    aura_env.mistcaller = \"Evocanebbie\"\nelseif aura_env.locale == \"ptPT\" then\n    aura_env.mistZone = \"Enleio do Véu da Névoa\" \nend\nif aura_env.eLang == 2 and not aura_env.useEng then --Dutch\n    aura_env.solution = \"Nog geen antwoorden\"\n    aura_env.solutionText = \"Antwoord\"\n    aura_env.nocircleText =\"Geen cirkel\"\n    aura_env.circleText = \"Cirkel\"\n    aura_env.leafText = \"Blad\"\n    aura_env.lotusText = \"Lotus\"\n    aura_env.emptyText = \"Leeg\"\n    aura_env.filledText = \"Gekleurd\"\n    aura_env.noSolveText = \"Geen antwoord gevonden\"\n    aura_env.sigSentText = \"Sigil verstuurd door\"\n    aura_env.totalText = \"Totaal ingevoerd\"\n    aura_env.dupeText = \"Invoer gedupliceerd\"\n    aura_env.noSubText = \"Nog geen antwoorden\"\n    aura_env.lastSolveText = \"Vorig Antwoord\" \n    aura_env.resetText = \"Reset\"\nelseif aura_env.eLang == 3 and not aura_env.useEng then --Norwegian\n    aura_env.solution = \"Forløpig ingen data\"\n    aura_env.solutionText = \"UTEN SIRKEL\"\n    aura_env.nocircleText =\"UTEN SIRKEL\"\n    aura_env.circleText = \"SIRKEL\"\n    aura_env.leafText = \"LØV\"\n    aura_env.lotusText = \"LOTUS\"\n    aura_env.emptyText = \"IKKE FYLT\"\n    aura_env.filledText = \"FYLT\"\n    aura_env.noSolveText = \"Ingen løsning funnet\"\n    aura_env.sigSentText = \"Symbol sendt av\"\n    aura_env.totalText = \"Totalt tilbudt\"\n    aura_env.dupeText = \"Duplikert tilførsel\"\n    aura_env.noSubText = \"Forløpig ingen data\"\n    aura_env.lastSolveText = \"Forrige løsning\" \n    aura_env.resetText = \"Tilbakestill\"\nelseif aura_env.eLang == 4 and not aura_env.useEng then --Hungarian\n    aura_env.solution = \"Nincs megadva\"\n    aura_env.solutionText = \"Megoldás\"\n    aura_env.nocircleText =\"Nincs karika\"\n    aura_env.circleText = \"Karika\"\n    aura_env.leafText = \"Levél\"\n    aura_env.lotusText = \"Virág\"\n    aura_env.emptyText = \"Üres\"\n    aura_env.filledText = \"Teli\"\n    aura_env.noSolveText = \"Nem található megoldás\"\n    aura_env.sigSentText = \"Jel beküldve\"\n    aura_env.totalText = \"Összes\"\n    aura_env.dupeText = \"Dupla bemenet\"\n    aura_env.noSubText = \"Nincs megadva\"\n    aura_env.lastSolveText = \"Előző megoldás\" \n    aura_env.resetText = \"Töröl\"\nend\nif aura_env.useEng then\n    aura_env.solution = \"No submissions yet\"\n    aura_env.solutionText = \"Solution\"\n    aura_env.nocircleText =\"NO CIRCLE\"\n    aura_env.circleText = \"CIRCLE\"\n    aura_env.leafText = \"LEAF\"\n    aura_env.lotusText = \"LOTUS\"\n    if aura_env.lotusFlower == 2 then aura_env.lotusText = \"FLOWER\" end\n    aura_env.emptyText = \"EMPTY\"\n    aura_env.filledText = \"FILLED\"\n    aura_env.noSolveText = \"No solution found\"\n    aura_env.sigSentText = \"Sigil sent by\"\n    aura_env.totalText = \"Total provided\"\n    aura_env.dupeText = \"Duplicated input\"\n    aura_env.noSubText = \"No submissions yet\"\n    aura_env.lastSolveText = \"Previous Solution\"\n    aura_env.resetText = \"Reset\"\nend\n\nC_ChatInfo.RegisterAddonMessagePrefix(\"Tirna Ver2\")\naura_env.logged_images = {}\naura_env.testArray = {}\naura_env.dupe = false\naura_env.button = 9\n\naura_env.noc = {4,5,6,7}\naura_env.cir = {0,1,2,3}\naura_env.leaf = {0,1,4,5}\naura_env.lotus = {2,3,6,7}\naura_env.nofi = {1,3,5,7}\naura_env.fill = {0,2,4,6}\n\naura_env.LCG = LibStub(\"LibCustomGlow-1.0\")",
					["do_custom"] = true,
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "custom",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HELPFUL",
						["event"] = "Health",
						["unit"] = "player",
						["custom_type"] = "status",
						["spellIds"] = {
						},
						["events"] = "ZONE_CHANGED, PLAYER_LOGIN, PLAYER_ENTERING_WORLD, ENCOUNTER_START",
						["names"] = {
						},
						["check"] = "event",
						["custom"] = "function(event, eid,...)\n    if aura_env.debugmode then \n        return true \n    end\n    local text = GetMinimapZoneText()\n    if text == aura_env.mistZone then\n        return true\n    else\n        return false\n    end\nend",
						["subeventPrefix"] = "SPELL",
						["custom_hide"] = "custom",
					},
					["untrigger"] = {
					},
				}, -- [1]
				{
					["trigger"] = {
						["debuffType"] = "HELPFUL",
						["type"] = "custom",
						["custom"] = "function(event,eid,...)\n    if event == \"ENCOUNTER_END\" then \n        if IsPartyLFG() then\n            C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", \"9 9 9 9v\"..aura_env.version, \"INSTANCE_CHAT\")\n            C_ChatInfo.SendAddonMessageLogged(\"TIRNASCYTHE\", \"9 9 9 9v\"..aura_env.version, \"INSTANCE_CHAT\")\n        elseif UnitInParty(\"player\") then\n            C_ChatInfo.SendAddonMessageLogged(\"Tirna Ver2\", \"9 9 9 9v\"..aura_env.version, \"PARTY\")\n            C_ChatInfo.SendAddonMessageLogged(\"TIRNASCYTHE\", \"9 9 9 9v\"..aura_env.version, \"PARTY\")\n        end\n        if eid == 2392 and aura_env.showEncounter == false then \n            return false \n        end\n        return true \n    elseif event == \"ENCOUNTER_START\" then \n        if aura_env.showEncounter == false then \n            return false\n        else\n            return true\n        end\n    end\n    return true\nend",
						["custom_type"] = "status",
						["check"] = "event",
						["customName"] = "function()\n    return \nend",
						["events"] = "ENCOUNTER_START, ENCOUNTER_END, PLAYER_ENTERING_WORLD",
						["unit"] = "player",
					},
					["untrigger"] = {
					},
				}, -- [2]
				{
					["trigger"] = {
						["use_alwaystrue"] = true,
						["use_genericShowOn"] = true,
						["genericShowOn"] = "showOnCooldown",
						["names"] = {
						},
						["dynamicDuration"] = false,
						["unit"] = "player",
						["custom_hide"] = "timed",
						["spellName"] = 0,
						["duration"] = "60",
						["type"] = "custom",
						["custom_type"] = "event",
						["subeventSuffix"] = "_CAST_START",
						["custom"] = "function(event, prefix, msg, dist, sender)\n    -- Ignore non-WA messages\n    if prefix ~= \"Tirna Ver2\" then return false end\n    -- Limit version checks vs other players to 8 times\n    if aura_env.versionCheck < 8 then\n        if string.match(msg, \"v\") then\n            local version = string.match(msg, \"v%d%d%d\") or string.match(msg, \"v%d%d\") or string.match(msg, \"v%d\")\n            version = string.match(version, \"%d%d%d\") or string.match(version, \"%d%d\") or string.match(version, \"%d\")\n            if tonumber(version) >= tonumber(aura_env.version + aura_env.versionPersonal) then \n                if aura_env.maxVersion < tonumber(version) then\n                    aura_env.maxVersion = tonumber(version)\n                    print(\"|cFFFF0000Mists of Tirna Scithe maze guessing game is \" .. (tonumber(version) - tonumber(aura_env.version)).. \" versions old, please update.|r\")\n                end            \n                aura_env.versionCheck = aura_env.versionCheck + 1\n            end \n        end\n    end\n    \n    if aura_env.debugmode then\n        print(\"Received:\",prefix,\":\", msg, dist, sender)\n    end\n    if dist ~= \"WHISPER\" and aura_env.noCommunication then \n        if aura_env.debugmode then print(\"Ignored ^\") end\n        return false \n    end\n    \n    local length = table.getn(aura_env.logged_images)\n    if length == 0 then WeakAuras.ScanEvents(\"GlowReset\", true) end\n    -- If we're resetting\n    local circle, shape, fill, dup = string.match(msg, \"(%d) (%d) (%d) (%d)\")\n    if (circle == \"9\" and shape == \"9\" and fill == \"9\" and dup == \"9\") then\n        aura_env.solution = aura_env.resetText .. \": \" .. sender .. \"\\n\" .. aura_env.noSubText\n        aura_env.logged_images = {}\n        aura_env.matchFound = false\n        WeakAuras.ScanEvents(\"GlowReset\", true)\n        return true \n    end\n    \n    -- Main logic\n    local array = {circle, shape, fill, dup}\n    aura_env.dupe = false\n    aura_env.edgeCase = false\n    local remove = false\n    if string.match(msg, \"RightButton\") then\n        remove = true\n    end\n    -- Check for duplicates and check for removals\n    if length > 0 then\n        for i=1, table.getn(aura_env.logged_images) do \n            if aura_env.logged_images[i][4] == array[4] then \n                if remove then\n                    WeakAuras.ScanEvents(\"TirnaScitheButtonOrder\", tonumber(aura_env.logged_images[i][4]), 9)\n                    WeakAuras.ScanEvents(\"GlowReset\", false, tonumber(aura_env.logged_images[i][4]))\n                    table.remove(aura_env.logged_images,i)\n                    for j=1, table.getn(aura_env.logged_images) do \n                        WeakAuras.ScanEvents(\"GlowReset\", false, tonumber(aura_env.logged_images[j][4]))\n                        WeakAuras.ScanEvents(\"TirnaScitheButtonOrder\", tonumber(aura_env.logged_images[j][4]), j)\n                        aura_env.LCG.PixelGlow_Start(aura_env.testArray[tonumber(aura_env.logged_images[j][4])], aura_env.selectionColour,8,0,1000,3,0,0,true,2)\n                    end\n                    length = table.getn(aura_env.logged_images)\n                    if length == 4 and aura_env.fiveFour then \n                        aura_env.edgeCase = true\n                    end\n                    break\n                else\n                    aura_env.dupe = true\n                    if length == 4 then \n                        aura_env.edgeCase = true\n                    end\n                end\n            end\n        end\n    end\n    -- If not a duplicate then calculate answer\n    if not aura_env.dupe and not remove then\n        aura_env.logged_images[length + 1] = array\n        local new_length = table.getn(aura_env.logged_images)\n        if new_length == 1 then \n            aura_env.lastSolve = aura_env.lastSolvePH\n        end\n        aura_env.solution = aura_env.sigSentText .. \" \" .. sender .. \"\\n\" .. aura_env.totalText .. \": \" .. new_length .. \"/4\"\n        for i=1, new_length do \n            aura_env.button = tonumber(aura_env.logged_images[i][4])\n            aura_env.LCG.PixelGlow_Start(aura_env.testArray[aura_env.button], aura_env.selectionColour,8,0,1000,3,0,0,true,2)\n            if i == new_length then\n                aura_env.announcement(aura_env.announceClicked,i,true,0)\n            end\n        end\n        WeakAuras.ScanEvents(\"TirnaScitheButtonOrder\", aura_env.button, new_length)\n    end\n    -- Ignore additional signs when at 4\n    if length > 3 and not aura_env.edgeCase then return false end\n    if not aura_env.dupe or aura_env.edgeCase then\n        local new_length = table.getn(aura_env.logged_images)\n        if new_length == 3 then \n            local checkOpp12 = {}\n            local checkOpp13 = {}\n            local checkOpp23 = {}\n            local checkAttTotal = {}\n            local countCout = 0\n            local countSout = 0\n            local countFout = 0\n            local count12match = 0\n            local count13match = 0\n            local count23match = 0\n            local annOut = 0\n            for i=1,3 do\n                checkOpp12[i] = (tonumber(aura_env.logged_images[1][i]) + tonumber(aura_env.logged_images[2][i]))\n                checkOpp13[i] = (tonumber(aura_env.logged_images[1][i]) + tonumber(aura_env.logged_images[3][i]))\n                checkOpp23[i] = (tonumber(aura_env.logged_images[2][i]) + tonumber(aura_env.logged_images[3][i]))\n            end\n            if aura_env.debugmode then \n                print(tonumber(aura_env.logged_images[1][1]), tonumber(aura_env.logged_images[1][2]), tonumber(aura_env.logged_images[1][3]))\n                print(tonumber(aura_env.logged_images[2][1]), tonumber(aura_env.logged_images[2][2]), tonumber(aura_env.logged_images[2][3]))\n                print(tonumber(aura_env.logged_images[3][1]), tonumber(aura_env.logged_images[3][2]), tonumber(aura_env.logged_images[3][3]))\n                print(\"-----\")\n                print(checkOpp12[1],checkOpp12[2],checkOpp12[3])\n                print(checkOpp13[1],checkOpp13[2],checkOpp13[3])\n                print(checkOpp23[1],checkOpp23[2],checkOpp23[3])\n            end\n            if (checkOpp12[1] == 1 and checkOpp12[2] == 1 and checkOpp12[3] == 1) or \n            (checkOpp13[1] == 1 and checkOpp13[2] == 1 and checkOpp13[3] == 1) or \n            (checkOpp23[1] == 1 and checkOpp23[2] == 1 and checkOpp23[3] == 1) then\n                if checkOpp12[1] == 0 or checkOpp12[1] == 2 then count12match = count12match + 1 end\n                if checkOpp12[2] == 0 or checkOpp12[2] == 2 then count12match = count12match + 1 end\n                if checkOpp12[3] == 0 or checkOpp12[3] == 2 then count12match = count12match + 1 end\n                if checkOpp13[1] == 0 or checkOpp13[1] == 2 then count13match = count13match + 1 end\n                if checkOpp13[2] == 0 or checkOpp13[2] == 2 then count13match = count13match + 1 end\n                if checkOpp13[3] == 0 or checkOpp13[3] == 2 then count13match = count13match + 1 end\n                if checkOpp23[1] == 0 or checkOpp23[1] == 2 then count23match = count23match + 1 end\n                if checkOpp23[2] == 0 or checkOpp23[2] == 2 then count23match = count23match + 1 end\n                if checkOpp23[3] == 0 or checkOpp23[3] == 2 then count23match = count23match + 1 end\n                if count12match ~= 2 and count13match ~= 2 then \n                    if checkOpp23[1] == 2 then annOut = 1 elseif checkOpp23[2] == 2 then annOut = 2 else annOut = 3 end\n                    aura_env.LCG.PixelGlow_Start(aura_env.testArray[tonumber(aura_env.logged_images[1][4])], aura_env.answerColour,8,0,1000,3,0,0,true,2)\n                    aura_env.announcement(aura_env.announceSolution,1,false,annOut,true,tonumber(aura_env.logged_images[1][1]),tonumber(aura_env.logged_images[1][2]),tonumber(aura_env.logged_images[1][3]))\n                    aura_env.matchFound = true\n                    return true\n                end\n                if count12match ~= 2 and count23match ~= 2 then \n                    if checkOpp13[1] == 2 then annOut = 1 elseif checkOpp13[2] == 2 then annOut = 2 else annOut = 3 end                    \n                    aura_env.LCG.PixelGlow_Start(aura_env.testArray[tonumber(aura_env.logged_images[2][4])], aura_env.answerColour,8,0,1000,3,0,0,true,2)\n                    aura_env.announcement(aura_env.announceSolution,1,false,annOut,true,tonumber(aura_env.logged_images[2][1]),tonumber(aura_env.logged_images[2][2]),tonumber(aura_env.logged_images[2][3]))\n                    aura_env.matchFound = true\n                    return true\n                end\n                if count23match ~= 2 and count13match ~= 2 then\n                    if checkOpp12[1] == 2 then annOut = 1 elseif checkOpp12[2] == 2 then annOut = 2 else annOut = 3 end\n                    aura_env.LCG.PixelGlow_Start(aura_env.testArray[tonumber(aura_env.logged_images[3][4])], aura_env.answerColour,8,0,1000,3,0,0,true,2)\n                    aura_env.announcement(aura_env.announceSolution,1,false,annOut,true,tonumber(aura_env.logged_images[3][1]),tonumber(aura_env.logged_images[3][2]),tonumber(aura_env.logged_images[3][3]))\n                    aura_env.matchFound = true\n                    return true\n                end\n            end\n            checkAttTotal[1] = checkOpp12[1] + checkOpp13[1] + checkOpp23[1]\n            checkAttTotal[2] = checkOpp12[2] + checkOpp13[2] + checkOpp23[2]\n            checkAttTotal[3] = checkOpp12[3] + checkOpp13[3] + checkOpp23[3]\n            if checkAttTotal[1] == 6 or checkAttTotal[1] == 0 or checkAttTotal[2] == 6 or checkAttTotal[2] == 0 or checkAttTotal[3] == 6 or checkAttTotal[3] == 0 then\n                if checkOpp12[1] == 0 or checkOpp13[1] == 0 or checkOpp23[1] == 0 then countCout = 1 end\n                if checkOpp12[2] == 0 or checkOpp13[2] == 0 or checkOpp23[2] == 0 then countSout = 1 end\n                if checkOpp12[3] == 0 or checkOpp13[3] == 0 or checkOpp23[3] == 0 then countFout = 1 end                \n                if checkAttTotal[1] == 0 then countCout = 1 annOut = 1 elseif checkAttTotal[1] == 6 then countCout = 0 annOut = 1 end\n                if checkAttTotal[2] == 0 then countSout = 1 annOut = 2 elseif checkAttTotal[2] == 6 then countSout = 0 annOut = 2 end                \n                if checkAttTotal[3] == 0 then countFout = 1 annOut = 3 elseif checkAttTotal[3] == 6 then countFout = 0 annOut = 3 end\n                local outframetest = (4*countCout) + ((countSout-1)*-2) + countFout\n                WeakAuras.ScanEvents(\"TirnaScitheButtonOrder\", outframetest, \"4\")\n                aura_env.LCG.PixelGlow_Start(aura_env.testArray[outframetest], aura_env.answerColour,8,0,1000,3,0,0,true,2)\n                aura_env.announcement(aura_env.announceSolution,1,false,annOut,true,countCout,countSout,countFout)\n                aura_env.logged_images[4] = {countCout,countSout,countFout,tostring(outframetest)}\n                aura_env.matchFound = true\n                return true\n            end\n        end\n        if new_length > 3 then\n            local circleSum = 0\n            local shapeSum = 0\n            local fillSum = 0\n            for i=1,4 do\n                circleSum = circleSum + tonumber(aura_env.logged_images[i][1])\n                shapeSum = shapeSum + tonumber(aura_env.logged_images[i][2])\n                fillSum = fillSum + tonumber(aura_env.logged_images[i][3])\n            end\n            \n            if circleSum == 1 then\n                for i=1, new_length do \n                    for j=1, 4 do \n                        if aura_env.noc[j] == tonumber(aura_env.logged_images[i][4]) then \n                            aura_env.LCG.PixelGlow_Start(aura_env.testArray[tonumber(aura_env.logged_images[i][4])], aura_env.answerColour,8,0,1000,3,0,0,true,2)\n                            aura_env.announcement(aura_env.announceSolution,i,false,1)\n                            aura_env.matchFound = true\n                        end\n                    end\n                end\n                aura_env.solution = aura_env.solutionText ..\": \" .. aura_env.nocircleText\n                aura_env.lastSolvePH  = aura_env.lastSolveText .. \": \" ..aura_env.nocircleText\n            elseif circleSum == 3 then\n                for i=1, new_length do \n                    for j=1, 4 do \n                        if aura_env.cir[j] == tonumber(aura_env.logged_images[i][4]) then \n                            aura_env.LCG.PixelGlow_Start(aura_env.testArray[tonumber(aura_env.logged_images[i][4])], aura_env.answerColour,8,0,1000,3,0,0,true,2)\n                            aura_env.announcement(aura_env.announceSolution,i,false,1)\n                            aura_env.matchFound = true\n                        end\n                    end\n                end\n                aura_env.solution = aura_env.solutionText ..\": \" .. aura_env.circleText                \n                aura_env.lastSolvePH  = aura_env.lastSolveText .. \": \" ..aura_env.circleText\n            elseif shapeSum == 1 then\n                for i=1, new_length do \n                    for j=1, 4 do \n                        if aura_env.leaf[j] == tonumber(aura_env.logged_images[i][4]) then \n                            aura_env.LCG.PixelGlow_Start(aura_env.testArray[tonumber(aura_env.logged_images[i][4])], aura_env.answerColour,8,0,1000,3,0,0,true,2)\n                            aura_env.announcement(aura_env.announceSolution,i,false,2)\n                            aura_env.matchFound = true\n                        end\n                    end\n                end\n                aura_env.solution = aura_env.solutionText ..\": \" .. aura_env.leafText\n                aura_env.lastSolvePH  = aura_env.lastSolveText .. \": \" ..aura_env.leafText\n            elseif shapeSum == 3 then\n                for i=1, new_length do \n                    for j=1, 4 do \n                        if aura_env.lotus[j] == tonumber(aura_env.logged_images[i][4]) then \n                            aura_env.LCG.PixelGlow_Start(aura_env.testArray[tonumber(aura_env.logged_images[i][4])], aura_env.answerColour,8,0,1000,3,0,0,true,2)\n                            aura_env.announcement(aura_env.announceSolution,i,false,2)\n                            aura_env.matchFound = true\n                        end\n                    end\n                end\n                aura_env.solution = aura_env.solutionText ..\": \" .. aura_env.lotusText\n                aura_env.lastSolvePH  = aura_env.lastSolveText .. \": \" ..aura_env.lotusText\n            elseif fillSum == 1 then\n                for i=1, new_length do \n                    for j=1, 4 do \n                        if aura_env.nofi[j] == tonumber(aura_env.logged_images[i][4]) then \n                            aura_env.LCG.PixelGlow_Start(aura_env.testArray[tonumber(aura_env.logged_images[i][4])], aura_env.answerColour,8,0,1000,3,0,0,true,2)\n                            aura_env.announcement(aura_env.announceSolution,i,false,3)\n                            aura_env.matchFound = true\n                        end\n                    end\n                end\n                aura_env.solution = aura_env.solutionText ..\": \" .. aura_env.emptyText\n                aura_env.lastSolvePH  = aura_env.lastSolveText .. \": \" ..aura_env.emptyText\n            elseif fillSum == 3 then\n                for i=1, new_length do \n                    for j=1, 4 do \n                        if aura_env.fill[j] == tonumber(aura_env.logged_images[i][4]) then \n                            aura_env.LCG.PixelGlow_Start(aura_env.testArray[tonumber(aura_env.logged_images[i][4])], aura_env.answerColour,8,0,1000,3,0,0,true,2)\n                            aura_env.announcement(aura_env.announceSolution,i,false,3)\n                            aura_env.matchFound = true\n                        end\n                    end\n                end\n                aura_env.solution = aura_env.solutionText ..\": \" .. aura_env.filledText\n                aura_env.lastSolvePH  = aura_env.lastSolveText .. \": \" ..aura_env.filledText\n            else\n                aura_env.solution = aura_env.noSolveText\n            end\n        else\n            aura_env.solution = aura_env.sigSentText .. \" \" .. sender .. \"\\n\" .. aura_env.totalText .. \": \" .. new_length .. \"/4\"         \n        end\n    else\n        aura_env.solution = aura_env.dupeText\n    end\n    return true\nend",
						["use_spellName"] = true,
						["event"] = "Combat Log",
						["unevent"] = "timed",
						["realSpellName"] = 0,
						["customName"] = "",
						["spellIds"] = {
						},
						["events"] = "CHAT_MSG_ADDON_LOGGED",
						["check"] = "event",
						["debuffType"] = "HELPFUL",
						["use_track"] = true,
						["subeventPrefix"] = "SPELL",
					},
					["untrigger"] = {
					},
				}, -- [3]
				{
					["trigger"] = {
						["use_genericShowOn"] = true,
						["genericShowOn"] = "showOnCooldown",
						["unit"] = "player",
						["debuffType"] = "HELPFUL",
						["type"] = "custom",
						["spellName"] = 0,
						["unevent"] = "auto",
						["subeventSuffix"] = "",
						["subeventPrefix"] = "",
						["event"] = "Cooldown Progress (Spell)",
						["events"] = "TirnaScitheButtonData",
						["realSpellName"] = 0,
						["use_spellName"] = true,
						["custom"] = "function(event, frameNumber, frameData)\n    -- Add button data to array\n    if not aura_env.testArray[frameNumber] or aura_env.testArray[frameNumber] ~= frameData then\n        if aura_env.debugmode then\n            print(\"ButtonData:\",frameNumber, frameData, aura_env.testArray[frameNumber])\n        end\n        aura_env.testArray[frameNumber] = frameData\n    end\n    return true\nend",
						["custom_type"] = "event",
						["check"] = "event",
						["duration"] = "0.5",
						["use_track"] = true,
						["custom_hide"] = "custom",
					},
					["untrigger"] = {
					},
				}, -- [4]
				{
					["trigger"] = {
						["use_genericShowOn"] = true,
						["genericShowOn"] = "showOnCooldown",
						["unit"] = "player",
						["debuffType"] = "HELPFUL",
						["type"] = "custom",
						["spellName"] = 0,
						["unevent"] = "auto",
						["subeventSuffix"] = "",
						["subeventPrefix"] = "",
						["event"] = "Cooldown Progress (Spell)",
						["events"] = "GlowReset",
						["realSpellName"] = 0,
						["use_spellName"] = true,
						["custom"] = "function(event,full,button)\n    if full then\n        for i=0, 7 do \n            aura_env.LCG.PixelGlow_Stop(aura_env.testArray[i], 2)\n        end\n        WeakAuras.ScanEvents(\"TirnaScitheButtonOrder\", 9, 9)\n    else\n        aura_env.LCG.PixelGlow_Stop(aura_env.testArray[button], 2)\n    end\n    return true\nend",
						["custom_type"] = "status",
						["check"] = "event",
						["duration"] = "0.5",
						["use_track"] = true,
						["custom_hide"] = "timed",
					},
					["untrigger"] = {
					},
				}, -- [5]
				{
					["trigger"] = {
						["type"] = "custom",
						["unevent"] = "timed",
						["duration"] = "1",
						["event"] = "Combat Log",
						["unit"] = "player",
						["custom_type"] = "status",
						["events"] = "CHAT_MSG_MONSTER_SAY, CHAT_MSG_MONSTER_YELL",
						["subeventSuffix"] = "",
						["check"] = "event",
						["subeventPrefix"] = "",
						["custom"] = "function(event, ...)\n    if select(2,...) == aura_env.mistcaller then\n        if aura_env.matchFound then\n            WeakAuras.ScanEvents(\"GlowReset\", true)\n            aura_env.logged_images = {}\n            aura_env.matchFound = false\n        end\n    end\n    return true\nend",
						["debuffType"] = "HELPFUL",
					},
					["untrigger"] = {
					},
				}, -- [6]
				["activeTriggerMode"] = -10,
			},
			["displayText_format_p_format"] = "timed",
			["internalVersion"] = 45,
			["wordWrap"] = "Elide",
			["font"] = "Arial Narrow",
			["version"] = 67,
			["subRegions"] = {
			},
			["load"] = {
				["ingroup"] = {
					["single"] = "group",
					["multi"] = {
						["group"] = true,
					},
				},
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["spec"] = {
					["multi"] = {
					},
				},
				["use_encounterid"] = false,
				["difficulty"] = {
					["multi"] = {
						["mythic"] = true,
						["challenge"] = true,
					},
				},
				["use_zone"] = false,
				["zoneIds"] = "1669",
				["use_zoneIds"] = true,
				["zone"] = "",
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["fontSize"] = 21,
			["displayText_format_n_format"] = "none",
			["shadowXOffset"] = 1,
			["displayText_format_3.n_format"] = "none",
			["preferToUpdate"] = true,
			["regionType"] = "text",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["selfPoint"] = "TOP",
			["conditions"] = {
			},
			["displayText_format_p_time_precision"] = 1,
			["shadowYOffset"] = -1,
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["uid"] = "mO6xel(DgdK",
			["semver"] = "2.0.2",
			["tocversion"] = 90005,
			["id"] = "Solution Display",
			["authorOptions"] = {
				{
					["type"] = "toggle",
					["key"] = "encounter",
					["default"] = false,
					["name"] = "Show during Boss",
					["useDesc"] = false,
					["width"] = 1,
				}, -- [1]
				{
					["type"] = "color",
					["key"] = "selection",
					["default"] = {
						0, -- [1]
						0, -- [2]
						1, -- [3]
						1, -- [4]
					},
					["name"] = "Selection Border",
					["useDesc"] = false,
					["width"] = 1,
				}, -- [2]
				{
					["type"] = "color",
					["key"] = "answer",
					["default"] = {
						1, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
					["name"] = "Solution Border",
					["useDesc"] = false,
					["width"] = 1,
				}, -- [3]
				{
					["type"] = "select",
					["values"] = {
						"Disabled", -- [1]
						"SAY", -- [2]
						"YELL", -- [3]
						"PARTY > INSTANCE > SAY", -- [4]
					},
					["default"] = 1,
					["name"] = "Announce Selection",
					["useDesc"] = false,
					["key"] = "announceClicked",
					["width"] = 1,
				}, -- [4]
				{
					["type"] = "select",
					["values"] = {
						"Disabled", -- [1]
						"SAY", -- [2]
						"YELL", -- [3]
						"PARTY > INSTANCE > SAY", -- [4]
					},
					["default"] = 4,
					["name"] = "Announce Solution",
					["useDesc"] = false,
					["key"] = "announceSolution",
					["width"] = 1,
				}, -- [5]
				{
					["type"] = "select",
					["values"] = {
						"Trait", -- [1]
						"X + Y + Z > Trait", -- [2]
						"X + Y + Z", -- [3]
					},
					["default"] = 1,
					["name"] = "Announcement style",
					["useDesc"] = false,
					["key"] = "announceStyle",
					["width"] = 1,
				}, -- [6]
				{
					["type"] = "select",
					["values"] = {
						"LOTUS", -- [1]
						"FLOWER", -- [2]
					},
					["default"] = 2,
					["name"] = "Lotus or Flower",
					["useDesc"] = false,
					["key"] = "lotusFlower",
					["width"] = 1,
				}, -- [7]
				{
					["type"] = "multiselect",
					["values"] = {
						"Party Leader", -- [1]
						"Tank", -- [2]
						"Healer", -- [3]
						"Always", -- [4]
					},
					["default"] = {
						true, -- [1]
						false, -- [2]
						false, -- [3]
						false, -- [4]
					},
					["name"] = "Announce as:",
					["useDesc"] = false,
					["key"] = "announcer",
					["width"] = 1,
				}, -- [8]
				{
					["type"] = "header",
					["useName"] = false,
					["text"] = "",
					["noMerge"] = false,
					["width"] = 1,
				}, -- [9]
				{
					["type"] = "toggle",
					["key"] = "noCommunication",
					["default"] = false,
					["name"] = "Disable Cross-Communication with others running the WA",
					["useDesc"] = false,
					["width"] = 2,
				}, -- [10]
				{
					["type"] = "toggle",
					["key"] = "useEng",
					["width"] = 2,
					["name"] = "Use English instead of Client Regional Language",
					["useDesc"] = true,
					["default"] = false,
					["desc"] = "(Overwrites selection below)",
				}, -- [11]
				{
					["type"] = "select",
					["values"] = {
						"Use Client Region Language", -- [1]
						"Dutch", -- [2]
						"Norwegian (Bokmål)", -- [3]
						"Hungarian", -- [4]
					},
					["default"] = 1,
					["name"] = "Language (non-client region Language)",
					["useDesc"] = false,
					["key"] = "eLang",
					["width"] = 1,
				}, -- [12]
				{
					["type"] = "range",
					["useDesc"] = false,
					["max"] = 10,
					["step"] = 1,
					["width"] = 1,
					["min"] = 1,
					["key"] = "versionPersonal",
					["default"] = 1,
					["name"] = "Notify when version outdated:",
				}, -- [13]
				{
					["type"] = "toggle",
					["key"] = "fiveFour",
					["width"] = 2,
					["name"] = "5 - 4 on removal check (Mouseover for info)",
					["useDesc"] = true,
					["default"] = true,
					["desc"] = "Make a check on going from 5 to 4 symbols. Can potentially announce an impossible combination if a false symbol remains in the four selected.",
				}, -- [14]
				{
					["type"] = "header",
					["useName"] = false,
					["text"] = "",
					["noMerge"] = false,
					["width"] = 1,
				}, -- [15]
				{
					["type"] = "toggle",
					["key"] = "debug",
					["default"] = false,
					["name"] = "|cffff0000Debug testing|r (requires reload for full debug)",
					["useDesc"] = false,
					["width"] = 2,
				}, -- [16]
			},
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["justify"] = "LEFT",
			["config"] = {
				["lotusFlower"] = 2,
				["debug"] = false,
				["encounter"] = false,
				["announceClicked"] = 1,
				["announceStyle"] = 1,
				["fiveFour"] = true,
				["eLang"] = 1,
				["selection"] = {
					0, -- [1]
					0, -- [2]
					1, -- [3]
					1, -- [4]
				},
				["announceSolution"] = 4,
				["versionPersonal"] = 1,
				["announcer"] = {
					true, -- [1]
					false, -- [2]
					false, -- [3]
					false, -- [4]
				},
				["noCommunication"] = false,
				["answer"] = {
					1, -- [1]
					0, -- [2]
					0, -- [3]
					1, -- [4]
				},
				["useEng"] = false,
			},
			["automaticWidth"] = "Auto",
			["displayText_format_p_time_dynamic_threshold"] = 60,
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["fixedWidth"] = 300,
			["information"] = {
				["ignoreOptionsEventErrors"] = true,
			},
			["parent"] = "Mists of Tirna Scithe guessing game v2",
		},
		["No Backup Warning"] = {
			["outline"] = "OUTLINE",
			["color"] = {
				1, -- [1]
				1, -- [2]
				1, -- [3]
				1, -- [4]
			},
			["displayText"] = "%c",
			["customText"] = "function()\n    if (aura_env.state and aura_env.mark) or WeakAuras.IsOptionsOpen() then\n        return \"|cFFFF0000 NO BACKUP FOR:\"..aura_env.icons[aura_env.mark]\n    end\nend",
			["yOffset"] = 295.99975585938,
			["anchorPoint"] = "CENTER",
			["displayText_format_p_time_format"] = 0,
			["customTextUpdate"] = "event",
			["url"] = "https://wago.io/kJFQFy0V8/3",
			["actions"] = {
				["start"] = {
					["sound"] = "Interface\\AddOns\\WeakAuras\\Media\\Sounds\\AirHorn.ogg",
					["do_sound"] = false,
				},
				["finish"] = {
				},
				["init"] = {
					["custom"] = "aura_env.mark = 1\naura_env.icons= { \n    [1]  = \n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_1:12\\124t\",\n    [2] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_2:12\\124t\", \n    [3] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_3:12\\124t\", \n    [4] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_4:12\\124t\", \n    [5] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_5:12\\124t\", \n    [6] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_6:12\\124t\",   \n    [7] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_7:12\\124t\",\n    [8] =\n    \" \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_8:12\\124t\",\n    [9] =\n    \" Right of \\124TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_1:12\\124t\",\n    [10] =\n    \" 10\",\n    [11] =\n    \" 11\",\n    [12] =\n    \" 12\",\n    [13] =\n    \" 13\",\n    [14] =\n    \" 14\",\n    [15] =\n    \" 15\",\n}",
					["do_custom"] = true,
				},
			},
			["triggers"] = {
				{
					["trigger"] = {
						["type"] = "custom",
						["subeventSuffix"] = "_CAST_START",
						["debuffType"] = "HELPFUL",
						["duration"] = "5.2",
						["event"] = "Health",
						["unit"] = "player",
						["names"] = {
						},
						["dynamicDuration"] = false,
						["custom"] = "function(event, mark)\n    if mark then \n        aura_env.mark = mark\n        return true\n    end\nend",
						["events"] = "RELOE_BACKUP_ALERT2",
						["spellIds"] = {
						},
						["subeventPrefix"] = "SPELL",
						["custom_type"] = "event",
						["custom_hide"] = "timed",
					},
					["untrigger"] = {
					},
				}, -- [1]
				["activeTriggerMode"] = -10,
			},
			["displayText_format_p_format"] = "timed",
			["internalVersion"] = 45,
			["wordWrap"] = "WordWrap",
			["font"] = "Friz Quadrata TT",
			["version"] = 3,
			["subRegions"] = {
			},
			["load"] = {
				["use_encounterid"] = true,
				["zoneIds"] = "",
				["use_never"] = false,
				["talent"] = {
					["multi"] = {
					},
				},
				["encounterid"] = "2417",
				["spec"] = {
					["multi"] = {
					},
				},
				["class"] = {
					["multi"] = {
					},
				},
				["size"] = {
					["multi"] = {
					},
				},
			},
			["fontSize"] = 60,
			["shadowXOffset"] = 1,
			["displayText_format_p_time_dynamic_threshold"] = 60,
			["regionType"] = "text",
			["parent"] = "09 Generals Soak Alerts",
			["selfPoint"] = "BOTTOM",
			["conditions"] = {
			},
			["displayText_format_p_time_precision"] = 1,
			["authorOptions"] = {
			},
			["automaticWidth"] = "Auto",
			["config"] = {
			},
			["justify"] = "LEFT",
			["tocversion"] = 90002,
			["id"] = "No Backup Warning",
			["xOffset"] = 0,
			["frameStrata"] = 1,
			["anchorFrameType"] = "SCREEN",
			["shadowYOffset"] = -1,
			["uid"] = "w6p0HXjF))0",
			["semver"] = "1.0.2",
			["animation"] = {
				["start"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["main"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
				["finish"] = {
					["easeStrength"] = 3,
					["type"] = "none",
					["duration_type"] = "seconds",
					["easeType"] = "none",
				},
			},
			["shadowColor"] = {
				0, -- [1]
				0, -- [2]
				0, -- [3]
				1, -- [4]
			},
			["fixedWidth"] = 200,
			["information"] = {
			},
			["preferToUpdate"] = false,
		},
	},
	["registered"] = {
	},
	["frame"] = {
		["xOffset"] = -2677.579711914063,
		["yOffset"] = -525.3277587890625,
		["height"] = 681.6665649414062,
		["width"] = 782.4998168945312,
	},
	["editor_theme"] = "Monokai",
}
